  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        'user-agent': 'OMEGAv6-R316-Builder-Control-Plane',
...
async function observeCanonical(env) {
  ...
  const githubPrsPromise = fetchJson(`${githubBase}/pulls?state=open&base=main&per_page=50`);
  ...
  return {
    schema: 'OMEGA_R316_OBSERVATION_V1',
    observedAt: now(),
    repository,
    publicBase,
...
function deriveDecision(observation) {
  ...
  return {
    schema: 'OMEGA_R316_BUILD_DECISION_V1',
    revision: REVISION,
    decidedAt: now(),
    decision,
    reason,
...
async function persistReceipt(env, observation, decision, trigger) {
  ...
  const receipt = {
    schema: 'OMEGA_R316_BUILD_RECEIPT_V1',
    receiptId: `R316-${Date.now()}-${crypto.randomUUID().slice(0, 8)}`,
    recordedAt: now(),
    trigger,
    observation,
    decision,
  ...
  async fetch(request) {
    ...
    if (request.method === 'POST' && url.pathname === '/append') {
      ...
      const state = (await this.ctx.storage.get('state')) || {
        schema: 'OMEGA_R316_LEDGER_STATE_V1',
        revision: REVISION,
        receipts: [],
      };
