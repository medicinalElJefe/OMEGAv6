...
export const R141_LAWS=Object.freeze([
 ...
 'OUTPUT_CANNOT_MUTATE_CANONSTATE_WITHOUT_ADMISSION',
 'FAILED_OR_STALE_EXECUTION_REMAINS_VISIBLE'
]);
...
export type CapabilityExecutionReceiptR141={
 schema:typeof R141_SCHEMA;revision:typeof R141_REVISION;receiptId:string;capabilityId:string;domain:CapabilityDomainR141;route:string|null;intent:string|null;
 state:CapabilityExecutionStateR141;authorized:boolean;available:boolean;invoked:boolean;returned:boolean;verified:boolean;startedAt:string|null;finishedAt:string|null;
 latencyMs:number|null;requestHash:string|null;responseHash:string|null;source:string;failureReason:string|null;freshUntil:string|null;proofRef:string|null;
...
export function deriveCapabilityExecutionStateR141(x:any):CapabilityExecutionStateR141{
 ...
 if(x?.verified===true)return 'VERIFIED';if(x?.returned===true)return 'RETURNED';if(x?.invoked===true)return 'INVOKED';if(x?.available===true)return 'AVAILABLE';if(x?.authorized===true)return 'AUTHORIZED';
...
export function pluginManifestReceiptR141(plugin:{id:string;entry:string;enabled:boolean;source:string}){
 ...
 return normalizeCapabilityExecutionReceiptR141({capabilityId:plugin.id,domain:'PLUGIN',route:'Plugins',authorized:plugin.enabled===true,available:plugin.enabled===true&&!remote,source:`plugin-registry:${plugin.source}`,failureReason:plugin.enabled===true&&remote?'remote provider presence is not proven by a local registry manifest':null,lineage:['R45_PLUGIN_REGISTRY','R139_UNIFIED_CAPABILITY_ENGINE','R141_EXECUTION_RECEIPT']});
}
export function summarizeCapabilityReceiptsR141(rows:CapabilityExecutionReceiptR141[]){
 const states=Object.fromEntries(['DISCOVERED','AUTHORIZED','AVAILABLE','INVOKED','RETURNED','VERIFIED','UNAVAILABLE','FAILED','REJECTED','STALE'].map(s=>[s,rows.filter(r=>r.state===s).length]));
 return{schema:'OMEGA_CAPABILITY_EXECUTION_SUMMARY_R141',total:rows.length,states,verified:states.VERIFIED||0,failed:(states.FAILED||0)+(states.REJECTED||0)+(states.STALE||0),canonicalMutation:false,admissionAuthority:'R125'};
}
