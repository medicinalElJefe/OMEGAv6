...
export function archiveGenomeCompleteSummaryR288(){const rows=[...ARCHIVE_GENOME_COMPLETE_R288];return{rows:rows.length,scars:ARCHIVE_SCARS_R288.length,priority1:rows.filter(x=>x.priority===1).length,active:rows.filter(x=>x.currentCoverage==='ACTIVE').length,partial:rows.filter(x=>x.currentCoverage==='PARTIAL').length,absent:rows.filter(x=>x.currentCoverage==='ABSENT').length,gated:rows.filter(x=>x.currentCoverage==='GATED').length};}