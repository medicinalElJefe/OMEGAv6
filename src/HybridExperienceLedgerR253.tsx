...
import './hybridExperienceLedgerR253.css';

type Ledger={ok:boolean;schema:string;revision:string;generatedAt:number;cloudHeadSha?:string|null;currentAuthenticatedHeartbeat?:boolean;aggregate?:any;runs?:any[];devices?:any[];sanitation?:Record<string,boolean>;truthBoundary?:string};
...
export default function HybridExperienceLedgerR253(){
 ...
 const exportJson=()=>{if(!data)return;const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`omega-r253-hybrid-ledger-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url)};
 return <section className='r253-ledger' data-r253-heartbeat={data?.currentAuthenticatedHeartbeat?'CURRENT':'UNPROVED'}>
  <header><div><span>R253 · AUTHENTICATED HYBRID EXPERIENCE LEDGER</span><h3>Turn repeated PC runs into longitudinal execution evidence.</h3><p>Read-only, sanitized telemetry over already-returned Hybrid job/mission history. It tracks operation outcomes, timing, repeated failure signatures and mutation/build/test/package evidence without exposing bridge secrets, raw device IDs, paths or logs.</p></div><div className={data?.currentAuthenticatedHeartbeat?'r253-live pass':'r253-live hold'}><Activity/><span><b>{data?.currentAuthenticatedHeartbeat?'CURRENT AUTHENTICATED HEARTBEAT':'HEARTBEAT NOT PROVED IN THIS EXPORT'}</b><small>{data?.cloudHeadSha?`head ${data.cloudHeadSha.slice(0,12)}…`:'source SHA not exposed by release evidence'}</small></span></div></header>
  {error&&<div className='r253-error'><TriangleAlert/><span>{error}</span></div>}
  <div className='r253-summary'>
  ...
  <div className='r253-columns'>
   <div><h4>Operation calibration</h4><div className='r253-table'><div className='head'><span>OP</span><span>N</span><span>PASS</span><span>P50</span><span>P95</span></div>{operations.map((op:any)=><div key={op.op}><span>{op.op}</span><span>{op.count}</span><span>{Math.round((Number(op.successRate)||0)*100)}%</span><span>{ms(op.p50DurationMs)}</span><span>{ms(op.p95DurationMs)}</span></div>)}</div></div>
   <div><h4>Latest returned / active jobs</h4><div className='r253-runs'>{recent.map((run:any)=><article key={run.jobId||Math.random()} className={ACTIVE.has(String(run.status).toUpperCase())?'active':''}><span><b>{run.status}</b><small>{run.action||run.stage||'Hybrid job'}</small></span><span><em>{run.steps?.map((s:any)=>s.op).filter(Boolean).join(' → ')||'no step proof yet'}</em><small>{run.executionDurationMs!=null?ms(run.executionDurationMs):run.startedAt?`started ${age(run.startedAt)} ago`:'queued'} · {run.deviceKey||'device withheld'}</small></span></article>)}</div></div>
  </div>
  {(aggregate.repeatedFailureSignatures||[]).length>0&&<div className='r253-repeat'><TriangleAlert/><span><b>Repeated failure fingerprints detected</b><small>{aggregate.repeatedFailureSignatures.slice(0,6).map((x:any)=>`${x.signature} ×${x.count}`).join(' · ')}</small></span></div>}
  <div className='r253-actions'><button onClick={()=>void refresh()} disabled={busy}><RefreshCw className={busy?'spin':''}/>{busy?'Refreshing':'Refresh ledger'}</button><button onClick={exportJson} disabled={!data}><Download/>Export sanitized JSON</button></div>
  <footer><ShieldCheck/><span>{data?.truthBoundary||'R253 has observation/export authority only. It cannot dispatch, mutate, merge, deploy, replay execution or admit CanonState.'}</span></footer>
 </section>;
}
