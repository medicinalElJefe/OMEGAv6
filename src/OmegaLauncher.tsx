...
export default function OmegaLauncher({onNavigate}:Props){
 ...
 const keyActivate=(name:string,e:React.KeyboardEvent<HTMLDivElement>)=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();go(name)}};
 return <><button className='omega-nexus-trigger' onClick={()=>setOpen(true)} aria-label='Open OMEGA navigator'><Command/><span>OPEN</span></button>{open&&<div className='omega-nexus-backdrop' role='presentation' onMouseDown={e=>{if(e.target===e.currentTarget)setOpen(false)}}><section className='omega-nexus omega-nexus-r13' role='dialog' aria-modal='true' aria-label='OMEGA navigator'>
  ...
  <main className='omega-r13-app-list'>{rows.map(x=><div className='omega-r13-app' key={x.id} role='button' tabIndex={0} onClick={()=>go(x.name)} onKeyDown={e=>keyActivate(x.name,e)} aria-label={`Open ${x.name}`}><code>{x.id}</code><span><b>{x.name}</b><small>{x.hint}</small></span><em>{x.effect.replace('_',' ')}</em><button type='button' className={'omega-nexus-fav '+(favorites.includes(x.name)?'active':'')} onClick={e=>fav(x.name,e)} aria-label={(favorites.includes(x.name)?'Remove ':'Add ')+x.name+' favorite'}><Heart/></button><ChevronRight/></div>)}{rows.length===0&&<div className='omega-r13-empty'>No matching OMEGA application.</div>}</main>
