...
export default function OmegaLauncher({onNavigate}:Props){
 ...
 return <><button className='omega-nexus-trigger' onClick={()=>setOpen(true)} aria-label='Open OMEGA navigator'><Command/><span>Menu</span></button>{open&&<div className='omega-nexus-backdrop' role='presentation' onMouseDown={e=>{if(e.target===e.currentTarget)setOpen(false)}}><section className='omega-nexus omega-nexus-r13' role='dialog' aria-modal='true' aria-label='OMEGA navigator'>
  <aside className='omega-r13-domains' aria-label='OMEGA application domains'><div className='omega-r13-brand' aria-label='OMEGA'><strong>Ω</strong></div>{DOMAIN_SECTIONS.map(d=><button key={d.id} className={domain===d.id&&!q?'active':''} onClick={()=>{setDomain(d.id);setQ('')}} aria-label={d.label} title={`${d.label} — ${d.subtitle}`}>{icon(d.icon)}<span>{d.label}</span></button>)}<div className='omega-r13-domain-foot'><ShieldCheck/></div></aside>
  <div className='omega-r13-content'><header className='omega-r13-head'><div><span>{q?'Search':activeDomain.label}</span><small>{q?`${rows.length} matching applications`:activeDomain.subtitle}</small></div><button onClick={()=>setOpen(false)} aria-label='Close navigator'><X/></button></header>
  <div className='omega-nexus-search'><Search/><input autoFocus value={q} onChange={e=>setQ(e.target.value)} placeholder='Find an application or capability'/><kbd>⌘K</kbd></div>
