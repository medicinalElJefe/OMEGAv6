...
export default function OmegaLauncher({onNavigate}:Props){
 ...
 const fav=(name:string)=>{const n=favorites.includes(name)?favorites.filter(x=>x!==name):[...favorites,name];setFavorites(n);localStorage.setItem('omega.v6.favorites',JSON.stringify(n))};
 return <><button className='omega-nexus-trigger' onClick={()=>setOpen(true)} aria-label='Open OMEGA navigator'><Command/><span>Menu</span></button>{open&&<div className='omega-nexus-backdrop' role='presentation' onMouseDown={e=>{if(e.target===e.currentTarget)setOpen(false)}}><section className='omega-nexus omega-nexus-r13' role='dialog' aria-modal='true' aria-label='OMEGA navigator'>
  ...
  <main className='omega-r13-app-list' aria-label={q?'Search results':`${activeMenu.label} applications`}>{rows.map(x=><div className='omega-nexus-card' data-r13-app data-master-menu={omegaMasterMenuForRouteR289(x.name)?.id||''} data-authority={x.authority} data-effect={x.effect} key={x.id}><button type='button' className='omega-nexus-route' onClick={()=>go(x.name)} aria-label={`Open ${x.name}`}><span><b>{x.name}</b><small>{x.hint}</small></span><ChevronRight/></button><button type='button' className={'omega-nexus-fav '+(favorites.includes(x.name)?'active':'')} onClick={()=>fav(x.name)} aria-label={(favorites.includes(x.name)?'Remove ':'Add ')+x.name+' favorite'}><Heart/></button></div>)}{rows.length===0&&<div className='omega-r13-empty'>No matching application.</div>}</main>
  <footer><span>{q?`${rows.length} results`:`${activeMenu.routes.length} applications · menu ${activeMenu.id}`}</span><span>{LAUNCHER_SURFACES.length} routes · {LAUNCHER_MASTER_MENUS_R289.length} recovered menus · ALL MODES</span></footer></div>
 </section></div>}</>}
