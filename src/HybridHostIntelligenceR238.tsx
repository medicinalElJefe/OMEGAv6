...
export default function HybridHostIntelligenceR238(){
 ...
 return <section className='r238-host-intelligence' aria-label='R238 Hybrid host intelligence' data-r238-host-intelligence={p?'RETURNED_HOST_PROOF':'AWAITING_RETURNED_PROFILE'} data-r238-selected-device={selectedDeviceId||'NONE'} data-r238-snapshot-epoch={epoch}>
  <header><div><span>R238 · HYBRID HOST INTELLIGENCE · SHARED SNAPSHOT + RETURNED HASH-BOUND PROOF</span><h3>Use the machine you actually have.</h3><p>Resource truth now consumes the same selected authenticated host and the same Hybrid/Mission snapshot epoch as R212 observation and R237 command admission. CPU, memory, GPU, approved-root storage, Python and RCWA state appear only when that selected host returns them inside a DESKTOP_HEALTH proof packet covered by the existing R141 exact-payload fingerprint.</p></div><button onClick={()=>void refresh()}><RefreshCw/>Refresh shared snapshot</button></header>
  <div className='r238-truth-row'>
   <article className={currentHeartbeat?'pass':'hold'}><ShieldCheck/><div><small>AUTHENTICATED HOST</small><b>{currentHeartbeat?'CURRENT HEARTBEAT':stale?'SNAPSHOT STALE':'DEVICE PROOF REQUIRED'}</b><span>{device?`${text(device.name,device.id)} · ${age(device.lastSeen)} · epoch ${epoch}`:'No current non-revoked host heartbeat'}</span></div></article>
  ...
  <div className='r238-grid'>
   <article><Cpu/><div><small>CPU</small><b>{p?text(p.cpu?.model):'—'}</b><span>{p?`${p.cpu?.logicalProcessors||'—'} logical processors · advisory workers ${p.schedulerAdvisory?.recommendedCpuWorkers??'—'}`:'Awaiting selected-host proof'}</span></div></article>
   <article><MemoryStick/><div><small>MEMORY</small><b>{p?bytes(p.memory?.totalBytes):'—'}</b><span>{p?`${bytes(p.memory?.availableBytes)} available when sampled`:'Awaiting selected-host proof'}</span></div></article>
   <article><Cpu/><div><small>GPU</small><b>{p?text(p.gpu?.name,p.gpu?.present?'GPU detected':'No supported GPU query returned'):'—'}</b><span>{p?`${bytes(p.gpu?.vramBytes)} VRAM · driver ${text(p.gpu?.driverVersion)}`:'Awaiting selected-host proof'}</span></div></article>
   <article><HardDrive/><div><small>APPROVED ROOT STORAGE</small><b>{p?bytes(p.storage?.totalBytes):'—'}</b><span>{p?`${bytes(p.storage?.freeBytes)} free · root ${text(p.storage?.rootLabel)}`:'Awaiting selected-host proof'}</span></div></article>
   <article><Database/><div><small>PYTHON</small><b>{p?text(p.python?.version):'—'}</b><span>{p?`${text(p.python?.executable)} · ${text(p.python?.architecture)}`:'Awaiting selected-host proof'}</span></div></article>
   <article><ShieldCheck/><div><small>LOCAL MACRO STORE</small><b>{m?`${m.verifiedCount||0} VERIFIED`:'—'}</b><span>{m?`${m.totalCount||0} stored · ${m.invalidCount||0} held · contents not uploaded`:'Inventory is emitted with selected-host proof only'}</span></div></article>
  </div>
  {proof&&<footer><ShieldCheck/><span>Proof source: selected authenticated Hybrid job <code>{proof.job.id}</code> · DESKTOP_HEALTH step · snapshot epoch {epoch} observed {age(observedAt)} · R141 exact return closure. Hardware presence does not prove CUDA runtime, RCWA numerical validity, scientific truth, source mutation, or CanonState admission. R238 only improves resource truth and safe scheduling decisions.</span></footer>}
