import type {SystemFamily,SystemFamilyStatus} from './systemAtlasRuntime';

export type FamilyOperationalProofStateR137='CURRENT_EXECUTION_PROOF'|'CURRENT_SERVICE_OBSERVATION'|'DECLARED_BOUNDARY_ONLY'|'GATE_CURRENTLY_HELD'|'STATIC_LINEAGE_ONLY'|'UNKNOWN';
...
export const FAMILY_OPERATIONAL_PROOF_LAWS_R137=Object.freeze({
 ...
 successor:'R48_R153_CURRENT_SUCCESSOR_STATUS_MAY_CLASSIFY_THE_PRESENT_BOUNDARY_WITHOUT_REWRITING_V24_LINEAGE',
 ...
 livingWorld:'R136_LIVING_WORLD_FRAME_REMAINS_CURRENT_EVIDENCE_MISSION_PROJECTION_CONTEXT',
 nextAction:'CURRENT_PROOF_MAY_ADVANCE_NEXT_ACTION_WITHOUT_REWRITING_DECLARED_STATUS_OR_SUCCESSOR_AUTHORITY',
...
export function familyOperationalProofR137(family:SystemFamily,operational:any,hybrid:any,currentStatus?:SystemFamilyStatus):FamilyOperationalProofR137{
 const at=observedAt(operational),pc=pcProved(operational,hybrid),devices=onlineDevices(hybrid),status=currentStatus||family.status,statusSource=currentStatus?'R48/R153 current successor ledger':'authoritative family registry';
 if(family.id==='S03')return pc?
  {familyId:family.id,state:'CURRENT_EXECUTION_PROOF',current:true,label:'PC EXECUTION GATE CURRENTLY SATISFIED',detail:`Authenticated heartbeat is current${devices.length?` on ${devices.length} non-revoked device${devices.length===1?'':'s'}`:''}. The DEVICE_GATED successor boundary remains; this observation satisfies it only for the current proof window.`,source:'R130 operational matrix + Hybrid authenticated heartbeat',observedAt:at}:
 ...
 if(status==='EVIDENCE_GATED'||status==='DEVICE_GATED')return{familyId:family.id,state:'GATE_CURRENTLY_HELD',current:false,label:status==='EVIDENCE_GATED'?'DOMAIN EVIDENCE GATE NOT SATISFIED HERE':'DEVICE GATE NOT SATISFIED HERE',detail:'Generic runtime health cannot satisfy this family’s current proof gate. Use its specialist validation/host evidence path.',source:statusSource,observedAt:at};
 if(status==='RESTORATION_DEBT'||status==='DONOR_ONLY'||status==='NATIVE_TARGET')return{familyId:family.id,state:'STATIC_LINEAGE_ONLY',current:false,label:'NO CURRENT EXECUTION CLAIM',detail:'Known lineage/target value is preserved, but this operational overlay does not claim a live implementation.',source:statusSource,observedAt:at};
 return{familyId:family.id,state:'DECLARED_BOUNDARY_ONLY',current:false,label:'CURRENT SUCCESSOR BOUNDARY',detail:'This family has a current web/source/local successor within its declared boundary. No stronger per-family live or native proof is inferred from generic health.',source:statusSource,observedAt:at};
}

export function currentNextActionR137(family:SystemFamily,proof:FamilyOperationalProofR137,declaredAction:string){
 if(family.id==='S03'&&proof.state==='CURRENT_EXECUTION_PROOF')return'Run a bounded authenticated PC workload, return its execution receipt into the R134 append-only world/scar chain and R136 living-world evidence frame, replay it, and verify the receipt before expanding machine authority.';
