...
import RelativityCalibrationR334 from './RelativityCalibrationR334';
import RelativityCalibrationR340 from './RelativityCalibrationR340';
...
export default function RelativityLab({record,state,onNavigate}:Props){
 ...
 return <section className='panel relativity-lab rel36' data-omega-view-guard='R158'>
  ...
  {tab==='DIMENSIONAL'&&<div className='rel36-reference'><DimensionalRelativityEvolutionR156 record={record}/><DimensionalRelativityPanelR24 record={record}/><RelativityCalibrationR334/><RelativityCalibrationR340/><p>Workbook-defined 0D–12D skins are representation stages; R156 extends their observer/address-resolution relationships but does not create new physical dimensions. β changes only the apparent field projection—longitudinal compression, angular aberration, apparent depth and phase rate. CanonState, route admission and donor provenance remain unchanged.</p><details className='r93-legacy-renderer'><summary>Optional observer-field rendering</summary><CalculusFieldR37 address={record.address} mode='RELATIVITY' steps={30} observerBeta={observerBeta} label={`DERIVED OBSERVER RENDER · β ${observerBeta.toFixed(2)} · γ ${gamma.toFixed(2)}`}/></details></div>}
