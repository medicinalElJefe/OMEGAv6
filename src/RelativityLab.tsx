...
type Tab='FIELD'|'DIMENSIONAL'|'MOTION'|'CONTROLLER'|'VIOLET'|'EQUATIONS';
...
export default function RelativityLab({record,state,onNavigate}:Props){
 ...
 return <section className='panel relativity-lab rel36' data-omega-view-guard='R158'>
  <div className='section-head'><div><p className='overline'>RELATIVITY · OBSERVER / MOTION / CALCULUS PROJECTION</p><h2>See the operating fabric, then inspect what changes with frame</h2><p className='muted'>R158 exposes the current 20,736-state atlas, 241 provenance-separated mode/lens channels, R155 capability-family graph, relative capacity, causal NOW, Woven carry and dimensional-relativity shells as one source-bound instrument. The exact Drive-backed R24 workbook remains the detailed dimensional authority below it.</p></div><div className='pill-row'><span className='pill'>STATE {record.stateId}</span><span className='pill'>FRAME {record.identity.frame}</span><span className='pill ok'>{record.metrics.decision}</span></div></div>
  <CapabilityUniverseR158 record={record} onNavigate={onNavigate}/>
  ...
  <div className='rel-tabs' data-omega-control-plane='reserved'>{(['DIMENSIONAL','MOTION','FIELD','CONTROLLER','VIOLET','EQUATIONS'] as Tab[]).map(x=><button key={x} className={tab===x?'active':''} onClick={()=>setTab(x)}>{x}</button>)}</div>
  {tab==='DIMENSIONAL'&&<div className='rel36-reference'><DimensionalRelativityEvolutionR156 record={record}/><DimensionalRelativityPanelR24 record={record}/><p>Workbook-defined 0D–12D skins are representation stages; R156 extends their observer/address-resolution relationships without manufacturing physical dimensions. β changes only the apparent field projection—longitudinal compression, angular aberration, apparent depth and phase rate. CanonState, route admission and donor provenance remain unchanged.</p><details className='r93-legacy-renderer'><summary>Optional observer-field rendering</summary><CalculusFieldR37 address={record.address} mode='RELATIVITY' steps={30} observerBeta={observerBeta} label={`DERIVED OBSERVER RENDER · β ${observerBeta.toFixed(2)} · γ ${gamma.toFixed(2)}`}/></details></div>}
