...
export default function EarthObservatoryR8({address}:Props){
 ...
 return <section className='earth-r8 earth-r72 earth-r279'>
  ...
  <div className={`earth-r72-workspace ${view==='SAR'?'sar-active':''}`}>
   <div className='earth-r72-stage earth-r279-stage'>{view==='SATELLITE'?renderSatellite():view==='PLANET'?<EarthObservedGlobeR281 address={address} evidence={evidence} targetLat={lat} targetLon={lon}/>:view==='SAR'?<section className='earth-r283-sar earth-r285-sar' data-earth-view='SAR'><SARLiveTruthR285 lat={lat} lon={lon} onTargetChange={setSarTarget}/></section>:view==='GROUND'?<section className='earth-r279-ground' data-earth-view='GROUND'><EarthGroundTraversalR9 lat={lat} lon={lon}/></section>:view==='CALCULUS'?<section className='earth-r279-calculus' data-earth-view='CALCULUS' data-provenance='REPRESENTATIONAL'><EarthLivingFieldR36 address={address} lat={lat} lon={lon} evidence={evidence}/><p>REPRESENTATIONAL ONLY · returned measurements determine observed quantities; OMEGA calculus changes relational rendering and inference, not source pixels.</p></section>:<EarthNowInstrument address={address} evidence={evidence} targetLat={lat} targetLon={lon} mode={instrumentMode}/>}</div>
