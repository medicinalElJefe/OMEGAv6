...
export default function EarthObservatoryR8({address}:Props){
 ...
 return <section className='earth-r8 earth-r72'>
  <header className='earth-r72-bar'><div><span>EARTH NOW · SOURCE-BACKED DIRECT WORKSPACE</span><h2>Operate the evidence in geographic context.</h2><small>WGS84 geography and returned public observations remain distinct from OMEGA model/interface shells.</small></div><button className='primary' onClick={load} disabled={busy}><RefreshCw className={busy?'spin':''}/>{busy?'Refreshing…':'Refresh returned evidence'}</button></header>
  <div className='earth-r72-workspace'><div className='earth-r72-stage'><EarthNowInstrument address={address} evidence={evidence} targetLat={lat} targetLon={lon}/></div><aside className='earth-r72-console'><div className='earth-r72-location'><Globe2/><div><b>WGS84 target</b><small>Directly changes the returned-source query target.</small></div></div><div className='earth-r72-coords'><label>Latitude<input type='number' min='-90' max='90' step='.01' value={lat} onChange={e=>setLat(Math.max(-90,Math.min(90,Number(e.target.value))))}/></label><label>Longitude<input type='number' min='-180' max='180' step='.01' value={lon} onChange={e=>setLon(Math.max(-180,Math.min(180,Number(e.target.value))))}/></label></div><button className='earth-r72-apply' onClick={load} disabled={busy}>Query this location</button><button className='earth-r72-reset' onClick={()=>{setLat(initial.lat);setLon(initial.lon)}}>Return to model-mapped target</button><div className='earth-r72-focus-head'><b>Evidence layers</b><button className={focus==='ALL'?'active':''} onClick={()=>setFocus('ALL')}>Show all</button></div><div className='earth-r72-focus'>{focusRows.map(row=><button key={row.id} className={focus===row.id?'active':''} onClick={()=>setFocus(v=>v===row.id?'ALL':row.id)}>{row.icon}<span><b>{row.label}</b><strong>{row.value}</strong><small>{row.detail}</small></span></button>)}</div><div className='earth-r72-truth'><ShieldCheck/><span><b>{evidence?.evidenceHash?'RETURNED EVIDENCE BOUND':'EVIDENCE NOT YET BOUND'}</b><small>{evidence?.verifiedAt||'No verification timestamp returned.'}</small></span></div></aside></div>
  ...
  <section className='earth-r72-noaa'><header><div><span>NOAA STAR · GEOCOLOR</span><b>{current?.label||'Loading coverage…'}</b><small>{current?.state||'UNVERIFIED'} · {current?.lastModified||'no Last-Modified returned'}</small></div><div><button onClick={()=>move(-1)} disabled={!catalog.length}><ChevronLeft/></button><button onClick={()=>setPlaying(v=>!v)} disabled={!catalog.length}>{playing?<Pause/>:<Play/>}</button><button onClick={()=>move(1)} disabled={!catalog.length}><ChevronRight/></button></div></header><div className='earth-r72-noaa-stage'>{current&&<img src={`/api/earth/noaa/image?coverage=${encodeURIComponent(current.id)}`} alt={`${current.label} NOAA GeoColor latest alias`}/>}<nav>{catalog.map(x=><button key={x.id} onClick={()=>setSelected(x.id)} className={x.id===current?.id?'active':''}><span>{x.label}</span><small>{x.state}</small></button>)}</nav></div><p><CloudSun/> GeoColor is a NOAA/CIRA derived sensor composite. Missing or unavailable source material is not synthetically replaced.</p></section>
  <EarthSarPanelR181 lat={lat} lon={lon}/>
  <div className='earth-r72-disclosure'><button className={showGround?'active':''} onClick={()=>setShowGround(v=>!v)}>Ground / street evidence</button><button className={showCalculus?'active':''} onClick={()=>setShowCalculus(v=>!v)}>Representational calculus comparison</button></div>
