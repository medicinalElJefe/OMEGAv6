...
export async function sarCatalogR285(url){
 const lat=clamp(Number(url.searchParams.get('lat')||0),-90,90),lon=clamp(Number(url.searchParams.get('lon')||0),-180,180),days=Math.round(clamp(Number(url.searchParams.get('days')||60),1,180)),limit=Math.round(clamp(Number(url.searchParams.get('limit')||12),1,30)),product=String(url.searchParams.get('product')||'GRD').toUpperCase()==='SLC'?'SLC':'GRD',span=clamp(Number(url.searchParams.get('span')||0.45),0.05,2);
 const collection=COLLECTIONS[product],now=new Date(),start=new Date(now.getTime()-days*86400000),body={collections:[collection],bbox:[Number((lon-span).toFixed(5)),Number((lat-span).toFixed(5)),Number((lon+span).toFixed(5)),Number((lat+span).toFixed(5))],datetime:`${start.toISOString()}/${now.toISOString()}`,limit};
 const started=Date.now();
 try{
  const response=await fetch(CDSE_STAC_SEARCH_R285,{method:'POST',headers:{'accept':'application/geo+json,application/json','content-type':'application/json','user-agent':'OMEGAv6-R285/1.0'},body:JSON.stringify(body),cf:{cacheTtl:0,cacheEverything:false}}),raw=await response.text();
  ...
  const payload={ok:true,schema:SAR_LIVE_CATALOG_SCHEMA_R285,state:products.length?'CATALOG_RETURNED':'NO_MATCHING_ACQUISITIONS',verifiedAt:new Date().toISOString(),latencyMs:Date.now()-started,target:{lat,lon,crs:'WGS84 / EPSG:4326'},query:{product,collection,days,limit,bbox:body.bbox,datetime:body.datetime},source:{provider:'Copernicus Data Space Ecosystem STAC',endpoint:CDSE_STAC_SEARCH_R285},products,latest:products[0]||null,truthBoundary:'Returned STAC records are real acquisition catalogue metadata. They do not prove product bytes, calibrated backscatter, complex I/Q, phase, coherence, deformation, elevation or any other measurement array. Native/derived fields remain unavailable until their exact assets and processing lineage are bound and verified.'};
  payload.evidenceHash=await sha256(payload);return payload;
 }catch(e){
  const payload={ok:false,schema:SAR_LIVE_CATALOG_SCHEMA_R285,state:'UPSTREAM_UNAVAILABLE',verifiedAt:new Date().toISOString(),latencyMs:Date.now()-started,target:{lat,lon,crs:'WGS84 / EPSG:4326'},query:{product,collection,days,limit,bbox:body.bbox,datetime:body.datetime},source:{provider:'Copernicus Data Space Ecosystem STAC',endpoint:CDSE_STAC_SEARCH_R285},products:[],latest:null,error:e instanceof Error?e.message:String(e),truthBoundary:'Upstream catalogue failure is shown as unavailable. OMEGA does not substitute fabricated acquisitions or pixels.'};
  payload.evidenceHash=await sha256(payload);return payload;
 }
}
