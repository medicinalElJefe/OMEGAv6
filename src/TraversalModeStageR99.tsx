...
export default function TraversalModeStageR99({variant,address,onAddress}:Props){
 const canvas=useRef<HTMLCanvasElement|null>(null),routeHit=useRef<{x:number;y:number;address:number}[]>([]),clock=useRef({last:0,elapsed:0});
 const[mode,setMode]=useState<TraversalDesignModeR99>('UNIFIED'),[animated,setAnimated]=useState(true),[detail,setDetail]=useState(1),[routeDepth,setRouteDepth]=useState(36),[timeScale,setTimeScale]=useState(1),[weaveReadout,setWeaveReadout]=useState(()=>({phaseBand:0,pulse:.5}));
 ...
 const routeWeaves=useMemo(()=>route.path.map((step:any)=>{const ru=unifiedFromRecord(corpusState(step.address));return deriveWeaveStateR100(step.address,ru,0,1)}),[route]);
 useEffect(()=>{clock.current={last:0,elapsed:0}},[address]);
 useEffect(()=>{const el=canvas.current;if(!el)return;let raf=0,alive=true,lastReadout=-1;const ctx=el.getContext('2d');if(!ctx)return;const render=(ms:number)=>{if(!alive)return;const rect=el.getBoundingClientRect(),dpr=Math.min(2,window.devicePixelRatio||1),w=Math.max(420,rect.width),h=Math.max(420,rect.height);if(el.width!==Math.floor(w*dpr)||el.height!==Math.floor(h*dpr)){el.width=Math.floor(w*dpr);el.height=Math.floor(h*dpr)}ctx.setTransform(dpr,0,0,dpr,0,0);
  const dt=clock.current.last?Math.min(.05,Math.max(0,(ms-clock.current.last)/1000)):0;clock.current.last=ms;if(animated)clock.current.elapsed+=dt*timeScale;const t=clock.current.elapsed,weave=deriveWeaveStateR100(address,u,t,1);if(weave.phaseBand!==lastReadout&&Math.floor(ms/120)!==Math.floor((ms-16)/120)){lastReadout=weave.phaseBand;setWeaveReadout({phaseBand:weave.phaseBand,pulse:weave.pulse})}
  ctx.fillStyle='#020709';ctx.fillRect(0,0,w,h);const cx=w*.5,cy=h*.5,scale=Math.min(w,h)*.315*profile.radial;
  ...
  // R100 woven continuity geometry: nested atlas resolutions are represented as recursive
  // address shells. They are not claimed as literal physical dimensions.
  ctx.save();ctx.translate(cx,cy);ctx.rotate(weave.torsion*.08);
  for(let i=0;i<weave.ringCount;i++){const f=(i+1)/weave.ringCount,r=scale*(.12+.96*f),ell=1-.12*u.shape.anisotropy+Math.sin(weave.phase+i*.33)*.018*weave.pulse;ctx.beginPath();ctx.ellipse(0,0,r,r*ell,weave.torsion*f*.12,0,Math.PI*2);const level=i%4,ch=weaveChannelR100(level,weave,u),major=i===0||i===weave.ringCount-1||i%Math.max(1,Math.round(weave.ringCount/4))===0;ctx.strokeStyle=`rgba(${ch.r},${ch.g},${ch.b},${major?.12+.12*ch.strength:.018+.055*ch.strength})`;ctx.lineWidth=major?1.05:.42;ctx.stroke()}
  for(let l=0;l<weave.lobeCount;l++){const offset=l/weave.lobeCount*Math.PI*2;ctx.beginPath();for(let j=0;j<=180;j++){const a=j/180*Math.PI*2,r=scale*(.22+.61*(.5+.5*Math.sin(a*weave.lobeCount+offset+weave.phase))*(.38+.62*weave.aperture)),tw=a+offset+weave.torsion*Math.sin(a+weave.phase)*.18,x=Math.cos(tw)*r,y=Math.sin(tw)*r*(.78+.16*weave.depth);j?ctx.lineTo(x,y):ctx.moveTo(x,y)}const ch=weaveChannelR100(l,weave,u);ctx.strokeStyle=`rgba(${ch.r},${ch.g},${ch.b},${.018+.075*ch.strength})`;ctx.lineWidth=.45+.5*weave.invariantCarry;ctx.stroke()}
  // Four sparse resolution boundaries encode 12 → 144 → 1,728 → 20,736 as hierarchy.
  ...
  // Source-driven particles. R99's declared mode warp remains intact; R100 then applies
  // continuity exchange, invariant carry, scar/residual carry and oriented re-contextualization.
  const count=Math.max(700,Math.round((1500+1200*detail)*profile.density));ctx.save();ctx.globalCompositeOperation='lighter';for(let i=0;i<count;i++){const base=visualFieldPoint(i,count,u,t),modePoint=warpTraversalPointR99(mode,base,i,count,u,t,profile),p=applyWovenContinuityR100(modePoint,i,count,u,weave,t),rot=t*(.015+.035*u.motionRelativity)*(weave.orientation||1),cr=Math.cos(rot),sr=Math.sin(rot),x=(p.x*cr-p.z*sr)*scale+cx,y=(p.y*.76+p.z*.1)*scale+cy,z=p.x*sr+p.z*cr,depth=clamp((z+1.4)/2.8),proof=mode==='PROOF'?(.08+.92*u.evidence)*(1-.55*u.q):1,alpha=(.021+.22*p.weight)*profile.alpha*proof*(.46+.54*depth),rad=(.42+2.35*profile.particleScale*(.3+.7*depth)),channel=weaveChannelR100(i,weave,u);ctx.beginPath();ctx.arc(x,y,rad,0,Math.PI*2);ctx.fillStyle=mode==='SCAR'?`rgba(207,74,94,${alpha})`:mode==='LIGHT'||mode==='PROOF'?`rgba(231,211,150,${alpha})`:mode==='WATER'?`rgba(61,204,191,${alpha})`:mode==='RELATIVITY'?`rgba(95,167,205,${alpha})`:mode==='FORECAST'?`rgba(154,190,214,${alpha})`:`rgba(${channel.r},${channel.g},${channel.b},${alpha*(.72+.28*channel.strength)})`;ctx.fill()}ctx.restore();

  // Actual canonical admitted route, using canonical projection points. Route geometry carries
  // each state's own weave strength but never changes address authority.
  const hits:{x:number;y:number;address:number}[]=[];ctx.save();ctx.lineJoin='round';route.path.forEach((step:any,i:number)=>{const p=projectionPoint(step.address,'MANDALA',1000),x=(p.x/1000-.5)*Math.min(w,h)*.92+cx,y=(p.y/1000-.5)*Math.min(w,h)*.92+cy,rw=routeWeaves[i]||weave;hits.push({x,y,address:step.address});if(i){const prev=hits[i-1],strength=clamp((rw.continuityFlux+rw.invariantCarry)/2);ctx.beginPath();ctx.moveTo(prev.x,prev.y);ctx.lineTo(x,y);ctx.strokeStyle=mode==='FORECAST'?`rgba(218,182,106,${.32+.35*strength})`:`rgba(99,210,194,${.2+.32*strength})`;ctx.lineWidth=(mode==='FORECAST'?1.7:1)+(1.2*strength);ctx.stroke()}if(i%Math.max(1,Math.floor(route.path.length/10))===0||i===0){ctx.beginPath();const rr=i===0?5.5:2.2+2*rw.invariantCarry;ctx.arc(x,y,rr,0,Math.PI*2);ctx.fillStyle=i===0?'#f0cf84':rw.orientation<0?'#7d9fd1':'#65d0bf';ctx.fill()}});ctx.restore();routeHit.current=hits;
  ...
  raf=requestAnimationFrame(render)};raf=requestAnimationFrame(render);return()=>{alive=false;cancelAnimationFrame(raf)}},[address,animated,detail,mode,profile,route,routeWeaves,timeScale,u]);
