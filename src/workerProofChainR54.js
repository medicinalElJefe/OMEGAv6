...
export async function verifyProofChainR54(input){
 ...
 const errors=[],duplicates=[],seen=new Set();let previous=null;
 for(let i=0;i<segment.length;i++){
  ...
  if(r.parentCheckpointHash!=null&&!hashOk(r.parentCheckpointHash))errors.push(`${label}.parentCheckpointHash must be null or 64-hex SHA-256`);
  ...
  if(previous){
   if(String(r.parentCheckpointHash||'')!==String(previous.checkpointHash||''))errors.push(`${label}.parentCheckpointHash does not match prior checkpoint`);
   if(r.mode==='DELTA'&&String(r.baseHash||'')!==String(previous.currentHash||''))errors.push(`${label}.baseHash does not match prior currentHash`);
  }else if(body.expectedParentCheckpointHash!=null&&String(r.parentCheckpointHash||'')!==String(body.expectedParentCheckpointHash||''))errors.push(`${label}.parentCheckpointHash does not match expected parent`);
  previous=r;
 }
 if(duplicates.length)errors.push(`duplicate checkpointHash at indexes ${duplicates.join(',')}`);
 const normalized=segment.map(r=>({mode:r.mode,parentCheckpointHash:r.parentCheckpointHash??null,baseHash:r.baseHash??null,currentHash:r.currentHash,deltaHash:r.deltaHash??null,snapshotHash:r.snapshotHash??null,checkpointHash:r.checkpointHash,changedComponents:Array.isArray(r.changedComponents)?r.changedComponents:[],patchCount:Number(r.patchCount||0),resynced:r.resynced===true}));
 ...
 return{ok:errors.length===0,status:errors.length?422:200,schema:'OMEGA_SUPERVISED_PROOF_CHAIN_R54',health,segmentHash,receiptCount:segment.length,headCheckpointHash:head?.checkpointHash||null,errors,duplicates,proof:{algorithm:'SHA-256',authority:'NON_AUTHORITATIVE_CHAIN_VERIFIER',receivedNotObserved:true,supervision:'B012-compatible submitted event-chain verification'},boundary:'R54 verifies structural continuity of a submitted local proof-chain segment. It does not persist the journal, own canonical state, repair history silently, claim a native watchdog, or prove external/device facts.'};
}
