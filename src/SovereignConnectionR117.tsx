...
export const R151_EXECUTION_SPINE='CURRENT HEARTBEAT -> INDEX -> HASH_TREE -> PROOF-CONDITIONED REPAIR -> BUILD -> TEST -> PACKAGE -> R141 CLOSURE';
...
export default function SovereignConnectionR117({compact=false,onState}:Props){
 ...
 const refreshBuildMission=async()=>{try{const r=await api.get<any>('/api/missions'),missions=Array.isArray(r.data?.missions)?r.data.missions:[];const current=buildMission?.id?missions.find((m:any)=>m.id===buildMission.id):missions.find((m:any)=>['ACTIVE','PAUSED'].includes(String(m?.status||'')))||missions[0];if(current){setBuildMission(current);const embedded=current.currentJob;if(embedded)setBuildJob(embedded);else if(current.currentJobId){const found=(Array.isArray(live?.jobs)?live.jobs:[]).find((j:any)=>j.id===current.currentJobId);if(found)setBuildJob(found)}}}catch{}};
 ...
 const runFullBuild=async()=>{
  ...
  try{
   const initial=defaultCommandPlan('BUILD','AUTO_BUILD',buildRoot),validation=validateCommandPlan(initial,buildRoot,[]);
   ...
   const stateContext=JSON.stringify({surface:'SovereignConnectionR151',executionSpine:R151_EXECUTION_SPINE,canonicalHost:'https://omegav6.jeffdeweyeljefe.workers.dev',rootPolicy:'approved non-system root only; no C: OMEGA runtime writes',proofAuthority:'R141 exact returned payload; R142 execution-truth classification; R125 CanonState admission remains unchanged',mutationPolicy:'returned evidence first; preimage-bound repair; no delete/move/rename; preserve existing capability layers',goal:'inventory -> hash -> repair as proven -> build -> test -> package -> proof closure'});
   const r=await api.post<any>('/api/missions',{objective,threadId:'',stateContext,draft,targetDeviceId:activeDevice.id,allowedOps:[...FULL_BUILD_ALLOWED_OPS],maxCycles:18,confirmedMission:true});
 ...
 return <section className={'r112-sovereign '+(compact?'compact ':'')+(online?'live':bridge?'prepared':'idle')} aria-label='Connect this PC to OMEGA'>
  ...
  {online&&!compact&&<section className={'r151-build-spine '+(missionActive?'active':terminal(jobStatus)?'returned':'ready')} aria-label='R151 sovereign full build execution spine'>
   <header><div className='r151-build-title'><Cpu/><span><small>R151 · AUTHENTICATED PC → GOVERNED BUILD → R141 PROOF CLOSURE</small><b>Sovereign full-build execution spine</b><em>Inventory → hash → proof-conditioned repair → build → test → package → returned proof</em></span></div><div className='r151-build-actions'>{missionStatus==='PAUSED'?<button onClick={()=>void controlMission('resume')} disabled={buildBusy}><Play/>Resume mission</button>:missionActive?<button onClick={()=>void controlMission('pause')} disabled={buildBusy}><PauseCircle/>Pause mission</button>:<button className='primary' onClick={()=>void runFullBuild()} disabled={buildBusy}>{buildBusy?<Loader2 className='spin'/>:<Play/>}{buildBusy?'Admitting mission…':'RUN FULL SOVEREIGN BUILD'}</button>}</div></header>
   <div className='r151-build-facts'><span><small>HOST</small><b>{activeDevice?.name||activeDevice?.id||'paired PC'}</b></span><span><small>PROJECT ROOT</small><b>{buildRoot==='.'?'DISCOVER INSIDE APPROVED ROOT':buildRoot}</b></span><span><small>MISSION</small><b>{buildMission?.id?short(buildMission.id,22):'NOT STARTED'}</b></span><span><small>STATE</small><b>{missionStatus}{jobStatus?` · ${jobStatus}`:''}</b></span></div>
   {buildJob&&<div className='r151-build-return'><ShieldCheck/><span><b>{terminal(jobStatus)?'HOST RETURN AVAILABLE':'HOST WORKLOAD IN FLIGHT'}</b><small>job {buildJob.id||buildMission?.currentJobId||'pending'} · proof steps {buildJob.returnPacket?.stepProofs?.length||0} · result {short(buildJob.returnPacket?.resultFingerprint,18)}</small></span></div>}
   {buildError&&<div className='r151-build-error'><TriangleAlert/>{buildError}</div>}
   <footer><ShieldCheck/><span><b>No mystery execution.</b> This button creates a durable confirmed mission only while the authenticated heartbeat is current. Broad-root runs are discovery/hash first; later mutation is proof-conditioned and preimage-bound. R141/R142 below remains the authority for returned/verified execution truth.</span></footer>
  </section>}
