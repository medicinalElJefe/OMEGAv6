...
export async function assembleFederationAttestationWorldLensR169({evidence=null,context={}}={}){
 ...
 const frame=await assembleLivingWorldFrameR136({
  ...
  mission,
  federation:verified?{
   state:'ATTESTED_NOT_PROMOTED',
   receiptId:scarId,
   source:'R168_GENESIS_R192_ATTESTATION',
   genesisRole:'PROPOSE',
 ...
 return{
  ...
  frame,
  visualOverlay:{state,verified,action:verified?'REVIEW_RETURNED_FEDERATION_EVIDENCE':'PROOF_REQUIRED',lod:frame.visualState.lod,sampleBudget:frame.visualState.sampleBudget,scarId},
