...
import {R334_CALIBRATION_RECEIPT} from './system/calibratedCernRelativityR334';
...
export const R314_BUILD_STAGES:readonly R314BuildStage[]=[
 ...
 {id:'R314-B04',order:4,title:'675-row Implementation Canon reconciliation + R328 source-exact + R334 calibrated-evidence crosswalk',objective:'Keep the 675-row Implementation Canon, 3,743-row R328 semantic/provenance canon and R334 calibrated CERN/Dewey evidence layer separate, then crosswalk them to current artifacts and proof without promoting reconstructed or approximate quantities into raw observation.',dependsOn:['R314-B02'],sourceFamilies:['AG-021','R328 SOURCE-EXACT CANON','R334 CERN/DEWEY CALIBRATION'],deliverables:['675-row implementation compiler','3,743-row semantic registry','59-row calibrated bridge/closure registry','module/symbol/shader/API/DB/test/build-gate crosswalk','record-level semantic crosswalk','conflict-variant ledger','single-writer reconciliation','machine-readable completion ledger'],proof:['675 implementation rows classified','3,743 semantic rows preserved and classified','R334 23+36 calibrated rows hash/census verified','ACTIVE requires exact source evidence','reported/reconstructed/approximate evidence classes remain distinct','conflict variants remain intact','no PLANNED/source row promoted by naming alone'],automationClass:'DATA_COMPILER',mutationClass:'DATA_ONLY'},
 ...
 {id:'R314-B06',order:6,title:'Motion, relativity and continuity engine',objective:'Unify observer/frame transforms, motion derivatives, uncertainty, scar/history carry and resolution-relative addressing without converting atlas resolutions into physical dimensions.',dependsOn:['R314-B05'],sourceFamilies:['AG-003','AG-004','AG-005','R265','R266','R334 CERN/DEWEY CALIBRATION'],deliverables:['frame transform operators','motion hypothesis','parent/fold relativity adapter','Water transport registry','Woven carry engine','Violet re-expression adapter','calibration receipts','distribution-first evidence bridge','physicality gate','cross-skin covariance/contradiction carry'],proof:['transform round trips','orientation reversal invariants','time-step validity','golden-row replay','R334 calibrated-row replay','reported vs reconstructed evidence separation','native ATLAS replication gate remains open','no literal-dimension claims'],automationClass:'RUNTIME',mutationClass:'BOUNDED_SOURCE'},
...
export function buildConvergenceMasterR314(){
 ...
 return {
  ...
  calibratedCernRelativity:{...R334_CALIBRATION_RECEIPT},
