...
export type OrthoWgs84={inverse:(nx:number,ny:number)=>InversePoint|null;forward:(lat:number,lon:number)=>ProjectedPoint};
...
export function createWgs84Orthographic(centerLat:number,centerLon:number):OrthoWgs84{
 const {center,east,north,up}=cameraFrame(centerLat,centerLon),ib2=1/(WGS84_B*WGS84_B),A=up[0]*up[0]+up[1]*up[1]+up[2]*up[2]*ib2;
 const inverse=(nx:number,ny:number):InversePoint|null=>{
  const q=add(center,add(scale(east,nx),scale(north,ny))),B=2*(q[0]*up[0]+q[1]*up[1]+q[2]*up[2]*ib2),C=q[0]*q[0]+q[1]*q[1]+q[2]*q[2]*ib2-1,disc=B*B-4*A*C;if(disc<0)return null;
  const t=(-B+Math.sqrt(Math.max(0,disc)))/(2*A),p=add(q,scale(up,t)),lon=wrapLon(deg(Math.atan2(p[1],p[0]))),xy=Math.hypot(p[0],p[1]),lat=deg(Math.atan2(p[2],Math.max(1e-12,xy)*(1-WGS84_E2))),normal=geodeticNormal(lat,lon),mu=dot3(normal,up);if(mu<-1e-8)return null;return{lat,lon,mu:clamp(mu,0,1),normal};
 };
 const forward=(lat:number,lon:number):ProjectedPoint=>{const p=geodeticToEcef(lat,lon),d:Vec3=[p[0]-center[0],p[1]-center[1],p[2]-center[2]],normal=geodeticNormal(lat,lon),mu=dot3(normal,up);return{x:dot3(d,east),y:dot3(d,north),z:mu,mu}};
 return{inverse,forward};
}

export function inverseOrthoWgs84(nx:number,ny:number,centerLat:number,centerLon:number):InversePoint|null{return createWgs84Orthographic(centerLat,centerLon).inverse(nx,ny)}
export function forwardOrthoWgs84(lat:number,lon:number,centerLat:number,centerLon:number):ProjectedPoint{return createWgs84Orthographic(centerLat,centerLon).forward(lat,lon)}
...
export function solarIllumination(lat:number,lon:number,date:Date){const sun=solarPoint(date);return dot3(geodeticNormal(lat,lon),geodeticNormal(sun.lat,sun.lon))}
