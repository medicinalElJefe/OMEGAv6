...
export default function ForecastSovereignPanel({address}:Props){
 ...
 return <section className='panel forecast-r36'><header className='fr36-head'><div><p className='overline'>FORECAST · COMPETING ADMISSIBLE ROUTES</p><h2>What could happen next, and why?</h2><p>Competing legal futures without pretending to observe the future. OMEGA compares legal transitions from the current packet, keeps alternatives visible, and shows where the branches begin to disagree.</p></div><span className='fr36-count'>{plan.corridors.length} scenarios</span></header>
 <div className='fr36-controls'><label><span>Look ahead</span><input type='range' min='1' max='36' value={horizon} onChange={e=>setHorizon(Number(e.target.value))}/><b>{horizon} transitions</b></label><button className='gold' onClick={freeze}><Save size={15}/>Freeze source-only prior</button></div>
