...
import {closeHybridReturnR139,readHybridClosureR139,replayHybridClosureR139,manifestR139,R139_REVISION} from './hybridProofClosureR139.js';
...
const JSON_HEADERS={'content-type':'application/json; charset=utf-8','cache-control':'no-store','x-omega-runtime-successor':REVISION,'x-omega-connector-revision':CONNECTOR_REVISION,'x-omega-proof-closure':R139_REVISION};
...
function withCorsR116(response,request){
 const headers=new Headers(response.headers);headers.set('x-omega-runtime-successor',REVISION);headers.set('x-omega-connector-revision',CONNECTOR_REVISION);headers.set('x-omega-proof-closure',R139_REVISION);const cors=corsHeadersR116(request);if(cors)for(const[k,v]of Object.entries(cors))headers.set(k,v);
...
async function convergenceR116(request,env){
 ...
 return{
  ok:Boolean(status&&machine),schema:'OMEGA_SYSTEM_CONVERGENCE_R116',runtimeRevision:REVISION,connectorRevision:CONNECTOR_REVISION,proofClosureRevision:R139_REVISION,canonicalAuthority:'omega-v6',
...
async function durablePairR117(request,env){
 ...
 return json({
  ...
  proofClosureRevision:R139_REVISION,
  bridgeId,
  secret,
  pairingCode,
  createdAt:Date.now(),
...
function hybridRuntimeIdR139(request){return safeId(request.headers.get('x-omega-bridge-id'))||safeId(request.headers.get('x-omega-session-id'))}
...
export class OmegaRuntime extends OmegaRuntimeR115 {
 async fetch(request){
  ...
  if(path==='/agent/result'&&request.method==='POST'){
   ...
   const closure=await closeHybridReturnR139(this,job,body);return json({...data,proofClosure:{schema:closure.schema,state:closure.state,fingerprint:closure.fingerprint,continuity:closure.continuity,finalHeadSha256:closure.finalHeadSha256,canonicalMutation:false,canonicalAdmissionAuthority:'R125'}});
  }
  const closureMatch=path.match(/^\/jobs\/([A-Za-z0-9._:-]+)\/closure$/);
  if(closureMatch&&request.method==='GET'){if(!await this.authorized(request))return json({ok:false,code:'DEVICE_PROOF_REQUIRED'},401);const closure=await readHybridClosureR139(this,closureMatch[1]);return closure?json({ok:true,closure}):json({ok:false,code:'PROOF_CLOSURE_NOT_FOUND'},404)}
  const replayMatch=path.match(/^\/jobs\/([A-Za-z0-9._:-]+)\/replay$/);
  if(replayMatch&&request.method==='POST'){if(!await this.authorized(request))return json({ok:false,code:'DEVICE_PROOF_REQUIRED'},401);const receipt=await replayHybridClosureR139(this,replayMatch[1]);return receipt?json({ok:receipt.ok,receipt},receipt.ok?200:409):json({ok:false,code:'PROOF_CLOSURE_NOT_FOUND'},404)}
