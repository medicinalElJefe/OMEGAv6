...
const ACTIVE_CONNECTOR_REVISION='R127';
const HYBRID_PROTOCOL_R127='OMEGA_HYBRID_PROTOCOL_R127';
const HEARTBEAT_TTL_R127=30000;
const POLL_INTERVAL_R127=4000;
const CANONICAL_ORIGIN_R127='https://omegav6.jeffdeweyeljefe.workers.dev';
const JSON_HEADERS={'content-type':'application/json; charset=utf-8','cache-control':'no-store','x-omega-runtime-successor':REVISION,'x-omega-connector-revision':ACTIVE_CONNECTOR_REVISION,'x-omega-hybrid-protocol':HYBRID_PROTOCOL_R127};
...
const HYBRID_OPS_R127=Object.freeze(['TRAIN_LOCAL','INDEX','READ_TEXT','SEARCH_TEXT','HASH_TREE','SAFE_IMPORT','WORKBOOK_AUDIT','BUILD','TEST','PACKAGE','SUPPORT_BUNDLE','APPLY_PATCH','OPEN_URL','WAIT','CLICK','KEY','TYPE_TEXT','SCROLL','ASSERT_WINDOW','READ_VISIBLE_TEXT','RECORD_MACRO','REPLAY_MACRO']);

async function sha256TextR127(source){const digest=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(source));return [...new Uint8Array(digest)].map(x=>x.toString(16).padStart(2,'0')).join('')}
async function canonicalHybridAgentR127(env){
 if(!env?.ASSETS?.fetch)return{ok:false,code:'HYBRID_AGENT_ASSET_BINDING_UNAVAILABLE'};
 const asset=await env.ASSETS.fetch(new Request(new URL('/omega-hybrid-agent.py',CANONICAL_ORIGIN_R127),{headers:{'cache-control':'no-cache'}}));
 if(!asset.ok)return{ok:false,code:'HYBRID_AGENT_ASSET_NOT_FOUND',status:asset.status};
 const source=await asset.text();
 const valid=source.length>1000&&source.startsWith('#!/usr/bin/env python3')&&source.includes("DEFAULT_SERVER='https://omegav6.jeffdeweyeljefe.workers.dev'")&&source.includes('OMEGA R34 local Hybrid Link agent')&&source.includes(`HYBRID_PROTOCOL='${HYBRID_PROTOCOL_R127}'`);
 if(!valid)return{ok:false,code:'HYBRID_AGENT_ASSET_INVALID'};
 const version=(source.match(/VERSION='([^']+)'/)||[])[1]||'UNKNOWN',sha256=await sha256TextR127(source);
 return{ok:true,source,version,sha256,protocol:HYBRID_PROTOCOL_R127,bytes:new TextEncoder().encode(source).byteLength};
}
...
function withCorsR116(response,request){
 const headers=new Headers(response.headers);headers.set('x-omega-runtime-successor',REVISION);headers.set('x-omega-connector-revision',ACTIVE_CONNECTOR_REVISION);headers.set('x-omega-hybrid-protocol',HYBRID_PROTOCOL_R127);const cors=corsHeadersR116(request);if(cors)for(const[k,v]of Object.entries(cors))headers.set(k,v);
 return new Response(response.body,{status:response.status,statusText:response.statusText,headers});
}
function preflightR116(request){const headers=corsHeadersR116(request);return headers?new Response(null,{status:204,headers:{...headers,'x-omega-runtime-successor':REVISION,'x-omega-connector-revision':ACTIVE_CONNECTOR_REVISION,'x-omega-hybrid-protocol':HYBRID_PROTOCOL_R127}}):new Response(null,{status:403,headers:{'x-omega-runtime-successor':REVISION,'x-omega-connector-revision':ACTIVE_CONNECTOR_REVISION,'x-omega-hybrid-protocol':HYBRID_PROTOCOL_R127}})}
...
async function convergenceR116(request,env){
 ...
 return{
  ok:Boolean(status&&machine),schema:'OMEGA_SYSTEM_CONVERGENCE_R116',runtimeRevision:REVISION,connectorRevision:ACTIVE_CONNECTOR_REVISION,hybridProtocol:HYBRID_PROTOCOL_R127,canonicalAuthority:'omega-v6',
  ...
  connectorPolicy:{canonicalOrigin:CANONICAL_ORIGIN_R127,currentRevision:ACTIVE_CONNECTOR_REVISION,runtimeRevision:REVISION,hybridProtocol:HYBRID_PROTOCOL_R127,retiredOrigin:'omega-sovereign-convergence.foundasound.chatgpt.site',retiredLaunchersMustNotBeUsed:true,silentPairRotationForbidden:true,agentHashPinRequired:true,reason:'R127 reuses a valid pairing, pins the exact canonical agent bytes, and separates registration from heartbeat proof.'},
  truthBoundary:'Surface availability, machine-service availability, browser pairing, device registration, current host heartbeat, exact agent integrity, solver freshness, and canonical admission are distinct states. R127 never promotes registration or credential issuance into PC ONLINE.'
 };
}
...
async function durablePairR117(request,env){
 ...
 const agent=await canonicalHybridAgentR127(env);
 ...
 return json({
  ...
  hybridProtocol:HYBRID_PROTOCOL_R127,
  bridgeId,
  secret,
  pairingCode,
  createdAt:Date.now(),
  connectorFilename:'START_OMEGA_PC_LINK_R127_SEALED.cmd',
  canonicalOrigin:CANONICAL_ORIGIN_R127,
  retiredOrigin:'omega-sovereign-convergence.foundasound.chatgpt.site',
  agentPath:'/api/hybrid/agent-download?r117=1&r120=1&r127=1',
  rcwaAgentPath:'/api/federation/rcwa/agent-download?r117=1&r120=1&r127=1',
  ...
  truthBoundary:'This endpoint rotates a fresh bridge credential only after the canonical agent itself passes integrity validation. Registration is not PC ONLINE; a real authenticated R127 heartbeat is still required.'
 });
}

function deviceIntegrityR127(device){
 if(device?.protocol!==HYBRID_PROTOCOL_R127)return'LEGACY_UNSEALED';
...
export class OmegaRuntime extends OmegaRuntimeR115 {
 ...
 async fetch(request){
  ...
  if(['/agent/heartbeat','/agent/poll','/agent/result'].includes(path)&&request.method==='POST'){
   ...
   if(row.protocol!==HYBRID_PROTOCOL_R127||row.bootId!==bootId)return json({ok:false,code:'DEVICE_SESSION_SUPERSEDED',reply:'A different sealed agent session owns this device identity. Stop stale connector windows and keep only the newest one.'},409);
   if(deviceIntegrityR127(row)!=='CURRENT_AGENT')return json({ok:false,code:'AGENT_INTEGRITY_REQUIRED'},409);
   if(path==='/agent/heartbeat'){
    ...
    const at=Date.now();rows=rows.map(x=>x.id===id?({...x,lastSeen:at,lastHeartbeatAt:at,heartbeatSeq:seq,version:text(b.version||x.version).slice(0,80)}):x);await this.put('devices',rows);return json({ok:true,at,protocol:HYBRID_PROTOCOL_R127,heartbeatSeq:seq,heartbeatTtlMs:HEARTBEAT_TTL_R127,pollIntervalMs:POLL_INTERVAL_R127,nativeExecutionClaimed:true});
   }
   const online=Date.now()-Number(row.lastSeen||0)<HEARTBEAT_TTL_R127;if(!online)return json({ok:false,code:'HEARTBEAT_STALE',reply:'A current sealed heartbeat is required before polling or returning native work.'},409);
...
async function fetchR116(request,env){
 ...
 if(path==='/api/hybrid/agent-manifest'&&request.method==='GET'){
  const agent=await canonicalHybridAgentR127(env);if(!agent.ok)return withCorsR116(json({ok:false,code:agent.code},503),request);
  return withCorsR116(json({ok:true,schema:'OMEGA_HYBRID_AGENT_MANIFEST_R127',connectorRevision:ACTIVE_CONNECTOR_REVISION,protocol:agent.protocol,version:agent.version,sha256:agent.sha256,bytes:agent.bytes,filename:'omega-hybrid-agent.py',canonicalOrigin:CANONICAL_ORIGIN_R127,downloadPath:'/api/hybrid/agent-download?r117=1&r120=1&r127=1',heartbeatTtlMs:HEARTBEAT_TTL_R127,pollIntervalMs:POLL_INTERVAL_R127,truthBoundary:'The launcher must pin these exact bytes. A hash mismatch is a hard stop, not a fallback or mutation.'}),request);
 }
 ...
 if(path==='/api/hybrid/agent/register'&&request.method==='POST'){
  const agent=await canonicalHybridAgentR127(env);if(!agent.ok)return withCorsR116(json({ok:false,code:agent.code},503),request);
  const headers=new Headers(request.headers);headers.set('x-omega-expected-agent-sha256',agent.sha256);headers.set('x-omega-hybrid-protocol',HYBRID_PROTOCOL_R127);return withCorsR116(await r115.fetch(new Request(request,{headers}),env),request);
 }
 ...
 if(path==='/api/hybrid/agent-download'&&request.method==='GET'&&response.ok){const headers=new Headers(response.headers);headers.set('x-omega-hybrid-protocol',HYBRID_PROTOCOL_R127);headers.set('x-omega-connector-revision',ACTIVE_CONNECTOR_REVISION);return withCorsR116(new Response(response.body,{status:response.status,statusText:response.statusText,headers}),request)}
