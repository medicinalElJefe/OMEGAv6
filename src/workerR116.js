...
import {graphManifestR148,publicGraphApiR148,runtimeGraphApiR148,R148_REVISION} from './execution/executionGraphApiR148.js';
...
async function convergenceR116(request,env){
 ...
 return{
  ok:Boolean(status&&machine),schema:'OMEGA_SYSTEM_CONVERGENCE_R116',runtimeRevision:REVISION,connectorRevision:CONNECTOR_REVISION,proofClosureRevision:R141_REVISION,durableExecutionRevision:R146_REVISION,executorFabricRevision:R147_REVISION,operationGraphRevision:R148_REVISION,canonicalAuthority:'omega-v6',
  ...
  truthBoundary:'Surface availability, R139 unified capability routing, R140 browser operation-world projection, machine-service availability, browser pairing, current host heartbeat, R146 durable operation history, R147 executor binding/dispatch, R148 dependency graph scheduling, returned execution proof, deterministic replay, solver freshness, and canonical admission are distinct states. R116/R117/R141/R146/R147/R148 never promotes one into another.'
 };
}
...
async function fetchR116(request,env){
 ...
 const graphResponse=await publicGraphApiR148(request,internalPath=>executionRuntimeProxyR146(request,env,internalPath));if(graphResponse)return withCorsR116(graphResponse,request);
...
export class OmegaRuntime extends OmegaRuntimeR115 {
 async fetch(request){
  ...
  if(path.startsWith('/execution/')){if(!await this.authorized(request))return json({ok:false,code:'PAIR_AUTH_FAILED',reply:'Durable execution history requires the authenticated OMEGA runtime bridge.'},401);
   const queueHybrid=async jobInput=>{const headers=new Headers(request.headers);headers.set('content-type','application/json');const response=await super.fetch(new Request('https://omega-runtime.internal/jobs',{method:'POST',headers,body:JSON.stringify(jobInput)})),data=await response.clone().json().catch(()=>({}));return{...data,ok:response.ok,status:response.status}};
   if(path.startsWith('/execution/graphs')){const graphResponse=await runtimeGraphApiR148(this,request,{queueHybrid});if(graphResponse)return graphResponse}
