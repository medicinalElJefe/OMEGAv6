...
import {compileSaiHybridLineageR208,publishPendingLineageR208} from './missionLineageR208';
...
export default function SaiHybridHandoffPanelR1961({record,modeSummary}:{record:any;modeSummary:{appliedCount:number;gatedCount:number;catalogCount:number}}){
 ...
 const validate=async()=>{
  ...
  try{
   ...
   const correlated=await compileSaiHybridLineageR208({proposalId:handoff.proposalId,stateId:handoff.stateId,address:handoff.address,draft,validation:next,evidenceRefs:[]});
   publishPendingLineageR208(correlated);setLineage(correlated);
   await appendSaiLedger('R208_LINEAGE_HELD_FOR_OPERATOR_IMPORT',{lineageId:correlated.lineageId,ancestrySha256:correlated.ancestrySha256,proposalId:correlated.proposalId,draftFingerprint:correlated.draftFingerprint,state:correlated.validationState,operatorImportRequired:true,queueMutation:false,dispatchMutation:false,executionClaimed:false,canonicalMutation:false,canonicalAdmissionAuthority:'R125'});
  }catch(e){setError(e instanceof Error?e.message:String(e))}finally{setBusy('')}
 };
 return <section className='sai-r1961' data-sai-hybrid-handoff='R196.1' data-r208-lineage={lineage?.lineageId||'NONE'} data-queue-mutation='false'>
  <header><div><span>R196.1 → R208 · SAI → HYBRID GOVERNED HANDOFF</span><h3>Proposal becomes a typed draft—not a hidden action.</h3><p>B059 diagnoses and proposes. R196.1 translates that exact proposal into the existing V90 Hybrid planning contract. R208 now binds the validated held draft to a SHA-256 ancestry envelope for explicit operator import. Neither layer confirms, binds a device, queues, dispatches or executes native work.</p></div><ShieldCheck/></header>
