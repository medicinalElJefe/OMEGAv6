...
export default function SovereignRuntimeConstellationR62({record,onNavigate,compact=false}:Props){
 ...
 return <section className={`r62-constellation ${compact?'compact':''}`}>
  ...
  <div className='r62-layout'><div className='r62-stage-column'><div className='r62-stage' style={{'--core-size':`${coreSize}px`,'--pressure-blur':`${pressureBlur}px`} as React.CSSProperties}>
   ...
   {families.map(f=><span key={f.id} className={`r62-visual-node reality-${f.dominant.toLowerCase()} ${selected===f.id?'active':''}`} style={{left:`${f.x/5}%`,top:`${f.y/4.2}%`}} aria-hidden='true'><i/><b>{cleanFamily(f.id)}</b></span>)}
  </div><div className='r62-node-grid' aria-label='Capability family controls'>{families.map(f=><button key={f.id} className={`r62-node reality-${f.dominant.toLowerCase()} ${selected===f.id?'active':''}`} onClick={()=>setSelected(f.id)} aria-pressed={selected===f.id} aria-label={`${cleanFamily(f.id)} · ${CAPABILITY_REALITY_LABEL[f.dominant]} · ${f.items.length} capabilities`}><b>{cleanFamily(f.id)}</b><span>{f.items.length}</span><small>{CAPABILITY_REALITY_LABEL[f.dominant]}</small></button>)}</div></div>
  ...
  <footer><ShieldCheck/><span><b>Geometry truth:</b> family membership, node counts and launch targets come from the operational capability registry. Ring distance is a UI encoding of execution boundary. Visual ring markers are non-interactive; the adjacent family controls provide collision-free pointer and keyboard selection. The view does not create evidence, capability or physical dimensions.</span></footer>
 </section>
}
