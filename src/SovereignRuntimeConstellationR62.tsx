...
type FamilyNode={id:string;items:typeof OPERATIONAL_CAPABILITIES;dominant:CapabilityReality;angle:number;x:number;y:number};
...
export default function SovereignRuntimeConstellationR62({record,onNavigate,compact=false}:Props){
 const families=useMemo<FamilyNode[]>(()=>{const live=OPERATIONAL_CAPABILITIES.filter(x=>x.routable),map=new Map<string,typeof live>();for(const item of live){const arr=map.get(item.familyId)||[];arr.push(item);map.set(item.familyId,arr)}const groups=[...map.entries()].sort(([a],[b])=>a.localeCompare(b));return groups.map(([id,items],i)=>{const dominant=[...items].sort((a,b)=>REALITY_RANK[a.reality]-REALITY_RANK[b.reality])[0]?.reality||'SOURCE_ACTIVE';const angle=(Math.PI*2*i/groups.length)-Math.PI/2,r=RADIUS[dominant];return{id,items,dominant,angle,x:250+Math.cos(angle)*r,y:210+Math.sin(angle)*r}})},[]);
 ...
 const coreSize=42+Math.round(Math.max(0,Math.min(1,Number(m.continuity||0)))*18),pressure=Math.max(0,Math.min(1,(Number(m.contradiction||0)+Number(m.burden||0))/2));
 return <section className={`r62-constellation ${compact?'compact':''}`}>
  ...
  <div className='r62-layout'><div className='r62-stage' style={{'--core-size':`${coreSize}px`,'--pressure':pressure} as React.CSSProperties}>
