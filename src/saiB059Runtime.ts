...
export async function appendSaiLedger(event:string,payload:any){const rows=readSaiLedger(),previous=rows.at(-1)?.entry_hash||'0'.repeat(64),body={timestamp_utc:new Date().toISOString(),event,previous_hash:previous,payload},entry_hash=await sha256Hex(previous+JSON.stringify(body,Object.keys(body).sort()));const row={...body,entry_hash};localStorage.setItem(LEDGER_KEY,JSON.stringify([...rows,row].slice(-500)));return row;}
export async function verifySaiLedger(rows=SaiLedgerEntry[] as any){let previous='0'.repeat(64),count=0;for(const row of rows){const body={timestamp_utc:row.timestamp_utc,event:row.event,previous_hash:row.previous_hash,payload:row.payload};const expected=await sha256Hex(previous+JSON.stringify(body,Object.keys(body).sort()));if(row.previous_hash!==previous||row.entry_hash!==expected)return{passed:false,entries:count,failure_at:count+1,head:previous};previous=row.entry_hash;count++}return{passed:true,entries:count,head:previous}}
