...
const OMEGA_WORKSPACE_ROUTES_R82=OMEGA_WORKSPACES_R82.flatMap(x=>x.routes);
export const OMEGA_ALL_ROUTES_R82=OMEGA_NAV_NAMES;
...
export function validateExperienceRegistryR82(){
 ...
 return{
  ...
  workspaceMissing,
  workspaceExtra,
  workspaceRouteCount:workspaceRoutes.length,
  pass:routes.length>0&&unique.size===routes.length&&workspaceRoutes.length===routes.length&&workspaceUnique.size===workspaceRoutes.length&&workspaceMissing.length===0&&workspaceExtra.length===0&&workspaceIds.size===OMEGA_WORKSPACES_R82.length&&emptyWorkspaces.length===0&&projectionIds.size===OMEGA_FIELD_PROJECTIONS_R82.length,
