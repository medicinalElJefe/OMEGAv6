...
import {familyOperationalProofR135,familyOperationalProofSummaryR135,FAMILY_OPERATIONAL_PROOF_LAWS_R135} from './familyOperationalProofR135';
...
export default function OmegaBuildPotentialR133({onNavigate,compact=false}:Props){
 ...
 return <section className={'r133-potential '+(compact?'compact':'full')}>
  ...
  <div className='r133-family-grid'>{rows.map(row=>{const I=ICON[row.lane],proof=familyOperationalProofR135(row.family,operational,hybrid);return <article key={row.family.id} className={TONE[row.lane]}>
