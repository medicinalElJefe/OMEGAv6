...
export default function OmegaBuildPotentialR133({onNavigate,compact=false}:Props){
 ...
 return <section className={'r133-potential '+(compact?'compact':'full')}>
  <header className='r133-potential-head'><div><span>R137 LIVE PROOF · R136 LIVING WORLD · R134 WORLD + WOVEN CONTINUITY · R133 BUILD POTENTIAL / RESTORATION MAP</span><h3>What works, what is gated, and what should be built next</h3><p>The declared 24-family execution status stays immutable while R137 overlays current operational evidence. R136 remains the living evidence/mission/projection frame; both R134 continuity authorities remain intact. R137 observes current proof and advances the next action without rewriting those authorities.</p></div><div className='r133-summary'>{BUILD_POTENTIAL_LANES_R133.map(x=><button key={x.lane} className={TONE[x.lane]} onClick={()=>setLane(lane===x.lane?'ALL':x.lane)} aria-pressed={lane===x.lane}><b>{x.count}</b><span>{LABEL[x.lane]}</span><small>{x.priority}</small></button>)}</div></header>
  ...
  {proofError&&<div className='r137-proof-error'><TriangleAlert/>{proofError} · missing runtime evidence remains unknown; declared family status is unchanged.</div>}
  ...
  <div className='r133-family-grid'>{rows.map(row=>{const I=ICON[row.lane],proof=familyOperationalProofR137(row.family,operational,hybrid),nextAction=currentNextActionR137(row.family,proof,row.action);return <article key={row.family.id} className={TONE[row.lane]}>
   ...
   <div className='r133-family-proof'><span>DECLARED FAMILY STATUS · CURRENT TRUTH</span><b>{row.family.status.replaceAll('_',' ')}</b><small>{row.family.statusNote}</small></div>
   <div className={'r137-family-live '+proofTone(proof.state)}><span>CURRENT OPERATIONAL PROOF</span><b>{proof.label}</b><small>{proof.detail}</small><em>{proof.source}{proof.observedAt?` · ${proof.observedAt}`:''}</em></div>
