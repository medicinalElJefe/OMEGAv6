...
export default function OmegaBuildPotentialR133({onNavigate,compact=false}:Props){
 ...
 return <section className={'r133-potential '+(compact?'compact':'full')}>
  <header className='r133-potential-head'><div><span>R135 LIVE PROOF · R134 WORLD + WOVEN CONTINUITY · R133 BUILD POTENTIAL / RESTORATION MAP</span><h3>What works, what is gated, and what should be built next</h3><p>The declared 24-family execution status stays immutable while R135 overlays current operational evidence. Both R134 continuity authorities remain intact: the append-only world/scar/proof chain and the woven structural/reference/route continuum. R135 only observes current proof and advances the next action without rewriting either authority.</p></div><div className='r133-summary'>{BUILD_POTENTIAL_LANES_R133.map(x=><button key={x.lane} className={TONE[x.lane]} onClick={()=>setLane(lane===x.lane?'ALL':x.lane)} aria-pressed={lane===x.lane}><b>{x.count}</b><span>{LABEL[x.lane]}</span><small>{x.priority}</small></button>)}</div></header>
