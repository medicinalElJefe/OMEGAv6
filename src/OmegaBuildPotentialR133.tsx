...
export default function OmegaBuildPotentialR133({onNavigate,compact=false}:Props){
 ...
 const proofCounts=useMemo(()=>{const proofs=BUILD_POTENTIAL_ROWS_R133.map(x=>familyOperationalProofR137(x.family,operational,hybrid));return{execution:proofs.filter(x=>x.state==='CURRENT_EXECUTION_PROOF').length,service:proofs.filter(x=>x.state==='CURRENT_SERVICE_OBSERVATION').length,held:proofs.filter(x=>x.state==='GATE_CURRENTLY_HELD').length}},[operational,hybrid]);
 const q=query.trim().toLowerCase();
 const rows=useMemo(()=>BUILD_POTENTIAL_ROWS_R133.filter(x=>(lane==='ALL'||x.lane===lane)&&(!q||[x.family.id,x.family.name,x.family.status,x.effectiveStatus,x.statusSource,x.family.statusNote,x.family.inventoryPurpose,x.effectiveFamily.operatorRoute,x.lane,x.action,...x.planeLabels].join(' ').toLowerCase().includes(q))),[lane,q]);
 ...
 return <section className={'r133-potential '+(compact?'compact':'full')}>
  <header className='r133-potential-head'><div><span>R168 EFFECTIVE EXECUTION · R137 LIVE PROOF · R136 LIVING WORLD · R134 WORLD + WOVEN CONTINUITY · R133 BUILD POTENTIAL / RESTORATION MAP</span><h3>What works, what is gated, and what should be built next</h3><p>What works now is calculated from the preserved 24-family V24 registry plus bounded R46/R153 successor executors. Historical status stays immutable as lineage; R168 stops restored software from being counted as missing. R137 current service/device proof remains a separate evidence axis.</p></div><div className='r133-summary'>{BUILD_POTENTIAL_LANES_R133.map(x=><button key={x.lane} className={TONE[x.lane]} onClick={()=>setLane(lane===x.lane?'ALL':x.lane)} aria-pressed={lane===x.lane}><b>{x.count}</b><span>{LABEL[x.lane]}</span><small>{x.priority}</small></button>)}</div></header>
  ...
  <div className='r133-family-grid'>{rows.map(row=>{const I=ICON[row.lane],proof=familyOperationalProofR137(row.family,operational,hybrid),nextAction=currentNextActionR137(row.family,proof,row.action);return <article key={row.family.id} className={TONE[row.lane]}>
   <header><code>{row.family.id}</code><div><b>{row.family.name}</b><small>{row.family.role} · effective {row.effectiveStatus}</small></div><I/></header>
   <p>{row.family.inventoryPurpose}</p>
   <div className='r133-family-proof'><span>CURRENT TRUTH · CURRENT EFFECTIVE EXECUTION · R168</span><b>{row.effectiveStatus.replaceAll('_',' ')}</b><small>{row.effectiveFamily.executionBoundary}</small><em>{row.statusSource}</em></div>
   ...
   <div className={'r137-family-live '+proofTone(proof.state)}><span>CURRENT OPERATIONAL PROOF · R137</span><b>{proof.label}</b><small>{proof.detail}</small><em>{proof.source}{proof.observedAt?` · ${proof.observedAt}`:''}</em></div>
   ...
   <footer><button onClick={()=>open(row.effectiveFamily.operatorRoute,row.family.id)}>OPEN {row.effectiveFamily.operatorRoute||'System Atlas'} <ArrowRight/></button><small>{row.breadth} expression plane{row.breadth===1?'':'s'} · family registration ≠ execution · historical registration ≠ successor execution ≠ external proof</small></footer>
  </article>})}</div>
  ...
  <footer className='r133-truth'><ShieldCheck/><span><b>Authority boundary:</b> {BUILD_POTENTIAL_LAWS_R133.authority} {FAMILY_OPERATIONAL_PROOF_LAWS_R137.separation}. {FAMILY_OPERATIONAL_PROOF_LAWS_R137.reachability}. {FAMILY_OPERATIONAL_PROOF_LAWS_R137.worldContinuity}. {FAMILY_OPERATIONAL_PROOF_LAWS_R137.wovenContinuum}. {FAMILY_OPERATIONAL_PROOF_LAWS_R137.livingWorld}. {FAMILY_OPERATIONAL_PROOF_LAWS_R137.nextAction}.</span><Target/><small>Priority is an organization signal. R168 corrects current execution accounting and direct operator routing; neither priority nor successor accounting can fabricate evidence, scientific validity, native breadth, physical truth or CanonState.</small></footer>
 </section>;
}
