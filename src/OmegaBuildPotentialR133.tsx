...
export default function OmegaBuildPotentialR133({onNavigate,compact=false}:Props){
 ...
 return <section className={'r133-potential '+(compact?'compact':'full')}>
  <header className='r133-potential-head'><div><span>R168 EFFECTIVE EXECUTION · R137 LIVE PROOF · R136 LIVING WORLD · R134 WOVEN CONTINUITY · R133 BUILD POTENTIAL</span><h3>What works now, what is gated, and what should actually be built next</h3><p>R168 preserves the immutable V24 family status as historical lineage while using already-proved R46/R153 successor executors for current build prioritization. Restored software is no longer miscounted as missing. Runtime/device/evidence proof remains a separate R137 axis.</p></div><div className='r133-summary'>{BUILD_POTENTIAL_LANES_R133.map(x=><button key={x.lane} className={TONE[x.lane]} onClick={()=>setLane(lane===x.lane?'ALL':x.lane)} aria-pressed={lane===x.lane}><b>{x.count}</b><span>{LABEL[x.lane]}</span><small>{x.priority}</small></button>)}</div></header>
  ...
  <div className='r133-family-grid'>{rows.map(row=>{const I=ICON[row.lane],proof=familyOperationalProofR137(row.family,operational,hybrid),nextAction=currentNextActionR137(row.family,proof,row.action);return <article key={row.family.id} className={TONE[row.lane]}>
   ...
   <div className='r133-family-proof'><span>CURRENT EFFECTIVE EXECUTION · R168</span><b>{row.effectiveStatus.replaceAll('_',' ')}</b><small>{row.effectiveFamily.executionBoundary}</small><em>{row.statusSource}</em></div>
   ...
   <div className={'r137-family-live '+proofTone(proof.state)}><span>CURRENT EXTERNAL / SERVICE PROOF · R137</span><b>{proof.label}</b><small>{proof.detail}</small><em>{proof.source}{proof.observedAt?` · ${proof.observedAt}`:''}</em></div>
   ...
   <footer><button onClick={()=>open(row.effectiveFamily.operatorRoute,row.family.id)}>OPEN {row.effectiveFamily.operatorRoute||'System Atlas'} <ArrowRight/></button><small>{row.breadth} expression plane{row.breadth===1?'':'s'} · historical registration ≠ current successor execution ≠ external proof</small></footer>
  </article>})}</div>
