...
export default function OmegaBuildPotentialR133({onNavigate,compact=false}:Props){
 ...
 return <section className={'r133-potential '+(compact?'compact':'full')}>
  <header className='r133-potential-head'><div><span>R168 CURRENT SUCCESSOR EXECUTION · R137 LIVE PROOF · R136 LIVING WORLD · R134 WOVEN CONTINUITY · R133 BUILD POTENTIAL</span><h3>What works now, what is proof-gated, and what should be built next</h3><p>Development priority now uses the current R48/R153 successor ledger rather than stale V24 debt labels. Historical registration stays immutable as lineage, current successor execution stays separate, and R137 current service/device evidence remains an independent proof axis.</p></div><div className='r133-summary'>{BUILD_POTENTIAL_LANES_R133.map(x=><button key={x.lane} className={TONE[x.lane]} onClick={()=>setLane(lane===x.lane?'ALL':x.lane)} aria-pressed={lane===x.lane}><b>{x.count}</b><span>{LABEL[x.lane]}</span><small>{x.priority}</small></button>)}</div></header>
  ...
  {proofError&&<div className='r137-proof-error'><TriangleAlert/>{proofError} · missing runtime evidence remains unknown; declared family status is unchanged. Current successor execution accounting is also left unchanged by a failed runtime probe.</div>}
  ...
  <div className='r133-family-grid'>{rows.map(row=>{const I=ICON[row.lane],proof=familyOperationalProofR137(row.family,operational,hybrid),nextAction=currentNextActionR137(row.family,proof,row.action);return <article key={row.family.id} className={TONE[row.lane]}>
   ...
   <footer><button onClick={()=>open(row.effectiveRoute,row.family.id)}>OPEN {row.effectiveRoute||'System Atlas'} <ArrowRight/></button><small>{row.breadth} expression plane{row.breadth===1?'':'s'} · successor surface {row.effectiveSurface} · historical registration ≠ current successor execution ≠ external proof</small></footer>
  </article>})}</div>
