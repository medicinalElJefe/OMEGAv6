...
const BINDINGS:Record<string,ModeBindingR280>=Object.freeze({
  'OVERALL CANON MODE':{
    artifacts:['src/allModesTruthFusionR151.ts','src/universalTruthEnvelopeR152.ts','src/system/calibrationR334.js','src/system/calibrationR340.js'],
  ...
  'Dewey Calculus Mode':{
    artifacts:['src/sourceBackedModeRuntimeR21.ts','src/AppliedCalculusR168.tsx','src/system/calibrationR334.js','src/system/calibrationR340.js'],tests:['tests/r107-full-calculus-capability-fabric-invariants.mjs','tests/r334-calibration-convergence-invariants.mjs'],executionClass:'DOMAIN_RUNTIME',inputContract:'CΩ Φ q Λ Scar + declared operators',outputContract:'bounded derived calculus/control variables',
...
export function compileModeRealizationRegistryR280(record:any){
  ...
  return{
    ...
    forecastCalibrationContext:{...calibrationManifestR340(),propagationRevision:'R340',use:'POST_FREEZE_ABLATION_AND_FORECAST_ONLY'},
    rows,criticalGaps,
    canonicalMutation:false,canonicalAdmissionAuthority:'R125',
    truthBoundary:'R280 distinguishes a named/charted mode, a derived lens, a source-executed operator, and a domain runtime. All 62 canon authorities have executable read-only lenses through the inherited R12 evaluator, but lens execution is not equivalent to realizing every historical domain executor. R340 carries the R334 calibrated CERN/Omega evidence context plus the frozen V4 ablation/forecast contract across the registry without treating every mode as a CERN executor or independent replication. Gated inputs remain explicit. Promotion status requires bound runtime/test evidence and never overrides empirical evidence, physical validation, execution receipts, or R125 CanonState admission.'
  };
}
