...
export function resourceEnvelopeR239(input:{profile:any; snapshotCurrent:boolean; activeNativeWork:boolean}):ResourceEnvelopeR239{
 ...
 const verify=profileProved&&input.snapshotCurrent&&!input.activeNativeWork&&tier!=='HOLD';
 ...
 return{
  schema:'OMEGA_HYBRID_RESOURCE_ENVELOPE_R239',revision:'R239',tier,profileProved,snapshotCurrent:input.snapshotCurrent,activeNativeWork:input.activeNativeWork,
  ...
  truthBoundary:'R239 is a deterministic admission/sizing envelope over returned R238 host evidence and the shared selected-host snapshot. It does not infer CUDA/RCWA validity, install dependencies, create a new executor, bypass R237 backpressure, mutate CanonState, or convert advisory resource evidence into scientific proof.'
 };
}
