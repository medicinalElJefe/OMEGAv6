...
export default function SystemFoundryR268({deviceHeartbeat=false,externalBindings=false,resourceEnvelopeR239=null}:Props){
 ...
 return <section className='foundry-r268' aria-label='R268 System Foundry' data-runtime-truth='R269' data-inspection-truth='R272_READ_ONLY' data-dependency-truth='R273_READ_ONLY' data-impact-truth='R274_READ_ONLY' data-evidence-truth='R275_READ_ONLY' data-action-truth='R277_AUTH_REQUIRED'>
  <header><div><span>R268→R277 · SYSTEM-CONSTRUCTION + LIVE TRUTH + CAUSAL INSPECTION</span><h3>OMEGA System Foundry</h3><p>Compile a declarative system genome into an active capability frontier, shared operator DAG, evidence-bound executor placements, lenses and proof obligations. This surface plans and inspects; it does not create a second executor or production writer. R273–R275 expose causal, impact and evidence review; R276 propagates blocked dependencies; R277 keeps every side-effect capability AUTH_REQUIRED so this planning surface cannot self-authorize or bypass the existing R179 → R147 action path.</p></div><div className='foundry-proof'><ShieldCheck/><b>FAIL-CLOSED</b><small>{plan.fingerprint}</small></div></header>
  ...
  <footer><div><b>Active planning frontier</b><span>Estimated abstract cost {plan.estimatedCost} · latency {plan.estimatedLatency} · {plan.dormantCapabilities} registry capabilities remain addressable without materialization.</span></div><div><b>R277 action truth</b><span>AUTH_REQUIRED — Foundry cannot self-authorize or dispatch; R179 authorization and R147 dispatch remain separate existing authorities.</span></div><div><b>Device truth</b><span>{deviceHeartbeat?'Current authenticated heartbeat supplied to this planner.':'DEVICE_PROOF_REQUIRED — browser/CI state is not treated as private-PC proof.'}</span></div><div><b>R239 resource truth</b><span>{plan.runtimeTruth.resourceAuthority==='R239_CURRENT_ENVELOPE'?`${plan.runtimeTruth.resourceTier} · ${plan.runtimeTruth.deviceAssistAdmitted?'DEVICE ASSIST ADMITTED':'DEVICE ASSIST HELD'} · ${plan.runtimeTruth.resourceReasons.join(' · ')||'fresh governed envelope'}`:'R239_RESOURCE_PROOF_REQUIRED — device placement remains blocked without a fresh returned R238 host profile and current shared snapshot.'}</span></div></footer>
 </section>
}
