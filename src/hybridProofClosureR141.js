...
import {compileSovereignExecutionConvergenceR159,R159_REVISION} from './execution/sovereignExecutionConvergenceR159.js';
...
export async function closeHybridReturnR141(runtime,job,body){
 ...
 const replayReceipt=await replayHybridClosureR141(runtime,closure.jobId),replayedClosure=await runtime.get('r141Closure:'+closure.jobId,closure),sovereignConvergence=await compileSovereignExecutionConvergenceR159(runtime,job,replayedClosure,replayReceipt),finalClosure={...replayedClosure,sovereignConvergence,sovereignExecutionConvergenceRevision:R159_REVISION};
 await runtime.put('r141Closure:'+closure.jobId,finalClosure);
 const jobs=await runtime.get('jobs',[]);await runtime.put('jobs',jobs.map(x=>x.id===closure.jobId?({...x,proofClosure:{schema:R141_SCHEMA,state:finalClosure.state,fingerprintVerified:finalClosure.fingerprint.verified,finalHeadSha256:finalClosure.finalHeadSha256,continuity:finalClosure.continuity,deterministicReplayVerified:replayReceipt?.ok===true,sovereignConvergenceSha256:sovereignConvergence.convergenceSha256,admissionCandidateState:sovereignConvergence.admissionCandidate.state,canonicalMutation:false}}):x));
 await runtime.event('JOB_PROOF_CLOSED',`Job ${closure.jobId} return closed into deterministic R141 proof/scar evidence and R159 sovereign convergence.`,{jobId:closure.jobId,state:finalClosure.state,fingerprintVerified:finalClosure.fingerprint.verified,finalHeadSha256:finalClosure.finalHeadSha256,deterministicReplayVerified:replayReceipt?.ok===true,sovereignConvergenceSha256:sovereignConvergence.convergenceSha256,admissionCandidateState:sovereignConvergence.admissionCandidate.state});
 return finalClosure;
}
...
export function manifestR141(){return{ok:true,schema:R141_SCHEMA,revision:R141_REVISION,fingerprintSchema:R141_FINGERPRINT_SCHEMA,laws:R141_LAWS,canonicalMutation:false,canonicalAdmissionAuthority:'R125',preservesUnifiedCapabilityRevision:'R139',preservesLivingWorldOperationBridgeRevision:'R140',successorConvergenceRevision:R159_REVISION,durability:'OMEGA_RUNTIME_DURABLE_OBJECT_EVIDENCE_STORE',r97ContinuityPromotion:'NOT_AUTO_PROMOTED',truthBoundary:'Manifest only. R141 adds exact-payload host-return closure/replay evidence inside the already-proven Hybrid Durable Object runtime while preserving the separate R139 capability engine and R140 living-world operation bridge. R159 adds deterministic sovereign execution convergence and an R125 admission candidate state without changing CanonState. It does not itself claim a PC is online, a workload succeeded, or CanonState changed.'}}
