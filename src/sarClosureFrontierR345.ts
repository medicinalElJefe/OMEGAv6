...
export const SAR_CLOSURE_FRONTIER_SCHEMA_R345='OMEGA_SAR_CLOSURE_FRONTIER_R345';
export type SarClosureActionKindR345='BIND_SOURCE'|'DECODE'|'CALIBRATE'|'COREGISTER'|'VALIDATE_PHASE'|'TERRAIN'|'UNWRAP'|'LOS'|'CORRECT'|'ADD_GEOMETRY'|'VERIFY';
export type SarClosureFrontierItemR345={
 rank:number;layer:SarR342LayerId;kind:SarClosureActionKindR345;gate:string;reason:string;
...
function kind(id:SarR342LayerId):SarClosureActionKindR345{
...
export function buildSarClosureFrontierR345(layers:SarEstablishmentLayerR342[],validation?:SarClosureValidationR344|null,receipt?:SarHostClosureReceiptR344|null){
 ...
 const out:SarClosureFrontierItemR345[]=held.map(layer=>{
 ...
 return{schema:SAR_CLOSURE_FRONTIER_SCHEMA_R345,state:out.length?'OPEN_FRONTIER':'PHYSICAL_CLOSURE_COMPLETE',next:out.find(x=>x.admissible)??null,items:out,validationScars:validation?.scars||[],truthBoundary:'R345 chooses the next admissible evidence-producing action. It may order work, but it cannot manufacture Sentinel-1 measurements, DEM/orbit/correction evidence, unwrap closure, or independent viewing geometry.'};
}
