...
export default function OmegaExperienceShellR257({children,onNavigate,onHome,home}:Props){
 ...
 return <div className={`r257-shell ${immersive?'immersive':''}`} data-r257-experience={experience} data-r257-depth={depth} data-r257-immersive={immersive?'true':'false'} data-r257-authority={EXPERIENCE_TRUTH_CONTRACT_R257.authority}>
  ...
  <header className='r257-experience-bar'>
   <button type='button' className='r257-home' onClick={onHome} aria-label='OMEGA Home'><Compass aria-hidden='true'/><span><b>OMEGA EXPERIENCE</b><small>{active.label} · {active.copy}</small></span></button>
   <nav aria-label='Experience mode'>{OMEGA_EXPERIENCES_R257.map(x=><button type='button' key={x.id} aria-label={`Experience mode: ${x.label}`} aria-pressed={experience===x.id} className={experience===x.id?'active':''} onClick={()=>choose(x.id)} title={`${x.label}: ${x.copy}`}><b>{x.label}</b><small>{x.copy}</small></button>)}</nav>
   <div className='r257-shell-actions'><button type='button' onClick={()=>setImmersive(!immersive)} aria-pressed={immersive} aria-label={immersive?'Exit immersive experience':'Enter immersive experience'}>{immersive?<Minimize2 aria-hidden='true'/>:<Maximize2 aria-hidden='true'/>}<span>{immersive?'Exit immersive':'Immersive'}</span></button><button type='button' onClick={resetAll} aria-label='Reset experience preferences' title='Reset presentation preferences'><RotateCcw aria-hidden='true'/></button></div>
  </header>
  <section className='r257-context-strip' aria-label={`${active.label} experience context`}>
  ...
  {!immersive&&<OmegaUniversalInterfaceR263 onNavigate={onNavigate} compact={!home}/>} 
