...
export type WorkflowIntentR108='EXPLORE'|'ANALYZE'|'FORECAST'|'BUILD'|'REPAIR'|'PROVE'|'CREATE'|'CONNECT';
...
const BASE_BY_INTENT:Record<WorkflowIntentR108,readonly string[]>={
...
function capabilityReason(cap:MasterCapabilityR83,intent:WorkflowIntentR108,matched:Map<string,string[]>){
...
export function compileUltimateCapabilityPlanR108(args:{intent:WorkflowIntentR108;goal:string;surface:string;record:any;federationStatus?:any}){
 ...
 const selections:CapabilitySelectionR108[]=MASTER_CAPABILITIES_R83.map(cap=>({id:cap.id,name:cap.name,state:requiredIds.has(cap.id)?'REQUIRED':supportingIds.has(cap.id)?'SUPPORTING':'AVAILABLE',route:routeForCapabilityR83(cap),menu:cap.menu,proofGate:cap.proofGate,roles:cap.roles,reason:capabilityReason(cap,intent,matched)}));
 ...
 const federationStatusKnown=Boolean(args.federationStatus&&typeof args.federationStatus==='object');
 ...
 return{
  ...
  federation:{...federation,statusObserved:federationStatusKnown},proof:{proofGates,sourceGates,empiricalValidationRequired:empirical,empiricalAuthority:empirical?'SCIENTIFIC_VALIDATION_BRIDGE':'AVAILABLE_IF_CLAIM_REQUIRES_IT'},
  next:{routes,federationGate:federationStatusKnown?String(federation?.gate||'UNKNOWN'):'STATUS_NOT_OBSERVED',firstRoute:routes[0]||args.surface},
  boundary:'R108 compiles one intent into the already accepted source → state → calculus → modes → layers → capability → runtime → observation → action → proof → admission chain. It selects the minimum required capability graph while keeping all recovered capabilities/modes/source authorities available. Selection never turns catalog metadata into execution, a Drive design row into live runtime proof, a cloud registration into ONLINE state, or a representational scale into a physical dimension. Federation readiness is reported as current only when a status observation is supplied.'
 };
}
