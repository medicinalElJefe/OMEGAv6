...
export async function selectExecutorR147(runtime,run,input={}){
 ...
 return{schema:'OMEGA_EXECUTOR_PLAN_R147',runId:run.id,domain,executorId,eligible,state,strategy:strategy||'AUTO',canonicalMutation:false,truthBoundary:'Selection binds an eligible executor to a durable run. Selection itself is not invocation or execution proof.'};
}
...
export async function dispatchRunR147(runtime,id,input={},callbacks={}){
 ...
 if(plan.executorId==='WORKERS_AI')return runWorkersAI(runtime,run,input,runtime.env||{});
 if(plan.executorId==='LOCAL_PROOF')return runLocalProof(runtime,run,input);
 if(plan.executorId==='LOCAL_RUNTIME')return runLocalRuntime(runtime,run,input);
 if(plan.executorId==='FEDERATION_CHAIN')return runFederation(runtime,run,input,runtime.env||{});
 if(plan.executorId==='AUTONOMIC_SWARM')return runSwarm(runtime,run,input,runtime.env||{});
 if(plan.executorId==='HYBRID_HOST')return queueHybrid(runtime,run,input,callbacks);
 if(plan.executorId==='PLUGIN_CONNECTOR')return{ok:false,status:409,code:'R147_PLUGIN_CONCRETE_ADAPTER_REQUIRED',plan,run,truthBoundary:'A registered plugin/provider name is not execution. R147 refuses to invoke without a concrete provider transport adapter and returned provider receipt.'};
 return{ok:false,status:409,code:'R147_EXECUTOR_NOT_IMPLEMENTED',plan,run};
}
...
export function manifestR147(){return{ok:true,schema:'OMEGA_UNIFIED_EXECUTOR_FABRIC_MANIFEST_R147',revision:R147_REVISION,executors:R147_EXECUTORS,laws:R147_LAWS,upstream:{operationContract:'R143',lifecycle:'R142',hybridProof:'R141',durableRun:'R146',runtimeAttestation:'R144',worldScar:'R145',swarm:'R125',federation:'R115'},canonicalMutation:false,canonicalAdmissionAuthority:'R125',truthBoundary:'R147 unifies concrete execution transports around one R146 durable run. It distinguishes selection, availability, dispatch, invocation, return, return-integrity verification, evidence/world-scar carry, and CanonState admission. No executor, model, plugin, swarm quorum, federation packet, host return, or deployment attestation automatically mutates CanonState.'}}
