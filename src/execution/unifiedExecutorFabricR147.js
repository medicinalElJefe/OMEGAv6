...
import {assembleDifferentialPartitionsR195,manifestR195,planDifferentialPartitionsR195,publishDifferentialPartitionsR195} from './differentialPartitionExecutionR195.js';
import {manifestR196} from './boundedPartitionSchedulerR196.js';
import {executeAdaptivePartitionsR197,manifestR197} from './adaptivePartitionBackpressureR197.js';
...
export const R147_LAWS=Object.freeze([
 ...
 'R195_UNCHANGED_PARTITIONS_MAY_CARRY_ONLY_FROM_VERIFIED_EXACT_KEYS',
 'R195_CHANGED_PARTITIONS_RECOMPUTE_UNDER_THE_SELECTED_R147_EXECUTOR',
 'R196_ONLY_R195_RECOMPUTE_PARTITIONS_ENTER_BOUNDED_PARALLEL_EXECUTION',
 'R196_PARALLELISM_IS_REPLANNED_FROM_CURRENT_R193_COMPUTE_AXIS_AND_DIRTY_SET',
 'R196_STAY_TURN_IS_SCHEDULING_BEHAVIOR_NOT_TRUTH_PROMOTION',
 'R197_FEEDBACK_MAY_ONLY_REDUCE_OR_RECOVER_WITHIN_THE_CURRENT_R196_CEILING',
 'R197_FAILURE_BACKPRESSURE_AND_HEALTHY_RECOVERY_ARE_SCHEDULING_ONLY',
 'R197_FEEDBACK_IS_SCOPE_LOCAL_AND_NEVER_EXECUTION_AUTHORIZATION',
...
export async function executorDirectoryR147(runtime,env=runtime?.env||{}){
 ...
 return{
  ...
  performance:{revision:'R185',loop:'PREDICT_CARRY_CORRECT_REALLOCATE',adaptiveSelection:'TERMINAL_HISTORY_MATURED_ONLY',verifiedReuse:'R194_EXACT_CONTENT_ONLY',differentialPartitionExecution:'R195_AI_SAI_EXACT_PARTITION_CARRY',boundedPartitionParallelism:'R196_DIRTY_ONLY_DYNAMIC_R193_CEILING',adaptivePartitionBackpressure:'R197_SCOPE_LOCAL_AIMD_WITHIN_R196_CEILING'},
  runtimeStorageCompatibility:'R168',
  canonicalMutation:false,canonicalAdmissionAuthority:'R125',
  truthBoundary:'Executor availability means a concrete transport/binding is presently available. CONTENT_CACHE additionally requires an exact R194 verified-content hit before selection. R195 can reduce a selected Workers AI execution to changed partitions only when explicit partition content is exact-key comparable. R196 bounds only that current dirty set from the current R193 compute ceiling. R197 may further reduce or cautiously recover the dirty-work window from durable scope-local measured outcomes but can never exceed R196. Availability, carry, STAY/TURN, DOWN/UP/CLAMP feedback, parallelism and performance history are not invocation, return, factual truth, solver validity, PC heartbeat proof, or CanonState admission.'
 };
}
...
export async function selectExecutorR147(runtime,run,input={}){
 ...
 return{schema:'OMEGA_EXECUTOR_PLAN_R147',runId:run.id,domain,executorId,eligible:planEligible,state,strategy:strategy||'AUTO',temporalPerformance,reuse,canonicalMutation:false,truthBoundary:'Selection binds an eligible executor to a durable run. R185 may reorder only eligible AVAILABLE executors after measured terminal history matures. R194 may substitute CONTENT_CACHE only after an exact verified-content hit and only when the operator did not explicitly choose an executor or strategy. If WORKERS_AI remains selected, R195 may carry exact unchanged partitions, R196 defines the current hard bounded dirty-work plan, and R197 may adapt only downward/recovery within that plan from durable measured scope-local history. Selection itself is not invocation or execution proof.'};
}

async function runWorkersAI(runtime,run,input,env,multiAxis=null){
 if(!env?.AI?.run)return{ok:false,status:503,code:'R147_WORKERS_AI_UNAVAILABLE'};
 const partitionPlan=await planDifferentialPartitionsR195(runtime,{run,input,multiAxis,model:MODEL});if(partitionPlan?.enabled&&partitionPlan?.ok===false)return{ok:false,status:400,code:partitionPlan.code||'R195_PARTITION_PLAN_INVALID',partitionPlan,run};
 ...
 try{
  ...
  if(partitionPlan?.enabled){
   const dirty=(partitionPlan.partitions||[]).filter(p=>p.action==='RECOMPUTE');
   const batch=await executeAdaptivePartitionsR197(runtime,dirty,async p=>{const response=await env.AI.run(MODEL,{messages:[{role:'system',content:system},{role:'user',content:p.prompt}],max_tokens:Math.max(128,Math.min(1600,Number(input.maxTokens)||900)),temperature:Math.max(0,Math.min(1,Number.isFinite(Number(input.temperature))?Number(input.temperature):.2)),chat_template_kwargs:{enable_thinking:false}}),text=txt(modelText(response),16000);return{id:p.id,result:{provider:'CLOUDFLARE_WORKERS_AI',model:MODEL,text,executionTruth:'RETURNED_MODEL_PARTITION_NOT_FACTUAL_VERIFICATION'}}},{run,multiAxis,input,executorId:'WORKERS_AI',model:MODEL});
   if(!batch.ok){const failed=await transitionRunR146(runtime,run.id,{state:'FAILED',reason:'R197 adaptive / R196 bounded dirty-partition execution failed',evidence:{proofRef:'R196_BOUNDED_PARTITION_BATCH_FAILURE',providerReceipt:txt(batch?.failure?.message||batch?.code,1000)}});return{ok:false,status:batch.status||502,code:batch.code||'R196_PARTITION_EXECUTION_FAILED',run:failed.run||await readRunR146(runtime,run.id),differential:{plan:partitionPlan,scheduling:batch},truthBoundary:'R197/R196 failed closed before R195 reassembly, R147 return verification, R195 publication or R194 whole-result publication. Already-running bounded calls may have settled; their partial bytes are not admitted to the verified partition cache. The failed measured batch may only reduce a future scope-local scheduling window.'};}
   const freshResults=batch.results||[],assembly=assembleDifferentialPartitionsR195(partitionPlan,freshResults),payload={provider:'CLOUDFLARE_WORKERS_AI',model:MODEL,text:assembly?.text||'',executionTruth:'R195_R196_R197_MIXED_CARRIED_AND_ADAPTIVE_BOUNDED_FRESH_MODEL_PARTITIONS_NOT_FACTUAL_VERIFICATION',differential:{schema:assembly?.schema,revision:'R195',schedulerRevision:'R196',adaptiveRevision:'R197',partitionCount:partitionPlan.partitionCount,carried:assembly?.carried||0,executed:assembly?.executed||0,carryFraction:partitionPlan.carryFraction,recomputeFraction:partitionPlan.recomputeFraction,partitions:assembly?.partitions||[],freshProviderCalls:freshResults.length,scheduling:{turn:batch.plan?.turn,turnReason:batch.plan?.turnReason,logicalLanes:batch.plan?.logicalLanes,derivedMax:batch.plan?.derivedMax,requested:batch.plan?.requested,concurrency:batch.plan?.concurrency,hardMax:batch.plan?.hardMax,metrics:batch.metrics,adaptive:{scope:batch.adaptivePlan?.scope,controllerWindow:batch.adaptivePlan?.controllerWindow,effectiveConcurrency:batch.adaptivePlan?.effectiveConcurrency,operatorLimited:batch.adaptivePlan?.operatorLimited,turnDirection:batch.adaptivePlan?.turnDirection,feedbackReason:batch.adaptivePlan?.feedbackReason,prior:batch.adaptivePlan?.prior||null,record:batch.feedback?.recorded?{reason:batch.feedback.reason,controllerWindow:batch.feedback?.state?.controllerWindow,successStreak:batch.feedback?.state?.successStreak,lastOutcome:batch.feedback?.state?.lastOutcome,lastAdjustment:batch.feedback?.state?.lastAdjustment,sampleCount:batch.feedback?.state?.sampleCount}:null}}}};
   const saved=await persistResult(runtime,run.id,'WORKERS_AI',payload,'R195_R196_R197_DIFFERENTIAL_EXECUTION_RETURN_INTEGRITY_NOT_FACTUAL_TRUTH');await returned(runtime,run.id,'WORKERS_AI',saved.resultFingerprint,`Workers AI ${MODEL} · R195 ${freshResults.length} dirty partitions · R196 base ${batch.adaptivePlan?.basePlan?.concurrency||0} · R197 effective ${batch.adaptivePlan?.effectiveConcurrency||0}`);const verified=await verifyNormal(runtime,run.id,'WORKERS_AI',saved.resultFingerprint,`Workers AI ${MODEL} · R195/R196/R197 differential assembly`),verifiedRun=verified.run||await readRunR146(runtime,run.id);let partitionPublication=null;if(verified.ok&&verifiedRun?.state==='VERIFIED')partitionPublication=await publishDifferentialPartitionsR195(runtime,{run:verifiedRun,executorId:'WORKERS_AI',plan:partitionPlan,freshResults});return{ok:verified.ok,status:verified.status||200,run:verifiedRun,result:saved,differential:{plan:partitionPlan,scheduling:batch,assembly,publication:partitionPublication}};
  }
...
export async function dispatchRunR147(runtime,id,input={},callbacks={}){
 ...
 if(plan.executorId==='WORKERS_AI')out=await runWorkersAI(runtime,run,input,runtime.env||{},plan.temporalPerformance?.multiAxis);
...
export function manifestR147(){return{ok:true,schema:'OMEGA_UNIFIED_EXECUTOR_FABRIC_MANIFEST_R147',revision:R147_REVISION,executors:R147_EXECUTORS,laws:R147_LAWS,performance:manifestR185(),contentReuse:manifestR194(),differentialPartitionExecution:manifestR195(),boundedPartitionParallelism:manifestR196(),adaptivePartitionBackpressure:manifestR197(),runtimeStorageCompatibility:'R168',upstream:{operationContract:'R143',lifecycle:'R142',hybridProof:'R141',durableRun:'R146',runtimeAttestation:'R144',worldScar:'R145',swarm:'R125',federation:'R115',temporalPerformance:'R185',multiAxis:'R193',verifiedContentReuse:'R194',differentialPartitionExecution:'R195',boundedPartitionParallelism:'R196',adaptivePartitionBackpressure:'R197'},canonicalMutation:false,canonicalAdmissionAuthority:'R125',truthBoundary:'R147 unifies concrete execution transports around one R146 durable run. R168 keeps executor artifacts on the real Durable Object storage shape. R185 may optimize eligible executor priority from measured terminal history, R193 supplies independent refinement targets, R194 may eliminate exact whole-result recomputation, R195 may carry exact verified unchanged AI/SAI partitions, R196 defines the hard bounded current dirty-work scheduler, and R197 may reduce or cautiously recover its scope-local window only within the current R196/R193 ceiling. Dispatch acceptance never counts as execution success, whole-cache retrieval never claims fresh original-workload execution, partition carry never claims fresh provider execution or observation, and bounded/adaptive scheduling changes latency/throughput only. R147 still distinguishes selection, availability, dispatch, invocation, return, return-integrity verification, evidence/world-scar carry, convergence and CanonState admission. No executor, model, plugin, swarm quorum, federation packet, cached result, carried partition, scheduler turn, feedback direction, parallel batch, host return, performance trend or deployment attestation automatically mutates CanonState.'}}