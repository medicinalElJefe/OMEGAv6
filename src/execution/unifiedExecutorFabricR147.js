...
async function runWorkersAI(runtime,run,input,env,multiAxis=null){
 ...
 try{
  ...
  if(partitionPlan?.enabled){
   const freshResults=[];for(const p of partitionPlan.partitions||[]){if(p.action!=='RECOMPUTE')continue;const response=await env.AI.run(MODEL,{messages:[{role:'system',content:system},{role:'user',content:p.prompt}],max_tokens:Math.max(128,Math.min(1600,Number(input.maxTokens)||900)),temperature:Math.max(0,Math.min(1,Number.isFinite(Number(input.temperature))?Number(input.temperature):.2)),chat_template_kwargs:{enable_thinking:false}}),text=txt(modelText(response),16000);freshResults.push({id:p.id,result:{provider:'CLOUDFLARE_WORKERS_AI',model:MODEL,text,executionTruth:'RETURNED_MODEL_PARTITION_NOT_FACTUAL_VERIFICATION'}})}
   const assembly=assembleDifferentialPartitionsR195(partitionPlan,freshResults),payload={provider:'CLOUDFLARE_WORKERS_AI',model:MODEL,text:assembly?.text||'',executionTruth:'R195_MIXED_CARRIED_AND_FRESH_MODEL_PARTITIONS_NOT_FACTUAL_VERIFICATION',differential:{schema:assembly?.schema,revision:'R195',partitionCount:partitionPlan.partitionCount,carried:assembly?.carried||0,executed:assembly?.executed||0,carryFraction:partitionPlan.carryFraction,recomputeFraction:partitionPlan.recomputeFraction,partitions:assembly?.partitions||[],freshProviderCalls:freshResults.length}};
   const saved=await persistResult(runtime,run.id,'WORKERS_AI',payload,'R195_DIFFERENTIAL_EXECUTION_RETURN_INTEGRITY_NOT_FACTUAL_TRUTH');await returned(runtime,run.id,'WORKERS_AI',saved.resultFingerprint,`Workers AI ${MODEL} · R195 ${freshResults.length} fresh partitions`);const verified=await verifyNormal(runtime,run.id,'WORKERS_AI',saved.resultFingerprint,`Workers AI ${MODEL} · R195 differential assembly`),verifiedRun=verified.run||await readRunR146(runtime,run.id);let partitionPublication=null;if(verified.ok&&verifiedRun?.state==='VERIFIED')partitionPublication=await publishDifferentialPartitionsR195(runtime,{run:verifiedRun,executorId:'WORKERS_AI',plan:partitionPlan,freshResults});return{ok:verified.ok,status:verified.status||200,run:verifiedRun,result:saved,differential:{plan:partitionPlan,assembly,publication:partitionPublication}};
  }
...
export function manifestR147(){return{ok:true,schema:'OMEGA_UNIFIED_EXECUTOR_FABRIC_MANIFEST_R147',revision:R147_REVISION,executors:R147_EXECUTORS,laws:R147_LAWS,performance:manifestR185(),contentReuse:manifestR194(),differentialPartitionExecution:manifestR195(),runtimeStorageCompatibility:'R168',upstream:{operationContract:'R143',lifecycle:'R142',hybridProof:'R141',durableRun:'R146',runtimeAttestation:'R144',worldScar:'R145',swarm:'R125',federation:'R115',temporalPerformance:'R185',multiAxis:'R193',verifiedContentReuse:'R194',differentialPartitionExecution:'R195'},canonicalMutation:false,canonicalAdmissionAuthority:'R125',truthBoundary:'R147 unifies concrete execution transports around one R146 durable run. R168 keeps executor artifacts on the real Durable Object storage shape. R185 may optimize eligible executor priority from measured terminal history, R193 supplies independent refinement targets, R194 may eliminate exact whole-result recomputation, and R195 may carry exact verified unchanged AI/SAI partitions while invoking Workers AI only for changed partitions. Dispatch acceptance never counts as execution success, whole-cache retrieval never claims fresh original-workload execution, and partition carry never claims fresh provider execution or observation. R147 still distinguishes selection, availability, dispatch, invocation, return, return-integrity verification, evidence/world-scar carry, and CanonState admission. No executor, model, plugin, swarm quorum, federation packet, cached result, carried partition, host return, performance trend or deployment attestation automatically mutates CanonState.'}}