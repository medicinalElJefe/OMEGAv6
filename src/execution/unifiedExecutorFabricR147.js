...
import {executeBoundedPartitionsR196,manifestR196} from './boundedPartitionSchedulerR196.js';
...
export async function executorDirectoryR147(runtime,env=runtime?.env||{}){
 ...
 return{
  ...
  performance:{revision:'R185',loop:'PREDICT_CARRY_CORRECT_REALLOCATE',adaptiveSelection:'TERMINAL_HISTORY_MATURED_ONLY',verifiedReuse:'R194_EXACT_CONTENT_ONLY',differentialPartitionExecution:'R195_AI_SAI_EXACT_PARTITION_CARRY',boundedPartitionParallelism:'R196_DIRTY_ONLY_DYNAMIC_R193_CEILING'},
  runtimeStorageCompatibility:'R168',
  canonicalMutation:false,canonicalAdmissionAuthority:'R125',
  truthBoundary:'Executor availability means a concrete transport/binding is presently available. CONTENT_CACHE additionally requires an exact R194 verified-content hit before selection. R195 can reduce a selected Workers AI execution to changed partitions only when explicit partition content is exact-key comparable. R196 may then execute only that current dirty set with bounded, freshly replanned concurrency derived from R193. Availability, carry, STAY/TURN scheduling and parallelism are not invocation, return, factual truth, solver validity, PC heartbeat proof, or CanonState admission.'
 };
}
...
async function runWorkersAI(runtime,run,input,env,multiAxis=null){
 ...
 try{
  ...
  if(partitionPlan?.enabled){
   ...
   const batch=await executeBoundedPartitionsR196(dirty,async p=>{const response=await env.AI.run(MODEL,{messages:[{role:'system',content:system},{role:'user',content:p.prompt}],max_tokens:Math.max(128,Math.min(1600,Number(input.maxTokens)||900)),temperature:Math.max(0,Math.min(1,Number.isFinite(Number(input.temperature))?Number(input.temperature):.2)),chat_template_kwargs:{enable_thinking:false}}),text=txt(modelText(response),16000);return{id:p.id,result:{provider:'CLOUDFLARE_WORKERS_AI',model:MODEL,text,executionTruth:'RETURNED_MODEL_PARTITION_NOT_FACTUAL_VERIFICATION'}}},{multiAxis,input});
   if(!batch.ok){const failed=await transitionRunR146(runtime,run.id,{state:'FAILED',reason:'R196 bounded dirty-partition execution failed',evidence:{proofRef:'R196_BOUNDED_PARTITION_BATCH_FAILURE',providerReceipt:txt(batch?.failure?.message||batch?.code,1000)}});return{ok:false,status:batch.status||502,code:batch.code||'R196_PARTITION_EXECUTION_FAILED',run:failed.run||await readRunR146(runtime,run.id),differential:{plan:partitionPlan,scheduling:batch},truthBoundary:'R196 failed closed before R195 reassembly, R147 return verification, R195 publication or R194 whole-result publication. Already-running bounded calls may have settled but their partial bytes are not admitted to the verified partition cache.'};}
   const freshResults=batch.results||[],assembly=assembleDifferentialPartitionsR195(partitionPlan,freshResults),payload={provider:'CLOUDFLARE_WORKERS_AI',model:MODEL,text:assembly?.text||'',executionTruth:'R195_R196_MIXED_CARRIED_AND_BOUNDED_FRESH_MODEL_PARTITIONS_NOT_FACTUAL_VERIFICATION',differential:{schema:assembly?.schema,revision:'R195',schedulerRevision:'R196',partitionCount:partitionPlan.partitionCount,carried:assembly?.carried||0,executed:assembly?.executed||0,carryFraction:partitionPlan.carryFraction,recomputeFraction:partitionPlan.recomputeFraction,partitions:assembly?.partitions||[],freshProviderCalls:freshResults.length,scheduling:{turn:batch.plan?.turn,turnReason:batch.plan?.turnReason,logicalLanes:batch.plan?.logicalLanes,derivedMax:batch.plan?.derivedMax,requested:batch.plan?.requested,concurrency:batch.plan?.concurrency,hardMax:batch.plan?.hardMax,metrics:batch.metrics}}};
   const saved=await persistResult(runtime,run.id,'WORKERS_AI',payload,'R195_R196_DIFFERENTIAL_EXECUTION_RETURN_INTEGRITY_NOT_FACTUAL_TRUTH');await returned(runtime,run.id,'WORKERS_AI',saved.resultFingerprint,`Workers AI ${MODEL} · R195 ${freshResults.length} dirty partitions · R196 concurrency ${batch.plan?.concurrency||0}`);const verified=await verifyNormal(runtime,run.id,'WORKERS_AI',saved.resultFingerprint,`Workers AI ${MODEL} · R195/R196 differential assembly`),verifiedRun=verified.run||await readRunR146(runtime,run.id);let partitionPublication=null;if(verified.ok&&verifiedRun?.state==='VERIFIED')partitionPublication=await publishDifferentialPartitionsR195(runtime,{run:verifiedRun,executorId:'WORKERS_AI',plan:partitionPlan,freshResults});return{ok:verified.ok,status:verified.status||200,run:verifiedRun,result:saved,differential:{plan:partitionPlan,scheduling:batch,assembly,publication:partitionPublication}};
  }
...
export function manifestR147(){return{ok:true,schema:'OMEGA_UNIFIED_EXECUTOR_FABRIC_MANIFEST_R147',revision:R147_REVISION,executors:R147_EXECUTORS,laws:R147_LAWS,performance:manifestR185(),contentReuse:manifestR194(),differentialPartitionExecution:manifestR195(),boundedPartitionParallelism:manifestR196(),runtimeStorageCompatibility:'R168',upstream:{operationContract:'R143',lifecycle:'R142',hybridProof:'R141',durableRun:'R146',runtimeAttestation:'R144',worldScar:'R145',swarm:'R125',federation:'R115',temporalPerformance:'R185',multiAxis:'R193',verifiedContentReuse:'R194',differentialPartitionExecution:'R195',boundedPartitionParallelism:'R196'},canonicalMutation:false,canonicalAdmissionAuthority:'R125',truthBoundary:'R147 unifies concrete execution transports around one R146 durable run. R168 keeps executor artifacts on the real Durable Object storage shape. R185 may optimize eligible executor priority from measured terminal history, R193 supplies independent refinement targets, R194 may eliminate exact whole-result recomputation, R195 may carry exact verified unchanged AI/SAI partitions, and R196 may dynamically STAY or TURN only the remaining dirty partitions into bounded provider execution under the current R193 compute ceiling. Dispatch acceptance never counts as execution success, whole-cache retrieval never claims fresh original-workload execution, partition carry never claims fresh provider execution or observation, and bounded parallelism changes latency/throughput only. R147 still distinguishes selection, availability, dispatch, invocation, return, return-integrity verification, evidence/world-scar carry, convergence and CanonState admission. No executor, model, plugin, swarm quorum, federation packet, cached result, carried partition, scheduler turn, parallel batch, host return, performance trend or deployment attestation automatically mutates CanonState.'}}