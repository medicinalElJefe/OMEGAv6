...
import {manifestR185,planTemporalPerformanceR185,recordTemporalPerformanceR185} from './temporalRelativityPerformanceR185.js';
...
async function storagePut(runtime,key,value){await runtime.state.storage.put(key,value);return value}
async function binding(runtime,runId){return runtime.state.storage.get(BIND_PREFIX+runId)}
async function result(runtime,runId){return runtime.state.storage.get(RESULT_PREFIX+runId)}
async function artifactLink(runtime,kind,id){return runtime.state.storage.get(`${ARTIFACT_PREFIX}${kind}:${id}`)}
...
export async function executorDirectoryR147(runtime,env=runtime?.env||{}){
 ...
 return{
  ...
  performance:{revision:'R185',loop:'PREDICT_CARRY_CORRECT_REALLOCATE',adaptiveSelection:'HISTORY_MATURED_ONLY'},
...
export async function selectExecutorR147(runtime,run,input={}){
 ...
 return{schema:'OMEGA_EXECUTOR_PLAN_R147',runId:run.id,domain,executorId,eligible,state,strategy:strategy||'AUTO',temporalPerformance,canonicalMutation:false,truthBoundary:'Selection binds an eligible executor to a durable run. R185 may reorder only eligible AVAILABLE executors after measured history matures; selection itself is not invocation or execution proof.'};
}
...
export async function dispatchRunR147(runtime,id,input={},callbacks={}){
 ...
 let sample=null;try{sample=await recordTemporalPerformanceR185(runtime,{run:out?.run||run,executorId:plan.executorId,ok:out?.ok===true,status:out?.status||0,latencyMs:Date.now()-started,phase:out?.status===202?'DISPATCH_ACCEPTED':'RETURN_OR_TERMINAL',verified:out?.run?.state==='VERIFIED',input})}catch{}
...
export async function syncHybridReturnR147(runtime,job,closure){const link=job?.id?await artifactLink(runtime,'HYBRID_JOB',job.id):null;if(!link?.runId)return null;let run=await readRunR146(runtime,link.runId);if(!run)return null;const bind=await binding(runtime,run.id),fingerprint=txt(job?.returnPacket?.resultFingerprint||job?.resultFingerprint,256)||await sha({jobId:job.id,status:job.status,returnPacket:job.returnPacket||null});const saved=await persistResult(runtime,run.id,'HYBRID_HOST',{jobId:job.id,status:job.status,returnPacket:job.returnPacket||null,closure:closure||null},'R141_EXACT_HOST_RETURN_REQUIRED');if(run.state==='AVAILABLE'){const inv=await transitionRunR146(runtime,run.id,{state:'INVOKED',reason:'Hybrid result arrived after an unobserved claim; host return proves the job was executed but not yet verified',evidence:{proofRef:'R147_HYBRID_RETURN_IMPLIES_PRIOR_INVOCATION'}});run=inv.run||run}if(run.state==='INVOKED'){const ret=await transitionRunR146(runtime,run.id,{state:'RETURNED',reason:`Hybrid host returned linked job ${job.id}`,evidence:{proofRef:'R147_HYBRID_RETURN',resultFingerprint:fingerprint,providerReceipt:saved.resultFingerprint}});run=ret.run||run}if(run.state==='RETURNED'&&closure?.state==='VERIFIED_EXECUTION_RETURN'&&closure?.fingerprint?.verified===true&&closure?.fingerprint?.digestMatch===true&&closure?.fingerprint?.semanticMatch===true){const ver=await transitionRunR146(runtime,run.id,{state:'VERIFIED',reason:'R141 exact payload digest and semantic closure verified the linked Hybrid return',evidence:{proofRef:'R141_EXACT_PAYLOAD_SHA_SEMANTIC_EQUALITY',resultFingerprint:fingerprint,r141Closure:closure,providerReceipt:saved.resultFingerprint}});run=ver.run||run}try{await recordTemporalPerformanceR185(runtime,{run,executorId:'HYBRID_HOST',ok:['RETURNED','VERIFIED'].includes(run.state),status:200,latencyMs:bind?.createdAt?Date.now()-Number(bind.createdAt):0,phase:'ASYNC_HOST_RETURN',verified:run.state==='VERIFIED'})}catch{}return run}

export async function pollRunR147(runtime,id){let run=await readRunR146(runtime,id);if(!run)return{ok:false,status:404,code:'R147_RUN_NOT_FOUND'};const bind=await binding(runtime,id);if(!bind)return{ok:true,status:200,run,binding:null,result:await result(runtime,id)};if(bind.kind==='SWARM_MISSION'&&run.state==='INVOKED'&&runtime.env?.OMEGA_SWARM_AUTONOMIC){const root=runtime.env.OMEGA_SWARM_AUTONOMIC.get(runtime.env.OMEGA_SWARM_AUTONOMIC.idFromName('omega-autonomic-root-r125')),response=await root.fetch(new Request(`https://autonomic.internal/missions/${encodeURIComponent(bind.artifactId)}`)),mission=await jsonBody(response);if(response.ok&&['COMPLETE','FAILED','CANCELLED'].includes(mission?.status)){if(mission.status==='COMPLETE'){const payload={mission,authority:'AUTONOMIC_RECEIPT_NOT_CANON'},saved=await persistResult(runtime,id,'AUTONOMIC_SWARM',payload,'SWARM_EXECUTION_RECEIPT_INTEGRITY_NOT_TRUTH_CONSENSUS'),fingerprint=mission?.checkpointSha256||mission?.receipt?.merkleRoot||saved.resultFingerprint;const ret=await returned(runtime,id,'AUTONOMIC_SWARM',fingerprint,saved.resultFingerprint);run=ret.run||run;if(run.state==='RETURNED'){const ver=await verifyNormal(runtime,id,'AUTONOMIC_SWARM',fingerprint,saved.resultFingerprint);run=ver.run||run}}else{const failed=await transitionRunR146(runtime,id,{state:'FAILED',reason:`Autonomic swarm mission ${mission.status}`,evidence:{proofRef:'R147_SWARM_TERMINAL_RETURN',providerReceipt:mission?.checkpointSha256||null}});run=failed.run||run}try{await recordTemporalPerformanceR185(runtime,{run,executorId:'AUTONOMIC_SWARM',ok:run.state==='VERIFIED',status:response.status||200,latencyMs:bind?.createdAt?Date.now()-Number(bind.createdAt):0,phase:'ASYNC_SWARM_TERMINAL',verified:run.state==='VERIFIED'})}catch{}}return{ok:true,status:200,run,binding:bind,mission,result:await result(runtime,id)}}return{ok:true,status:200,run,binding:bind,result:await result(runtime,id)}}
export async function readResultR147(runtime,id){const run=await readRunR146(runtime,id);if(!run)return null;return{schema:'OMEGA_EXECUTOR_RESULT_VIEW_R147',revision:R147_REVISION,runId:id,state:run.state,binding:await binding(runtime,id),result:await result(runtime,id),canonicalMutation:false,canonicalAdmissionAuthority:'R125'}}
export function manifestR147(){return{ok:true,schema:'OMEGA_UNIFIED_EXECUTOR_FABRIC_MANIFEST_R147',revision:R147_REVISION,executors:R147_EXECUTORS,laws:R147_LAWS,performance:manifestR185(),upstream:{operationContract:'R143',lifecycle:'R142',hybridProof:'R141',durableRun:'R146',runtimeAttestation:'R144',worldScar:'R145',swarm:'R125',federation:'R115',temporalPerformance:'R185'},canonicalMutation:false,canonicalAdmissionAuthority:'R125',truthBoundary:'R147 unifies concrete execution transports around one R146 durable run. R185 may optimize eligible executor priority from measured history, but R147 still distinguishes selection, availability, dispatch, invocation, return, return-integrity verification, evidence/world-scar carry, and CanonState admission. No executor, model, plugin, swarm quorum, federation packet, host return, performance trend or deployment attestation automatically mutates CanonState.'}}