...
function verifiedReturn(closure){return Boolean(
...
function admissionState(closure,replay){
 ...
 if(!verifiedReturn(closure))return'HELD_R141_RETURN_NOT_VERIFIED';
...
export async function compileSovereignExecutionConvergenceR159(runtime,job,closure,replayReceipt=null){
 const jobId=safeId(job?.id||closure?.jobId),verified=verifiedReturn(closure),replayOk=replayVerified(replayReceipt),candidateState=admissionState(closure,replayReceipt),deviceId=safeId(closure?.hybridProofAtClose?.deviceId||job?.targetDeviceId,'device-unknown');
 const core={
  ...
  stages:[
   ...
   stage('EXACT_RETURN_PROOF',verified?'VERIFIED':'HELD',closure?.fingerprint?.supplied||null),
  ...
  admissionCandidate:{
   ...
   invariants:{exactR141ReturnVerified:verified,deterministicReplayVerified:replayOk,r134ScarBound:Boolean(closure?.finalHeadSha256),r136WorldFrameBound:closure?.livingWorldFrame?.revision==='R136'},
  ...
  truthBoundary:'R159 converges authenticated host availability, job invocation/return, R141 exact-payload proof, R134 scar continuity, R136 living-world evidence, deterministic replay and an R125 admission candidate into one durable evidence object. It never turns heartbeat, execution, replay, world projection, or executor state into CanonState automatically.'
 };
...
export function manifestR159(){return{ok:true,schema:R159_SCHEMA,revision:R159_REVISION,laws:R159_LAWS,canonicalMutation:false,canonicalAdmissionAuthority:'R125',truthBoundary:'R159 is a durable convergence/admission-candidate layer over existing proven authorities. It does not replace R127, R132, R140, R141, R142, R143, R146, R147, R134, R136 or R125.'}}
