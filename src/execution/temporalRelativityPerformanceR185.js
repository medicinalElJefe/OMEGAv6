...
function performanceHint(run,input={}){
 const explicit=input?.performanceHint&&typeof input.performanceHint==='object'?input.performanceHint:{},r154=run?.metadata?.relativeCapacityR154&&typeof run.metadata.relativeCapacityR154==='object'?run.metadata.relativeCapacityR154:{},p=r154.pressures||{};
 return{
  ...
  source:'R154_METADATA_PLUS_OPTIONAL_EXECUTION_HINT'
 };
}
...
export async function planTemporalPerformanceR185(runtime,{run,directory,eligible=[],defaultExecutorId=null,input={}}={}){
 ...
 else if(best&&best.id!==defaultExecutorId&&best.maturity>=.25&&best.score>finite(defaultRow?.score)+.06){recommendedExecutorId=best.id;recommendationReason='MEASURED_HISTORY_OUTPERFORMS_DEFAULT_WITH_MARGIN'}
 ...
 const prewarm=candidates.filter(x=>x.available).slice(0,2).map(x=>({executorId:x.id,score:x.score,reason:'PREPARE_ONLY_NOT_INVOCATION'})),totalSamples=candidates.reduce((n,x)=>n+x.count,0),historyMaturity=cl(totalSamples/24);
 return{ok:true,schema:R185_SCHEMA,revision:R185_REVISION,routeId:routeIdentity(run),runId:run?.id||null,mode,temporal:{currentPressure,velocity,acceleration,predictedPressure,targetTemporalHz},work:{carryFraction,recomputeFraction,workingSetResolution,prewarm},history:{samples:totalSamples,maturity:historyMaturity},selection:{defaultExecutorId,recommendedExecutorId,recommendationReason,adaptiveApplied:recommendedExecutorId!==defaultExecutorId&&!explicitExecutor&&!explicitStrategy,candidates},hint,canonicalMutation:false,canonicalAdmissionAuthority:'R125',truthBoundary:'R185 converts measured execution history plus R154 motion/residual pressure into scheduling and executor-priority advice. Carry, prewarm, predicted pressure and adaptive executor choice are performance operations only; they do not prove observation, invocation, return, scientific validity, or CanonState admission.'};
}

export async function recordTemporalPerformanceR185(runtime,{run,executorId,ok=false,status=0,latencyMs=0,phase='DISPATCH',verified=false,input={}}={}){
 if(!run||!executorId)return null;const storage=runtimeStorageR168(runtime),key=routeKey(run),prior=await storage.get(key)||{schema:R185_SCHEMA,revision:R185_REVISION,routeId:routeIdentity(run),route:{},executors:{},updatedAt:0,canonicalMutation:false,canonicalAdmissionAuthority:'R125'},hint=performanceHint(run,input),pressure=pressureOf(hint),routePrior=prior.route||{},velocity=pressure-finite(routePrior.pressure,pressure),acceleration=velocity-finite(routePrior.velocity,0),id=txt(executorId,64).toUpperCase(),old=prior.executors?.[id]||{},latency=Math.max(0,Math.min(86400000,finite(latencyMs))),oldLatency=finite(old.latencyEwmaMs,latency),newLatency=ewma(old.latencyEwmaMs,latency),latencyVelocity=newLatency-oldLatency,latencyAcceleration=latencyVelocity-finite(old.latencyVelocityMs,0),success=ok?1:0,verifiedValue=verified?1:0,row={executorId:id,count:finite(old.count)+1,successEwma:ewma(old.successEwma,success),verifiedEwma:ewma(old.verifiedEwma,verifiedValue),latencyEwmaMs:newLatency,latencyVelocityMs:latencyVelocity,latencyAccelerationMs:latencyAcceleration,lastLatencyMs:latency,lastStatus:finite(status),lastPhase:txt(phase,40),lastRunId:run.id,lastAt:Date.now()};
 const next={...prior,route:{pressure,velocity,acceleration,predictedPressure:cl(pressure+.46*velocity+.18*acceleration),lastHint:hint,lastAt:Date.now()},executors:{...(prior.executors||{}),[id]:row},updatedAt:Date.now(),canonicalMutation:false,canonicalAdmissionAuthority:'R125'};await storage.put(key,next);return{schema:R185_SCHEMA,revision:R185_REVISION,routeId:next.routeId,route:next.route,executor:row,canonicalMutation:false,canonicalAdmissionAuthority:'R125'};
}

export function manifestR185(){return{ok:true,schema:'OMEGA_TEMPORAL_RELATIVITY_PERFORMANCE_MANIFEST_R185',revision:R185_REVISION,laws:R185_LAWS,loop:['PREDICT','CARRY','CORRECT','REALLOCATE'],inputs:['R146 durable run metadata','R154 motion/residual/capacity envelope','R147 executor availability','measured dispatch/return latency','measured success/failure','verified-return state'],outputs:['predicted change pressure','carry/recompute fraction','adaptive working-set resolution','target temporal cadence','prewarm candidates','history-matured executor recommendation'],authority:{executorDispatch:'R147',durableHistory:'R146',relativeCapacity:'R154',canonicalAdmission:'R125'},canonicalMutation:false,canonicalAdmissionAuthority:'R125',truthBoundary:'R185 is a performance adaptation fabric. It learns execution cost and temporal trend from first-hand runtime history and may reprioritize eligible executors after sufficient samples. It cannot authorize execution, invent observations, change R141/R159 proof closure, validate scientific claims, or admit CanonState.'}}
