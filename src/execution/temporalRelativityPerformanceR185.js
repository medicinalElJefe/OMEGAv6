...
import {compileMultiAxisRelativityR193,manifestR193} from './multiAxisRelativityCompilerR193.js';
...
export const R185_LAWS=Object.freeze([
 ...
 'R193_MULTI_AXIS_REFINEMENT_MAY_ADVANCE_SCALE_TIME_FIDELITY_FRAME_COMPUTE_PROOF_AND_MODE_COVERAGE_CONCURRENTLY',
...
export async function planTemporalPerformanceR185(runtime,{run,directory,eligible=[],defaultExecutorId=null,input={}}={}){
 ...
 const prewarm=candidates.filter(x=>x.available).slice(0,2).map(x=>({executorId:x.id,score:x.score,reason:'PREPARE_ONLY_NOT_INVOCATION'})),totalSamples=candidates.reduce((n,x)=>n+x.count,0),dispatchSamples=candidates.reduce((n,x)=>n+x.dispatchCount,0),historyMaturity=cl(totalSamples/24),multiAxis=compileMultiAxisRelativityR193({run,hint,currentPressure,predictedPressure,input,history:{maturity:historyMaturity}}),refinedWorkingSetResolution=Math.max(workingSetResolution,multiAxis.axes.address.targetResolution),refinedTargetTemporalHz=Math.max(targetTemporalHz,multiAxis.axes.time.targetHz);
 return{ok:true,schema:R185_SCHEMA,revision:R185_REVISION,routeId:routeIdentity(run),runId:run?.id||null,mode,temporal:{currentPressure,velocity,acceleration,predictedPressure,targetTemporalHz:refinedTargetTemporalHz},work:{carryFraction,recomputeFraction,workingSetResolution:refinedWorkingSetResolution,prewarm},multiAxis,history:{terminalSamples:totalSamples,dispatchSamples,maturity:historyMaturity},selection:{defaultExecutorId,recommendedExecutorId,recommendationReason,adaptiveApplied:recommendedExecutorId!==defaultExecutorId&&!explicitExecutor&&!explicitStrategy,candidates},hint,canonicalMutation:false,canonicalAdmissionAuthority:'R125',truthBoundary:'R185 converts measured terminal execution history plus R154 motion/residual pressure into scheduling and executor-priority advice. R193 independently refines address scale, time, fidelity, reference frame, compute, proof depth and lawful mode coverage over that same plan. Dispatch acceptance is tracked separately and never matures reliability. Carry, prewarm, predicted pressure, multi-axis refinement and adaptive executor choice are performance operations only; they do not prove observation, invocation, return, scientific validity, or CanonState admission.'};
}
...
export function manifestR185(){return{ok:true,schema:'OMEGA_TEMPORAL_RELATIVITY_PERFORMANCE_MANIFEST_R185',revision:R185_REVISION,laws:R185_LAWS,loop:['PREDICT','CARRY','CORRECT','REALLOCATE'],inputs:['R146 durable run metadata','R154 motion/residual/capacity envelope','R147 executor availability','measured dispatch latency','measured terminal return latency','measured terminal success/failure','verified-return state'],outputs:['predicted change pressure','carry/recompute fraction','adaptive working-set resolution','target temporal cadence','prewarm candidates','history-matured executor recommendation','R193 multi-axis refinement plan'],multiAxis:manifestR193(),historyPolicy:{dispatchAcceptance:'TRACKED_SEPARATELY_NOT_SUCCESS',terminalOutcome:'REQUIRED_TO_MATURE_RELIABILITY',verification:'SEPARATE_RATE'},authority:{executorDispatch:'R147',durableHistory:'R146',relativeCapacity:'R154',multiAxisRefinement:'R193',canonicalAdmission:'R125'},canonicalMutation:false,canonicalAdmissionAuthority:'R125',truthBoundary:'R185 is a performance adaptation fabric. It learns execution cost and temporal trend from first-hand runtime history and may reprioritize eligible executors after sufficient terminal samples. R193 adds simultaneous independent refinement budgets without becoming an executor or truth authority. Dispatch acceptance never counts as execution success. It cannot authorize execution, invent observations, change R141/R159 proof closure, validate scientific claims, or admit CanonState.'}}
