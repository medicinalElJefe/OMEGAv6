...
export const R197_LAWS=Object.freeze([
 ...
 'FAILURE_DECREASE_IS_BOUNDED_BY_OBSERVED_ACTIVE_WORK_NOT_UNEXECUTED_CAPACITY',
 'TWO_CONSECUTIVE_HEALTHY_UNCAPPED_BATCHES_ALLOW_ONE_ADDITIVE_RECOVERY_STEP',
 'RECOVERY_CREDIT_REQUIRES_CONSECUTIVE_UNCAPPED_BATCHES_AND_RESETS_AT_CEILING_OR_OPERATOR_CAP',
...
export function evolvePartitionFeedbackR197(prior,{ok=false,started=0,completed=0,durationMs=0,observedMaxActive=0,baseCeiling=1,operatorLimited=false,scope='unknown'}={}){
 ...
 if(!ok){const observed=int(observedMaxActive),failureBase=observed>0?Math.max(1,Math.min(boundedOld,observed)):boundedOld;controllerWindow=Math.max(1,Math.floor(failureBase/2));successStreak=0;lastAdjustment=controllerWindow<boundedOld?'DOWN':'NONE'}
 else if(operatorLimited){successStreak=0;lastAdjustment=oldWindow>ceiling?'CLAMP':'NONE'}
 else{
  successStreak+=1;
  if(oldWindow>ceiling){controllerWindow=ceiling;successStreak=0;lastAdjustment='CLAMP'}
  else if(boundedOld>=ceiling){controllerWindow=ceiling;successStreak=0;lastAdjustment='NONE'}
  else if(successStreak>=2){controllerWindow=Math.min(ceiling,boundedOld+1);successStreak=0;lastAdjustment='UP'}
 }
 ...
 return{state:next,recorded:true,reason:ok?(operatorLimited?'SUCCESS_OPERATOR_LIMITED_NO_RECOVERY_CREDIT':lastAdjustment==='UP'?'HEALTHY_STREAK_ADDITIVE_RECOVERY':lastAdjustment==='CLAMP'?'CURRENT_CEILING_CLAMPED':'HEALTHY_SAMPLE_ACCUMULATED'):'FAILURE_MULTIPLICATIVE_DECREASE'};
}
