...
async function sha256R243(value){
 const bytes=new TextEncoder().encode(typeof value==='string'?value:JSON.stringify(value));
 const digest=await crypto.subtle.digest('SHA-256',bytes);
 return [...new Uint8Array(digest)].map(x=>x.toString(16).padStart(2,'0')).join('');
}

async function boundInitialDiscoveryR243(runtime,mission){
 ...
 const step={...index,id:'S01',path:'.',maxResults:4000,discoveryOnly:true,label:'R243 bounded project discovery before any project tree hash'};
 const rewritten={...job,steps:[step],r243InitialDiscoveryBounded:true,r243RemovedRootHash:true};
 rewritten.inputFingerprint=await sha256R243({steps:rewritten.steps,targetDeviceId:rewritten.targetDeviceId,projectPath:rewritten.projectPath||'.'});
 jobs=jobs.map(j=>j.id===rewritten.id?rewritten:j);await runtime.put('jobs',jobs);
 const next={...mission,currentJob:rewritten,executionMotionRevision:'R243',initialDiscoveryPolicy:'INDEX_PROJECT_FIRST_HASH_SELECTED_PROJECT_AFTER_DISCOVERY',updatedAt:Date.now()};
 let missions=await runtime.get('missions',[]);missions=missions.map(m=>m.id===next.id?next:m);await runtime.put('missions',missions);
 await runtime.event('R243_INITIAL_DISCOVERY_BOUNDED',`Mission ${next.id} will discover the project before hashing its tree.`,{missionId:next.id,jobId:rewritten.id,targetDeviceId:rewritten.targetDeviceId,removedRootHash:true});
 return next;
}
...
export async function tagSovereignMissionR152(runtime,requestBody,mission){
 ...
 return boundInitialDiscoveryR243(runtime,tagged);
}
