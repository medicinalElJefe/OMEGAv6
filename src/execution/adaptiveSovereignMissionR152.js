...
export const R152_MAX_CYCLES=12;
...
export async function tagSovereignMissionR152(runtime,requestBody,mission){
 if(requestBody?.draft?.schema!==R152_SOURCE_SCHEMA||!mission?.id)return mission;let missions=await runtime.get('missions',[]),stored=missions.find(m=>m.id===mission.id)||mission,projectPath=safeRel(requestBody?.draft?.projectPath)||'.',stage=projectPath==='.'?'DISCOVERY':'BUILD_VERIFY',requestedCycles=Math.max(2,Number(requestBody?.maxCycles||stored.maxCycles||R152_MAX_CYCLES));const tagged={...stored,schema:R152_MISSION_SCHEMA,sourceSchema:R152_SOURCE_SCHEMA,adaptiveRevision:R152_REVISION,maxCycles:Math.min(R152_MAX_CYCLES,requestedCycles),stage,projectPath,repairAttempts:0,history:[],truthLaw:R152_LAW,updatedAt:now()};missions=missions.map(m=>m.id===tagged.id?tagged:m);await runtime.put('missions',missions);await runtime.event('R152_MISSION_TAGGED',`Sovereign mission ${tagged.id} bound to proof-driven adaptive continuation.`,{missionId:tagged.id,stage,maxCycles:tagged.maxCycles});return tagged;
}
