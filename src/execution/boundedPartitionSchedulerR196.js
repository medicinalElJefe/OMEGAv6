...
export function planPartitionConcurrencyR196({multiAxis=null,input={},dirtyCount=0}={}){
 ...
 return{ok:true,schema:'OMEGA_PARTITION_CONCURRENCY_PLAN_R196',revision:R196_REVISION,dirtyCount:dirty,logicalLanes,derivedMax,requested,concurrency,hardMax:R196_HARD_CONCURRENCY_MAX,requestedRaisedAboveDerived:Boolean(requested!==null&&requested>derivedMax),source:'R193_COMPUTE_AXIS_WITH_OPTIONAL_OPERATOR_LOWER_CAP',canonicalMutation:false,canonicalAdmissionAuthority:'R125',truthBoundary:'R196 concurrency is a bounded scheduling parameter derived from the R193 compute axis. More parallel calls can reduce wall-clock latency but do not increase evidence quality, provider truth, execution authority, proof depth, or CanonState authority.'};
}
...
export async function executeBoundedPartitionsR196(items,worker,{multiAxis=null,input={}}={}){
 ...
 if(!items.length)return{ok:true,schema:R196_SCHEMA,revision:R196_REVISION,plan,results:[],metrics:{started:0,completed:0,unscheduled:0,observedMaxActive:0,durationMs:0},canonicalMutation:false,canonicalAdmissionAuthority:'R125'};
 ...
 const runners=[];for(let i=0;i<plan.concurrency;i+=1)runners.push(loop());await Promise.all(runners);const durationMs=Math.max(0,Date.now()-startedAt),metrics={started,completed,unscheduled:Math.max(0,items.length-started),observedMaxActive,durationMs};
 ...
 return{ok:true,status:200,schema:R196_SCHEMA,revision:R196_REVISION,plan,metrics,results,canonicalMutation:false,canonicalAdmissionAuthority:'R125',truthBoundary:'R196 completed bounded dirty-partition scheduling. Result order is input partition order even when provider calls finish out of order. Parallel completion is execution performance only and does not change proof or CanonState authority.'};
}

export function manifestR196(){return{ok:true,schema:'OMEGA_BOUNDED_PARTITION_SCHEDULER_MANIFEST_R196',revision:R196_REVISION,laws:R196_LAWS,hardConcurrencyMax:R196_HARD_CONCURRENCY_MAX,derivedConcurrency:[{logicalLanes:'1',max:1},{logicalLanes:'2-12',max:2},{logicalLanes:'13-144',max:4},{logicalLanes:'145-1728',max:8},{logicalLanes:'>1728',max:12}],inputs:['R195 dirty partitions only','R193 compute logical-lane target','optional operator partitionConcurrency lower cap'],outputs:['bounded concurrency plan','ordered results','observed active-call metrics','fail-closed batch result'],failurePolicy:'FIRST_FAILURE_STOPS_NEW_ASSIGNMENTS_ALREADY_RUNNING_BOUNDED_CALLS_SETTLE_NO_R195_PUBLICATION',authority:{multiAxis:'R193',differentialCarry:'R195',dispatch:'R147',history:'R146',admission:'R125'},canonicalMutation:false,canonicalAdmissionAuthority:'R125',truthBoundary:'R196 is a latency/throughput scheduler over already-authorized dirty work. It cannot authorize execution, fabricate provider returns, upgrade carried evidence, establish factual truth, validate a solver, or admit CanonState.'}};
