...
export async function dispatchAuthorizedLivingWorldMissionR180(runtime,authorization={},input={},callbacks={}){
 ...
 for(const item of preflight){
  if(item.hostConfirmationRequired&&input.confirmHostExecution!==true){results.push({runId:item.runId,state:item.run.state,status:'HELD_HOST_EXECUTION_CONFIRMATION_REQUIRED',executionInvoked:false,returned:false,verified:false,canonicalMutation:false});continue}
  const stepInput=(input.dispatchByRun&&typeof input.dispatchByRun==='object'&&input.dispatchByRun[item.runId])||{},dispatchInput={...stable(stepInput)};if(item.hostConfirmationRequired)dispatchInput.confirmed=true;
  const dispatched=await dispatchRunR147(runtime,item.runId,dispatchInput,callbacks),current=dispatched?.run||await readRunR146(runtime,item.runId),replay=current?await replayRunR146(runtime,item.runId):null,poll=current&&['INVOKED','RETURNED'].includes(current.state)?await pollRunR147(runtime,item.runId).catch(()=>null):null,finalRun=poll?.run||current;
