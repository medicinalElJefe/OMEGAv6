...
function exactBasis(run,multiAxis){
 ...
 if(!reuse?.eligible||!reuse?.requiresExactKeyMatch||!reuse?.requiresSha256Binding||!reuse?.keyBasis)return{ok:false,reason:'R193_REUSE_NOT_ELIGIBLE',domain};
...
export async function reuseKeyR194({run,multiAxis}={}){
 const exact=exactBasis(run,multiAxis);if(!exact.ok)return{...exact,keySha256:null};
...
export async function lookupVerifiedReuseR194(runtime,{run,multiAxis}={}){
 ...
 return{hit:true,reason:'EXACT_VERIFIED_CONTENT_HIT',...keyed,entry,sourceRun,sourceResult,truthBoundary:'R194 hit proves an exact stored-key match to bytes captured from an R146 VERIFIED source run. It does not prove a fresh model/provider/solver/device invocation, a fresh observation, scientific validity, or CanonState admission.'};
}
...
export async function publishVerifiedReuseR194(runtime,{run,executorId,result,multiAxis}={}){
 ...
 const sourceResult=stable(result),sourceResultStorageSha256=await sha(sourceResult),entry={
  ...
  publishedAt:Date.now(),canonicalMutation:false,canonicalAdmissionAuthority:'R125'
 };
 entry.entrySha256=await sha(entry);await storage(runtime).put(CACHE_PREFIX+keyed.keySha256,entry);
 return{published:true,reusedExistingEntry:false,keySha256:keyed.keySha256,entry,truthBoundary:'Publication records a verified computation result under an exact R193-derived content key. Publication is not CanonState admission and does not convert result integrity into factual truth.'};
}

export function manifestR194(){return{ok:true,schema:'OMEGA_VERIFIED_CONTENT_REUSE_MANIFEST_R194',revision:R194_REVISION,allowedDomains:R194_ALLOWED_DOMAINS,laws:R194_LAWS,keyBinds:['execution domain','route identity','input fingerprint','operator fingerprint','frame fingerprint','address target','temporal target','solver/fidelity target','observer frame/policy','logical lanes','proof requirement','source-mode budget','canon-lens budget'],sourceRequirement:'R146 VERIFIED plus persisted R147 result fingerprint and SHA-256-bound result bytes',selectionPolicy:'R147 may choose CONTENT_CACHE only after an exact R194 hit and only when the operator did not explicitly select an executor or non-AUTO strategy',excluded:['BUILD','HYBRID','PLUGIN','independent-evidence proof requirements','cache-of-cache publication'],authority:{planning:'R193',dispatch:'R147',history:'R146',hybridProof:'R141',admission:'R125'},canonicalMutation:false,canonicalAdmissionAuthority:'R125',truthBoundary:'R194 is a computation-elision fabric. It can avoid repeating byte-identical, operator-identical stable work by executing deterministic cache retrieval against a verified source result. It never claims fresh execution of the original workload, fresh observation, solver validity, provider truth, or CanonState admission.'}};
