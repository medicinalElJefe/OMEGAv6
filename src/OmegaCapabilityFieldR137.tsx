...
export default function OmegaCapabilityFieldR137({panel,record,address,onAddress,onNavigate,status,restore}:Props){
 ...
 return <section className='r137-capability-field' data-panel={panel} data-visual-policy='CAPABILITY_FIRST_NO_STALE_CHART_PRIMARY'>
  <header className='r137-capability-head'><div><span>R137 · ACTIVE CAPABILITY FIELD</span><h2>{panel}</h2><p>Operate the system from the living state. Charts are no longer the primary destination: choose a route, commit a candidate state, execute a host path, prove a result, or open a specialist instrument.</p></div><div className='r137-health'><b>{runtimeHealthy?'RUNTIME PRESENT':'RUNTIME UNVERIFIED'}</b><small>{restoreHealthy?'continuity surface present':'restore surface unverified'}</small></div></header>
  <div className='r137-capability-layout'>
   <button className='r137-live-stage' onClick={()=>candidates[0]&&onAddress(candidates[0].address)} aria-label='Capability field; activate admitted next state'>
    <svg viewBox='0 0 640 460' role='img' aria-label='Interactive capability topology around the current canonical state'>
     ...
     {nodes.map((n,i)=><g key={n.id} className='r137-node'><line x1='320' y1='230' x2={n.x} y2={n.y}/><circle cx={n.x} cy={n.y} r={n.kind==='STATE'?13:18}/><text x={n.x} y={n.y+32} textAnchor='middle'>{n.label.slice(0,19)}</text><text x={n.x} y={n.y+45} textAnchor='middle' className='r137-node-sub'>{n.kind}</text></g>)}
    ...
    <span className='r137-stage-hint'>Click field to commit the highest-ranked admitted candidate</span>
   </button>
  ...
  <footer className='r137-capability-ledger'><span>CΩ <b>{fmt(law.u.C)}</b></span><span>Φ <b>{fmt(law.u.Phi)}</b></span><span>q <b>{fmt(law.u.q)}</b></span><span>Λ <b>{fmt(law.u.Lambda)}</b></span><span>evidence <b>{fmt(law.u.evidence)}</b></span><span>source-backed modes <b>{modes.appliedCount}</b></span><span>gated <b>{modes.gatedCount}</b></span></footer>
 </section>
}
