...
export async function createWholeOperationRunR143(runtime,input={}){
 ...
 const existing=await runtime.get(key(id),null);if(existing)return existing;
...
export async function applyWholeOperationReceiptR143(runtime,id,nodeId,receipt={},frames={}){
 ...
 const causalResidual=receipt?.source==='R143_CAUSAL_DEPENDENCY_GATE'&&String(receipt?.state||'').toUpperCase()==='STALE'&&String(dep.reason||'').startsWith('DEPENDENCY_');
 if(before.id!=='N00_FRAME'&&!dep.ready&&!['RETURNED','INVOKED','RUNNING'].includes(before.state)&&!causalResidual)return{error:'DEPENDENCY_NOT_READY',dependency:dep};
 const graph=applyNodeReceiptR143(run.graph,nodeId,receipt,frames),updated={...run,graph,updatedAt:new Date().toISOString()};await runtime.put(key(id),updated);await runtime.event('R143_NODE_RECEIPT',`Whole-operation node ${nodeId} recorded ${graph.nodes.find(x=>x.id===nodeId)?.state}.`,{runId:id,nodeId,state:graph.nodes.find(x=>x.id===nodeId)?.state,verified:receipt.verified===true,causalResidual});return updated;
}
...
export async function setWholeOperationHostPlanR143(runtime,id,hostPlan,confirmed=false){
 const run=await readWholeOperationRunR143(runtime,id);if(!run)return null;if(confirmed!==true||!hostPlan||typeof hostPlan!=='object')return{error:'EXPLICIT_HOST_PLAN_CONFIRMATION_REQUIRED'};
 const hostNode=run.graph.nodes.find(x=>x.id==='N40_HOST');if(!hostNode)return{error:'HOST_NODE_NOT_PRESENT'};if(hostNode.state==='UNAVAILABLE')return{error:'HOST_EXECUTOR_UNAVAILABLE'};if(hostNode.invocation)return{error:'HOST_NODE_ALREADY_INVOKED'};
 const plan={...clone(hostPlan),confirmed:true},updated={...run,hostPlan:plan,updatedAt:new Date().toISOString()};await runtime.put(key(id),updated);await runtime.event('R143_HOST_PLAN_ATTACHED',`Confirmed native plan attached to ${id}; no host job was queued by attachment.`,{runId:id,stepCount:Array.isArray(plan.steps)?plan.steps.length:0,targetDeviceId:plan.targetDeviceId||null});return updated;
}

export async function joinWholeOperationRunR143(runtime,id){
 const run=await readWholeOperationRunR143(runtime,id);if(!run)return null;if(run.joinReceipt)return run;
 const dep=dependencyStateR143(run.graph,'N80_JOIN');if(!dep.ready)return{error:'JOIN_DEPENDENCIES_NOT_READY',dependency:dep,run};
 ...
 if(joinReceipt.worldHead)await runtime.put('r143WorldHead',joinReceipt.worldHead);await runtime.put(key(id),updated);await runtime.event('R143_RUN_JOINED',`Whole-operation weave ${id} joined as ${status}.`,{runId:id,status,finalHeadSha256:joinReceipt.finalHeadSha256,heldNodeCount:joinReceipt.heldNodeCount});return updated;
}

export function wholeOperationStoreManifestR143(){return{...manifestR143(),runSchema:R143_RUN_SCHEMA,durability:'EXISTING_OMEGA_RUNTIME_DURABLE_OBJECT',newDurableAuthorityIntroduced:false,mutations:['create run','attach explicitly confirmed host plan without execution','record invocation','record executor receipt','deterministic join'],nativeExecution:'REQUIRES_EXISTING_HYBRID_EXPLICIT_CONFIRMATION',joinIdempotent:true,canonicalMutation:false,canonicalAdmissionAuthority:'R125'}}
