...
const storedRoutes=(key:string,limit:number)=>{try{const parsed=JSON.parse(localStorage.getItem(key)||'[]');return Array.isArray(parsed)?parsed.filter(x=>typeof x==='string'&&OMEGA_ALL_ROUTES_R82.includes(x as any)).filter((x,i,a)=>a.indexOf(x)===i).slice(0,limit):[]}catch{return[]}};
const short=(value:string,n=74)=>value.length>n?`${value.slice(0,n)}…`:value;
...
export default function OmegaSideNavigatorR88({currentPanel='',onNavigate,onHome}:Props){
 ...
 const operationAudit=useMemo(()=>auditAuthoritativeOperationChainR143(),[]),live=useLiveNavigationR156();
 useEffect(()=>{
 ...
 useEffect(()=>{if(!currentPanel||!OMEGA_ALL_ROUTES_R82.includes(currentPanel as any))return;setRecent(prev=>{const next=[currentPanel,...prev.filter(x=>x!==currentPanel)].slice(0,7);try{localStorage.setItem('omega.r156.globalRecent',JSON.stringify(next))}catch{}return next})},[currentPanel]);
 const togglePin=(route:string)=>setPinned(prev=>{const next=prev.includes(route)?prev.filter(x=>x!==route):[route,...prev].slice(0,8);try{localStorage.setItem('omega.r156.globalPins',JSON.stringify(next))}catch{}return next});
 ...
 const nextActions=useMemo(()=>currentPanel?adaptiveNextRoutes(currentPanel,'',live):[],[currentPanel,live]);
 ...
 return <aside className={'r94-side-toolbar '+(expanded?'expanded':'collapsed')+' '+(railWide?'rail-wide':'rail-compact')+' r100-professional-nav r104-readable-nav r105-context-nav r120-adaptive-nav r132-organized-nav r156-global-nav'} aria-label='OMEGA global navigation toolbar' data-operation-chain='R143' data-operation-chain-pass={operationAudit.pass?'true':'false'} data-pc-online={live.pcOnline?'true':'false'} data-mission-state={live.missionStatus}>
  ...
  <section className='r88-navigator r89-flat-navigator r94-nav-panel r100-nav-panel r104-nav-panel r156-nav-panel' aria-hidden={!expanded}>
   <header className='r88-navigator-head r100-navigator-head r104-navigator-head'>
    <div><span>OMEGA V6 · ADAPTIVE INSTRUMENT OS · R156 OVER R143 OPERATION CHAIN</span><b>{layer==='EVERYWHERE'?(activeWorkspace?`${activeWorkspace.label} tools`:'Live operating surface'):'Software / capability map'}</b><small>{layer==='EVERYWHERE'?(`${rows.length}/${routeCount} destinations · ${live.pcOnline?'PC ONLINE':'PC heartbeat held'} · ${missionRunning?`${live.missionStatus} mission active`:'no active mission'} · ${live.rcwaOnline?'RCWA LIVE':'RCWA held'} · all 44 destinations remain reachable`):'Full software lineage, capability, execution-domain, receipt and proof authority map'}</small></div>
   ...
   {layer==='EVERYWHERE'?<>
    ...
    <label className='r88-search r100-search r104-search r156-semantic-search'><Search/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder='Describe the outcome: repair build, prove it, connected PC, Earth forecast…' aria-label={activeWorkspace?`Search ${activeWorkspace.label} tools by goal and capability`:'Search all OMEGA instruments and task workspaces by goal'}/><kbd>⌘K</kbd></label>
    {query.trim()&&taskMatches.length>0&&<section className='r156-task-matches'><header><span>TASK WORKSPACES</span><small>compositions of existing instruments · no automatic execution</small></header>{taskMatches.slice(0,3).map(stack=><button key={stack.id} onClick={()=>go(stack.routes[0])}><span><b>{stack.label}</b><small>{stack.intent}</small></span><i>{stack.routes.join(' → ')}</i></button>)}</section>}
