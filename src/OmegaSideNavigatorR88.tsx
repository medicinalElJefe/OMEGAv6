 ...
 return <aside className={'r94-side-toolbar '+(expanded?'expanded':'collapsed')+' r100-professional-nav r104-readable-nav r105-context-nav'} aria-label='OMEGA global navigation toolbar'>
  ...
  <section className='r88-navigator r89-flat-navigator r94-nav-panel r100-nav-panel r104-nav-panel' aria-hidden={!expanded}>
   ...
   {layer==='EVERYWHERE'?<>
    ...
    <label className='r88-search r100-search r104-search'><Search/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder='Search all 44 OMEGA applications' aria-label={activeWorkspace?`Search ${activeWorkspace.label} tools within the shared 44-route navigator`:'Search all 44 OMEGA applications'}/><kbd>⌘K</kbd></label>
