...
export default function OmegaSideNavigatorR88({currentPanel='',onNavigate,onHome}:Props){
 ...
 useEffect(()=>{
  ...
  const routeCommitted=()=>{setExpanded(false);setLayer('EVERYWHERE');setWorkspaceFilter('ALL');setMasterMenu('ALL');setQuery('');setNavigationScar('')};window.addEventListener('omega-r88-open-navigator',openNavigator as EventListener);window.addEventListener('omega-r356-route-committed',routeCommitted as EventListener);window.addEventListener('keydown',key);
