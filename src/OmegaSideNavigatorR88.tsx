...
import {auditInterfaceConvergenceR146} from './interfaceConvergenceR146';
...
import './omegaInterfaceConvergenceR146.css';
...
export default function OmegaSideNavigatorR88({currentPanel='',onNavigate,onHome}:Props){
 ...
 const operationAudit=useMemo(()=>auditAuthoritativeOperationChainR143(),[]),interfaceAudit=useMemo(()=>auditInterfaceConvergenceR146(),[]);
 useEffect(()=>{
 ...
 useEffect(()=>{document.documentElement.dataset.omegaNavPresent='true';document.documentElement.dataset.omegaNavExpanded=expanded?'true':'false';document.documentElement.dataset.omegaInterfaceConvergence='R146';return()=>{delete document.documentElement.dataset.omegaNavPresent;delete document.documentElement.dataset.omegaNavExpanded;delete document.documentElement.dataset.omegaInterfaceConvergence}},[expanded]);
 ...
 return <aside className={'r94-side-toolbar '+(expanded?'expanded':'collapsed')+' '+(railWide?'rail-wide':'rail-compact')+' r100-professional-nav r104-readable-nav r105-context-nav r120-adaptive-nav r132-organized-nav r146-interface-convergence'} aria-label='OMEGA global navigation toolbar' data-operation-chain='R143' data-operation-chain-pass={operationAudit.pass?'true':'false'} data-interface-convergence='R146' data-interface-convergence-pass={interfaceAudit.pass?'true':'false'}>
  <button className='r146-nav-backdrop' aria-label='Close OMEGA navigator backdrop' tabIndex={expanded?0:-1} onClick={()=>setExpanded(false)}/>
  ...
  <section className='r88-navigator r89-flat-navigator r94-nav-panel r100-nav-panel r104-nav-panel' aria-hidden={!expanded} aria-label='OMEGA application and capability navigator'>
   <header className='r88-navigator-head r100-navigator-head r104-navigator-head'>
    <div><span>OMEGA V6 · INSTRUMENT OS · R146 PRESERVATION CONVERGENCE</span><b>{layer==='EVERYWHERE'?(activeWorkspace?`${activeWorkspace.label} tools`:'Everywhere'):'Software / capability map'}</b><small>{layer==='EVERYWHERE'?(`${rows.length}/${routeCount} registered destinations visible · ${operationAudit.mappedRoutes}/${operationAudit.totalRoutes} route→capability contracts · ${activeWorkspace?activeWorkspace.copy:'all destinations remain reachable'}`):'Full software lineage, capability, execution-domain, receipt and proof authority map'}</small><div className='r146-nav-health'><i className={interfaceAudit.pass?'pass':''}>R146 {interfaceAudit.pass?'PRESERVED':'HOLD'}</i><i>{routeCount} ROUTES</i><i>{OMEGA_WORKSPACES_R82.length} WORKSPACES</i><i>R143 CONTRACTS {operationAudit.pass?'PASS':'HOLD'}</i></div></div>
    <div className='r88-head-actions'><button className='r146-nav-close' onClick={()=>setExpanded(false)} aria-label='Collapse navigator'><ChevronLeft/><span>CLOSE</span></button></div>
   </header>
   <nav className='r89-nav-mode r100-nav-mode r104-nav-mode' aria-label='Navigator mode'><button aria-label='Browse everywhere' className={layer==='EVERYWHERE'?'active':''} onClick={()=>{setLayer('EVERYWHERE');setWorkspaceFilter('ALL')}}><Menu/>Everywhere <b>{routeCount}</b></button><button aria-label='Browse software and capability map' className={layer==='SOFTWARE'?'active':''} onClick={()=>{setLayer('SOFTWARE');setWorkspaceFilter('ALL')}}><Layers3/>Software map</button></nav>
   ...
   {layer==='EVERYWHERE'?<>
    <nav className='r105-workspace-filter' aria-label='Application workspace submenu'><button aria-label='Show all workspaces' className={workspaceFilter==='ALL'?'active':''} onClick={()=>setWorkspaceFilter('ALL')}>ALL <b>{routeCount}</b></button>{OMEGA_WORKSPACES_R82.map(workspace=><button key={workspace.id} aria-label={`Show ${workspace.label} workspace`} className={workspaceFilter===workspace.id?'active':''} onClick={()=>setWorkspaceFilter(workspace.id)} title={workspace.copy}>{workspace.label} <b>{workspace.routes.length}</b></button>)}</nav>
