...
import TraversalFieldCockpitR347 from './TraversalFieldCockpitR347';
...
export default function OmegaTraversalStudio({variant,address,state,onAddress}:Props){
 ...
 return <section className={'special-app traversal-studio r99-traversal-studio '+variant.toLowerCase().replaceAll(' ','-')} data-r168-traversal-role={spec.role} data-route-depth={spec.depth} data-performance-quality={performance.quality}>
  <header className='special-head r99-special-head'><div><span>R347 HUMAN-CORRELATED TRAVERSAL · WOVEN CONTINUITY · SOURCE STATE PRESERVED</span><h2>{variant}</h2><p>{spec.lens}. Primary geometry now uses one declared human visual grammar: worldline position, continuity thickness, plasticity branch aperture, q/Λ uncertainty, scar persistence, evidence focus, signed orientation and representational resolution. Across all three depth variants, canonical state, transition law and proof authority remain shared. R100/R35/R23 remain preserved below as independent comparison/proof layers.</p></div><div className='special-status'><b>SOURCE-BACKED · {spec.depth} STEP DEPTH</b><small>STATE {r.stateId} · D{coords.d} P{coords.p} R{coords.r} L{coords.l} · {performance.quality} {performance.targetFps} FPS TARGET</small></div></header>
  <TraversalFieldCockpitR347 variant={variant} address={address} onAddress={onAddress}/>
  ...
  <details className='r99-mode-data r347-r100-layer'><summary>R100 WOVEN FIELD · preserved mathematical renderer / comparison</summary><TraversalModeStageR99 variant={variant} address={address} onAddress={onAddress}/></details>
