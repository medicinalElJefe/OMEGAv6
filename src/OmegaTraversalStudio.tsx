...
export default function OmegaTraversalStudio({variant,address,state,onAddress}:Props){
 ...
 return <section className={'special-app traversal-studio r99-traversal-studio '+variant.toLowerCase().replaceAll(' ','-')}>
  <header className='special-head r99-special-head'><div><span>TRAVERSAL KERNEL · WOVEN CONTINUITY · SOURCE STATE PRESERVED</span><h2>{variant}</h2><p>The canonical packet is expressed as a time-synchronized weave state: geometry responds to continuity exchange, invariant carry, scar/history carry and signed orientation while proof, motion telemetry and restored donors remain outside the visual stage.</p></div><div className='special-status'><b>SOURCE-BACKED</b><small>STATE {r.stateId} · D{coords.d} P{coords.p} R{coords.r} L{coords.l}</small></div></header>
  ...
  <div className='instrument-controls r99-traversal-actions'><button onClick={()=>onAddress(r.autoPing.previous)}><Rotate3d/>Previous source state</button><button className='gold primary-action' onClick={()=>onAddress(r.autoPing.dataNext)}><FastForward/>Admitted next</button><span><ShieldCheck/> Mode and weave changes alter lawful depiction only. Previous/current/next, corridor and receipts remain bound to the same canonical packet.</span></div>
