...
const TABS:VisualAtlasLensR183[]=['FIELD','WEAVE','MOTION','SCALE','PROOF','DUALVERSE'];
...
export default function VisualAtlasR183({packet,onSelectAddress}:{packet:VisualAtlasPacketR183;onSelectAddress:(address:number)=>void}){
 const[tab,setTab]=useState<VisualAtlasLensR183>('FIELD');
 const routeSeries=useMemo(()=>({
  continuity:packet.route.map(x=>x.continuity),plasticity:packet.route.map(x=>x.plasticity),contradiction:packet.route.map(x=>x.contradiction),burden:packet.route.map(x=>x.burden),motion:packet.route.map(x=>x.motion),velocity:packet.route.map(x=>x.velocity)
 }),[packet]);
 ...
 const neighborBy=(axis:string,sign:number)=>packet.neighbors.find(x=>x.axis===axis&&x.sign===sign);
 const deltaRows=[['CΩ continuity',packet.deltas.continuity],['Φ plasticity',packet.deltas.plasticity],['q contradiction',packet.deltas.contradiction],['Λ burden',packet.deltas.burden],['scar',packet.deltas.scar],['motion',packet.deltas.motion],['velocity',packet.deltas.velocity],['ΔΦ',packet.deltas.phase]] as const;
 return <section className='visual-atlas-r183'>
  ...
  <div className='va183-grid'>
   <section className='va183-stage'>
    {tab==='FIELD'&&<>
     ...
     <svg viewBox='0 0 520 360' className='va183-glyph' role='img' aria-label='R183 field membrane encoding of the current OMEGA packet'>
      ...
      <circle cx='260' cy='180' r={8+18*packet.encoding.scarKnot} className='va183-scar-knot'/><circle cx='260' cy='180' r='4' className='va183-core'/>
      <path d={`M260 180 L${260+packet.orientation.tangent.x*85} ${180-packet.orientation.tangent.y*85}`} className='va183-forward'/>
   ...
   <aside className='va183-ledger'>
    <header><GitBranch/><div><b>Why this view looks this way</b><small>Every channel is declared.</small></div></header>
    {Object.entries(packet.channels).filter(([,v])=>typeof v==='number').map(([k,v])=><div key={k}><span>{k}</span><b>{typeof v==='number'?f(v,4):String(v)}</b></div>)}
    <footer><ShieldCheck/><span>{packet.boundary}</span></footer>
   </aside>
  </div>
 </section>
}
