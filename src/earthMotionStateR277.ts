export type EarthMotionSampleR277={
...
export type EarthMotionStateR277={schema:'OMEGA_EARTH_MOTION_STATE_R277';revision:'R277';observedAt:string;provider:string;sampleCount:number;observedCount:number;samples:EarthMotionSampleR277[];summary:{quality:number;flow:number;boundary:number;scar:number;memory:number;continuity:number;woven:number;futurePreservation:number;maxWindKph:number};truthBoundary:string};

const clamp=(n:number,a=0,b=1)=>Math.max(a,Math.min(b,Number.isFinite(n)?n:a));
const num=(v:any)=>Number.isFinite(Number(v))?Number(v):null;
const angleDelta=(a:number,b:number)=>{let d=Math.abs((((a-b)+540)%360)-180);return clamp(d/180)};
const STORAGE='omega.r277.earth-motion.previous';
...
function savePrevious(rows:EarthMotionSampleR277[]){if(typeof localStorage==='undefined')return;try{localStorage.setItem(STORAGE,JSON.stringify(rows.map(x=>({id:x.id,windKph:x.windKph,windDirectionDeg:x.windDirectionDeg,surfacePressureHpa:x.surfacePressureHpa,observedAt:x.observedAt}))))}catch{}}
function mean(rows:EarthMotionSampleR277[],key:keyof EarthMotionSampleR277){const xs=rows.map(x=>Number(x[key])).filter(Number.isFinite);return xs.length?xs.reduce((a,b)=>a+b,0)/xs.length:0}

export async function fetchEarthMotionStateR277(signal?:AbortSignal):Promise<EarthMotionStateR277>{
