...
import {deriveGeometricMotionFrameR169} from './selfBuildRuntimeR124';
import './autonomousBuildContinuumR169.css';
...
export default function AutonomousBuildContinuumR169({onNavigate}:{onNavigate:(name:string)=>void}){
 ...
 const roadmap=Array.isArray(state?.roadmap)?state.roadmap:[],frame=useMemo(()=>state&&roadmap.length?deriveGeometricMotionFrameR169(state,roadmap):null,[state,roadmap.length]);
 ...
 return <section className='r169-autobuild' aria-label='R169 geometric motion autonomous build continuum'>
  <header><div><span>R169 · GEOMETRIC MOTION AUTONOMOUS BUILD</span><h3>Observe → prioritize → sandbox → prove → freshness check → admit → pulse again</h3><p>The self-build loop no longer stops falsely at legacy generation 8. R169 extends the declared governed roadmap to generation 12 and wakes on recurring three-hour observation pulses; successful admissions immediately trigger the next generation.</p></div><button onClick={()=>void refresh()} disabled={busy}><RefreshCw className={busy?'spin':''}/>{busy?'READING':'REFRESH BUILD STATE'}</button></header>
  <div className='r169-autobuild-kpis'><article><span>GENERATION</span><b>{generation}/{effectiveMax}</b><small>{running?'continuation available':'observe / hold boundary'}</small></article><article><span>NEXT DECLARED</span><b>{next||'OBSERVE'}</b><small>{remaining[0]?.[1]||'no undeclared capability is invented'}</small></article><article><span>MOTION PHASE</span><b>{phase}</b><small>{orientation}</small></article><article><span>RESOLUTION</span><b>{frame?.effectiveResolution?.toLocaleString?.('en-US')||'—'}</b><small>representational scheduling level</small></article><article><span>CONTINUITY</span><b>{frame?Number(frame.continuity).toFixed(3):'—'}</b><small>invariant carry {frame?Number(frame.invariantCarry).toFixed(3):'—'}</small></article><article><span>RESIDUAL</span><b>{frame?Number(frame.residual).toFixed(3):'—'}</b><small>repartition {frame?Number(frame.repartitionDemand).toFixed(3):'—'}</small></article></div>
  <div className='r169-motion-line'>{CONTINUATION.map(([id,label],i)=>{const done=admitted.has(id),active=id===next;return <article key={id} data-done={done?'true':'false'} data-active={active?'true':'false'}><code>{id}</code><span><b>{label}</b><small>{done?'ADMITTED':active?'NEXT RELATIVE-MOTION TARGET':'DEPENDENCY-ORDERED'}</small></span>{i<CONTINUATION.length-1&&<Waypoints/>}</article>})}</div>
  <div className='r169-autobuild-proof'><ShieldCheck/><span><b>{running?'AUTONOMOUS CONTINUATION ARMED':'OBSERVATION BOUNDARY'}</b><small>Geometric motion changes build priority only inside the declared backlog. Repository mutation still requires an isolated sandbox candidate, focused + inherited tests, unchanged main SHA, rollback lineage and explicit proof-gated admission. RETURNED ≠ VERIFIED and R125 remains the only CanonState admission authority.</small></span></div>
  {candidate&&<div className='r169-candidate'><Activity/><span><b>LAST CANDIDATE · {candidate?.capsule?.id||candidate?.receipt?.capsuleId||'UNKNOWN'}</b><small>{candidate?.admission||candidate?.receipt?.status||'proposal state'} · generation {candidate?.generation??'—'} · score {Number(candidate?.score||0).toFixed(3)}</small></span></div>}
  {error&&<div className='r169-error'>{error}</div>}
