...
uniform float uTime,uYaw,uPitch,uZoom,uAspect,uSigma,uHybrid;uniform int uScene;uniform vec4 uScale;
out vec4 vColor;out float vProof;out float vRoute;
${TRANSFORM}
void main(){float C=aMetric.x,P=aMetric.y,q=aMetric.z,L=aMetric.w,scar=aAux.x,E=aAux.y,M=aAux.z,mode=aAux.w;float warp=clamp(.28+.32*C+.18*P+.16*mode-.12*q-.10*L,0.,1.);float flow=clamp(.28*M+.28*P+.24*mode+.20*C,0.,1.);if(uScene==2)warp+=.12*sin(uTime*.8+pow(max(.01,M),.5)*6.283);if(uScene==4)warp+=.12*abs(aPosition.w)*(.35+.65*uScale.z);if(uScene==5){float forceShape=uScale.w<.5?(1.-abs(aPosition.x)):(uScale.w<1.5?(1.-abs(aPosition.y)):(1.-abs(aPosition.z)));warp+=.05+.18*forceShape*(.25+.75*uScale.z);flow=clamp(flow+.16*uScale.x+.10*uScale.y,0.,1.);}if(uScene==6)warp*=.48+.75*E*(1.-q)*(1.-L);if(uScene==7)warp+=.10*uHybrid*aRoute*sin(uTime*1.7);vec3 q3=project4(aPosition,uTime,warp,flow);float cam=2.65-q3.z*.38;vec2 xy=q3.xy*(uZoom/max(.72,cam));xy.x/=max(.5,uAspect);gl_Position=vec4(xy,clamp(q3.z*.28,-.95,.95),1.);float size=1.1+2.2*C+1.4*E+4.8*aRoute;if(uScene==6)size*=.45+1.75*E;gl_PointSize=clamp(size*(1.+.6/max(.8,cam)),1.,10.);float alpha=.045+.20*C+.15*P+.22*E+.20*mode+.28*aRoute-.10*q-.07*L;if(uScene==1)alpha*=.78;if(uScene==3)alpha*=.18+.95*aRoute;if(uScene==6)alpha*=.18+1.45*E*(1.-q)*(1.-L);vec3 col=aColor;if(uScene==5){vec3 forceColor=uScale.w<.5?vec3(.91,.63,.27):(uScale.w<1.5?vec3(.33,.78,.91):vec3(.71,.79,.94));col=mix(col,forceColor,.24+.36*uScale.z);}col=mix(col,vec3(.76,.96,.91),clamp(aRoute*.62,0.,.7));if(uScene==7)col=mix(col,vec3(.40,.88,1.),uHybrid*aRoute*.65);vColor=vec4(col,clamp(alpha,.018,.94));vProof=E*(1.-q)*(1.-L);vRoute=aRoute;}`;
...
precision highp float;layout(location=0)in vec4 aPosition;layout(location=1)in vec4 aMeta;uniform float uTime,uYaw,uPitch,uZoom,uAspect,uSigma,uHybrid;uniform int uScene;uniform vec4 uScale;out vec4 vColor;
${TRANSFORM}
void main(){float transport=aMeta.x,scar=aMeta.y,proof=aMeta.z,route=aMeta.w;float warp=.26+.32*transport+.12*scar;if(uScene==5)warp+=.12*uScale.z;vec3 q3=project4(aPosition,uTime,warp,transport);float cam=2.65-q3.z*.38;vec2 xy=q3.xy*(uZoom/max(.72,cam));xy.x/=max(.5,uAspect);gl_Position=vec4(xy,clamp(q3.z*.28,-.95,.95),1.);float a=.025+.12*transport+.08*proof;if(uScene==1)a=.055+.27*transport;if(uScene==2)a=.035+.30*transport+.16*scar;if(uScene==3)a=route*(.18+.62*proof);if(uScene==5)a=.025+.18*transport+.10*uScale.z;if(uScene==6)a*=.15+1.35*proof;if(uScene==7)a=route*(.12+.58*uHybrid);vec3 c=route>0.?vec3(.86,.68,.32):mix(vec3(.08,.32,.35),vec3(.30,.88,.76),transport);if(uScene==2)c=mix(c,vec3(.76,.27,.35),scar*.55);if(uScene==5){vec3 forceColor=uScale.w<.5?vec3(.91,.63,.27):(uScale.w<1.5?vec3(.33,.78,.91):vec3(.71,.79,.94));c=mix(c,forceColor,.34);}if(uScene==6)c=mix(vec3(.13,.18,.20),vec3(.58,.94,.84),proof);if(uScene==7)c=mix(c,vec3(.34,.80,1.),uHybrid);vColor=vec4(c,clamp(a,0.,.88));}`;
...
export default function OmegaWovenRelativityContinuumR134({address,onAddress,projection='MANDALA',view='SOURCE_COLOR'}:Props){
 ...
 const scaleUniform=useMemo(()=>{const forceCode=scale.dominantForce==='Strong interaction'?0:scale.dominantForce==='Electromagnetic interaction'?1:2;return[scale.coherence,scale.form,scale.aggregation,forceCode] as const},[scale]);
 ...
 },[gpu,field.points.length,scene,running,yaw,pitch,zoom,sigma,pcOnline,scaleUniform]);
 ...
 return <section className='r134-continuum' data-scene={scene} data-lod={effectiveLod} data-frame={frame} data-truth='structural-route-observed-execution-operational-separated'>
  ...
  {(scene==='SCALE'||scene==='FORCES')&&<section className='r134-scale'><header><Layers3/><span><b>{scale.scale}</b><small>{scale.dominantForce} · reference hierarchy lens, not a claim that OMEGA geometry is a measured physical field</small></span></header><input type='range' min='0' max={R132_SCALE_HIERARCHY.length-1} value={scaleIndex} onChange={e=>setScaleIndex(Number(e.target.value))}/><p>{scale.role}</p></section>}
