...
import ArchiveContinuityAura from './ArchiveContinuityAura';
import GrowthSequenceRail from './GrowthSequenceRail';
...
function App(){const[home,setHome]=useState(true);useEffect(()=>{const open=()=>setHome(true);window.addEventListener('omega-home-request',open as EventListener);return()=>window.removeEventListener('omega-home-request',open as EventListener)},[]);const navigate=(name:string)=>{localStorage.setItem('omega.v6.panel',JSON.stringify(name));setHome(false)};return <AppBoundary><ArchiveContinuityAura home={home}/><GrowthSequenceRail home={home}/>{home?<OmegaHome onEnter={navigate}/>:<Suspense fallback={<div className='boot'><b>OMEGA {RUNTIME_IDENTITY.hostedBuild} · {RUNTIME_IDENTITY.runtimeContract}</b><span>Starting full specialist workstation…</span></div>}><OmegaWorkstation/><button className='omega-home-launch' onClick={()=>setHome(true)} aria-label='Return to OMEGA home'><Home/></button></Suspense>}</AppBoundary>}
export default App;
