...
function App(){const[buildOut,setBuildOut]=useState(false);const address=()=>{const n=Number(localStorage.getItem('omega.v6.address')||11498);return Number.isFinite(n)?Math.max(0,Math.min(20735,Math.floor(n))):11498};const navigate=(name:string)=>{localStorage.setItem('omega.v6.panel',JSON.stringify(name));setBuildOut(false);window.dispatchEvent(new Event('storage'))};return <AppBoundary><Suspense fallback={<div className='boot'><b>OMEGA {RUNTIME_IDENTITY.hostedBuild} · {RUNTIME_IDENTITY.runtimeContract}</b><span>Starting restored + surpass sovereign workstation…</span></div>}><OmegaWorkstation/><button className='woven-buildout-launch' onClick={()=>setBuildOut(true)} aria-label='Open Build Out mode'><GitBranch/>BUILD OUT</button>{buildOut&&<div className='woven-buildout-overlay'><header><div><span>ALL MODES · BUILD OUT</span><b>Woven Continuity Control Plane</b></div><button onClick={()=>setBuildOut(false)} aria-label='Close Build Out'><X/></button></header><main><WovenBuildOutPanel address={address()} onNavigate={navigate}/></main></div>}</Suspense></AppBoundary>}
export default App;