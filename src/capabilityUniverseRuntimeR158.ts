...
export function compileCapabilityUniverseR158(record:any,options:{power?:number;orientation?:R156Orientation;observerFrame?:R158ObserverFrame}={}){
 const address=Math.max(0,Math.min(STATE_COUNT-1,Math.floor(Number(record?.address)||0))),fusion=compileAllModesTruthFusionR151(record),whole=wholeSystemConvergenceManifestR155(),evolution=compileDimensionalRelativityEvolutionR156(address,options.power??4,options.orientation),orientation=evolution.orientation;
