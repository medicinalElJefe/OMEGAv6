...
export type R158ReflexNode={index:number;stage:string;angle:number;x:number;y:number};
...
export function compileCapabilityUniverseR158(record:any,options:{power?:number;orientation?:R156Orientation;observerFrame?:R158ObserverFrame}={}){
 const address=Math.max(0,Math.min(STATE_COUNT-1,Math.floor(Number(record?.address)||0))),fusion=compileAllModesTruthFusionR151(record),whole=wholeSystemConvergenceManifestR155(),organismReflex=organismReflexManifestR156(),evolution=compileDimensionalRelativityEvolutionR156(address,options.power??4,options.orientation),orientation=evolution.orientation;
 const directionalOrientation=orientation===0?1:orientation;
 const modes:R158ModeNode[]=fusion.channels.map((row:any,index:number)=>{const angle=directionalOrientation*(index/Math.max(1,fusion.channels.length))*TAU-Math.PI/2,radius=176+52*cl(row.score)+18*cl(row.weight),p=polar(angle,radius);return{index,id:String(row.id),name:String(row.name),group:String(row.group),gate:String(row.gate),provenance:String(row.provenance),score:cl(row.score),weight:cl(row.weight),angle,radius,x:p.x,y:p.y}});
 const families:R158FamilyNode[]=whole.families.map((row:any,index:number)=>{const angle=-directionalOrientation*(index/Math.max(1,whole.families.length))*TAU-Math.PI/2,p=polar(angle,316);return{index,family:String(row.family),state:String(row.state),authority:String(row.authority),purpose:String(row.purpose),dependencies:Array.isArray(row.dependencies)?row.dependencies.map(String):[],boundary:String(row.boundary),angle,x:p.x,y:p.y}});
 const reflexNodes:R158ReflexNode[]=organismReflex.stages.map((stage:string,index:number)=>{const angle=index/Math.max(1,organismReflex.stages.length)*TAU-Math.PI/2,p=polar(angle,104);return{index,stage,angle,x:p.x,y:p.y}});
 const reflexEdges=reflexNodes.map((node,index)=>({from:index,to:(index+1)%reflexNodes.length,fromStage:node.stage,toStage:reflexNodes[(index+1)%reflexNodes.length].stage}));
 ...
 return{schema:R158_SCHEMA,laws:R158_LAWS,address,stateId:record.stateId,nextStateId:evolution.canonicalNextStateId,observerFrame:options.observerFrame||'GLOBAL',orientation,directionalOrientation,fusion,evolution,wholeSystem:whole,organismReflex,reflexNodes,reflexEdges,modes,families,dependencyEdges,resolution:R156_RESOLUTION_LADDER,provenanceCounts,gateCounts,familyStates,counts:{channels:fusion.channelCount,sourceModes:fusion.sourceModeCount,canonAuthorities:fusion.canonAuthorityCount,capabilityFamilies:families.length,dependencyEdges:dependencyEdges.length,reflexStages:organismReflex.stages.length,reflexActions:organismReflex.actions.length,reflexResidualKinds:organismReflex.residual_kinds.length,resolutionFrames:R156_RESOLUTION_LADDER.length,canonicalStates:STATE_COUNT},revisionAuthorities:{wholeSystem:'R155 capability-family ownership',organismReflex:'R156 promoted cross-family control/reflex protocol',dimensionalRelativity:'R156 derived dimensional-relativity candidate on R158 branch',collisionLaw:'REVISION_NUMBER_ALONE_NEVER_DETERMINES_AUTHORITY'},truthBoundary:'R158 is a source-bound visual composition over the proven R155 capability-family ownership model, promoted R156 organism-reflex control flow, R151 241-channel truth fusion and R156 dimensional-relativity candidate. The two R156-labeled lines are distinct capability authorities; the label alone grants no ownership. Mode agreement remains internal coherence, reflex stages remain bounded software control flow rather than execution proof or consciousness, animation remains projection, neutral σ remains neutral even when a deterministic display ordering is required, and R125 remains the only CanonState admission authority.'};
}
