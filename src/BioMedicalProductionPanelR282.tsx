...
export default function BioMedicalProductionPanelR282({record,samples}:Props){
 ...
 return <section className='bio282' aria-label='Heavy Bio medical production validation control plane'>
  <header className='bio282-head'><div><span>R282 · MEDICAL PRODUCTION BASELINE</span><h3>Clinical Validation & Release Control Plane</h3><p>Instrument evidence, intended-use locking, risk controls, all-mode validation, audit integrity and controlled updates are evaluated independently. Software cannot manufacture clinical authority.</p></div><div className={`bio282-release ${result.releaseState==='AUTHORIZED_CLINICAL_RELEASE_READY'?'pass':result.releaseState==='ENGINEERING_INCOMPLETE'?'fail':'hold'}`}><b>{pretty(result.releaseState)}</b><small>{profile.manifest.regulatoryStage.replaceAll('_',' ')} · {profile.manifest.jurisdiction}</small></div></header>
  <div className='bio282-kpis'>
   ...
   <article><span>AUDIT CHAIN</span><b>{audit?.ok?'PASS':audit?'FAIL':'BUILDING'}</b><small>{audit?.count??0} events · {audit?.head.slice(0,12)??'—'}</small></article>
