...
export default function BioMedicalProductionPanelR282({record,samples}:Props){
 ...
 const releaseTone=result.releaseState==='AUTHORIZED_CLINICAL_RELEASE_READY'?'ready':result.releaseState==='ENGINEERING_INCOMPLETE'?'blocked':'hold';
 const releaseLabel=pretty(result.releaseState);
 return <section className='bio282' aria-label='Heavy Bio medical production validation control plane'>
  <header className='bio282-head'><div><span>R282 · MEDICAL PRODUCTION BASELINE</span><h3>Clinical Validation & Release Control Plane</h3><p>Instrument evidence, intended-use locking, risk controls, all-mode validation, audit integrity and controlled updates are evaluated independently. Software cannot manufacture clinical authority.</p></div><div className={`bio282-release bio282-release-${releaseTone}`} data-release-state={result.releaseState} data-release-label={releaseLabel} data-profile-id={profile.manifest.releaseId} data-audit-state={auditState} aria-live='polite' aria-label={`R282 release state: ${releaseLabel}`}><strong className='bio282-release-label'>{releaseLabel}</strong><small>{profile.manifest.regulatoryStage.replaceAll('_',' ')} · {profile.manifest.jurisdiction}</small></div></header>
