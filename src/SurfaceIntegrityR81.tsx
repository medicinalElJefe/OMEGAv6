...
export default function SurfaceIntegrityR81({panel,children,onRecover,record}:Props){
 const[proofOpen,setProofOpen]=useState(false),[proofDeep,setProofDeep]=useState(false),[interactionReady,setInteractionReady]=useState(false),proofDetailsRef=useRef<HTMLDetailsElement|null>(null),proofOpenTimer=useRef<number|undefined>(undefined),proofDeepTimer=useRef<number|undefined>(undefined);
 ...
 useEffect(()=>{
  ...
  let cancelled=false,raf1=0,raf2=0,idleId:any=null,timer=0;
  const commit=()=>{if(!cancelled)setInteractionReady(true)};
  raf1=window.requestAnimationFrame(()=>{raf2=window.requestAnimationFrame(()=>{
   const ric=(window as any).requestIdleCallback;
   if(typeof ric==='function')idleId=ric(commit,{timeout:900});
   else timer=window.setTimeout(commit,120);
  })});
  return()=>{cancelled=true;window.cancelAnimationFrame(raf1);window.cancelAnimationFrame(raf2);if(idleId!==null&&(window as any).cancelIdleCallback)(window as any).cancelIdleCallback(idleId);if(timer)window.clearTimeout(timer)};
 },[panel]);
 useEffect(()=>{
