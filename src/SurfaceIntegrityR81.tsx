...
export default function SurfaceIntegrityR81({panel,children,onRecover,record}:Props){
 ...
 return <PanelBoundary panel={panel} onRecover={onRecover}>
  <section className='omega-surface-r81' style={canon?canonSurfaceStyleR120(canon) as any:undefined} data-surface={slug(panel)} data-surface-name={panel} data-decision={String(record?.metrics?.decision||'UNBOUND')} data-provenance-primary={provenance.primary} data-layer-primary={layer.primary} data-layer-bindings={layer.layers.join(' ')} data-layer-contract='R104/R107' data-calculus-fabric='R107' data-canon-contract='R120' data-canon-context={canon?.schema||'UNBOUND'} data-canon-orientation={canon?.woven.orientation??0} data-canon-compute={canon?.carry.computeReadiness?.toFixed?.(4)||'0'} data-canon-proof={canon?.carry.proofReadiness?.toFixed?.(4)||'0'}>
