...
export default function SurfaceIntegrityR81({panel,children,onRecover,record}:Props){
 ...
 const provenance=provenanceForSurfaceR94(panel),layer=surfaceLayerBindingR104(panel),visual=visualTruthForSurfaceR157(panel);
 ...
 const address=Number.isFinite(record?.address)?Number(record.address):null;
 return <PanelBoundary panel={panel} onRecover={onRecover}>
  <section className='omega-surface-r81' data-surface={slug(panel)} data-surface-name={panel} data-decision={String(record?.metrics?.decision||'UNBOUND')} data-provenance-primary={provenance.primary} data-layer-primary={layer.primary} data-layer-bindings={layer.layers.join(' ')} data-layer-contract='R104/R107' data-calculus-fabric='R107' data-visual-truth-r157={visual?'true':'false'} data-visual-kind={visual?.kind||'NONE'} data-visual-space={visual?.space||'NONE'} data-visual-canon-effect={visual?.canonEffect||'NONE'} data-visual-state-id={record?.stateId??''} data-visual-address={address===null?'':String(address)}>
   ...
   <VisualTruthR157 surface={panel} record={record}/>
