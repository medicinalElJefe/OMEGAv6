...
export function createWgs84Orthographic(centerLat:number,centerLon:number):OrthoWgs84{
 ...
 const inverse=(nx:number,ny:number):InversePoint|null=>{
  const q=add(center,add(scale(east,nx),scale(north,ny))),B=2*(q[0]*up[0]+q[1]*up[1]+q[2]*up[2]*ib2),C=q[0]*q[0]+q[1]*q[1]+q[2]*q[2]*ib2-1,disc=B*B-4*A*C;
  ...
  if(mu<-1e-8)return null;
  return{lat,lon,mu:clamp(mu,0,1),normal};
 };
 const forward=(lat:number,lon:number):ProjectedPoint=>{
  const p=geodeticToEcef(lat,lon),d:Vec3=[p[0]-center[0],p[1]-center[1],p[2]-center[2]],normal=geodeticNormal(lat,lon),mu=dot3(normal,up);
...
export function solarElevationDegrees(lat:number,lon:number,date:Date){return deg(Math.asin(clamp(solarIllumination(lat,lon,date),-1,1)))}
