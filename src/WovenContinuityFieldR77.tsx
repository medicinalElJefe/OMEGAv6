...
export default function WovenContinuityFieldR77({address,record,onAddress}:Props){
 ...
 useEffect(()=>{
  ...
  if(!gl){
   ...
   const draw=(now:number)=>{
    ...
    const active=decodeAddress(liveRef.current.address),step=budget.compact?12:6,t=(now-start)/1000;
 ...
 },[budget.dprCap,budget.refinementBatch,budget.compact,shell]);
 ...
 return <section className='r77-woven' data-renderer={renderer} data-profile={budget.profile}>
  <header className='r77-head'>
   <div><span>R77 · WOVEN CONTINUITY COMPUTATIONAL FIELD</span><h3>20,736 resident anchors · 61.9B exact virtual addresses</h3><p>All canonical anchors remain GPU-visible while source refinement, modes, and virtual outer shells are scheduled separately so display continuity is not tied to full materialization.</p></div>
   <div className='r77-live'><i/><b>{renderer}</b><small>{budget.profile} · {budget.targetFps} FPS target</small></div>
  </header>
  ...
  <div className='r77-stage'>
   ...
   <div className='r77-stage-label'><span>STATE {record.stateId}</span><b>CΩ {fmt(snapshot.kernel.C)} · Φ {fmt(snapshot.kernel.Phi)} · q {fmt(snapshot.kernel.q)} · Λ {fmt(snapshot.kernel.burden)}</b><small>{snapshot.kernel.decision} · MODE188 {snapshot.kernel.mode188} · virtual {virtualAddress.toLocaleString()}</small></div>
  </div>
  <div className='r77-controls'>
   <button onClick={()=>moveShell(-1)} disabled={shell<=0}><ArrowLeft/><span><b>Outer shell</b><small>{shell.toLocaleString()}</small></span></button>
   <div className='r77-address'><Boxes/><span><b>{virtualAddress.toLocaleString()}</b><small>exact address of {WOVEN_VIRTUAL_COUNT.toLocaleString()} · shell {shell.toLocaleString()} / {(WOVEN_OUTER_SHELL_COUNT-1).toLocaleString()}</small></span></div>
   <button onClick={()=>moveShell(1)} disabled={shell>=WOVEN_OUTER_SHELL_COUNT-1}><span><b>Outer shell</b><small>{Math.min(WOVEN_OUTER_SHELL_COUNT-1,shell+1).toLocaleString()}</small></span><ArrowRight/></button>
   <button className='r77-route' onClick={()=>onAddress(next)}><Activity/><span><b>Traverse admitted route</b><small>STATE {record.stateId} → {next+1}</small></span><ArrowRight/></button>
  </div>
  <div className='r77-proof'>
   <ShieldCheck/><span><b>Resource boundary is explicit, not visible as a broken view.</b> The resident 20,736 lattice is always rendered as one field. Exact 12¹⁰ addressing is virtualized by shell+anchor decomposition. Expensive source-state refinement is time-sliced; active mode evaluation and proof gates remain authoritative. This is a computational representation, not a claim of 20,736 physical dimensions.</span><Braces/>
  </div>
 </section>;
}
