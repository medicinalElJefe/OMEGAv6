...
const VERT=[
 ...
 ' gl_PointSize=(1.3+2.8*aQuality.w+2.4*relation+4.0*exact)*max(1.0,uDpr);',
...
export default function WovenContinuityFieldR77({address,record,onAddress}:Props){
 ...
 useEffect(()=>{
  ...
  if(!gl){
   ...
   let raf=0,start=performance.now(),lastFrame=0;
   const frameMs=1000/Math.max(12,budget.targetFps);
   const draw=(now:number)=>{
    if(now-lastFrame<frameMs){raf=requestAnimationFrame(draw);return}lastFrame=now;
    const rect=canvas.getBoundingClientRect(),res=applyCanvasResolutionR119(canvas,Math.max(1,rect.width),Math.max(1,rect.height),'AUTO'),dpr=res.dpr;
    ctx.setTransform(dpr,0,0,dpr,0,0);ctx.fillStyle='#02080b';ctx.fillRect(0,0,rect.width,rect.height);
    const active=decodeAddress(liveRef.current.address),t=(now-start)/1000+shellRef.current/WOVEN_OUTER_SHELL_COUNT*Math.PI*2;
    for(let a=0;a<WOVEN_CANONICAL_COUNT;a++){
     ...
     const rr=a===liveRef.current.address?3.4:Math.max(.45,1.0+1.4*rel);
  ...
  setRenderer('WEBGL2');
  let disposed=false,raf=0,refineCursor=0,currentDpr=1;
  ...
  const U={time:uniform(gl,p,'uTime'),aspect:uniform(gl,p,'uAspect'),shell:uniform(gl,p,'uShell'),dpr:uniform(gl,p,'uDpr'),active:uniform(gl,p,'uActive'),kernel:uniform(gl,p,'uKernel'),mode:uniform(gl,p,'uMode')};
  gl.clearColor(.006,.018,.022,1);gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);

  const resize=()=>{
   const rect=canvas.getBoundingClientRect(),res=applyCanvasResolutionR119(canvas,Math.max(1,rect.width),Math.max(1,rect.height),'AUTO');currentDpr=res.dpr;gl.viewport(0,0,res.backingWidth,res.backingHeight);
  };
 ...
 },[budget.refinementBatch,budget.compact,budget.targetFps]);
 ...
 return <section className='r77-woven' data-renderer={renderer} data-profile={budget.profile} data-resolution-authority='R119'>
  <div className='r77-stage'>
   <canvas ref={canvasRef} aria-label='Live 20,736-anchor Woven Continuity field'/>
   <div className='r77-stage-label'><span>STATE {record.stateId} · {renderer} · {budget.profile} · R119 FULL RESOLUTION</span><b>CΩ {fmt(snapshot.kernel.C)} · Φ {fmt(snapshot.kernel.Phi)} · q {fmt(snapshot.kernel.q)} · Λ {fmt(snapshot.kernel.burden)}</b><small>{snapshot.kernel.decision} · MODE188 {snapshot.kernel.mode188} · anchor {address.toLocaleString()} · shell {shell.toLocaleString()}</small></div>
