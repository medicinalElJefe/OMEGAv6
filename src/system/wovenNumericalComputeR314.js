...
export const R314_NUMERICAL_CAPABILITIES=Object.freeze(['SCALAR','VECTOR','MATRIX','DAG','GRADIENT','JACOBIAN','HESSIAN','INTEGRATION','ROOT','LINEAR_SOLVE','OPTIMIZATION','FRAME_TRANSFORM']);
...
export function solveLinearR314(A,b,{pivotEpsilon=1e-12}={}){
 A=matrix(A).map(r=>r.slice());b=vec(b).slice();if(A.length!==A[0].length||b.length!==A.length)throw new Error('R314 linear solve requires square A and matching b');
 const n=A.length;
 for(let c=0;c<n;c++){
  let p=c;for(let r=c+1;r<n;r++)if(Math.abs(A[r][c])>Math.abs(A[p][c]))p=r;
  if(Math.abs(A[p][c])<=pivotEpsilon)throw new Error('R314 singular or ill-conditioned system');
  [A[c],A[p]]=[A[p],A[c]];[b[c],b[p]]=[b[p],b[c]];
  const d=A[c][c];for(let j=c;j<n;j++)A[c][j]/=d;b[c]/=d;
  for(let r=0;r<n;r++){if(r===c)continue;const f=A[r][c];if(!f)continue;for(let j=c;j<n;j++)A[r][j]-=f*A[c][j];b[r]-=f*b[c]}
 }
 return{schema:R314_SCHEMA,kind:'LINEAR_SOLVE',value:b,residual:normR314(vectorSubR314(matvecR314(A.map((_,i)=>[]),[]),[]))};
}

function evaluatedLinearResidual(originalA,originalB,x){return normR314(vectorSubR314(matvecR314(originalA,x),originalB))}
export function linearSolveR314(A,b,opts={}){
 const originalA=matrix(A),originalB=vec(b);const M=originalA.map(r=>r.slice()),y=originalB.slice(),n=M.length;if(n!==M[0].length||y.length!==n)throw new Error('R314 linear solve requires square A and matching b');const eps=opts.pivotEpsilon??1e-12;
 for(let c=0;c<n;c++){let p=c;for(let r=c+1;r<n;r++)if(Math.abs(M[r][c])>Math.abs(M[p][c]))p=r;if(Math.abs(M[p][c])<=eps)throw new Error('R314 singular or ill-conditioned system');[M[c],M[p]]=[M[p],M[c]];[y[c],y[p]]=[y[p],y[c]];const d=M[c][c];for(let j=c;j<n;j++)M[c][j]/=d;y[c]/=d;for(let r=0;r<n;r++){if(r===c)continue;const f=M[r][c];for(let j=c;j<n;j++)M[r][j]-=f*M[c][j];y[r]-=f*y[c]}}
...
export function executeDagR314(nodes,{inputs={},memo=new Map(),provenance=[]}={}){
 ...
 const evalNode=id=>{id=String(id);if(values.has(id))return values.get(id);const node=byId.get(id);if(!node)throw new Error(`R314 missing DAG node ${id}`);if(visiting.has(id))throw new Error('R314 DAG cycle detected');visiting.add(id);let value;
  if(node.op==='const')value=node.value;else if(node.op==='input'){if(!(node.key in inputs))throw new Error(`R314 missing input ${node.key}`);value=inputs[node.key]}else{const fn=OPS[node.op];if(!fn)throw new Error(`R314 unsupported DAG op ${node.op}`);const args=(node.args||[]).map(evalNode);const key=structuralHashR314({op:node.op,args});if(memo.has(key))value=memo.get(key);else{value=fn(...args);memo.set(key,value)}}
  visiting.delete(id);values.set(id,value);trace.push({id,op:node.op,hash:structuralHashR314(value)});return value};
