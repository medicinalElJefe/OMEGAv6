...
export const R340_RELEASE_ID='DEWEY_OMEGA_CERN_V4_ABLATION_ROUNDTRIP_FORECAST_2026-09-19';
...
export function forecastCompatibilityR340(candidate){
 ...
 const df=f-R340_FORECAST.center.fL,dc=c-R340_FORECAST.center.cParallel;
 const det=sf*sf*sc*sc*(1-rho*rho);
 const d2=det>0?(df*df*sc*sc-2*rho*df*dc*sf*sc+dc*dc*sf*sf)/det:Infinity;
 return{state:d2<=R340_FORECAST.compatibilityThresholdD2?'PASS':'FAIL',pass:d2<=R340_FORECAST.compatibilityThresholdD2,d2,threshold:R340_FORECAST.compatibilityThresholdD2,frozen:true,noRetuning:true};
}

export function calibrationManifestR340(){
 return{schema:R340_SCHEMA,revision:R340_REVISION,releaseId:R340_RELEASE_ID,sources:R340_SOURCE_MANIFEST,rowHashes:R340_ROW_HASHES,roundTrip:R340_ROUNDTRIP,ablation:R340_ABLATION,forecast:R340_FORECAST,canonicalMutation:false,canonicalAdmissionAuthority:'R125',truthBoundary:'R340 is a post-freeze proof/ablation/forecast layer over R334. It preserves the uploaded V4 hashes, uses ablation only to classify information contribution inside the declared restricted model, and freezes a prospective compatibility rule. It is not new collider data, is not an official ATLAS/CMS combination, cannot retune after future results are seen, and cannot supply missing SAR measurement physics.'};
}
