...
export const R315_FIELD_BOUNDARY='R315.FIELD executes the established Woven Continuity operator over an addressed computational field beneath R315 cross-skin orchestration. It conserves declared scalar invariant through bounded transport/repartition, carries scar/history and provenance, separates structure from orientation, and measures software residuals. It does not claim literal physical dimensions, empirical truth, external execution, CanonState admission, or new dispatch/promotion authority.';
...
function normalizeField(operators=[]){
 ...
 return operators.filter(row=>row&&row.active!==false).map((row,index)=>({
  ...
  address:Math.floor(finite(row?.address?.address??row.address??index)),deepAddress:Math.floor(finite(row?.address?.deepAddress??row.deepAddress??((row?.address?.address??row.address??index)*12))),
...
function repartition(field,targetResolution){
 ...
 for(const row of field){
  const targetAddress=projectAddressR266(row.address,20736,targetResolution),key=String(targetAddress);
...
function roundTripProjectionResidual(source,target,targetResolution){
 ...
 const sourceMass=new Map(source.map(row=>[row.address,row.value])),returned=new Map();
 for(const bucket of target){const back=projectAddressR266(bucket.address,targetResolution,20736);returned.set(back,(returned.get(back)||0)+bucket.value)}
...
export function executeWovenStateEvolutionR315({operators=[],orientation=0,transportRate=.125,targetResolution=20736,scarLedger=[],provenance=[],context={}}={}){
 ...
 const source=normalizeField(operators),s=sigma(orientation),declaredProvenance=normalizeProvenance(provenance),invariantBefore=sum(source.map(x=>x.value));
 ...
 const carriedScar=[...(Array.isArray(scarLedger)?scarLedger:[]),{revision:R315_FIELD_REVISION,orientation:s,entries:scarDelta}];
 const targetField=repartition(transport.field,targetResolution),invariantAfterRepartition=sum(targetField.map(x=>x.value)),invariantResidual=Math.abs(invariantBefore-invariantAfterRepartition),roundTripResidual=roundTripProjectionResidual(source,targetField,targetResolution);
 ...
 const r265=compileWovenDimensionalRelativityR265({...context,orientation:s,sourceFrame:context.sourceFrame??'R240_ADDRESSED_FIELD',targetFrame:context.targetFrame??'R315_RECONTEXTUALIZED_FIELD',sourceSkin:context.sourceSkin??'COMPUTE',targetSkin:context.targetSkin??'COMPUTE',sourceResolution:20736,targetResolution,scar:scarMagnitude,residual:Math.min(1,invariantResidual),roundTripResidual,provenance:[...declaredProvenance,'R315_FIELD_EVOLUTION']});
 const receiptBase={schema:R315_FIELD_SCHEMA,revision:R315_FIELD_REVISION,operator:R315_FIELD_OPERATOR,orientation:s,transportRate:transport.rate,sourceResolution:20736,targetResolution,invariantBefore,invariantAfterTransport,invariantAfterRepartition,invariantResidual,roundTripResidual,scarMagnitude,sourceCount:source.length,targetCount:targetField.length,provenance:[...declaredProvenance],authority:R315_FIELD_AUTHORITY};
...
export function restoreWovenStateR315(evolution={}){
 ...
 return{schema:'OMEGA_WOVEN_STATE_RESTORE_R315',revision:R315_FIELD_REVISION,restored,restoredHash:structuralHashR314(restored),sourceReceiptHash:evolution?.receipt?.receiptHash??null,recoverablePathUsed:true,dispatchRequested:false,canonAdmissionClaimed:false,authority:R315_FIELD_AUTHORITY};
}
