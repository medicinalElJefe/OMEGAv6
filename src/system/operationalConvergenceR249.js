...
export const R249_BOUNDARY='R249 is a software-operational scheduling governor. It may narrow planning width, order already-validated steps, and bound recursion from returned evidence. It does not create execution authority, dispatch, native proof, source promotion, production deployment, empirical truth, or CanonState admission.';
...
export function compileOperationalConvergenceR249({metrics={},evidence=null,previousScars=[],configuredParallel=12,effectiveCpuWorkers=0,executionRequested=false}={}){
 ...
 const futurePlasticity=clamp01(m.plasticity*(1-.45*m.burden)*(1-.35*m.scar));
 const orientation=Math.abs(invariantCarry-residualPressure)<.04?0:invariantCarry>residualPressure?1:-1;
 const capacity=clamp01(mean(m.continuity,m.plasticity,m.evidence,1-m.burden,1-m.contradiction));
 ...
 return{schema:R249_SCHEMA,revision:R249_REVISION,continuityOperator:R249_CONTINUITY_OPERATOR,metrics:m,decision:base.decision,decisionScore:base.score,commonKernel,invariantCarry,residualPressure,futurePlasticity,orientation,capacity,policy:{frontierWidth,maxMissionCycles,stepMode,executionRequested:Boolean(executionRequested),parallelSourceMutation:false},authority:R249_AUTHORITY,boundary:R249_BOUNDARY};
}
