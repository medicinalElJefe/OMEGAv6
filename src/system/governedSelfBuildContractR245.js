...
export const R245_CAPSULE_BODIES=Object.freeze({
 ...
 SG004:header+`export type CandidateStatus='GENERATED_PENDING_PROOF'|'SANDBOX'|'PROVED_PENDING_PROMOTION'|'SOURCE_PROMOTED_PENDING_PRODUCTION'|'SOURCE_MERGE_OBSERVED'|'REJECTED';\nexport type CandidateProofReceipt={capsuleId:string;baseSha:string;candidateSha:string|null;branch:string|null;tests:Record<string,boolean>;residualGate:'PASS'|'BLOCK'|'UNPROVEN';status:CandidateStatus;canonicalAdmission:false};\nexport function provedForPromotionCandidate(x:CandidateProofReceipt){return x.residualGate==='PASS'&&Object.values(x.tests).every(Boolean)&&x.status==='PROVED_PENDING_PROMOTION'&&x.canonicalAdmission===false}\nexport function provedForPromotionCandidateomotion(x:CandidateProofReceipt){return provedForPromotionCandidate(x)}\nexport const GOVERNED_GENERATOR_CONTRACT='R245_SHARED_CANONICAL_GENERATOR' as const;\n`,
