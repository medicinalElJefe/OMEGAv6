...
function bindSteps(steps:any[],context:{targetDeviceId:string;snapshotEpoch:number;snapshotObservedAt:number;sourceProfileSha256:any;sessionId:string}){return steps.map((step:any,index:number)=>{const stepId=compact(step?.id)||`S${String(index+1).padStart(2,'0')}`;return{...step,calculusBridgeR240:buildHybridBridgeCalculusR240({op:step?.op,stepId,...context})}})}
export function bindHybridJobBridgeCalculusR240(body:any,sessionId:string){
 if(!body||typeof body!=='object')return body;
 const context={targetDeviceId:compact(body.targetDeviceId),snapshotEpoch:Math.max(0,Math.floor(Number(body.snapshotEpoch)||0)),snapshotObservedAt:Math.max(0,Math.floor(Number(body.snapshotObservedAt)||0)),sourceProfileSha256:body.resourceEnvelopeR239?.sourceProfileSha256||null,sessionId};
 if(Array.isArray(body.steps))return{...body,steps:bindSteps(body.steps,context)};
 if(Array.isArray(body.draft?.steps))return{...body,draft:{...body.draft,steps:bindSteps(body.draft.steps,context)}};
 return body;
}
