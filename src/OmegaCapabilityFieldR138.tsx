...
export default function OmegaCapabilityFieldR138({panel,record,address,onAddress,onNavigate,status,restore}:Props){
 const[durableRun,setDurableRun]=useState<any>(null);
 ...
 return <section className='r138-capability-field' data-panel={panel} data-visual-policy='CAPABILITY_FIRST_NO_STALE_CHART_PRIMARY' data-unified-runtime='R139' data-operation-fabric='R140' data-execution-lifecycle='R142' data-ui-operation-chain='R143' data-durable-execution='R146' data-operation-chain-pass={operationAudit.pass?'true':'false'} data-candidate-authority={CANDIDATE_AUTHORITY} data-canonical-mutation='false'>
  <header className='r138-capability-head'><div><span>R146 · DURABLE OPERATION EXECUTION · R143 UI CHAIN · R142 LIFECYCLE · R140/R139 ENGINE</span><h2>{panel}</h2><p>Every routed control resolves through one explicit route ID → capability ID → execution domain → lifecycle receipt authority. R139 resolves intent, R140 ranks lawful routes, R142 separates discovery/availability/invocation/return/verification, R143 prevents visible controls from floating outside the authoritative route chain, and R146 persists authenticated operator selections into hash-chained Durable Object execution history. UI selection and route priority never become execution proof; R144 attestation and R145 world scar remain evidence only. Selecting STATE changes only the local projection; R125 canonical admission remains a separate proof-gated operation.</p></div><div className='r138-health'><b>{runtimeObserved?'RUNTIME OBSERVATION PRESENT':'RUNTIME UNVERIFIED'}</b><small>{durableRun?.id?`R146 ${durableRun.state} · ${String(durableRun.id).slice(0,16)}…`:durableRun?.error?'R146 durable run unavailable':`${continuityObserved?'continuity observation present':'continuity observation unverified'} · UI chain ${operationAudit.pass?'PASS':'HOLD'}`}</small></div></header>
  <div className='r138-capability-layout'>
   <div className='r138-live-stage' role='group' aria-label='Interactive capability topology; select a spatial node to operate that capability or projected state'>
    ...
    <span className='r138-stage-hint'>Spatial nodes are live controls · R143 binds route/capability/domain identity · R146 persists authenticated intent · R142 lifecycle labels express execution truth · Hybrid cannot become VERIFIED until current exact R141 return proof exists</span>
   </div>
  ...
  <footer className='r138-capability-ledger'><span>CΩ <b>{fmt(law.u.C)}</b></span><span>Φ <b>{fmt(law.u.Phi)}</b></span><span>q <b>{fmt(law.u.q)}</b></span><span>Λ <b>{fmt(law.u.Lambda)}</b></span><span>evidence <b>{fmt(law.u.evidence)}</b></span><span>modes <b>{runtime.modes.executable}/{runtime.modes.considered}</b></span><span>mode coverage <b>{fmt(operation.modeCoverage)}</b></span><span>UI chain <b>{operationAudit.mappedRoutes}/{operationAudit.totalRoutes}</b></span><span>routes available <b>{executionSummary.states.AVAILABLE||0}</b></span><span>route verified <b>{executionSummary.verified}</b></span><span>held <b>{executionSummary.held}</b></span><span>R146 durable <b>{durableRun?.state||'NO RUN'}</b></span><span>authority <b>PROJECTION ≠ ADMISSION · ROUTE ≠ EXECUTION</b></span></footer>
 </section>
}
