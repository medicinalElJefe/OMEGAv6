...
export default function OmegaCapabilityFieldR138({panel,record,address,onAddress,onNavigate,status,restore}:Props){
 ...
 return <section className='r138-capability-field' data-panel={panel} data-visual-policy='CAPABILITY_FIRST_NO_STALE_CHART_PRIMARY' data-unified-runtime='R139' data-operation-fabric='R140' data-execution-lifecycle='R142' data-candidate-authority={CANDIDATE_AUTHORITY} data-canonical-mutation='false'>
  <header className='r138-capability-head'><div><span>R142 · PROOF-AWARE CAPABILITY LIFECYCLE · R140 OPERATION FABRIC · R139.1 ENGINE</span><h2>{panel}</h2><p>Operate one coherent runtime from the current source state. R139 resolves intent into registered capabilities; R140 prioritizes lawful routes from CΩ/Φ/evidence/contradiction/burden and executable-mode coverage; R142 separately states whether a capability is merely discovered, authorized, available, invoked, returned or verified. Route priority never becomes execution proof. Selecting STATE changes only the local projection; R125 remains canonical admission authority.</p></div><div className='r138-health'><b>{runtimeObserved?'RUNTIME OBSERVATION PRESENT':'RUNTIME UNVERIFIED'}</b><small>{continuityObserved?'continuity observation present':'continuity observation unverified'}</small></div></header>
  <div className='r138-capability-layout'>
   <div className='r138-live-stage' role='group' aria-label='Interactive capability topology; select a spatial node to operate that capability or projected state'>
    ...
    <span className='r138-stage-hint'>Spatial nodes are live controls · lifecycle labels express execution truth · STATE selects projection · Hybrid cannot become AVAILABLE or VERIFIED until current device/return proof exists</span>
   </div>
  ...
  <footer className='r138-capability-ledger'><span>CΩ <b>{fmt(law.u.C)}</b></span><span>Φ <b>{fmt(law.u.Phi)}</b></span><span>q <b>{fmt(law.u.q)}</b></span><span>Λ <b>{fmt(law.u.Lambda)}</b></span><span>evidence <b>{fmt(law.u.evidence)}</b></span><span>modes <b>{runtime.modes.executable}/{runtime.modes.considered}</b></span><span>mode coverage <b>{fmt(operation.modeCoverage)}</b></span><span>routes available <b>{executionSummary.states.AVAILABLE||0}</b></span><span>route verified <b>{executionSummary.verified}</b></span><span>held <b>{executionSummary.held}</b></span><span>authority <b>PROJECTION ≠ ADMISSION · ROUTE ≠ EXECUTION</b></span></footer>
 </section>
}
