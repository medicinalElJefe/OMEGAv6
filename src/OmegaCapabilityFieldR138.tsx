...
export default function OmegaCapabilityFieldR138({panel,record,address,onAddress,onNavigate,status,restore}:Props){
 ...
 const actions:Action[]=[...routes.map(route=>({id:'route:'+route,label:route,detail:route==='Hybrid Link'?'open paired-host execution; authenticated current host proof remains required':route==='Evidence & Proof'?'inspect evidence, proof, source and lineage':route==='Build Out'?'assemble and verify a governed build candidate':'open active capability surface',route,kind:actionKind(route)})),...candidates.slice(0,4).map((x,i)=>({id:'state:'+x.address,label:i===0?'Ranked projected next':'Projected candidate '+(i+1),detail:`${x.relation} · STATE ${x.record.stateId} · rank ${fmt(x.score)} · U ${fmt(x.law.u.unifiedCoherence)} · evidence ${fmt(x.law.u.evidence)}`,address:x.address,kind:ACTION_KIND.STATE}))];
 const uncertainty=n01(record?.metrics?.uncertainty);
