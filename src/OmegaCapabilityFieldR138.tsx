...
export default function OmegaCapabilityFieldR138({panel,record,address,onAddress,onNavigate,status,restore}:Props){
 const[durableRun,setDurableRun]=useState<any>(null),[executorDirectory,setExecutorDirectory]=useState<any>(null);
 ...
 const persistRoute=async(action:Action)=>{if(!action.route)return;try{const chain=operationContractForRouteR143(action.route),run=await createAuthorizedOperationRunR146(chain,{panel,address,stateId:record.stateId,routePriorityState:action.executionState||chain.state});setDurableRun(run)}catch(e:any){setDurableRun({state:'UNAVAILABLE',error:e?.message||String(e)})}};
 ...
 return <section className='r138-capability-field' data-panel={panel} data-visual-policy='CAPABILITY_FIRST_NO_STALE_CHART_PRIMARY' data-unified-runtime='R139' data-operation-fabric='R140' data-execution-lifecycle='R142' data-ui-operation-chain='R143' data-durable-execution='R146' data-executor-fabric='R147' data-operation-chain-pass={operationAudit.pass?'true':'false'} data-candidate-authority={CANDIDATE_AUTHORITY} data-canonical-mutation='false'>
  <header className='r138-capability-head'><div><span>R147 · UNIFIED EXECUTOR FABRIC · R146 DURABLE RUNS · R143 UI CHAIN · R142 LIFECYCLE</span><h2>{panel}</h2><p>Every routed control resolves through one route ID → capability ID → execution domain → durable run → eligible executor chain. R139 resolves intent, R140 ranks lawful routes, R142 separates discovery/availability/invocation/return/verification, R143 prevents visible controls from floating outside the authoritative route chain, R146 persists authenticated operator intent, and R147 binds that run to concrete Workers AI, Hybrid host, autonomic swarm, federation or bounded local execution without pretending selection is execution. R144 attestation and R145 world scar remain evidence only. Selecting STATE changes only the local projection; R125 canonical admission remains a separate proof-gated operation.</p></div><div className='r138-health'><b>{runtimeObserved?'RUNTIME OBSERVATION PRESENT':'RUNTIME UNVERIFIED'}</b><small>{durableRun?.id?`R146 ${durableRun.state} · ${String(durableRun.id).slice(0,16)}… · R147 ${executorIds.length} executors available`:durableRun?.error?'R146 durable run unavailable':`${continuityObserved?'continuity observation present':'continuity observation unverified'} · R147 ${executorIds.length} executors available · UI chain ${operationAudit.pass?'PASS':'HOLD'}`}</small></div></header>
  <div className='r138-capability-layout'>
   <div className='r138-live-stage' role='group' aria-label='Interactive capability topology; select a spatial node to operate that capability or projected state'>
    ...
    <span className='r138-stage-hint'>Spatial nodes are live controls · R143 binds route/capability/domain identity · R146 persists authenticated intent · R147 resolves concrete executor availability · queueing is not invocation · Hybrid cannot become VERIFIED until current exact R141 return proof exists</span>
   </div>
  ...
  <footer className='r138-capability-ledger' title={R147_EXECUTION_BOUNDARY}><span>CΩ <b>{fmt(law.u.C)}</b></span><span>Φ <b>{fmt(law.u.Phi)}</b></span><span>q <b>{fmt(law.u.q)}</b></span><span>Λ <b>{fmt(law.u.Lambda)}</b></span><span>evidence <b>{fmt(law.u.evidence)}</b></span><span>modes <b>{runtime.modes.executable}/{runtime.modes.considered}</b></span><span>mode coverage <b>{fmt(operation.modeCoverage)}</b></span><span>UI chain <b>{operationAudit.mappedRoutes}/{operationAudit.totalRoutes}</b></span><span>executors <b>{executorIds.length} available</b></span><span>routes available <b>{executionSummary.states.AVAILABLE||0}</b></span><span>route verified <b>{executionSummary.verified}</b></span><span>held <b>{executionSummary.held}</b></span><span>R146 durable <b>{durableRun?.state||'NO RUN'}</b></span><span>authority <b>PROJECTION ≠ ADMISSION · ROUTE ≠ EXECUTION</b></span></footer>
 </section>
}
