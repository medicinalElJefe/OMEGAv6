...
export default function OmegaCapabilityFieldR138({panel,record,address,onAddress,onNavigate,status,restore}:Props){
 ...
 return <section className='r138-capability-field' data-panel={panel} data-visual-policy='CAPABILITY_FIRST_NO_STALE_CHART_PRIMARY' data-unified-runtime='R139'>
  <header className='r138-capability-head'><div><span>R139 · UNIFIED CAPABILITY ENGINE · R138 LIVING FIELD</span><h2>{panel}</h2><p>Operate one coherent runtime from the living state. Full Overall mode planning now drives registered capability routes; exact/source/derived modes may execute, while gated and catalog-only modes remain visible without being falsely promoted.</p></div><div className='r138-health'><b>{runtimeHealthy?'RUNTIME PRESENT':'RUNTIME UNVERIFIED'}</b><small>{restoreHealthy?'continuity surface present':'restore surface unverified'}</small></div></header>
  ...
  <footer className='r138-capability-ledger'><span>CΩ <b>{fmt(law.u.C)}</b></span><span>Φ <b>{fmt(law.u.Phi)}</b></span><span>q <b>{fmt(law.u.q)}</b></span><span>Λ <b>{fmt(law.u.Lambda)}</b></span><span>evidence <b>{fmt(law.u.evidence)}</b></span><span>modes considered <b>{runtime.modes.considered}</b></span><span>executable <b>{runtime.modes.executable}</b></span><span>gated <b>{runtime.modes.gated}</b></span><span>catalog-only <b>{runtime.modes.catalogLens}</b></span></footer>
 </section>
}
