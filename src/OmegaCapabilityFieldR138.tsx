...
type Action={id:string;label:string;detail:string;route?:string;address?:number;kind:'STATE'|'ROUTE'|'EXECUTE'|'PROVE'|'BUILD'|'EXPLORE'|'INTELLIGENCE'|'GOVERN'|'SYSTEM';executionState?:string};
...
export default function OmegaCapabilityFieldR138({panel,record,address,onAddress,onNavigate,status,restore}:Props){
 const[durableReceipt,setDurableReceipt]=useState<any>(null);
 ...
 const actions:Action[]=[...operation.actions.slice(0,6).map((x,i)=>({id:'route:'+x.route,label:x.route,detail:`${x.reason} · ${x.readiness} · lifecycle ${routeReceipts[i]?.state||'DISCOVERED'} · signal ${fmt(x.score)}`,route:x.route,kind:x.kind,executionState:routeReceipts[i]?.state})),...candidates.slice(0,4).map((x,i)=>({id:'state:'+x.address,label:i===0?'Ranked projected next':'Projected candidate '+(i+1),detail:`${x.relation} · STATE ${x.record.stateId} · rank ${fmt(x.score)} · U ${fmt(x.law.u.unifiedCoherence)} · evidence ${fmt(x.law.u.evidence)} · projection only`,address:x.address,kind:'STATE' as const,executionState:'PROJECTION'}))];
 ...
 const persistRouteIntent=async(action:Action)=>{if(!action.route)return;try{const created=await api.post<any>('/api/execution/runs',{intent:`${panel} → ${action.route}`,executor:{kind:action.kind==='EXECUTE'?'HYBRID':'ROUTE',route:action.route,capabilityId:action.id},metadata:{panel,address,stateId:record.stateId,source:'OmegaCapabilityFieldR138'}}),id=created.data?.run?.id;if(!id)return;const authorized=await api.post<any>(`/api/execution/runs/${encodeURIComponent(id)}/transition`,{state:'AUTHORIZED',reason:'User selected this capability from the authenticated OMEGA runtime.',evidence:{proofRef:'R143_AUTHENTICATED_OPERATOR_SELECTION',resultFingerprint:null}});setDurableReceipt(authorized.data?.run||created.data?.run||null)}catch(e:any){setDurableReceipt({state:'UNAVAILABLE',id:null,error:e?.message||String(e)})}};
 const runAction=(action:Action)=>{if(action.route){void persistRouteIntent(action);onNavigate(action.route)}else if(action.address!==undefined)onAddress(action.address)};
 const runKey=(e:any,action:Action)=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();runAction(action)}};
 return <section className='r138-capability-field' data-panel={panel} data-visual-policy='CAPABILITY_FIRST_NO_STALE_CHART_PRIMARY' data-unified-runtime='R139' data-operation-fabric='R140' data-execution-lifecycle='R143' data-candidate-authority={CANDIDATE_AUTHORITY} data-canonical-mutation='false'>
  <header className='r138-capability-head'><div><span>R143 · DURABLE EXECUTION RUNTIME · R142 LIFECYCLE · R140 OPERATION FABRIC · R139.1 ENGINE</span><h2>{panel}</h2><p>Operate one coherent runtime from the current source state. R139 resolves intent into registered capabilities; R140 prioritizes lawful routes from CΩ/Φ/evidence/contradiction/burden and executable-mode coverage; R142 classifies lifecycle truth; R143 now persists authenticated capability selections as hash-chained execution runs. Route priority never becomes execution proof. Selecting STATE changes only the local projection; R125 canonical admission remains a separate proof-gated operation.</p></div><div className='r138-health'><b>{runtimeObserved?'RUNTIME OBSERVATION PRESENT':'RUNTIME UNVERIFIED'}</b><small>{durableReceipt?.id?`R143 ${durableReceipt.state} · ${String(durableReceipt.id).slice(0,18)}…`:durableReceipt?.error?'R143 durable receipt unavailable':continuityObserved?'continuity observation present':'continuity observation unverified'}</small></div></header>
  <div className='r138-capability-layout'>
   <div className='r138-live-stage' role='group' aria-label='Interactive capability topology; select a spatial node to operate that capability or projected state'>
    ...
    <span className='r138-stage-hint'>Spatial nodes are live controls · R143 persists authenticated route intent · lifecycle labels express execution truth · STATE selects projection · Hybrid cannot become AVAILABLE or VERIFIED until current device/return proof exists</span>
   </div>
   <aside className='r138-action-stack'>{actions.map(action=>{const Icon=action.route?ICONS[action.route]||ArrowRight:Waypoints;return <button key={action.id} data-kind={action.kind} data-execution-state={action.executionState||'DISCOVERED'} onClick={()=>runAction(action)}><Icon/><span><b>{action.label}</b><small>{action.detail}</small></span><ArrowRight/></button>})}</aside>
  </div>
  <footer className='r138-capability-ledger'><span>CΩ <b>{fmt(law.u.C)}</b></span><span>Φ <b>{fmt(law.u.Phi)}</b></span><span>q <b>{fmt(law.u.q)}</b></span><span>Λ <b>{fmt(law.u.Lambda)}</b></span><span>evidence <b>{fmt(law.u.evidence)}</b></span><span>modes <b>{runtime.modes.executable}/{runtime.modes.considered}</b></span><span>mode coverage <b>{fmt(operation.modeCoverage)}</b></span><span>routes available <b>{executionSummary.states.AVAILABLE||0}</b></span><span>route verified <b>{executionSummary.verified}</b></span><span>held <b>{executionSummary.held}</b></span><span>R143 durable <b>{durableReceipt?.state||'NO RUN'}</b></span><span>authority <b>PROJECTION ≠ ADMISSION · ROUTE ≠ EXECUTION</b></span></footer>
 </section>
}
