...
export default function OmegaCapabilityFieldR138({panel,record,address,onAddress,onNavigate,status,restore}:Props){
 ...
 return <section className='r138-capability-field' data-panel={panel} data-visual-policy='CAPABILITY_FIRST_NO_STALE_CHART_PRIMARY' data-unified-runtime='R139' data-candidate-authority={CANDIDATE_AUTHORITY} data-canonical-mutation='false'>
  <header className='r138-capability-head'><div><span>R140 · UNIFIED LIVING OPERATION FABRIC · R139.1 CAPABILITY ENGINE</span><h2>{panel}</h2><p>Operate one coherent runtime from the current source state. Full Overall mode planning drives registered capability routes; exact/source/derived modes may execute, while gated and catalog-only modes remain visible without false promotion. Selecting a STATE changes the local projection only; R125 canonical admission remains a separate proof-gated operation.</p></div><div className='r138-health'><b>{runtimeObserved?'RUNTIME OBSERVATION PRESENT':'RUNTIME UNVERIFIED'}</b><small>{continuityObserved?'continuity observation present':'continuity observation unverified'}</small></div></header>
  <div className='r138-capability-layout'>
   <div className='r138-live-stage' role='group' aria-label='Interactive capability topology; select a spatial node to operate that capability or projected state'>
    ...
    <span className='r138-stage-hint'>Spatial nodes are live controls · STATE selects a local projection · EXECUTE / BUILD / PROVE / INTELLIGENCE / GOVERN / SYSTEM open their bounded registered capability surfaces</span>
   </div>
  ...
  <footer className='r138-capability-ledger'><span>CΩ <b>{fmt(law.u.C)}</b></span><span>Φ <b>{fmt(law.u.Phi)}</b></span><span>q <b>{fmt(law.u.q)}</b></span><span>Λ <b>{fmt(law.u.Lambda)}</b></span><span>evidence <b>{fmt(law.u.evidence)}</b></span><span>modes considered <b>{runtime.modes.considered}</b></span><span>executable <b>{runtime.modes.executable}</b></span><span>gated <b>{runtime.modes.gated}</b></span><span>catalog-only <b>{runtime.modes.catalogLens}</b></span><span>authority <b>PROJECTION ≠ ADMISSION</b></span></footer>
 </section>
}
