...
export default function TraversalModeStageR100({variant,address,onAddress}:Props){
 ...
 useEffect(()=>{const el=canvas.current;if(!el)return;let raf=0,alive=true,lastBand=-1;const ctx=el.getContext('2d');if(!ctx)return;
  const render=(ms:number)=>{if(!alive)return;const rect=el.getBoundingClientRect(),dpr=Math.min(2,window.devicePixelRatio||1),w=Math.max(420,rect.width),h=Math.max(420,rect.height);if(el.width!==Math.floor(w*dpr)||el.height!==Math.floor(h*dpr)){el.width=Math.floor(w*dpr);el.height=Math.floor(h*dpr)}ctx.setTransform(dpr,0,0,dpr,0,0);
   const dt=clock.current.last?Math.min(.05,Math.max(0,(ms-clock.current.last)/1000)):0;clock.current.last=ms;if(animated)clock.current.elapsed+=dt*timeScale;const t=clock.current.elapsed,weave=deriveWeaveStateR100(address,u,t,1);if(weave.phaseBand!==lastBand){lastBand=weave.phaseBand;setWeaveReadout({phaseBand:weave.phaseBand,pulse:weave.pulse})}
   ...
   // Recursive weave shells. The four sparse reference boundaries encode atlas resolution,
   // not literal physical dimensions.
   ctx.save();ctx.translate(cx,cy);ctx.rotate(weave.torsion*.08);
   for(let i=0;i<weave.ringCount;i++){const f=(i+1)/weave.ringCount,r=scale*(.12+.96*f),ell=1-.12*u.shape.anisotropy+Math.sin(weave.phase+i*.33)*.018*weave.pulse,major=i===0||i===weave.ringCount-1||i%Math.max(1,Math.round(weave.ringCount/4))===0,ch=weaveChannelR100(i%4,weave,u),alpha=major?.12+.12*ch.strength:.018+.055*ch.strength;ctx.beginPath();ctx.ellipse(0,0,r,r*ell,weave.torsion*f*.12,0,Math.PI*2);ctx.strokeStyle=`rgba(${ch.r},${ch.g},${ch.b},${alpha})`;ctx.lineWidth=major?1.05:.42;ctx.stroke()}
