...
import './hybridExecutionMotionR242.css';
...
export default function HybridExecutionMotionR242(){
 ...
 const leaseLive=status==='RUNNING'&&leaseUntil>now&&p?.revision==='R242';
 ...
 return <section className='r242-motion' aria-label='R242 Hybrid execution motion' data-r242-motion={state} data-r242-job={active?.id||'NONE'} data-r242-epoch={epoch}>
  <header><div><span>R242 · HYBRID EXECUTION MOTION · AUTHENTICATED LEASE + STEP PROGRESS</span><h3>See what the PC is doing while it is doing it.</h3><p>A RUNNING label alone is no longer enough. R242 requires renewable host motion for new jobs and exposes the exact current allow-listed step, elapsed time and returned-step count. A lease proves only continued ownership/liveness of the running claim; final success still requires the normal R141 returned proof.</p></div><Activity/></header>
  <div className='r242-motion-grid'>
   ...
   <article className={leaseLive?'pass':leaseExpired?'hold':'neutral'}><Clock3/><div><small>MOTION / LEASE</small><b>{p?`${fmtMs(Number(p.elapsedMs||0))} elapsed`:'No R242 pulse yet'}</b><span>{leaseLive?`last pulse ${age(p.at)} ago · lease ${fmtMs(leaseUntil-now)} remaining`:leaseExpired?`lease expired ${age(leaseUntil)} ago`:legacyRunning?'This job predates R242 motion; restart the current PC connector after R242 deploy to recover it safely.':'No running lease required'}</span></div></article>
  ...
  {total>0&&<div className='r242-progress' role='progressbar' aria-valuemin={0} aria-valuemax={total} aria-valuenow={completed}><i style={{width:`${pct}%`}}/></div>}
  {(leaseExpired||legacyRunning)&&<div className='r242-motion-warning'><TriangleAlert/><span>{leaseExpired?'The Worker can no longer prove continued ownership of this RUNNING claim. R242 fails the stale claim closed and only auto-retries bounded non-mutating discovery after the authenticated agent polls again.':'This is the exact legacy condition visible in the screenshot: RUNNING exists, but no execution lease/progress protocol exists for that already-claimed job. Once the R242 agent is restarted, an old expired discovery claim can be replaced by bounded discovery rather than remaining permanently INVOKED.'}</span></div>}
  <footer><ShieldCheck/>R242 motion is observational control-plane evidence, not execution success. It does not kill a native process remotely, replay mutations, alter R141 exact-return closure, R146 history, R147 dispatch authority, or R125 CanonState admission.</footer>
 </section>;
}
