import {R208_INDEPENDENCE_LAW,R208_REVISION,R208_SCHEMA,type R208LineageEnvelope,type R208Node} from './missionLineageR208';

export const R208_EXECUTION_SCHEMA='OMEGA_R208_EXECUTION_LINEAGE_CORRELATION';
export const R208_EXECUTION_BOUNDARY='This compiler correlates receipts emitted by existing R179/R147/R141/R146/residual authorities. It cannot authorize, dispatch, execute, verify a host return, mutate durable execution history, or admit CanonState.';
...
export type R208ExecutionCorrelationInput={authorization?:AuthorizationReceipt;executor?:ExecutorReceipt;executionReturn?:ReturnReceipt;r141?:R141Receipt;r146?:R146Receipt;residual?:ResidualReceipt;repairProposal?:RepairReceipt};
...
const lastId=(nodes:R208Node[])=>nodes.at(-1)?.id||'';
const node=(kind:R208Node['kind'],id:string,authority:string,parentIds:string[],independence:R208Node['independence']):R208Node=>({kind,id,authority,parentIds,independence,canonicalMutation:false});

export async function compileExecutionLineageR208(base:R208LineageEnvelope,input:R208ExecutionCorrelationInput){
 if(base?.schema!==R208_SCHEMA||base?.revision!==R208_REVISION)throw new Error('R208 base lineage required');
 const nodes=[...base.nodes],errors:string[]=[];
 let completeThrough:R208Node['kind']='PLAN_VALIDATION';
 ...
 if(errors.length)return{ok:false as const,schema:R208_EXECUTION_SCHEMA,revision:R208_REVISION,lineageId:base.lineageId,errors,completeThrough,nodes,canonicalMutation:false as const,canonicalAdmissionAuthority:'R125' as const,boundary:R208_EXECUTION_BOUNDARY};
 const correlationSha256=await sha256({lineageId:base.lineageId,ancestrySha256:base.ancestrySha256,nodes,completeThrough,independenceLaw:R208_INDEPENDENCE_LAW});
 return{ok:true as const,schema:R208_EXECUTION_SCHEMA,revision:R208_REVISION,lineageId:base.lineageId,proposalId:base.proposalId,draftFingerprint:base.draftFingerprint,ancestrySha256:base.ancestrySha256,correlationSha256,completeThrough,nodes,independenceLaw:R208_INDEPENDENCE_LAW,authorities:{operatorAuthorization:'R179',executor:'R147',hybridReturnProof:'R141',durableHistory:'R146',canonicalAdmission:'R125'},queueAuthority:false,dispatchAuthority:false,executionAuthority:false,persistenceOwner:false,scientificTruthAuthority:false,canonicalMutation:false as const,canonicalAdmissionAuthority:'R125' as const,boundary:R208_EXECUTION_BOUNDARY};
}
