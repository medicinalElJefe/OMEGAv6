...
export type RelativeExecutionReadinessR154='ROUTE_READY'|'EVIDENCE_REQUIRED'|'DEVICE_PROOF_REQUIRED'|'PROOF_REQUIRED'|'SANDBOX_ONLY'|'ROUTE_UNAVAILABLE';
export type RelativeOperationPlanR154={
  ...
  capacity:{
    tier:RelativeCapacityTierR154;
    logicalLanes:number;
    logicalFanout:number;
    temporalHz:number;
    historyDepth:number;
    viewResolution:12|144|1728|20736|248832;
    accuracyLanes:number;
    solverFidelity:RelativeSolverFidelityR154;
  };
  relativity:{
    whole:string;
    part:string;
  ...
  lineage:{r153Fingerprint:string;truthFingerprint:string;lemmaFingerprint:string;nowAddress:string;canonicalStateId:number;routeContract:string};
  boundary:string;
};
...
function kindPressure(kind:string){
  if(kind==='PROVE')return 1;
  if(kind==='EXECUTE')return .94;
  if(kind==='BUILD')return .88;
  if(kind==='INTELLIGENCE')return .76;
  if(kind==='GOVERN')return .72;
  if(kind==='SYSTEM')return .68;
  return .60;
}
...
function readiness(action:OperationActionR140,truth:any,contractState:string):RelativeExecutionReadinessR154{
  if(contractState==='UNAVAILABLE')return'ROUTE_UNAVAILABLE';
...
export function compileRelativeCapacityFabricR154(record:any,input:R153Input&{panel?:string;intent?:string}){
  ...
  const runtime=compileUnifiedCapabilityRuntimeR139(record,input.panel||'',input.intent||''),operation=rankUnifiedCapabilityActionsR140(runtime,record),truth=now.truth;
  ...
  const plans:RelativeOperationPlanR154[]=operation.actions.map(action=>{
    const contract=operationContractForRouteR143(action.route),operationP=kindPressure(action.kind),routeBase=cl(action.score),combined=cl(.20*routeBase+.18*motion+.16*residual+.13*truthGap+.10*coherenceGap+.09*temporalError+.06*observer+.08*operationP),logicalFanout=fanout(combined),logicalLanes=Math.max(now.promotion.accuracy.lanes,logicalFanout),tier=tierFromLanes(logicalLanes),relativeView=Math.max(now.promotion.view.resolution,viewResolution(combined)) as 12|144|1728|20736|248832,solver=solverFidelity(action.route,action.kind,combined,truth),ready=readiness(action,truth,contract.state),relativePriority=cl(.70*routeBase+.30*combined);
    return{
      route:action.route,workspace:action.workspace,kind:action.kind,executionDomain:contract.executionDomain,capabilityId:contract.capabilityId,routeId:contract.routeId,contractState:contract.state,basePriority:routeBase,relativePriority,readiness:ready,
      capacity:{tier,logicalLanes,logicalFanout,temporalHz:temporalHz(cl(.55*motion+.25*temporalError+.20*combined)),historyDepth:historyDepth(cl(.45*residual+.25*motion+.20*coherenceGap+.10*combined)),viewResolution:relativeView,accuracyLanes:now.promotion.accuracy.lanes,solverFidelity:solver},
      relativity:{whole:`CANON_STATE:${now.canonical.stateId}`,part:`CAPABILITY:${contract.capabilityId}`,inner:`EXECUTION_DOMAIN:${contract.executionDomain}`,outer:'SYSTEM:R154',observer:input.frame.observerFrame,orientation:input.frame.orientation,projectionAddress:now.motion.projection.temporalProjectionAddress,canonicalAddress:now.canonical.address,nowAddress:now.now.id},
      pressures:{motion,residual,truthGap,coherenceGap,temporalError,observer,operation:operationP,combined},
      lineage:{r153Fingerprint:now.fingerprint,truthFingerprint:truth.fingerprint,lemmaFingerprint:now.selfModel.currentLemmaFingerprint,nowAddress:now.now.id,canonicalStateId:now.canonical.stateId,routeContract:`${contract.routeId}|${contract.capabilityId}|${contract.executionDomain}|${contract.state}`},
