...
export default function OmegaEarthRelativitySphereR121({address,onAddress,projection='MANDALA',view='SOURCE_COLOR'}:Props){
 const canvas=useRef<HTMLCanvasElement|null>(null),drag=useRef<{x:number;rotation:number}|null>(null);
 ...
 useEffect(()=>{if(!running)return;let raf=0,last=performance.now();const tick=(now:number)=>{const dt=Math.min(50,now-last);last=now;setFreeRotation(v=>wrapLon(v+dt*.0027));raf=requestAnimationFrame(tick)};raf=requestAnimationFrame(tick);return()=>cancelAnimationFrame(raf)},[running]);
 useEffect(()=>{const c=canvas.current;if(!c)return;let raf=0,alive=true,lastFrame=0;const reduced=window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const draw=(time=0)=>{if(!alive)return;if(!reduced&&time-lastFrame<32){raf=requestAnimationFrame(draw);return}lastFrame=time;
   ...
   const cx=W*.5,cy=H*.49,R=Math.min(W*.315,H*.335),t=time*.001,rot=wrapLon(freeRotation);
 ...
 const pointerDown=(e:React.PointerEvent<HTMLCanvasElement>)=>{drag.current={x:e.clientX,rotation:freeRotation};setRunning(false);e.currentTarget.setPointerCapture(e.pointerId)},pointerMove=(e:React.PointerEvent<HTMLCanvasElement>)=>{if(!drag.current)return;setFreeRotation(wrapLon(drag.current.rotation-(e.clientX-drag.current.x)*.35))},pointerUp=()=>{drag.current=null};
 const choose=(e:React.DoubleClickEvent<HTMLCanvasElement>)=>{if(!onAddress)return;const rect=e.currentTarget.getBoundingClientRect(),W=rect.width,H=rect.height,cx=W*.5,cy=H*.49,R=Math.min(W*.315,H*.335),nx=(e.clientX-rect.left-cx)/R,ny=(e.clientY-rect.top-cy)/R;if(nx*nx+ny*ny>1)return;const z=Math.sqrt(Math.max(0,1-nx*nx-ny*ny)),lat=deg(Math.asin(clamp(-ny,-1,1))),lon=wrapLon(deg(Math.atan2(nx,z))+freeRotation),d=clamp(Math.floor((lat+90)/180*12),0,11),lonBin=clamp(Math.floor((lon+180)/360*144),0,143),p=Math.floor(lonBin/12),r=lonBin%12,l=mapped.coords.l;onAddress(atlasAddress(d,p,r,l))};
 ...
 return <section className='r121-sphere r122-volume' data-truth-boundary='REFERENCE OBSERVED REPRESENTATIONAL' data-projection={projection} data-view={view}>
  <header className='r121-sphere-head'><div><span>R122 · VOLUMETRIC EARTH / RELATIVITY INSTRUMENT</span><b>Core → surface → atmosphere → orbital context → 20,736-cell relative field</b><small>{projection} geometry · {view.replaceAll('_',' ')} data skin · physical reference, returned observations and canonical representation remain separated.</small></div><div><button onClick={()=>setRunning(v=>!v)}>{running?<Pause/>:<Play/>}{running?'Pause':'Motion'}</button><button onClick={()=>void load()} disabled={busy}><RefreshCw className={busy?'spin':''}/>{busy?'Refreshing':'Refresh evidence'}</button></div></header>
  ...
  <div className='r121-sphere-body'>
   <div className='r121-sphere-stage'><canvas ref={canvas} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} onDoubleClick={choose} aria-label='Interactive whole-sphere Earth relativity instrument'/><div className='r121-sphere-hint'>DRAG ROTATION · DOUBLE-CLICK EARTH TO MOVE ADDRESS · ALL 20,736 CANONICAL CELLS PARTICIPATE IN THE RELATIVE FIELD</div></div>
   <aside className='r121-sphere-telemetry'>
    ...
    <section><header><Atom/><span><b>APPLIED RELATIVE FIELD</b><small>atlas address resolution · not physical dimensions</small></span></header><div className='r121-kpis'><article><span>CELLS</span><b>{STATE_COUNT.toLocaleString()}</b></article><article><span>CΩ</span><b>{fmt(record.metrics.continuity,3)}</b></article><article><span>Φ</span><b>{fmt(record.metrics.plasticity,3)}</b></article><article><span>q / Λ</span><b>{fmt(record.metrics.contradiction,2)} / {fmt(record.metrics.burden,2)}</b></article></div><p><Waypoints/>Relative radius is computed from layer role + invariant carry + plastic phase − contradiction/burden compression. L changes representational radius only; it is never asserted as literal geophysical depth.</p></section>
   </aside>
  </div>
  <div className='r121-source-band'><div><ShieldCheck/><span><b>TRUTH BOUNDARY</b><small>REFERENCE = published Earth constants · OBSERVED = returned provider data · REPRESENTATIONAL = OMEGA atlas/calculus projection</small></span></div><div><Layers3/><span><b>RELATIVE FRAME LAW</b><small>12-frame scaffold, radial canonical field, route motion and atmosphere/orbit context are visibly encoded as model layers rather than disguised as measured geology.</small></span></div></div>
