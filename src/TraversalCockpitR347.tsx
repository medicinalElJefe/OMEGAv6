...
import{R347_TRUTH_BOUNDARY,R347_UNIT_POLICY,R347_VISUAL_GRAMMAR,admitScalarChannelR347,lensScoreSetR347,modelMappedWgs84R347,routeR347,scaleReferenceR347,visualStateR347,type R347TaskView}from'./visualTraversalCockpitR347';
...
export default function TraversalCockpitR347({address,onSelect,onNavigate}:Props){
 ...
 ctx.fillStyle=isSelected?`rgba(241,199,107,${Math.max(.85,alpha)})`:isRoute&&view==='FORECAST'?`rgba(134,193,169,${Math.max(.18,alpha)})`:`rgba(92,170,177,${alpha})`;ctx.fillRect(p.px-r/2,p.py-r/2,r,r)}
 ctx.setLineDash(view==='FORECAST'?[6,7]:[]);ctx.lineJoin='round';ctx.lineCap='round';for(let i=0;i<route.length-1;i++){const a=project(route[i].address),b=project(route[i+1].address),rec=corpusState(route[i].address),line=.7+2.6*calibratedValue(cal,'C',Number(rec.metrics.continuity)),ev=calibratedValue(cal,'evidence',Number(rec.metrics.evidence));ctx.strokeStyle=view==='FORECAST'?`rgba(130,196,168,${.18+.56*ev})`:`rgba(226,187,104,${.12+.55*ev})`;ctx.lineWidth=line;ctx.beginPath();ctx.moveTo(a.px,a.py);ctx.lineTo(b.px,b.py);ctx.stroke()}ctx.setLineDash([]);
 ...
 const sourceName=(v:any,fallback:string)=>{const s=v?.source||v?.provider||v?.endpoint;return typeof s==='string'&&s.trim()?s:fallback};
 const liveScalars=[
  admitScalarChannelR347({id:'temperature',label:'Temperature',value:earth?.localConditions?.temperatureC,unit:'°C',source:sourceName(earth?.sources?.openMeteo,'Open-Meteo'),observedAt:earth?.sources?.openMeteo?.verifiedAt||earth?.verifiedAt,truth:'OBSERVED'}),
  admitScalarChannelR347({id:'wind',label:'Wind',value:earth?.localConditions?.windKph,unit:'km/h',source:sourceName(earth?.sources?.openMeteo,'Open-Meteo'),observedAt:earth?.sources?.openMeteo?.verifiedAt||earth?.verifiedAt,truth:'OBSERVED'}),
  admitScalarChannelR347({id:'seismic',label:'Seismic events',value:earth?.seismic?.count,unit:'events/24h',source:sourceName(earth?.sources?.usgs,'USGS'),observedAt:earth?.sources?.usgs?.verifiedAt||earth?.verifiedAt,truth:'OBSERVED'}),
  admitScalarChannelR347({id:'kp',label:'Kp index',value:earth?.spaceWeather?.kp,unit:'index',source:sourceName(earth?.sources?.swpc,'NOAA SWPC'),observedAt:earth?.spaceWeather?.observationTime||earth?.sources?.swpc?.verifiedAt||earth?.verifiedAt,truth:'OBSERVED'})
 ];
