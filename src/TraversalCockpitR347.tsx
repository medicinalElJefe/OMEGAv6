...
export default function TraversalCockpitR347({address,onSelect,onNavigate}:Props){
 const canvas=useRef<HTMLCanvasElement|null>(null),projected=useRef<Projected[]>([]);
 ...
 raf=requestAnimationFrame(draw)};raf=requestAnimationFrame(draw);return()=>{alive=false;cancelAnimationFrame(raf)}},[field,cal,address,lens,view,zoom,playing,reduced,route,routeSet,scale.label,scale.extent,scale.authority,hover]);

 const hit=(e:React.PointerEvent<HTMLCanvasElement>)=>{const r=e.currentTarget.getBoundingClientRect(),x=e.clientX-r.left,y=e.clientY-r.top;let best:null|Projected=null,bd=18;for(const p of projected.current){const d=Math.hypot(p.x-x,p.y-y);if(d<Math.max(bd,p.r)){best=p;bd=d}}setHover(best?.i??null)};
 ...
 return <section className='r347-cockpit'>
  ...
  <div className='r347-workspace'>
   <main className='r347-stage'>
    <canvas ref={canvas} onPointerMove={hit} onPointerLeave={()=>setHover(null)} onPointerDown={choose} aria-label='Calibrated 20,736 state traversal field'/>
