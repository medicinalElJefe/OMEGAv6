...
export default function TraversalCockpitR347({address,onSelect,onNavigate}:Props){
 ...
 for(let i=0;i<field.count;i+=stride){const p=project(i);if(p.px<-20||p.px>W+20||p.py<-20||p.py>H+20)continue;const w=mandalaLensWeight(field,i,lens),E=calibratedValue(cal,'evidence',field.evidence[i]),C=calibratedValue(cal,'C',field.C[i]),scar=calibratedValue(cal,'scar',field.scar[i]),qv=calibratedValue(cal,'q',field.q[i]),Lv=calibratedValue(cal,'Lambda',field.Lambda[i]),isRoute=routeSet.has(i),isSelected=i===address,isHover=i===hoverRef.current,sameDomain=field.domain[i]===field.domain[address];let visibility=1;if(view==='ROUTE')visibility=isRoute?1:.08;else if(view==='PROOF')visibility=.08+.92*w;else if(view==='SCAR')visibility=.08+.92*scar;else if(view==='FORECAST')visibility=isRoute ? .96 : .045;if(zoomBand==='CORRIDOR'&&!isRoute)visibility*=sameDomain ? .62 : .16;if(zoomBand==='DETAIL'&&!isRoute&&!isSelected)visibility*=sameDomain ? .30 : .035;const alpha=(.025+.42*E)*visibility*(.45+.55*w),stress=cl(.58*qv+.42*Lv),r=(.55+1.8*(.55*C+.45*w))*(isSelected?2.8:isHover?2.1:1);if(isSelected||isHover||isRoute)pts.push({i,x:p.px,y:p.py,r:Math.max(7,r*2.2)});
