...
export default function MissionWorldContinuityR206({onNavigate}:Props){
 ...
 return <section className='r206-world-mission' data-state={held?'HELD':view.sceneBound?'BOUND':'ASSEMBLED'} aria-label='Living world mission continuity'>
  ...
  <div className='r206-world-mission-copy'><small>R204 → R206 → R208 → R211 → R213 → R214 · INTENT · WORLD · PRE-RENDER LINEAGE · ADAPTIVE ATTEMPT · RECOVERABLE CONTINUITY</small><b>{stateLabel} · {view.current.command}</b><span><Route/> {view.route} · continuity {view.anchor.carryHash} · scar {view.anchor.scarCarry.toFixed(3)} · return {view.anchor.returnCarry.toFixed(3)}</span><em>{durableLabel} · {view.sceneBound?'Evidence-bound scene attached':'Scene evidence not attached'} · R122 {view.renderReady?'INPUT READY':'HELD'} · {renderLabel} · {attemptLabel} · {anchorLabel} · FRAME NOT RENDERED · COMPUTED PHOTOREAL REALITY UNPROVEN</em></div>
  <div className='r206-world-mission-proof'><ShieldCheck/><span>{view.proof}</span><span>World R134 · persist R149/R97</span><span>Render continuity R86/R87/R97</span><span>Canon R125 only</span><span>Federation receipt-gated</span></div>
