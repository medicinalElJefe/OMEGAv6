...
export default function MissionWorldContinuityR206({onNavigate}:Props){
 ...
 return <section className='r206-world-mission' data-state={held?'HELD':view.sceneBound?'BOUND':'ASSEMBLED'} aria-label='Living world mission continuity'>
  ...
  <div className='r206-world-mission-copy'><small>R204 → R206 → R208 · INTENT-ASSEMBLED MISSION · ONE CANONICAL WORLD</small><b>{stateLabel} · {view.current.command}</b><span><Route/> {view.route} · continuity {view.anchor.carryHash} · scar {view.anchor.scarCarry.toFixed(3)} · return {view.anchor.returnCarry.toFixed(3)}</span><em>{durableLabel} · {view.sceneBound?'Evidence-bound scene attached':'Scene evidence not attached'} · R122 {view.renderReady?'INPUT READY':'HELD'} · COMPUTED PHOTOREAL REALITY UNPROVEN</em></div>
