...
export const R139_FINGERPRINT_SCHEMA='OMEGA_AGENT_RETURN_FINGERPRINT_R139';
export const R139_LAWS=Object.freeze([
 'EXACT_AGENT_PAYLOAD_SHA_AND_SEMANTIC_EQUALITY_REQUIRED_BEFORE_PROOF',
 'LEGACY_AGENT_FINGERPRINT_IS_LINEAGE_NOT_R139_PROOF',
...
function fingerprintSourceR139(body={}){return{resultFingerprintSchema:text(body.resultFingerprintSchema,80),resultFingerprintR139Payload:typeof body.resultFingerprintR139Payload==='string'?body.resultFingerprintR139Payload:'',resultFingerprintR139:text(body.resultFingerprintR139,160),legacyResultFingerprint:text(body.resultFingerprint,160),proofClosureRevision:text(body.proofClosureRevision,40),baseAgentSha256:text(body.baseAgentSha256,160)}}
async function fingerprintEvidenceR139(source,core){
 const payload=source.resultFingerprintR139Payload||'',payloadBytes=new TextEncoder().encode(payload).byteLength,legacy=source.legacyResultFingerprint||null,coreSemanticDigest=await sha256R134(core);
 if(source.resultFingerprintSchema!==R139_FINGERPRINT_SCHEMA||!payload||!source.resultFingerprintR139||payloadBytes>MAX_FINGERPRINT_PAYLOAD_BYTES)return{schema:source.resultFingerprintSchema||null,state:'R139_FINGERPRINT_REQUIRED',supplied:source.resultFingerprintR139||null,expected:null,verified:false,digestMatch:false,semanticMatch:false,payloadBytes,coreSemanticDigest,legacy,legacyAuthority:'LINEAGE_ONLY_NOT_R139_PROOF'};
 const expected=await sha256R134(payload),digestMatch=source.resultFingerprintR139===expected;let parsed=null,parseOk=false;
 ...
 return{schema:source.resultFingerprintSchema,state:verified?'R139_FINGERPRINT_VERIFIED':'FINGERPRINT_MISMATCH',supplied:source.resultFingerprintR139,expected,verified,digestMatch,semanticMatch,parseOk,payloadBytes,payloadSemanticDigest,coreSemanticDigest,legacy,legacyAuthority:'LINEAGE_ONLY_NOT_R139_PROOF'};
}
...
export async function closeHybridReturnR139(runtime,job,body){
 const core=agentPacketCoreR139(body),source=fingerprintSourceR139(body),fingerprint=await fingerprintEvidenceR139(source,core),devices=await runtime.devices(),deviceId=safeId(job?.targetDeviceId),device=devices.find(x=>x?.id===deviceId),currentHeartbeatProved=Boolean(device&&device.online===true&&device.revoked!==true),eventTime=clamp(job?.returnPacket?.receivedAt||job?.completedAt||Date.now(),0,Number.MAX_SAFE_INTEGER),previousHead=await runtime.get('r139WorldHead',null);
 ...
 const closure={ok:true,schema:R139_SCHEMA,revision:R139_REVISION,jobId:input.job.id,state,fingerprint,hybridProofAtClose:input.hybridSnapshot,previousHeadSha256:previousHead?.headSha256||null,livingWorldFrame:compiled.living,continuity:compiled.operationRef,finalHeadSha256:compiled.finalHead.headSha256,input,canonicalMutation:false,canonicalAdmissionAuthority:'R125',durability:'OMEGA_RUNTIME_DURABLE_OBJECT_EVIDENCE_STORE',r97ContinuityPromotion:'NOT_AUTO_PROMOTED',truthBoundary:'R139 closes a returned bounded Hybrid job into R134 continuity/scar evidence and an R136 living-world evidence frame only after the exact R139 payload SHA-256 matches and the parsed payload is semantically identical to the returned packet core. Legacy Python resultFingerprint remains lineage only. Durable evidence, replay success, heartbeat proof, build/test success, and host claims never mutate CanonState or replace R125 admission.'};
...
export function manifestR139(){return{ok:true,schema:R139_SCHEMA,revision:R139_REVISION,fingerprintSchema:R139_FINGERPRINT_SCHEMA,laws:R139_LAWS,canonicalMutation:false,canonicalAdmissionAuthority:'R125',durability:'OMEGA_RUNTIME_DURABLE_OBJECT_EVIDENCE_STORE',r97ContinuityPromotion:'NOT_AUTO_PROMOTED',truthBoundary:'Manifest only. R139 adds exact-payload host-return closure/replay evidence inside the already-proven Hybrid Durable Object runtime. It does not itself claim a PC is online, a workload succeeded, or CanonState changed.'}}
