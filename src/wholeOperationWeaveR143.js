...
export function dependencyStateR143(graph,nodeId){
 ...
 if(n.id==='N80_JOIN'){const joinTerminal=new Set([...successTerminal,...FAILURE_STATES,'RETURNED']);const pending=parents.find(x=>!joinTerminal.has(x.state));return pending?{ready:false,reason:`WAITING_${pending.id}_${pending.state}`}:{ready:n.state!=='UNAVAILABLE',reason:n.state==='UNAVAILABLE'?'EXECUTOR_UNAVAILABLE':'JOIN_TERMINAL_DEPENDENCIES_SATISFIED_WITH_RESIDUAL_CARRY'}}
