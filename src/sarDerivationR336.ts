...
export function resolveSarFieldR336(obs:SarObservationR280,r0:SarRasterFieldR283|undefined,view:SarViewR325):SarFieldResultR336{
 ...
 if(view==='AMPLITUDE'){
  const calibratedPower=r.terrainFlattenedGamma0?.length?r.terrainFlattenedGamma0:r.gamma0?.length?r.gamma0:r.sigma0?.length?r.sigma0:r.beta0?.length?r.beta0:undefined;
  if(has(calibratedPower)){const field=r.terrainFlattenedGamma0?.length?'terrainFlattenedGamma0':r.gamma0?.length?'gamma0':r.sigma0?.length?'sigma0':'beta0';return result(view,'DISPLAYABLE','OBSERVED_CALIBRATED',field,'linear power',r,calibratedPower,true,'Sentinel-1 annotation-backed radiometric materialization','power=(DN^2-noiseLut)/calibrationLut^2',['native raster','calibration LUT','noise LUT/policy'],[],'Calibrated backscatter is bound as a separate physical array; terrain-flattened gamma0 is preferred only when DEM/local-geometry correction is also materialized.',r.validMask)}
 ...
 if(view==='INTERFEROGRAM'){
  if(has(r.correctedInterferometricPhaseRad))return result(view,'DISPLAYABLE','DERIVED_MEASUREMENT','correctedInterferometricPhaseRad','rad',r,r.correctedInterferometricPhaseRad,false,'validated interferogram + correction ledger','phi_corrected=wrap(phi_ifg-sum(correction_phase))',['full-resolution TOPS coregistration receipt','validated pair phase','declared correction ledger'],[],'Correction-ledger wrapped interferometric phase. Raw pair phase remains separately retained.',r.validMask);
  if(has(r.interferogramPhaseRad)){const physical=r.physicalClosureR343?.interferometricPhaseValidated===true;return result(view,'DISPLAYABLE','DERIVED_MEASUREMENT','interferogramPhaseRad','rad',r,r.interferogramPhaseRad,false,'complex conjugate product','I = master * conj(slave); wrapped phase = arg(I)',['proven identical sampled complex grid','complex SLC pair','TOPS subpixel coregistration proof for physical interferometry'],physical?[]:['SUBPIXEL_COREGISTRATION_NOT_PROVEN'],physical?'Physically admitted wrapped interferometric phase after full-resolution TOPS residual proof.':'Exact-grid wrapped cross-phase remains a computational pair field, not physically admitted TOPS interferometry.',r.validMask);}
 ...
 if(view==='DEFORMATION'){
  if(has(r.correctedLosDisplacementM))return result(view,'DISPLAYABLE','DERIVED_MEASUREMENT','correctedLosDisplacementM','m',r,r.correctedLosDisplacementM,false,'LOS correction ledger','d_corrected = d_LOS - sum(displacement corrections)',['metric LOS','atmosphere/ETAD/geodynamic corrections','correction provenance'],[],'Corrected metric line-of-sight displacement; raw LOS is retained independently.',r.validMask);
  if(has(r.losDisplacementM)){const proven=r.physicalClosureR343?.losDisplacementBound===true;return result(view,'DISPLAYABLE','DERIVED_MEASUREMENT','losDisplacementM','m',r,r.losDisplacementM,false,'phase-to-LOS after residual handling','d_LOS = sign * lambda * phi_unwrapped/(4*pi)',['unwrapped interferometric phase','orbit correction','topographic phase removal','wavelength','sign convention'],proven?[]:['R343_PHYSICAL_CLOSURE_RECEIPT_REQUIRED'],proven?'Metric LOS displacement from the proved R343 chain.':'A LOS array is present but its complete R343 physical-closure receipt is not bound.',r.validMask);}
...
export function sarDerivationTruthBoundaryR336(){
 return'R336/R341/R343 resolves every analytical lens through explicit evidence and dependency contracts. R343 can materialize annotation-backed beta0/sigma0/gamma0, terrain-flattened gamma0, full-resolution-coregistered interferometry, correction-ledger phase, quality-checked unwrapping and metric LOS when their evidence is present. Exact-grid R341 statistics remain computational only until the R343 TOPS receipt passes. Missing physical inputs remain named scars rather than UI-generated values.';
}
