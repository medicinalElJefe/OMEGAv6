...
export function resolveSarFieldR336(obs:SarObservationR280,r0:SarRasterFieldR283|undefined,view:SarViewR325):SarFieldResultR336{
 ...
 if(view==='COHERENCE'){
  if(has(r.coherence))return result(view,'DISPLAYABLE','DERIVED_MEASUREMENT','coherence','unitless',r,r.coherence,false,'normalized complex cross-correlation','gamma = |sum(s1*conj(s2))| / sqrt(sum|s1|^2 sum|s2|^2)',['same-grid or proven co-registered SLC pair','declared coherence window'],[],'Pair-derived interferometric coherence. R341 admits this only on an already-identical affine grid; general co-registration remains separate.',r.pairValidMask);
 ...
 if(view==='INTERFEROGRAM'){
  if(has(r.interferogramPhaseRad))return result(view,'DISPLAYABLE','DERIVED_MEASUREMENT','interferogramPhaseRad','rad',r,r.interferogramPhaseRad,false,'complex conjugate product','I = master * conj(slave); wrapped phase = arg(I)',['same-grid or proven co-registered SLC pair','wrapped phase'],[],'Bound wrapped interferometric phase from a distinct pair-derived field. Native single-scene phase remains separate.',r.pairValidMask);
  return result(view,obs.productLevel==='GRD'?'UNAVAILABLE_FROM_PRODUCT':'DEPENDENCY_MISSING','UNBOUND',null,'rad',r,undefined,false,'complex conjugate product','I = master * conj(slave)',['same-grid or proven co-registered SLC pair','wrapped phase difference'],['PAIR_FIELD_OR_COREGISTRATION_REQUIRED'],'An interferogram is a pair-derived field and is never substituted with native single-scene phase.');
 }
 if(view==='DEFORMATION'){
