...
export default function HybridProofClosureR141(){
 ...
 return <section className='r141-proof-ledger' aria-label='Hybrid execution proof closure and replay'>
  <header><div><span>R141 · PC WORKLOAD → PROOF → SCAR → REPLAY → R140 WORLD FRAME</span><h3>Execution proof closure</h3><p>Returned bounded PC work is exact-payload fingerprint-verified, appended as R134 evidence/scar continuity, framed by R136, replay-checked, then handed to the admitted R140 operation-world bridge only when an operator selects the durable receipt.</p></div><button onClick={()=>void refresh()} disabled={busy==='refresh'}><RefreshCw className={busy==='refresh'?'spin':''}/>Refresh</button></header>
  ...
  {closure&&selected===closure.jobId&&<section className='r141-receipt'>
