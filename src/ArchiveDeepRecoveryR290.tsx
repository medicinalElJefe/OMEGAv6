...
import {R290_DEEP_RECOVERY_BINDINGS,auditDeepArchiveRecoveryR290,deepArchiveRecoveryReceiptR290,type R290RecoveryState} from './archiveDeepRecoveryR290';
...
export default function ArchiveDeepRecoveryR290(){
 ...
 return <section className='r290-recovery' aria-label='R290 deep archive execution convergence'>
  <header className='r290-recovery-head'>
   <div><span>R290 · DEEP ARCHIVE EXECUTION CONVERGENCE</span><h3>Recovered lineage → correct menu → current successor → next proof</h3><p>Drive recovery is projected into the existing 12-menu / 44-route OMEGA shell. This layer plans recovery and exposes provenance; it does not create a route owner, state writer, proof authority, dispatcher or deployment authority.</p></div>
  ...
  <div className='r290-recovery-kpis'>
   ...
   <article><Layers3/><span>Drive artifacts</span><b>{R290_DEEP_RECOVERY_BINDINGS.reduce((n,x)=>n+x.driveArtifacts.length,0)}</b><small>explicit file/folder/ledger identities</small></article>
   <article className={audit.pass?'pass':'hold'}>{audit.pass?<CheckCircle2/>:<ShieldAlert/>}<span>Binding audit</span><b>{audit.pass?'PASS':'HOLD'}</b><small>{audit.pass?'no orphan menu/route/genome refs':'repair recovery binding before promotion'}</small></article>
  </div>
  ...
  <div className='r290-recovery-grid'>
   {rows.map(row=><article key={row.id} className={`r290-recovery-card state-${row.state.toLowerCase()}`}>
    ...
    <details><summary>Drive provenance · {row.driveArtifacts.length}</summary><ul>{row.driveArtifacts.map(a=><li key={a.id}><b>{a.kind}</b><span>{a.title}</span><code>{a.id}</code></li>)}</ul></details>
