...
export default function VisualCompositorR65({address,onAddress}:{address:number;onAddress:(n:number)=>void}){
 ...
 const selectedModeId=useMemo(()=>{try{return localStorage.getItem('omega.r82.selectedModeId')||'M001'}catch{return'M001'}},[]);
 const catalog=useMemo(()=>evaluateCorpusModes(record),[record]),selectedCatalog=useMemo(()=>(catalog.results as any[]).find(x=>x.id===selectedModeId)||null,[catalog,selectedModeId]),selectedExecution=useMemo(()=>modes.rows.find(x=>x.id===selectedModeId)||null,[modes,selectedModeId]),modeExpression=useMemo(()=>compileModeExpressionR82(selectedCatalog,selectedExecution,record),[selectedCatalog,selectedExecution,record]);
 ...
 return <section className='r65-compositor' data-lens={lens}>
  ...
  {lens!=='SYNTHESIS'&&<div className='r65-focus-stage'>
   {lens==='MODE'&&<ModeExpressionCanvasR82 expression={modeExpression} address={address} value={selectedExecution?.value}/>} 
  ...
  <footer className='r65-truth'><ShieldCheck/><span><b>Projection truth:</b> all panels are bound to canonical state {record.stateId}. Source-backed mode count is execution evidence only for implemented operators; catalog membership is not execution. Native GPU/video and device observations remain separately gated.</span></footer>
 </section>
}
