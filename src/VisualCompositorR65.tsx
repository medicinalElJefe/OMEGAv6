...
const LENSES:readonly {id:Lens;label:string;meaning:string}[]=[
 ...
 {id:'RELATIVITY',label:'Derived relativity',meaning:'optional observer rendering'},
 {id:'MOTION',label:'Transition data',meaning:'current vs admitted-next packet'},
 {id:'CINEMATIC',label:'Cinematic render',meaning:'optional representational browser renderer'}
] as const;
...
export default function VisualCompositorR65({address,onAddress}:{address:number;onAddress:(n:number)=>void}){
 ...
 return <section className='r65-compositor' data-lens={lens}>
  <header className='r65-compositor-head'><div><span>OMEGA VISUAL COMPOSITOR · ONE STATE / MANY LAWFUL PROJECTIONS</span><h3>Living multi-view instrument</h3><p>The default instrument shows only canonical packet values, evaluated mode output, and the admitted transition. Representational field/forecast/relativity/cinematic renderers remain optional and explicitly non-observational.</p></div><div className='r65-live'><Activity/><b>{record.metrics.decision}</b><small>STATE {record.stateId}</small></div></header>
