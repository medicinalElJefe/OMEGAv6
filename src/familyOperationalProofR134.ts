...
export type FamilyOperationalProofStateR134='CURRENT_EXECUTION_PROOF'|'CURRENT_SERVICE_OBSERVATION'|'DECLARED_BOUNDARY_ONLY'|'GATE_CURRENTLY_HELD'|'STATIC_LINEAGE_ONLY'|'UNKNOWN';
export type FamilyOperationalProofR134={familyId:string;state:FamilyOperationalProofStateR134;current:boolean;label:string;detail:string;source:string;observedAt:string|null};

export const FAMILY_OPERATIONAL_PROOF_LAWS_R134=Object.freeze({
 revision:'R134',
...
export function familyOperationalProofR134(family:SystemFamily,operational:any,hybrid:any):FamilyOperationalProofR134{
...
export function familyOperationalProofSummaryR134(operational:any,hybrid:any){
 const devices=onlineDevices(hybrid);return{revision:'R134',operationalObserved:Boolean(operational),observedAt:observedAt(operational),reachableCount:Number(operational?.summary?.reachableCount||0),requiredCount:Number(operational?.summary?.requiredCount||0),canonicalRuntime:String(operational?.summary?.canonicalRuntime||'UNKNOWN'),pcOnlineProved:pcProved(operational,hybrid),onlineDeviceCount:devices.length,authority:'OPERATIONAL_OBSERVATION_NOT_CANON',laws:FAMILY_OPERATIONAL_PROOF_LAWS_R134};
}
