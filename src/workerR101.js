...
export class OmegaRuntime extends OmegaRuntimeR34 {
 ...
 async fetch(request){
  ...
  if(path==='/agent/progress'&&request.method==='POST'){
   ...
   const b=await request.json().catch(()=>({})),deviceId=safeId(b.deviceId,''),jobId=safeId(b.jobId,'');if(!deviceId||!jobId)return json({ok:false,code:'R243_PROGRESS_ID_REQUIRED'},400);
   const t=Date.now(),jobs=await this.get('jobs',[]),target=jobs.find(j=>j?.id===jobId&&j?.targetDeviceId===deviceId&&String(j?.status||'').toUpperCase()==='RUNNING');if(!target)return json({ok:false,code:'R243_JOB_NOT_RUNNING'},409);
   const seq=Math.max(0,Math.floor(Number(b.seq)||0)),priorSeq=Math.max(-1,Math.floor(Number(target?.progress?.seq)||-1));if(seq<priorSeq)return json({ok:false,code:'R243_PROGRESS_SEQUENCE_STALE',expectedAtLeast:priorSeq},409);
   const state=['CLAIMED','STEP_RUNNING','STEP_COMPLETE','RETURNING'].includes(String(b.state||''))?String(b.state):'STEP_RUNNING',progress={schema:'OMEGA_HYBRID_EXECUTION_PROGRESS_R243',revision:EXECUTION_MOTION_REVISION,at:t,seq,state,stepId:safeId(b.stepId,''),stepOp:text(b.stepOp).slice(0,64),stepIndex:Math.max(0,Math.floor(Number(b.stepIndex)||0)),totalSteps:Math.max(0,Math.floor(Number(b.totalSteps)||0)),completedSteps:Math.max(0,Math.floor(Number(b.completedSteps)||0)),elapsedMs:Math.max(0,Math.floor(Number(b.elapsedMs)||0)),message:text(b.message).slice(0,240)};
