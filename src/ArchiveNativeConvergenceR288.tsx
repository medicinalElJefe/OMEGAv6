...
export default function ArchiveNativeConvergenceR288(){
 ...
 return <section className='r288-convergence' aria-label='R288 archive native capability convergence'>
  <header className='r288-head'>
   <div><span>R288 · ARCHIVE-NATIVE CAPABILITY CONVERGENCE</span><h3>Drive corpus → current successor reality</h3><p>Recovered software, calculus and build authorities are bound to the current OMEGA successor ledger without allowing archive donors to become shadow runtime authority.</p></div>
   <button className='r288-download' onClick={()=>downloadJson('OMEGA_R288_ARCHIVE_NATIVE_CONVERGENCE_RECEIPT.json',archiveNativeConvergenceReceiptR288())}><Download/>Export convergence receipt</button>
  </header>
  <div className='r288-kpis'>
   <article><Database/><span>Drive authorities</span><b>{R288_DRIVE_AUTHORITIES.length}</b><small>directly re-read in this pass</small></article>
  ...
  <div className='r288-family-toolbar'><div><b>CURRENT SUCCESSOR REALITY</b><span>Historical predecessor status is retained as provenance; current R48/R153 successor proof controls this view.</span></div><label>STATE<select value={state} onChange={e=>setState(e.target.value as any)}>{CURRENT_STATES.map(x=><option key={x}>{x}</option>)}</select></label></div>
  ...
  {R288_EMERGING_OPERATORS.map(row=><aside className='r288-emerging' key={row.id}><ShieldAlert/><div><b>{row.id} · {row.state}</b><p>{row.reason}</p><small>{row.admission}</small></div></aside>)}
