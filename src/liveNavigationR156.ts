...
const empty=():LiveNavigationR156=>({schema:LIVE_NAVIGATION_SCHEMA,observedAt:0,pcOnline:false,deviceName:'',missionStatus:'UNKNOWN',missionId:'',missionLabel:'',missionProgress:'',jobStatus:'UNKNOWN',currentJobId:'',currentJobOp:'',rcwaState:'UNKNOWN',rcwaOnline:false,proofState:'NO_RETURN_SELECTED',error:''});
...
export function deriveLiveNavigationR156(hybrid:any,missionData:any,federation:any):LiveNavigationR156{
 ...
 const proofState=String(job?.proofClosure?.state||job?.returnPacket?.state||job?.proofState||mission?.proofState||'NO_RETURN_SELECTED');
