...
function GlobalSearch({open,onClose,panel,onNavigate}:{open:boolean;onClose:()=>void;panel:string;onNavigate:(p:string)=>void}){const[q,setQ]=useState(''),input=useRef<HTMLInputElement|null>(null);useEffect(()=>{if(open){setQ('');setTimeout(()=>input.current?.focus(),20)}},[open]);const rows=useMemo(()=>ALL_ROUTES.filter(r=>r.toLowerCase().includes(q.trim().toLowerCase())),[q]);if(!open)return null;return <div className='nav20-search-backdrop' role='presentation' onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}><section className='nav20-search' role='dialog' aria-modal='true' aria-label='Search OMEGA applications'><header><Search size={18}/><input ref={input} value={q} onChange={e=>setQ(e.target.value)} placeholder='Search all 44 OMEGA surfaces…' onKeyDown={e=>e.key==='Escape'&&onClose()}/><kbd>ESC</kbd></header><div className='nav20-search-results'>{rows.map(r=><RouteButton key={r} route={r} panel={panel} onNavigate={onNavigate} close={onClose}/>)}</div><footer><span>{rows.length} destinations</span><small>One route registry · no decorative menu entries</small></footer></section></div>}
