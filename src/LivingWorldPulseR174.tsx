...
export default function LivingWorldPulseR174({onNavigate}:Props){
 ...
 const stageMission=()=>{if(!mission?.accepted)return;const next=composeLivingWorldMissionR177({...pulse,revision:'R175'},{operatorStaged:true});setStaged(true);try{localStorage.setItem('omega.r177.stagedMission',JSON.stringify(next))}catch{};window.dispatchEvent(new CustomEvent(R177_EVENT,{detail:next}))};
 ...
 return <section className='r174-pulse' aria-label='Living canonical world truth surface' data-closed={pulse.federationClosed?'true':'false'}>
  ...
  <div className='r177-mission' data-staged={staged?'true':'false'}><Sparkles/><span><b>{staged?'MISSION STAGED · NOT AUTHORIZED':'MISSION PROPOSAL'}</b><small>{missionSteps.length?missionSteps.map((step:any)=>step.action).join(' · '):'Awaiting accepted living-world state'} · execution remains explicitly governed</small></span><button disabled={!mission?.accepted||staged} onClick={stageMission}>{staged?'Staged':'Stage mission'}</button></div>
 </section>;
}
