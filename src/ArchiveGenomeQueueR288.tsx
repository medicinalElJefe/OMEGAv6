...
export default function ArchiveGenomeQueueR288({operators=false}:{operators?:boolean}){
 ...
 const receipt=()=>exportJson('OMEGA_R291_ARCHIVE_GENOME_RECEIPT.json',{schema:'OMEGA_ARCHIVE_GENOME_LEDGER_R291_CURRENT',generatedAt:new Date().toISOString(),summary,rows:[...ARCHIVE_GENOME_CURRENT_R289],scars:ARCHIVE_SCARS_R288,boundary:{archive:'Archive evidence is not execution, scientific, medical, device or production proof.',origin:'Third-party/dependency artifacts retain origin/version/license and never become OMEGA-original by archive presence.'}});
 return <section className='archive-genome-r288'>
  <header><div><span>R291 · ARCHIVE GENOME → OMEGAv6</span><h3>Upgrade Queue</h3><p>Drive-derived capability deltas, dependency boundaries, recovery scars and proof paths. Discovery does not grant execution authority.</p></div><button onClick={receipt}><Download/>Genome receipt</button></header>
