...
const hex32=value=>hash32(value).toString(16).padStart(8,'0');
...
function scarFingerprint(claimId,scar){return`r293-scar-${hex32(stable({claimId,kind:String(scar?.kind||'GATE'),id:String(scar?.id||'SCAR'),status:String(scar?.status||'OPEN'),detail:String(scar?.detail||'')}))}`}
function cellId(claimId,scar){const kind=String(scar?.kind||'GATE').replace(/[^A-Z0-9_-]/gi,'_'),id=String(scar?.id||'SCAR').replace(/[^A-Z0-9_.-]/gi,'_');return`R293-${kind}-${id}-${hex32(`${claimId}:${kind}:${id}`)}`}
