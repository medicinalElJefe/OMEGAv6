...
export default function HybridCommandDeckR237(){
 ...
 const[selectedDeviceId,setSelectedDeviceId]=useState(()=>{try{return window.localStorage.getItem('omega:hybrid:selectedDeviceId')||''}catch{return''}});
 ...
 useEffect(()=>{if(!device?.id)return;if(selectedDeviceId!==device.id)setSelectedDeviceId(device.id);try{window.localStorage.setItem('omega:hybrid:selectedDeviceId',device.id)}catch{}},[device?.id,selectedDeviceId]);
 ...
 const missionEntries=useMemo(()=>missions.map((mission:any)=>({mission,job:jobs.find((j:any)=>j.id===mission.currentJobId)||mission.currentJob||null})),[missions,jobs]);
 ...
 const currentMission=currentMissionEntry?.mission||null;
 const missionJob=currentMissionEntry?.job||null;
 const foreignActiveMissionCount=useMemo(()=>{if(!device)return 0;return missionEntries.filter(({mission,job})=>ACTIVE_MISSION.has(String(mission?.status||'').toUpperCase())&&targetForMission(mission,job)!==device.id).length},[missionEntries,device]);
 const advertised=useMemo(()=>new Set(Array.isArray(device?.capabilities)?device.capabilities:[]),[device]);
 const nativeReady=Boolean(live?.nativeExecutionClaimed===true&&device?.online&&!device?.revoked);
 ...
 const correlationLocked=Boolean(device)&&(!activeJob||activeJob.targetDeviceId===device.id)&&(!currentMission||missionTarget===device.id);
 const canRun=(preset:Preset)=>nativeReady&&correlationLocked&&rootValid&&queueClear&&!busy&&preset.ops.every(op=>advertised.has(op));
 const chooseDevice=(id:string)=>{setSelectedDeviceId(id);setMessage('');setError('')};
 const queuePreset=async(preset:Preset)=>{
  ...
  if(!correlationLocked){setError('Host/job/mission correlation is not locked to one authenticated device. Refresh or select the intended host before queueing work.');return}
 ...
 return <section className='r237-command-deck' aria-label='R237 Hybrid command authority' data-r237-command-authority='AUTHENTICATED_BOUNDED_NATIVE_CONTROL' data-r237-selected-device={device?.id||'NONE'} data-r237-correlation={correlationLocked?'LOCKED':'HELD'}>
  <header className='r237-head'><div><span>R237 · HYBRID COMMAND AUTHORITY · AUTHENTICATED + BACKPRESSURED + CORRELATED</span><h3>Use the PC as a governed compute node.</h3><p>R237 moves Hybrid Link from connection/proof visibility into an operator command deck: capability-negotiated native presets, one-active-job-per-device backpressure, authenticated mission control, queued-job cancellation, and exact handoff into the existing R212 host-effects and R141 proof closure. Host selection, active jobs and mission controls are now locked to the same authenticated device identity. These presets intentionally contain no APPLY_PATCH or WRITE_TEXT.</p></div><button onClick={()=>void refresh()} disabled={!!busy}><RefreshCw/>Refresh</button></header>
  ...
  {onlineDevices.length>0&&<div className='r237-root-row r237-device-row'><label><span>Authenticated compute host</span><select aria-label='Authenticated compute host' value={device?.id||''} onChange={e=>chooseDevice(e.target.value)}>{onlineDevices.map((row:any)=><option key={row.id} value={row.id}>{row.name||row.id} · {row.id}</option>)}</select><small>Every preset, queue gate, cancellation and mission-control action below is bound to this exact current device ID. Other hosts and their missions remain visible only as isolated counts, never as implicit control targets.</small></label></div>}
  ...
  <footer><ShieldCheck/><span>R237 adds operator access and runtime admission hardening, not a second executor. One selected authenticated device gets at most one QUEUED/RUNNING job, mission control is correlated to that same device ID, mission-control writes require the bridge secret, queued jobs can be cancelled before host claim, and running local work is never falsely reported as remotely killed. R141/R146/R147/R125 authority remains unchanged.</span></footer>
 </section>;
}
