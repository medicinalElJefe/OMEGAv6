...
export default function HybridCommandDeckR237(){
 ...
 return <section className='r237-command-deck' aria-label='R237 Hybrid command authority' data-r237-command-authority='AUTHENTICATED_BOUNDED_NATIVE_CONTROL' data-r237-selected-device={device?.id||'NONE'} data-r237-correlation={correlationLocked?'LOCKED':'HELD'} data-r237-snapshot-epoch={epoch} data-r239-resource-tier={resourceEnvelope.tier}>
  <header className='r237-head'><div><span>R237 + R238 + R239 · HYBRID COMMAND AUTHORITY · SHARED SNAPSHOT + RESOURCE ENVELOPE</span><h3>Use the PC as a governed compute node.</h3><p>R237 remains the authenticated command boundary. R238 supplies one selected-host snapshot and returned host profile. R239 now participates in admission: heavier presets are held when returned memory/storage headroom is unsafe, and HASH_TREE/TRAIN_LOCAL bounds are reduced deterministically under pressure. These presets still contain no APPLY_PATCH or WRITE_TEXT.</p></div><button onClick={()=>void refresh()} disabled={!!busy}><RefreshCw/>Refresh shared snapshot</button></header>
