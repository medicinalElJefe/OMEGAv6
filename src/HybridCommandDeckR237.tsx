...
import {applyEnvelopeToPresetR239,latestReturnedHostProofR239,resourceEnvelopeR239,type R239Preset} from './hybridResourceGovernorR239';
import './hybridCommandDeckR237.css';

type Preset={id:R239Preset;label:string;detail:string;ops:string[];steps:(root:string)=>any[]};
...
export default function HybridCommandDeckR237(){
 ...
 const resourceEnvelope=useMemo(()=>resourceEnvelopeR239({profile:hostProof?.profile||null,snapshotCurrent,activeNativeWork:activeJobs.length>0}),[hostProof?.profile,snapshotCurrent,activeJobs.length]);
 const canRun=(preset:Preset)=>nativeReady&&correlationLocked&&rootValid&&queueClear&&!busy&&preset.ops.every(op=>advertised.has(op))&&resourceEnvelope.admission[preset.id];
 ...
 const queuePreset=async(preset:Preset)=>{
  ...
  if(!resourceEnvelope.admission[preset.id]){setActionError(`R239 resource admission held ${preset.label}. ${resourceEnvelope.reasons.join(' · ')}. Run Prove host + tree when profile proof is missing, or wait for the selected host to regain safe memory/storage headroom.`);return}
  setBusy(preset.id);setMessage('');setActionError('');
  try{
   const steps=applyEnvelopeToPresetR239(preset.id,preset.steps(root.trim()),resourceEnvelope);
   const r=await api.post<any>('/api/hybrid/jobs',{schema:'OMEGA_HYBRID_OPERATOR_JOB_R237',action:preset.id,profile:'AUTO_BUILD',projectPath:root.trim(),instructions:preset.detail,allowedDomains:[],steps,targetDeviceId:device.id,confirmed:true,snapshotEpoch:epoch,snapshotObservedAt:observedAt,resourceEnvelopeR239:{schema:resourceEnvelope.schema,tier:resourceEnvelope.tier,effectiveCpuWorkers:resourceEnvelope.effectiveCpuWorkers,hashMaxResults:resourceEnvelope.hashMaxResults,trainMaxResults:resourceEnvelope.trainMaxResults,sourceProfileSha256:hostProof?.profile?.profileSha256||null}});
   ...
   setMessage(`Queued ${preset.label}: ${r.data.job.id} on ${device.name||device.id}. R239 envelope ${resourceEnvelope.tier} bounded the request to ${resourceEnvelope.effectiveCpuWorkers} advisory worker${resourceEnvelope.effectiveCpuWorkers===1?'':'s'}; the host must still claim and return first-hand proof.`);
   await refresh();
 ...
 return <section className='r237-command-deck' aria-label='R237 Hybrid command authority' data-r237-command-authority='AUTHENTICATED_BOUNDED_NATIVE_CONTROL' data-r237-selected-device={device?.id||'NONE'} data-r237-correlation={correlationLocked?'LOCKED':'HELD'} data-r237-snapshot-epoch={epoch} data-r239-resource-tier={resourceEnvelope.tier}>
  <header className='r237-head'><div><span>R237 + R238 + R239 · HYBRID COMMAND AUTHORITY · ROOT-CONFINED · SHARED SNAPSHOT + RESOURCE ENVELOPE</span><h3>Use the PC as a governed compute node.</h3><p>R237 remains the authenticated command boundary. R238 supplies one selected-host snapshot and returned host profile. R239 now participates in admission: heavier presets are held when returned memory/storage headroom is unsafe, and HASH_TREE/TRAIN_LOCAL bounds are reduced deterministically under pressure. These presets intentionally contain no APPLY_PATCH or WRITE_TEXT.</p></div><button onClick={()=>void refresh()} disabled={!!busy}><RefreshCw/>Refresh shared snapshot</button></header>
  <div className='r237-state-grid'>
   ...
   <article className={resourceEnvelope.tier==='READY'||resourceEnvelope.tier==='HIGH_CAPACITY'?'pass':resourceEnvelope.tier==='CONSTRAINED'?'running':'hold'}><Cpu/><span><small>R239 RESOURCE ENVELOPE</small><b>{resourceEnvelope.tier}</b><em>{resourceEnvelope.effectiveCpuWorkers} advisory worker{resourceEnvelope.effectiveCpuWorkers===1?'':'s'} · hash {resourceEnvelope.hashMaxResults} · train {resourceEnvelope.trainMaxResults}</em></span></article>
  </div>
  {onlineDevices.length>0&&<div className='r237-root-row r237-device-row'><label><span>Authenticated compute host</span><select aria-label='Authenticated compute host' value={device?.id||''} onChange={e=>chooseDevice(e.target.value)}>{onlineDevices.map((row:any)=><option key={row.id} value={row.id}>{row.name||row.id} · {row.id}</option>)}</select><small>This selection is shared by R212, R237, R238 and R239. Every preset, queue gate, resource envelope, return view and mission-control action is correlated to this exact device identity and snapshot plane.</small></label></div>}
  <div className='r237-root-row'><label><span>Project path inside approved root</span><input value={root} onChange={e=>setRoot(e.target.value)} aria-invalid={!rootValid}/><small>ROOT-CONFINED: use <code>.</code> for the launcher-approved root itself. Absolute Windows paths and parent traversal are deliberately rejected here.</small></label></div>
  ...
  <div className='r237-presets'>
   {PRESETS.map((preset,index)=>{const missing=preset.ops.filter(op=>!advertised.has(op));const resourceHeld=!resourceEnvelope.admission[preset.id];const icon=index===0?<ShieldCheck/>:index===1?<FileCheck2/>:index===2?<FolderOutput/>:<GraduationCap/>;return <article key={preset.id} className={canRun(preset)?'ready':'held'}>{icon}<div><span>{preset.id}</span><h4>{preset.label}</h4><p>{preset.detail}</p><div className='r237-oplist'>{preset.ops.map(op=><code key={op} className={advertised.has(op)?'ok':'missing'}>{op}</code>)}</div>{missing.length>0&&<small className='r237-missing'>missing host proof: {missing.join(', ')}</small>}{resourceHeld&&<small className='r237-missing'>R239 resource hold: {resourceEnvelope.reasons.join(' · ')}</small>}</div><button onClick={()=>void queuePreset(preset)} disabled={!canRun(preset)}>{busy===preset.id?'Queuing…':!queueClear?'Device busy':resourceHeld?'Resource held':'Run on this PC'}</button></article>})}
  </div>
  ...
  <footer><ShieldCheck/><span>R239 tightens resource admission without changing execution authority. R237 remains the bounded operator surface over the established executor: one selected authenticated device, one current snapshot epoch, at most one QUEUED/RUNNING job, pressure-aware preset admission, authenticated mission control, exact host-return closure through R212/R141, durable history through R146, dispatch authority through R147, and CanonState admission only through R125.</span></footer>
 </section>;
}
