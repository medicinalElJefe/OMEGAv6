...
export type AuthorityId='SOVEREIGN_CANON'|'SOURCE'|'DEPLOYMENT'|'RUNTIME'|'DEVICE'|'CANONSTATE_ADMISSION';
...
export const FOUNDRY_AUTHORITY_R268={
 ...
 preserved:['R141 exact-return closure','R146 history','R147 dispatch','R205 immutable executor semantics','R239 resource governance','R240 exact-head promotion','R243 Woven planning/execution-motion truth boundary']
} as const;
...
export const FOUNDRY_CAPABILITIES_R268:CapabilityDescriptor[]=[
 ...
 C({id:'action.dispatch',label:'Governed action request',layer:'ACTION',inputs:['bounded-recommendation'],outputs:['dispatch-request'],operators:['action.authorize','action.dispatch'],allowedExecutors:['CLOUD','DEVICE'],evidence:['NONE'],sideEffect:true,authority:'SOURCE',cost:3,latency:3,dependsOn:['intelligence.reason']}),
...
export function compileSystemGenomeR268(genome:SystemGenome,ctx:FoundryContext):CompiledSystemPlan{
 ...
 const proofObligations=[
  'R125 remains sole CanonState admission authority','R141 exact-return closure preserved','R146 history preserved','R147 dispatch preserved',
