...
function normalizedAssets(item){return Object.entries(item?.assets||{}).map(([key,v])=>({key,href:String(v?.href||''),type:v?.type?String(v.type):null,roles:arr(v?.roles),title:v?.title?String(v.title):null})).filter(x=>x.href)}
export async function sarAssetProbeR325(url){
 ...
 try{
  ...
  if(!trustedAssetUrl(asset.href))return{ok:false,...base,assetKey:asset.key,state:'UNTRUSTED_ASSET_ORIGIN',latencyMs:Date.now()-started,itemUrl,asset:{key:asset.key,type:asset.type,roles:asset.roles,href:asset.href},truthBoundary:'Asset probing is restricted to HTTPS Copernicus origins resolved from the exact STAC item.'};
  ...
  if(status===401||status===403){try{await response.body?.cancel()}catch{};return{ok:false,...base,assetKey:asset.key,state:'AUTH_REQUIRED',latencyMs:Date.now()-started,itemUrl,asset:{key:asset.key,type:asset.type,roles:asset.roles,href:asset.href},http:{status,contentType,contentLength,contentRange,acceptRanges,etag,lastModified},truthBoundary:'The exact asset exists but current cloud credentials do not authorize byte access. Catalogue identity is preserved; native pixels remain unbound.'}}
  if(!(response.ok||status===206)){try{await response.body?.cancel()}catch{};return{ok:false,...base,assetKey:asset.key,state:'ASSET_UNAVAILABLE',latencyMs:Date.now()-started,itemUrl,asset:{key:asset.key,type:asset.type,roles:asset.roles,href:asset.href},http:{status,contentType,contentLength,contentRange,acceptRanges,etag,lastModified},truthBoundary:'The exact asset pointer was resolved but no readable byte prefix returned.'}}
