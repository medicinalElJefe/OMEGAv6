 ...
 return <details className='r309-sar-assets r345-sar-hybrid' open>
  ...
  <div className='r345-grid'>
   ...
   <section className='r345-form'>
    ...
    <label className={recommended.has('wavelengthM')?'r346-recommended':''}><span>LOS wavelength (m)<small>Exact radar wavelength used by d=sign·λ·φ/(4π)</small></span><input type='number' min={0.000001} max={1} step={0.000001} value={form.wavelengthM} onChange={e=>set('wavelengthM',Number(e.target.value))}/></label>
    <label className={recommended.has('losSign')?'r346-recommended':''}><span>LOS sign<small>Explicit phase-to-displacement orientation</small></span><select value={form.losSign} onChange={e=>set('losSign',Number(e.target.value)===-1?-1:1)}><option value={1}>+1</option><option value={-1}>-1</option></select></label>
    <label className={recommended.has('signConvention')?'r346-recommended':''}><span>Sign convention<small>Human-readable physical orientation retained in proof</small></span><input value={form.signConvention} onChange={e=>set('signConvention',e.target.value)} /></label>
   </section>
   ...
   ]as [keyof Form,string][]).map(([key,label])=><label key={key} className={recommended.has(String(key))?'r346-recommended':''}><span>{label}</span><input value={String(form[key]??'')} onChange={e=>set(key,e.target.value as any)} placeholder='optional root-relative path'/></label>)}</div></details>
