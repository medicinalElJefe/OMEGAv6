...
export default function HybridExecutionMotionR243(){
 ...
 return <section className='r243-motion' aria-label='R243 Hybrid execution motion' data-r243-motion={state} data-r243-job={active?.id||'NONE'} data-r243-epoch={epoch}>
  ...
  {(leaseExpired||legacyRunning)&&<div className='r243-motion-warning'><TriangleAlert/><span>{leaseExpired?'The Worker can no longer prove continued ownership of this RUNNING claim. R243 fails the stale claim closed and only auto-retries bounded non-mutating discovery after the authenticated agent polls again.':'RUNNING exists, but this already-claimed legacy job has no R243 execution lease/progress protocol. Once the R243 agent is restarted, an old expired discovery claim can be replaced by bounded discovery rather than remaining permanently INVOKED.'}</span></div>}
