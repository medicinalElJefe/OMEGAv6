...
export default function HybridExecutionMotionR243(){
 ...
 return <section className='r243-motion' aria-label='R243 Hybrid execution motion' data-r243-motion={state} data-r243-job={active?.id||'NONE'} data-r243-epoch={epoch}>
  <header><div><span>R243 · HYBRID EXECUTION MOTION · AUTHENTICATED LEASE + STEP PROGRESS</span><h3>See what the selected PC is doing while it is doing it.</h3><p>A RUNNING label alone is no longer enough. R243 requires renewable authenticated host motion for newly claimed jobs and exposes the exact current allow-listed step, elapsed time and returned-step count. A lease proves continued ownership/liveness of the running claim only; final success still requires the normal R141 returned proof.</p></div><Activity/></header>
  <div className='r243-motion-grid'>
   ...
   <article className={leaseLive?'pass':leaseExpired?'hold':'neutral'}><Clock3/><div><small>MOTION / LEASE</small><b>{p?`${fmtMs(Number(p.elapsedMs||0))} elapsed`:'No R243 pulse yet'}</b><span>{leaseLive?`last pulse ${age(p.at)} ago · lease ${fmtMs(leaseUntil-now)} remaining`:leaseExpired?`lease expired ${age(leaseUntil)} ago`:legacyRunning?'This claim predates R243 motion; restart the canonical PC connector to establish the lease protocol.':'No running lease required'}</span></div></article>
  ...
  {(leaseExpired||legacyRunning)&&<div className='r243-motion-warning'><TriangleAlert/><span>{leaseExpired?'The Worker can no longer prove continued ownership of this RUNNING claim. R243 fails the stale claim closed; only bounded non-mutating discovery may be reissued after the authenticated agent polls again.':'This is a legacy RUNNING claim without R243 lease evidence. Restart the canonical connector; OMEGA will not fabricate motion or blindly replay mutation.'}</span></div>}
  <footer><ShieldCheck/>R243 motion is observational control-plane evidence, not execution success. It does not kill a native process remotely, replay mutations, alter R141 exact-return closure, R146 durable history, R147 dispatch authority, or R125 CanonState admission.</footer>
 </section>;
}
