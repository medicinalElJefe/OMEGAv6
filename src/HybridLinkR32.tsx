...
export default function HybridLinkR32({status,record}:Props){
 ...
 return <section className='hybrid-r32 special-app r112-hybrid-link'>
  <header className='r112-hybrid-hero'>
   <div><span>SOVEREIGN COMPUTE · HYBRID LINK · R238 WOVEN SNAPSHOT + HOST INTELLIGENCE + R237 COMMAND + R212 EFFECTS + R141 PROOF</span><h2>Your PC is an OMEGA compute node.</h2><p>R238 now converges the two successor lines into one Woven Hybrid continuity plane. One atomic Hybrid/Mission snapshot selects one authenticated host identity; returned CPU/RAM/GPU/storage/Python/RCWA evidence is attached only to that host; R212 observes only that host's returned effects; R237 admits only that host's advertised capabilities and fails closed when the shared epoch is stale; R141 closes exact returned payloads. The continuity operator is partition → exchange/transform → invariant carry → scar/history carry → re-contextualize/repartition.</p></div>
   <div className='r112-hybrid-truth'><ShieldCheck/><b>ONE HOST · ONE SNAPSHOT EPOCH · ONE RECOVERABLE PROOF PATH</b><small>Browser state, screenshots and hardware labels never substitute for a current authenticated heartbeat or returned proof. Host intelligence is advisory returned evidence, not a second executor. R238 shared sampling is read-only; R237 owns bounded authenticated command admission; R212 remains observation-only; R141 exact return closure, R146 history, R147 executor/dispatch authority and R125 admission authority remain unchanged. Woven Continuity is applied here as software lineage/correlation, not as a new physical primitive, physical dimension, scientific authority or CanonState mutation path.</small></div>
  </header>
