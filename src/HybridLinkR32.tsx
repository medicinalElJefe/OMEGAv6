...
export default function HybridLinkR32({status,record}:Props){
 ...
 return <section className='hybrid-r32 special-app r112-hybrid-link'>
  <header className='r112-hybrid-hero'>
   <div><span>SOVEREIGN COMPUTE · HYBRID LINK · R243 WOVEN SELF-BUILD + EXECUTION-MOTION CONVERGENCE · R242 NAVIGATION PRESERVED · R239 RESOURCE GOVERNOR · R238 HOST PROOF · R237 COMMAND · R141 RETURN CLOSURE</span><h2>Your PC is a governed OMEGA compute node.</h2><p>R243 advances the whole Hybrid path without replacing the proven spine. The existing R242 execution-motion transport closes the INVOKED→RETURNED liveness gap with authenticated renewable leases and step pulses; R243 converges that transport with the selected-host snapshot, bounded resource envelope and woven self-build planning fabric. R240 remains the single source-mutation/promotion gate, and the already-promoted R242 navigation lemma remains read-only.</p></div>
   <div className='r112-hybrid-truth'><ShieldCheck/><b>ONE HOST · ONE SNAPSHOT EPOCH · ONE EXECUTION LEASE · ONE RESOURCE ENVELOPE · ONE SOURCE-MUTATION CANDIDATE</b><small>Native execution is claimed only while an authenticated agent heartbeat is current. Lease motion proves continued ownership/liveness of a RUNNING claim and the currently entered allow-listed step; final success still requires a returned R141 packet. Build success is not source mutation. Exact APPLY_PATCH/WRITE_TEXT return proof remains mandatory for a source edit; expired motion fails closed; mutation is never blindly replayed; R240 alone may promote one exact proved source candidate; R125 alone may admit CanonState.</small></div>
  </header>
  ...
  <section className='r112-host-uses' aria-label='What Sovereign Compute adds'>
   ...
   <article><Cpu/><div><b>Transform work with visible motion</b><span>R243 converges the inherited execution-motion lease into the selected-host frame so long native steps expose a current step, elapsed time, lease freshness and returned-step count instead of an opaque RUNNING badge.</span></div></article>
   <article><GraduationCap/><div><b>Carry invariants, scars and planning state</b><span>The R243 planning fabric carries dependency state, residual scars, address identity and authority boundaries across 12→144→1,728→20,736→248,832 atlas/address levels. These are representational resolution levels, not physical dimensions.</span></div></article>
  ...
  <footer className='special-boundary'><ShieldCheck/>R243 is additive over the signed production R242 navigation release and the admitted R240/R239/R238 Hybrid/self-build line. It preserves the durable R101/R32 bridge, immutable R205 executor, R117 authenticated bootstrap, R125 admission authority, R134 world/scar continuity, R141 exact return closure, R146 history, R147 executor/dispatch authority, R209 correlation-only lineage, R212 first-hand host effects, R237 command authority, R238 selected-host/shared-epoch host intelligence, R239 resource governance and R240 exact one-candidate source promotion. R243 adds woven planning/evaluation plus execution-motion convergence; it does not create another production writer or Canon authority.</footer>
 </section>;
}
