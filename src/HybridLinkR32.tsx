...
// R264 may defer only the already-explicitly-collapsed donor/advanced surface.
// Every ordinary operational/proof surface remains mounted and visible on desktop and mobile.
const HybridMissionControlR8=lazy(()=>import('./HybridMissionControlR8'));

type Props={status:any;record:any};
const deepFallback=<div className='r264-deep-fallback' role='status' aria-live='polite'>Opening advanced federation and mission diagnostics…</div>;

export default function HybridLinkR32({status,record}:Props){
 const[deepOpen,setDeepOpen]=useState(false);
 return <section className='hybrid-r32 special-app r112-hybrid-link' data-r264-operational-surfaces='FULL' data-r264-responsive='DESKTOP_MOBILE'>
  <header className='r112-hybrid-hero'>
   <div><span>SOVEREIGN COMPUTE · HYBRID LINK · R117 CONNECTION + R247 CONNECTED ACTION + R243 WOVEN SELF-BUILD + EXECUTION-MOTION CONVERGENCE · R242 NAVIGATION PRESERVED · R239 RESOURCE GOVERNOR · R238 HOST PROOF · R237 COMMAND · R153 FULL REPAIR/BUILD · R141 PROOF CLOSURE · R254 OUTCOME CLOSURE · R255 EXPERIENCE LEDGER · R262 PARALLEL DEVELOPMENT · R264 ZERO-CONSTRICTION PERFORMANCE</span><h2>Full operational visibility, with performance work kept out of the way.</h2><p>R264 does not hide, progressively mount, or remove ordinary Hybrid controls. Connection, continuity, execution motion, returned host truth, resource governance, action, outcome closure, experience, adaptive development, host effects, command controls, proof closure and mission lineage remain mounted in the normal Hybrid flow on both desktop and mobile. Performance work is limited to non-urgent observer scheduling and the already-explicit advanced donor surface.</p></div>
  ...
  <details className='r112-hybrid-deep' onToggle={e=>setDeepOpen((e.currentTarget as HTMLDetailsElement).open)}>
   <summary><TerminalSquare/><span><b>Advanced federation, mission planning and diagnostics</b><small>This was already an explicit disclosure surface. R264 may code-split this one donor layer because opening it is the operator's explicit demand; nothing in the ordinary Hybrid control/proof stack is deferred.</small></span></summary>
