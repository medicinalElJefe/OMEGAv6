...
type Imported={
 id:string;name:string;size:number;kind:'CSV'|'JSON'|'XLSX'|'OTHER';sha256:string;
 rows?:number;columns?:number;keys?:string[];preview?:string[][];boundary:string
};
const STORE='omega.r46.data.imports';
const NAV:Record<string,string>={ask:'Command Center',command:'Command Center',build:'Build Out',code:'Development',create:'Create',render:'Visual Instrument',visual:'Visual Instrument',matter:'Matter Traversal',traverse:'Traversal',earth:'Earth Now',forecast:'Forecast',proof:'Evidence & Proof',evidence:'Evidence & Proof',memory:'Memory',archive:'Archive Census',plugin:'Plugins',adapter:'Plugins',data:'Plugins',csv:'Plugins',excel:'Plugins',language:'Instructions',lexicon:'Instructions',system:'System Atlas',mode:'Modes',calculus:'Modes',hybrid:'Hybrid Link',computer:'Hybrid Link',pc:'Hybrid Link'};
const OPS:Record<string,string>={carry:'CARRY',construct:'CONSTRUCT',build:'CONSTRUCT',prune:'PRUNE',remove:'PRUNE',turn:'TURN',change:'TURN',escalate:'ESCALATE',scar:'SCAR',translate:'TRANSLATE',prove:'PROVE',proof:'PROVE',forecast:'FORECAST',ledger:'LEDGER'};
...
export default function OmegaDataLexiconR46({onNavigate}:{onNavigate:(p:string)=>void}){
 ...
 const ingest=async(file:File)=>{
  ...
  setImports(next);localState.write(STORE,next);
 };
 return <section className='r46-data-language'>
  ...
  <div className='r46-two'>
   <section><label className='r46-drop'><Upload/><b>Fingerprint + inspect data</b><span>CSV/JSON preview · XLS/XLSX identity only</span><input type='file' accept='.csv,.json,.xls,.xlsx' onChange={e=>{const f=e.target.files?.[0];if(f)void ingest(f);e.currentTarget.value=''}}/></label><div className='r46-imports'>{imports.map(x=><article key={x.sha256}>{x.kind==='JSON'?<FileJson/>:<FileSpreadsheet/>}<div><b>{x.name}</b><small>{x.kind} · {x.size.toLocaleString()} bytes</small><code>{x.sha256}</code>{x.preview&&<pre>{x.preview.map(r=>r.slice(0,6).join(' | ')).join('\n')}</pre>}<span>{x.boundary}</span></div></article>)}</div></section>
   <section className='r46-lexicon'><label><Search/><textarea value={prompt} onChange={e=>setPrompt(e.target.value)}/></label><article><span>DETERMINISTIC ROUTE</span><h3>{language.route}</h3><div>{language.operators.map(x=><b key={x}>{x}</b>)}</div><button onClick={()=>onNavigate(language.route)}><Waypoints/>Open route</button><ol>{language.chain.map(x=><li key={x}>{x}</li>)}</ol><small>{language.boundary}</small></article></section>
  </div>
 </section>;
}
