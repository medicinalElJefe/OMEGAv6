...
import {R305_CAPABILITY_REACHABILITY} from './capabilityReachabilityR305';
...
export default function OmegaSystemInventoryR83({onNavigate,compact=false,initialTab='FABRIC'}:Props){
 ...
 return <section className={'r83-inventory '+(compact?'compact':'full')} data-reachability-revision='R305' data-reachability-pass={R305_CAPABILITY_REACHABILITY.pass?'true':'false'} data-reachability-residual-count={R305_CAPABILITY_REACHABILITY.residualCount}>
  <header className='r83-inventory-head'><div><span>OMEGA COMPLETE SOFTWARE + CALCULUS INDEX · PRESERVE BEFORE PRUNE</span><h3>Integrated capability fabric navigator</h3><p>Application destinations are an interface inventory only. The full machine is canonical state + calculus/modes + eight functional layers + software/runtime families + controls + federation + proof. R168 resolves Runtime families from the current R48/R153 successor ledger while retaining V24 status/targets as lineage evidence; R133 turns the same current ledger into build potential. R305 continuously audits that registered routes, recovered ledgers, master menus and layer contracts still converge without burying functions.</p></div><div className='r83-inventory-kpis'><b>{MASTER_SYSTEMS_R83.length}</b><small>systems</small><b>{FAMILIES.length}</b><small>families</small><b>{ALL_MODES_BOUNDARY.sourceModeEvaluations}</b><small>source modes</small><b>{ALL_MODES_BOUNDARY.canonAuthorities}</b><small>canon lenses</small><b>{MASTER_CAPABILITIES_R83.length}</b><small>capabilities</small><b>{R305_CAPABILITY_REACHABILITY.residualCount}</b><small>reachability scars</small></div></header>
