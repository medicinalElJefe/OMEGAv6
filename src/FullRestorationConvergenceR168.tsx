...
import {manifestR169} from './world/federationAttestationWorldLensR169.js';
...
export default function FullRestorationConvergenceR168({record,address,onNavigate}:Props){
 const[lens,setLens]=useState<any>(null),[genesis,setGenesis]=useState<any>(null),[busy,setBusy]=useState(false),[error,setError]=useState('');
 const federation=useMemo(()=>manifestR169(),[]);
 ...
 const load=async()=>{setBusy(true);setError('');try{
  const[accuracy,core,release,attestation,hybrid,genesisExpectation]=await Promise.all([
   ...
   safeJson(api.get<any>('/omega-genesis-attestation-r168.json'))
  ]);
  const coreLive=core?.ok===true&&core?.state==='LIVE';
  const world=await assembleDevelopmentResidualWorldLensR166({accuracyState:accuracy||{},runtimeEvidence:{coreHealth:core,releaseEvidence:release,runtimeAttestation:attestation,hybrid},workflowEvidence:[],context:{observerId:'omega-r168-full-restoration',address,eventTime:Date.now(),projection:'WOVEN',metrics:{continuity:Number(record?.metrics?.continuity||0),plasticity:Number(record?.metrics?.plasticity||0),evidence:coreLive?0.9:0.35,uncertainty:coreLive?0.18:0.55}}});
  setLens(world);setGenesis(genesisExpectation);
 }catch(e:any){setError(e?.message||String(e));setLens(null);setGenesis(null)}finally{setBusy(false)}};
 ...
 const federationState=genesis?.state||'PROOF_REQUIRED';
 return <section className='r168-restoration' aria-label='R168 full restoration convergence'>
  <header><div><span>R168 · FULL RESTORATION / CURRENT EXECUTION CONVERGENCE · R169 WORLD-SCAR CONTINUITY</span><h3>One lineage ledger · one current successor ledger · one live residual view</h3><p>Historical V24 registration remains provenance. Current operating truth comes from the R48/R153 successor ledger, R166 projects read-only residual pressure from R125/R164/runtime evidence, and R169 preserves federation-attestation evidence as R136/R134 world-scar continuity without converting attestation into execution or CanonState.</p></div><button onClick={()=>void load()} disabled={busy}><RefreshCw className={busy?'spin':''}/>{busy?'CHECKING':'REFRESH LIVE TRUTH'}</button></header>
  <div className='r168-kpis'><article><span>SOFTWARE FAMILIES</span><b>{R48_COMPLETION_FAMILIES.length}</b><small>single 24-family current completion ledger</small></article><article><span>CURRENT IMPLEMENTED</span><b>{R48_COMPLETION_SUMMARY.executable}</b><small>web + source + local successors</small></article><article><span>TRUTH GATED</span><b>{R48_COMPLETION_SUMMARY.gated}</b><small>{gated.map(x=>x.id).join(' · ')||'none'}</small></article><article><span>RESIDUAL WORLD</span><b>{overlay?.state||'LOADING'}</b><small>{overlay?`${overlay.counts.total} residual · ${overlay.counts.review} review · ${overlay.counts.blocking} block`:'R166 evidence assembling'}</small></article><article><span>FEDERATION WORLD SCAR</span><b>{federation.revision}</b><small>{federationState} · {federation.visualWorldAuthority}</small></article></div>
  ...
  <section className='r168-federation-world'><header><Waypoints/><div><span>R169 FEDERATION ATTESTATION → R136/R134 WORLD SCAR</span><b>{federationState}</b></div><small>attestation evidence ≠ execution ≠ promotion</small></header><div><article><span>ATTESTATION AUTHORITY</span><b>{federation.attestationAuthority}</b><small>{genesis?.expected?.contract||'R192 contract evidence required'}</small></article><article><span>WORLD AUTHORITY</span><b>{federation.visualWorldAuthority}</b><small>verified federation evidence may be carried into the existing living-world scar; it does not create a second world authority</small></article><article><span>CANON ADMISSION</span><b>{federation.canonicalAdmissionAuthority}</b><small>R169 canonical mutation: {String(federation.canonicalMutation)} · no federation closure is inferred</small></article></div></section>
  ...
  <footer><ShieldCheck/><span><b>Applied calculus boundary:</b> partition → exchange/transform → invariant carry → scar/residual carry → re-contextualize/repartition. Whole/part, inner/outer, orientation, and representation remain frame-relative roles. 12 → 144 → 1,728 → 20,736 → 248,832 remain address/view/scheduling resolution levels, not unsupported physical dimensions. Returned work is not automatically verified, logical swarm fanout is not physical-worker proof, federation attestation is not federation execution, and R125 remains the only CanonState admission authority.</span></footer>
 </section>;
}
