...
export interface SarRasterFieldR283{
 ...
 amplitudeDb?:number[];complexI?:number[];complexQ?:number[];phaseRad?:number[];interferogramReal?:number[];interferogramImag?:number[];interferogramPhaseRad?:number[];coherence?:number[];losDisplacementM?:number[];elevationM?:number[];polarimetricPower?:number[];multiBandRelative?:number[];timeStackRelative?:number[];uncertainty?:number[];quality?:number[];
 scarBurden?:number[];proofCoverage?:number[];pairValidMask?:number[];
 derivationR336?:{schema:string;sourceId:string;evidenceClass:string;operators:string[];truthBoundary:string};
 derivationR341?:{schema:string;masterSourceId:string;slaveSourceId:string;gridAdmission:string;gridTolerance:number;coherenceWindow:string;validPairSamples:number;coherenceSamples:number;meanCoherence:number|null;temporalBaselineDays:number|null;operators:string[];truthBoundary:string};
...
export function rasterVisualValueR283(r:SarRasterFieldR283,view:string,x01:number,y01:number){const i=rasterIndexR283(r,x01,y01);let v:number|null=null;
 ...
 if(view==='INTERFEROGRAM'){v=at(r.interferogramPhaseRad,i,r.pairValidMask);return v==null?null:normalizePhaseR280(v)}
