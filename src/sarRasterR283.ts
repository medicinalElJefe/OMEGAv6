...
export function rasterVisualValueR283(r:SarRasterFieldR283,view:string,x01:number,y01:number){const i=rasterIndexR283(r,x01,y01);let v:number|null=null;
 ...
 if(view==='AMPLITUDE'){v=at(r.amplitudeDb,i);if(v==null)return null;const q=r.ranges?.amplitudeDb||[-30,5];return normalizeDbR280(v,q[0],q[1])}
...
export function rasterCoverageR283(r:SarRasterFieldR283,view:string){const key=view==='SOURCE'?(r.nativeIntensity?.length?'nativeIntensity':'amplitudeDb'):view==='AMPLITUDE'?'amplitudeDb':view==='TIME_STACK'?'timeStackRelative':view==='PHASE'||view==='INTERFEROGRAM'?'phaseRad':view==='COHERENCE'?'coherence':view==='DEFORMATION'?'losDisplacementM':view==='ELEVATION'?'elevationM':view==='POLARIMETRY'?'polarimetricPower':view==='MULTI_BAND'?'multiBandRelative':view==='SCAR_UNCERTAINTY'?'uncertainty':view==='PROOF'?'quality':'amplitudeDb';const a=(r as any)[key] as number[]|undefined,mask=key==='nativeIntensity'?r.validMask:undefined,actual=a?.length?(mask?mask.filter(x=>x>0).length:a.filter(Number.isFinite).length):0;return{field:key,bound:actual>0,expected:r.width*r.height,actual,complete:actual===r.width*r.height}}
export function rasterTruthBoundaryR283(){return'R283/R326 raster rendering reads declared measurement arrays. SOURCE may read exact decoded native DN samples with an explicit validity mask; calibrated amplitude, time-stack and derived lenses require their own arrays. Missing arrays/pixels remain missing and never become claimed source evidence.'}
