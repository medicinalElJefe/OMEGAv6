...
export interface SarRasterFieldR283{
 ...
 amplitudeDb?:number[];beta0?:number[];sigma0?:number[];gamma0?:number[];terrainFlattenedGamma0?:number[];complexI?:number[];complexQ?:number[];phaseRad?:number[];interferogramPhaseRad?:number[];correctedInterferometricPhaseRad?:number[];unwrappedPhaseRad?:number[];unwrapComponentId?:number[];coherence?:number[];losDisplacementM?:number[];correctedLosDisplacementM?:number[];deformationEastM?:number[];deformationNorthM?:number[];deformationUpM?:number[];elevationM?:number[];polarimetricPower?:number[];multiBandRelative?:number[];timeStackRelative?:number[];uncertainty?:number[];quality?:number[];
 ...
 physicalClosureR343?:{schema:string;coregistrationBound:boolean;interferometricPhaseValidated:boolean;calibrationBound:boolean;terrainFlattenedBound:boolean;unwrappedPhaseBound:boolean;correctionLedgerBound:boolean;losDisplacementBound:boolean;proof:string[];scars:string[]};
