...
const tool=createOpticalExternalToolR1532({addressCandidate:addressCandidateR152,screenCandidate:normalizedScreen,atlasSize:R152_ATLAS_SIZE,truthBoundary:R152_TRUTH_BOUNDARY,service:R152_SERVICE});
...
async function augment(response,patch){const body=await response.json();const headers=new Headers(response.headers);headers.set('x-omega-tool-version',R1532_TOOL_VERSION);headers.set('cache-control','no-store');return new Response(JSON.stringify({...body,...patch},null,2),{status:response.status,headers})}
...
async function route(request){
 ...
 if(request.method==='GET'&&u.pathname==='/api/health'){
  ...
  return augment(original,{machineVersion:R1532_MACHINE_VERSION,externalTool:{name:R1532_TOOL_NAME,version:R1532_TOOL_VERSION,ready:true,descriptor:'/api/tool/descriptor',probe:'/api/tool/probe',invoke:'/api/tool/invoke',insight:'/api/tool/insight',cycle:'/api/tool/cycle',openapi:'/openapi.json',adaptiveRefine:true,adaptiveCycle:true,maxAdaptiveRounds:6,maxAdaptiveEvaluations:384,canonicalMutation:false}});
 }
 if(request.method==='GET'&&u.pathname==='/api/optical/manifest'){
  ...
  headers.set('x-omega-tool-version',R1532_TOOL_VERSION);headers.set('cache-control','no-store');
  ...
  return new Response(JSON.stringify({...body,machineVersion:R1532_MACHINE_VERSION,capabilities,externalTool:tool.descriptor(origin)},null,2),{status:original.status,headers});
 }
 const response=await opticalR152.fetch(request),headers=new Headers(response.headers);headers.set('x-omega-tool-version',R1532_TOOL_VERSION);return new Response(response.body,{status:response.status,statusText:response.statusText,headers});
}
