...
export const R1532_INHERITED_SCREEN_SERVICE=R152_SERVICE;
...
const corsHeaders={
 ...
 'access-control-expose-headers':'x-omega-machine-adapter,x-omega-truth-boundary,x-omega-tool-version,x-omega-machine-service,x-omega-machine-version,x-omega-inherited-screen-service',
 ...
 'x-omega-inherited-screen-service':R1532_INHERITED_SCREEN_SERVICE,
...
const identity={service:R1532_MACHINE_SERVICE,version:R1532_MACHINE_VERSION,machineVersion:R1532_MACHINE_VERSION,authority:R1532_MACHINE_AUTHORITY,inheritedScreenService:R1532_INHERITED_SCREEN_SERVICE};
async function augment(response,patch={}){const body=await response.json();const headers=new Headers(response.headers);for(const[k,v]of Object.entries(corsHeaders))headers.set(k,v);return new Response(JSON.stringify({...body,...identity,...patch},null,2),{status:response.status,headers})}
...
async function route(request){
 ...
 if(request.method==='GET'&&u.pathname==='/api/health'){
  ...
  return augment(original,{engine:'R1532_ADAPTIVE_EXTERNAL_SEARCH_OVER_R152_SCREEN',inheritedEngine:'R152_BOUNDED_OPTICAL_ATLAS_SCREEN',externalTool:{name:R1532_TOOL_NAME,version:R1532_TOOL_VERSION,ready:true,descriptor:'/api/tool/descriptor',probe:'/api/tool/probe',invoke:'/api/tool/invoke',insight:'/api/tool/insight',cycle:'/api/tool/cycle',openapi:'/openapi.json',adaptiveRefine:true,adaptiveCycle:true,maxAdaptiveRounds:6,maxAdaptiveEvaluations:384,canonicalMutation:false}});
 }
 if(request.method==='GET'&&u.pathname==='/api/optical/manifest'){
  ...
  return new Response(JSON.stringify({...body,...identity,baseManifestSchema:body.schema,capabilities,externalTool:tool.descriptor(origin)},null,2),{status:original.status,headers});
 }
 if(request.method==='POST'&&u.pathname==='/api/federation/screen'){
  const original=await opticalR152.fetch(request);
  return augment(original,{screeningService:R1532_INHERITED_SCREEN_SERVICE,screeningVersion:'R152.0'});
 }
 const response=await opticalR152.fetch(request),headers=new Headers(response.headers);for(const[k,v]of Object.entries(corsHeaders))headers.set(k,v);return new Response(response.body,{status:response.status,statusText:response.statusText,headers});
}
