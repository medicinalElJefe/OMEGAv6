...
def _build_eps_grid(np, nx, ny, pitch, width, length, theta_deg, eps_feature, eps_background):
    # Unit-cell coordinates in nanometers, centered at the origin. The mask is
    # rotated geometrically; the RCWA solver itself handles the Fourier expansion.
...
def _solve_once(job, nG):
    ...
    # grcwa uses consistent arbitrary units with c=1. Keeping every length in nm
    # and freq=1/lambda is dimensionally consistent.
    freq = 1.0 / wavelength
    ...
    # Finite buffer thicknesses in uniform half-space media mirror the reference
    # grcwa examples and leave the patterned region as the only structured layer.
