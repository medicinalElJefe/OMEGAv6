...
export function scoreCapsule(capsule) {
  ...
  return Number(capsule.expectedGain || 0) /
    Math.max(0.01, Number(capsule.complexity || 0) + Number(capsule.contradictionRisk || 0)) * riskFactor;
}

export function selectCapsule(state) {
  const admitted = new Set(state.admittedSourceCapsules || []);
  const candidates = (state.roadmap || [])
    .filter(c => !admitted.has(c.id) && (c.prerequisites || []).every(p => admitted.has(p)))
    .sort((a, b) => scoreCapsule(b) - scoreCapsule(a) || a.id.localeCompare(b.id));
...
export function classifyHeldCandidates({ currentMainSha, candidates }) {
  ...
  for (const candidate of candidates || []) {
    ...
    if (baseSha === currentMainSha) exact.push(candidate);
...
export function deriveResidualGate({ coreHealth, releaseEvidence, runtimeAttestation }) {
  if (!(coreHealth?.ok === true && coreHealth?.state === 'LIVE' && coreHealth?.schema === 'OMEGA_CANONICAL_CORE_HEALTH_R163')) {
    return { allow: false, state: 'BLOCK', reason: 'R163 canonical core health is not first-hand LIVE' };
  }
  const releaseSha = releaseEvidence?.source?.sha || null;
  const runtimeSha = runtimeAttestation?.source?.sha || null;
  if (releaseSha && runtimeSha && releaseSha !== runtimeSha) {
    return { allow: false, state: 'BLOCK', reason: 'release/runtime source SHA mismatch' };
  }
...
export function decideCycle({ currentMainSha, productionProofGreen, state, candidates, evidence }) {
  ...
  return {
    ...
    deploymentAuthority: AUTHORITY_BOUNDARIES.productionDeploymentWorkflow,
  };
}
