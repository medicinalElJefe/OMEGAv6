...
export async function inspectCycle({token,repo='medicinalElJefe/OMEGAv6',runtimeBase='https://omegav6.jeffdeweyeljefe.workers.dev'}){
  ...
  for(const ref of refs||[]){
    ...
    try{
      const candidate=await getRepoFile(token,repo,'public/omega-r170-selfbuild-candidate.json',branch);
      candidates.push({...candidate.json,branch,headSha:ref.object?.sha||null});
...
export async function proposeCycle({token,repo='medicinalElJefe/OMEGAv6',runtimeBase}){
  ...
  const {mainSha,state,decision}=inspection; const capsule=decision.capsule;
  ...
  const body=capsuleBody(capsule.id);
  await putRepoFile(token,repo,capsule.target,branch,`VCloud evolution g${generation}: ${capsule.id}`,body);
...
export async function promoteGreenVCloudPr({token,repo='medicinalElJefe/OMEGAv6',prNumber,requiredWorkflows=['OMEGA Cloud Bridge CI','R170 Current Convergence','R202 Operational Source Authority','R210 Release Controller']}){
  ...
  if(pr.base?.sha!==currentMain)return {action:'NONE',reason:'main drifted; stale candidate must not promote',expectedBase:pr.base?.sha,currentMain};
