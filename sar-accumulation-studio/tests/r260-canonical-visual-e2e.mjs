...
try{
  ...
  assert.equal(boot.runtime.featureRelease,'R259');assert.equal(boot.runtime.visualRelease,'R260');assert.equal(boot.runtime.patchRelease,'R260.0');assert.equal(boot.runtime.boundaries.structureTensorCreatesMeasurement,false);assert.equal(boot.runtime.boundaries.structureTensorIsInterferometricCoherence,false);for(const k of ['GNSS','STRAIN','SEISMIC','TILT','PORE_PRESSURE','ENVIRONMENT'])assert.equal(boot.domains[k].status,'ADAPTER_PENDING');
