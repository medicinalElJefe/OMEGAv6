...
try{
  ...
  await page.waitForFunction(()=>globalThis.OMEGA_SAR_R4_RUNTIME?.release==='R4-R252'&&globalThis.OMEGA_SAR_EXPERIENCE?.ready===true&&globalThis.OMEGA_SAR_SMOOTH_MOTION?.state==='READY',null,{timeout:30000});

  const initial=await page.evaluate(()=>{
    const rect=s=>document.querySelector(s)?.getBoundingClientRect(),style=s=>getComputedStyle(document.querySelector(s));
  ...
  assert.equal(initial.mode,'explore');assert.equal(initial.drawer,null);
  ...
  const evidence=await page.evaluate(()=>{const d=document.querySelector('.evidence-dock').getBoundingClientRect(),r=document.querySelector('#raster').getBoundingClientRect(),p=document.querySelector('.pixel-stage').getBoundingClientRect();return {d,r,p,drawer:document.body.dataset.drawer};});
  assert.equal(evidence.drawer,'evidence');assert.ok(evidence.d.right<=innerWidth+2&&evidence.d.left>innerWidth*.65,'evidence drawer should be visible on the right');
  ...
  const experience=await page.evaluate(()=>({state:globalThis.OMEGA_SAR_EXPERIENCE.state,mode:globalThis.OMEGA_SAR_EXPERIENCE.mode,updates:globalThis.OMEGA_SAR_EXPERIENCE.updates,runtime:globalThis.OMEGA_SAR_R4_RUNTIME.experience}));
  assert.equal(experience.state,'READY');assert.ok(experience.updates>0);assert.equal(experience.runtime.layout,'FULL_BLEED_EARTH_WITH_ON_DEMAND_MISSION_EVIDENCE_ANALYSIS_DRAWERS');
