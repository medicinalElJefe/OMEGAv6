...
try{
  ...
  await page.waitForFunction(()=>globalThis.OMEGA_SAR_R4_RUNTIME?.release==='R4-R248'&&globalThis.OMEGA_SAR_CONTINUOUS_FIELD?.cells?.length>100&&globalThis.OMEGA_SAR_LOCATION?.jump&&globalThis.OMEGA_SAR_NAVIGATION?.selectTarget&&globalThis.OMEGA_SAR_REGIONAL_MEASUREMENT&&globalThis.OMEGA_EARTH_AWARENESS,null,{timeout:30000});
  ...
  await page.waitForFunction(()=>{const r=globalThis.OMEGA_SAR_REGIONAL_MEASUREMENT;return r?.state==='READY'&&r.visible===true&&r.patch?.state==='CALIBRATED_SENTINEL1_REGIONAL_VIEWPORT'&&r.patch?.evidence?.measured===true&&r.patch?.evidence?.inferred===false&&r.patch?.stats?.validCount>100;},null,{timeout:90000});
  const sourceState=await page.evaluate(()=>({
    scale:globalThis.OMEGA_SAR_NAVIGATION?.view?.scale,
    browseVisible:document.querySelector('.sar-source-browse-canvas')?.dataset.mainMapVisible,
    sourceRegistration:globalThis.OMEGA_SAR_SOURCE_FRAME?.registration,
    src:globalThis.OMEGA_SAR_SOURCE_FRAME?.src,
    opacity:globalThis.OMEGA_SAR_RENDERER?.baseOpacity,
    regional:{state:globalThis.OMEGA_SAR_REGIONAL_MEASUREMENT?.state,visible:globalThis.OMEGA_SAR_REGIONAL_MEASUREMENT?.visible,patchState:globalThis.OMEGA_SAR_REGIONAL_MEASUREMENT?.patch?.state,validCount:globalThis.OMEGA_SAR_REGIONAL_MEASUREMENT?.patch?.stats?.validCount,evidence:globalThis.OMEGA_SAR_REGIONAL_MEASUREMENT?.patch?.evidence,geolocation:globalThis.OMEGA_SAR_REGIONAL_MEASUREMENT?.patch?.geolocation,overview:globalThis.OMEGA_SAR_REGIONAL_MEASUREMENT?.patch?.overview}
  }));
  assert.ok(sourceState.scale<=180,'regional stage jumped to unsupported local scale');assert.ok(sourceState.src);assert.notEqual(sourceState.sourceRegistration,'FOOTPRINT_QUAD_WARP','legacy quicklook quad warp returned');assert.equal(sourceState.regional.state,'READY');assert.equal(sourceState.regional.visible,true);assert.equal(sourceState.regional.patchState,'CALIBRATED_SENTINEL1_REGIONAL_VIEWPORT');assert.ok(sourceState.regional.validCount>100);assert.equal(sourceState.regional.evidence?.measured,true);assert.equal(sourceState.regional.evidence?.inferred,false);assert.ok(sourceState.opacity<=.14,'Earth context visually dominates measured regional SAR');
  if(sourceState.sourceRegistration?.startsWith('FOOTPRINT_ONLY_'))assert.equal(sourceState.browseVisible,'false','unregistered browse pixels became visible');

  // Topography and water geometry must be source/derived context on the same camera, never SAR measurement.
  await page.waitForFunction(()=>{const a=globalThis.OMEGA_EARTH_AWARENESS;return a?.state==='READY'&&a.terrain?.rawDem===true&&a.water?.geometry?.accumulation?.length>100;},null,{timeout:60000});
  const awareness=await page.evaluate(()=>globalThis.OMEGA_EARTH_AWARENESS.snapshot());
  assert.equal(awareness.terrain.rawDem,true);assert.equal(awareness.terrain.measuredSar,false);assert.equal(awareness.water.observedWater,false);assert.ok(awareness.water.summary.cells>100);assert.match(awareness.water.summary.boundary,/not observed water depth/i);assert.deepEqual(awareness.events.sources,['USGS','NASA_EONET']);assert.match(awareness.boundaries.canon,/not a replacement physical law/i);

  // Play changes actual source scene while holding target identity. Regional measurement follows each scene asynchronously.
  ...
  await page.waitForFunction(()=>globalThis.OMEGA_SAR_RENDERER?.sarOverlay?.patch?.state==='CALIBRATED_SENTINEL1_TARGET_PATCH'&&globalThis.OMEGA_SAR_FIELD_RUNTIME?.patchAnchors?.length>=4&&globalThis.OMEGA_SAR_BLADE_FOCUS?.lens?.state==='BLADE_LENS_READY'&&document.querySelector('#omegaFitSar')?.disabled===false,null,{timeout:15000});
  const exactBound=await page.evaluate(()=>({view:{...globalThis.OMEGA_SAR_RENDERER.view},regionalVisible:globalThis.OMEGA_SAR_REGIONAL_MEASUREMENT?.visible,overlay:globalThis.OMEGA_SAR_RENDERER?.sarOverlay?.patch?.state,anchors:globalThis.OMEGA_SAR_FIELD_RUNTIME?.patchAnchors?.length||0,blade:globalThis.OMEGA_SAR_BLADE_FOCUS?.lens?.state,rasterStats:document.querySelector('#rasterStats')?.textContent||'',opacity:globalThis.OMEGA_SAR_RENDERER?.baseOpacity}));
  assert.ok(Math.abs(exactBound.view.centerLon-beforeCal.centerLon)<1e-8&&Math.abs(exactBound.view.centerLat-beforeCal.centerLat)<1e-8&&Math.abs(exactBound.view.scale-beforeCal.scale)<1e-8,'exact calibration hijacked camera');assert.equal(exactBound.overlay,'CALIBRATED_SENTINEL1_TARGET_PATCH');assert.equal(exactBound.blade,'BLADE_LENS_READY');assert.ok(exactBound.anchors>=4);assert.match(exactBound.rasterStats,/CALIBRATED GRD/);assert.ok(exactBound.opacity<=.05,'Earth context is too strong over exact SAR');

  // User-controlled FIT SAR is the only operation that may frame the exact patch; regional measurement must yield to it.
  await page.click('#omegaFitSar');await page.waitForFunction(()=>globalThis.OMEGA_SAR_LOCAL_FOCUS?.state==='CALIBRATED_PATCH_FIT_EXPLICIT',null,{timeout:10000});await page.waitForTimeout(250);
  const local=await page.evaluate(()=>({scale:globalThis.OMEGA_SAR_NAVIGATION?.view?.scale,browseMain:document.querySelector('.sar-source-browse-canvas')?.dataset.mainMapVisible,regionalVisible:document.querySelector('.omega-regional-sar-layer canvas')?.dataset.visible,focus:globalThis.OMEGA_SAR_LOCAL_FOCUS,target:globalThis.OMEGA_SAR_NAVIGATION.target}));
  assert.ok(local.scale>180,'FIT SAR did not enter exact measurement scale');assert.equal(local.browseMain,'false','regional quicklook still covers exact SAR after FIT SAR');assert.equal(local.regionalVisible,'false','regional measured layer did not yield to exact local SAR');assert.ok(Math.abs(local.target.lon-tucson.lon)<1e-4&&Math.abs(local.target.lat-tucson.lat)<1e-4,'FIT SAR changed target identity');
  ...
  console.log(JSON.stringify({expected,sourceState,awareness,patch,exactBound,local,field},null,2));console.log('SAR_R4_R248_REGIONAL_EARTH_AWARENESS_PASS');
