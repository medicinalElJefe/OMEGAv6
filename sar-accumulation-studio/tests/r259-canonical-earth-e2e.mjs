...
try{
  ...
  assert.equal(boot.runtime.featureRelease,'R259');assert.ok(['R259.0','R260.0','R260.3'].includes(boot.runtime.patchRelease));assert.equal(boot.runtime.boundaries.documentedAdapterIsLiveData,false);assert.equal(boot.adapterFamilies.length,6);assert.ok(boot.adapterFamilies.every(x=>x.status==='DOCUMENTED_ADAPTER_PENDING'));assert.match(boot.readout,/CANON/i);
