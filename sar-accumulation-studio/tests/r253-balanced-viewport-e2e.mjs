...
try{
  ...
  await page.waitForFunction(()=>globalThis.OMEGA_SAR_R4_RUNTIME?.release==='R4-R255'&&globalThis.OMEGA_SAR_EXPERIENCE?.ready===true&&globalThis.OMEGA_SAR_PRIMARY_WORKSTATION?.state==='READY',null,{timeout:30000});
  ...
  const initial=await page.evaluate(()=>{
    ...
    const selectors=['#omegaExperienceStatus','#omegaQuickRail','.transport-deck','#omegaMapNav'];
    ...
    return {vw:innerWidth,vh:innerHeight,map,visibleMap,top,dock,search,controls,transport,hits,layout:globalThis.OMEGA_SAR_R4_RUNTIME.experience.layout,surface:globalThis.OMEGA_SAR_PRIMARY_WORKSTATION.surface};
  });
  assert.equal(initial.layout,'SAR_PRIMARY_VIEW_WITH_40PX_COMMAND_STRIP_AND_BOUNDED_DRAWERS');
  ...
  await page.waitForFunction(()=>{const r=globalThis.OMEGA_SAR_REGIONAL_MEASUREMENT;return r?.state==='READY'&&r.visible===true&&r.patch?.state==='CALIBRATED_SENTINEL1_REGIONAL_VIEWPORT'&&r.patch?.evidence?.measured===true&&r.patch?.stats?.validCount>100;},null,{timeout:120000,polling:300});

  const measured=await page.evaluate(()=>({
    view:{...globalThis.OMEGA_SAR_RENDERER.view},
    surface:globalThis.OMEGA_SAR_PRIMARY_WORKSTATION.surface,
    regional:{state:globalThis.OMEGA_SAR_REGIONAL_MEASUREMENT.state,visible:globalThis.OMEGA_SAR_REGIONAL_MEASUREMENT.visible,validCount:globalThis.OMEGA_SAR_REGIONAL_MEASUREMENT.patch?.stats?.validCount||0,evidence:globalThis.OMEGA_SAR_REGIONAL_MEASUREMENT.patch?.evidence||null},
  ...
  assert.equal(measured.surface,'REGIONAL_MEASURED');assert.equal(measured.regional.state,'READY');assert.equal(measured.regional.visible,true);assert.equal(measured.regional.evidence?.measured,true);assert.equal(measured.regional.evidence?.inferred,false);assert.ok(measured.regional.validCount>100);assert.ok(Number(measured.regionalCanvas)>=.95,'measured regional SAR is not visually primary');assert.ok(Number(measured.globalCanvas)<=.03,'global support fabric is washing out measured SAR');assert.match(measured.controls,/SAR/);assert.match(measured.controls,/WORLD/);assert.match(measured.controls,/DATA/);

  // Navigation must still work after measurement is on screen.
  ...
  await page.click('#omegaSarEvidence');await page.waitForTimeout(320);const evidence=await page.evaluate(()=>({drawer:document.body.dataset.drawer,d:document.querySelector('.evidence-dock').getBoundingClientRect(),vw:innerWidth}));assert.equal(evidence.drawer,'evidence');assert.ok(evidence.d.width<=342);assert.ok(evidence.d.left>evidence.vw*.76);await page.keyboard.press('Escape');
  assert.deepEqual(pageErrors,[],`page script errors: ${pageErrors.join(' | ')}`);
  console.log('SAR_R4_R255_PRIMARY_WORKSTATION_PASS',JSON.stringify({initial,measured,afterZoom,evidence},null,2));
