...
try{
  ...
  const initial=await page.evaluate(()=>{
    ...
    return {vw:innerWidth,vh:innerHeight,map,visibleMap,top,dock,search,controls,transport,hits,layout:globalThis.OMEGA_SAR_R4_RUNTIME.experience.layout,featureRelease:globalThis.OMEGA_SAR_R4_RUNTIME.featureRelease,patchRelease:globalThis.OMEGA_SAR_R4_RUNTIME.patchRelease,visualRelease:globalThis.OMEGA_SAR_R4_RUNTIME.visualRelease||null,surface:globalThis.OMEGA_SAR_PRIMARY_WORKSTATION.surface,nativeSurface:globalThis.OMEGA_DATA_NATIVE_SURFACE.surface,overlapPolicy:globalThis.OMEGA_SAR_R257_EXPERIENCE.overlapPolicy};
  });
  // Release-forward compatibility gate: preserve the collision-free R258 workstation geometry
  // while allowing later Canon/visual releases to advance feature and patch metadata.
  assert.equal(initial.layout,'ONE_IMAGE_PLANE_WITH_RESERVED_PROOF_EVIDENCE_MISSION_AND_ANALYSIS_ZONES');
  const featureMajor=Number(initial.featureRelease?.match(/^R(\d+)$/)?.[1]);
  const patchMatch=initial.patchRelease?.match(/^R(\d+)\.(\d+)$/),patchMajor=Number(patchMatch?.[1]),patchMinor=Number(patchMatch?.[2]);
  assert.ok(featureMajor>=258,`workstation geometry requires feature lineage R258+; found ${initial.featureRelease}`);
  assert.ok(patchMajor>258||(patchMajor===258&&patchMinor>=0),`workstation geometry requires patch lineage R258+; found ${initial.patchRelease}`);
  ...
  await page.click('#omegaSarEvidence');await page.waitForTimeout(120);const evidence=await page.evaluate(()=>({drawer:document.body.dataset.drawer,d:document.querySelector('.evidence-dock').getBoundingClientRect(),map:document.querySelector('.map-wrap').getBoundingClientRect(),dock:document.querySelector('.place-dock').getBoundingClientRect(),vw:innerWidth,proofDisplay:getComputedStyle(document.querySelector('#omegaR257ProofStack')).display,mapNavOpacity:getComputedStyle(document.querySelector('#omegaMapNav')).opacity,transition:getComputedStyle(document.querySelector('.map-wrap')).transitionDuration,overflow:document.querySelector('.evidence-dock').scrollWidth-document.querySelector('.evidence-dock').clientWidth}));assert.equal(evidence.drawer,'evidence');assert.ok(evidence.d.width<=382);assert.ok(evidence.d.left>evidence.vw*.70);assert.ok(evidence.d.right<=evidence.vw-4,'evidence drawer escapes viewport');assert.ok(evidence.map.right<=evidence.d.left+1,'evidence drawer overlaps SAR pixels');assert.ok(Math.abs(evidence.map.left-evidence.dock.left)<1&&Math.abs(evidence.map.right-evidence.dock.right)<1,'command strip and SAR surface reserve different geometry');assert.ok(evidence.overflow<=1,'evidence drawer has horizontal overflow');assert.equal(evidence.transition,'0s');assert.equal(evidence.proofDisplay,'none');assert.equal(Number(evidence.mapNavOpacity),0);await page.keyboard.press('Escape');
  assert.deepEqual(pageErrors,[],`page script errors: ${pageErrors.join(' | ')}`);
  console.log('SAR_RELEASE_FORWARD_PRIMARY_WORKSTATION_PASS',JSON.stringify({initial,measured,afterZoom,evidence},null,2));
