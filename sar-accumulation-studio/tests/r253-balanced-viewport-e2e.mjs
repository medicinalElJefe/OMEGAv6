...
try{
  ...
  const initial=await page.evaluate(()=>{
    ...
    return {vw:innerWidth,vh:innerHeight,map:rect('.map-wrap'),top:rect('.topbar'),search:rect('.place-search'),status:rect('#omegaExperienceStatus'),quick:rect('#omegaQuickRail'),transport:rect('.transport-deck'),drawer:document.body.dataset.drawer||null,mode:document.body.dataset.mode,hits,layout:globalThis.OMEGA_SAR_R4_RUNTIME.experience.layout};
  });
  ...
  assert.ok(initial.map.width>initial.vw*.98,`Earth width lost too much space: ${initial.map.width}/${initial.vw}`);
  assert.ok(initial.map.height>initial.vh*.93,`Earth height lost too much space: ${initial.map.height}/${initial.vh}`);
