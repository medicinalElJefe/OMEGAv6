...
try {
  for (const viewport of [{width:1440,height:1000},{width:900,height:900},{width:390,height:844}]) {
    ...
    await page.waitForSelector('#earthLocationConsole');
    await page.waitForSelector('#canonConsole');
    await page.waitForSelector('#nisarNativeConsole');
    ...
    await page.waitForFunction(()=>document.querySelector('#earthSelectedCoords')?.textContent.includes('32.222600'));
    await page.waitForFunction(()=>document.querySelector('#earthPlaceName')?.textContent==='Tucson');
    ...
    await page.waitForFunction(()=>!/move over map/.test(document.querySelector('#earthHoverCoords')?.textContent||''));
