...
export function buildGibsWmsUrl({bbox=[-180,-90,180,90],date=new Date().toISOString().slice(0,10),width=1200,height=600,layers=[GIBS_LAYERS.trueColor],transparent=false}={}){const b=normalizeBbox(bbox),p=new URLSearchParams({SERVICE:'WMS',REQUEST:'GetMap',VERSION:'1.1.1',SRS:'EPSG:4326',BBOX:b.join(','),WIDTH:String(Math.max(64,Math.min(2048,Math.round(width)))),HEIGHT:String(Math.max(64,Math.min(2048,Math.round(height)))),FORMAT:'image/png',TRANSPARENT:transparent?'TRUE':'FALSE',TIME:String(date).slice(0,10),LAYERS:layers.join(','),STYLES:''});return `${GIBS_ENDPOINT}?${p.toString()}`;}
export function gibsTransportUrl(url){if(!url)return null;const origin=globalThis.location?.origin;return origin?`${origin.replace(/\/$/,'')}/api/gibs?url=${encodeURIComponent(url)}`:url;}
