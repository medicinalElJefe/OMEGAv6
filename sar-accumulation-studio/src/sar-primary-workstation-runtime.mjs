...
const state={state:'INITIALIZING',intent:'sar',surface:'WORLD',target:null,restored:false,regionalScale:REGIONAL_SCALE};
...
function targetKey(p){return validTarget(p)?`${Number(p.lon).toFixed(6)},${Number(p.lat).toFixed(6)}`:null;}
...
function setSurface(){
  ...
  const label=$('#omegaSarPrimaryState'),sar=$('#omegaSarFocus'),fit=$('#omegaSarFit');
  if(label){
...
function applyRegionalFocus(point=state.target){
  const r=renderer();if(!r||!validTarget(point))return false;
  r.fitLocation(Number(point.lon),Number(point.lat),REGIONAL_SCALE);
  state.intent='sar';saveTarget(point);setSurface();
  setTimeout(()=>globalThis.OMEGA_SAR_REGIONAL_MEASUREMENT?.reload?.(),80);
  return true;
}
function focusSar(){
...
function evidence(){const b=$('#omegaQuickRail [data-drawer="evidence"]');if(b)b.click();else document.dispatchEvent(new KeyboardEvent('keydown',{key:'e',bubbles:true}));}
...
function installStyle(){
  ...
  /* R255: controls get a dedicated narrow command strip; SAR/Earth gets the rest. */
  ...
  /* The measured SAR layer becomes the visual surface; support layers recede. */
  body[data-sar-surface=regional_measured] .omega-global-sar-fabric canvas,body[data-sar-surface=exact_measured] .omega-global-sar-fabric canvas{opacity:.025!important;filter:none!important}
  body[data-sar-surface=regional_measured] .omega-woven-motion canvas,body[data-sar-surface=exact_measured] .omega-woven-motion canvas{opacity:.04!important}
  body[data-sar-surface=regional_measured] .omega-jrc-water-layer canvas,body[data-sar-surface=exact_measured] .omega-jrc-water-layer canvas{opacity:.08!important}
  body[data-sar-surface=regional_measured] .sar-source-browse-canvas,body[data-sar-surface=exact_measured] .sar-source-browse-canvas{opacity:.04!important}
  body[data-sar-surface=regional_measured] .omega-regional-sar-layer canvas{opacity:1!important;filter:contrast(1.18) brightness(1.02)!important}
  body[data-sar-surface=regional_measured] #omegaExperienceStatus,body[data-sar-surface=exact_measured] #omegaExperienceStatus{opacity:.36!important}
...
function patchNavigation(){
  ...
  n.selectTarget=async(point,options={})=>{
    ...
    state.intent='sar';saveTarget(point);applyRegionalFocus(point);return true;
  };
  return true;
}
function restore(){
  if(state.restored||nav()?.target)return;const p=savedTarget();if(!p)return;state.restored=true;setTimeout(()=>nav()?.selectTarget?.(p,{scale:REGIONAL_SCALE,reason:'restore previous SAR target'}),450);
}
function install(){
  installStyle();installControls();patchNavigation();restore();setSurface();
  const retry=setInterval(()=>{const ready=installControls()|patchNavigation();if(nav()?.selectTarget&&$('#omegaSarPrimaryControls')){clearInterval(retry);restore();setSurface();}},120);setTimeout(()=>clearInterval(retry),5000);
  state.state='READY';
}

map?.addEventListener('omega-map-select',event=>{
  const p=event.detail;if(!validTarget(p))return;saveTarget(p);
  // A direct click on the whole-Earth navigator is an explicit target choice. Move
  // into the regional measurement band so the next surface is actual calibrated SAR.
  const scale=Number(renderer()?.view?.scale)||1;if(scale<REGIONAL_MIN&&state.intent!=='world')setTimeout(()=>applyRegionalFocus(p),80);
  setSurface();
});
...
window.addEventListener('omega-regional-sar-measurement',event=>{
  if(event.detail?.state==='READY'&&state.intent==='sar'){
    const scale=Number(renderer()?.view?.scale)||1;if(scale<REGIONAL_MIN||scale>REGIONAL_MAX)applyRegionalFocus(nav()?.target||state.target);
  }
  setSurface();
});
...
state.focusSar=focusSar;state.world=world;state.evidence=evidence;state.surfaceState=setSurface;
