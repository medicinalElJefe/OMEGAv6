...
const runtime={
  ...
  visualState:'IMMERSIVE_MAP_FIRST_SAR_EXPERIENCE_PLUS_GLOBAL_SOURCE_BACKED_FABRIC_PLUS_SMOOTH_INERTIAL_CAMERA_PLUS_MODE188_LEMMA_TRANSLATOR_PLUS_WOVEN_CONTINUITY_MOTION_PLUS_CALIBRATED_REGIONAL_AND_EXACT_SAR',
  ...
  experience:{defaultMode:'EXPLORE',layout:'FULL_BLEED_EARTH_WITH_ON_DEMAND_MISSION_EVIDENCE_ANALYSIS_DRAWERS',modes:['EXPLORE','ANALYZE','PROOF'],cleanView:true,keyboard:['M_MISSION','E_EVIDENCE','A_ANALYSIS','F_CLEAN','ESC_CLOSE'],globalFabricVisual:'SOFT_CONTINUOUS_SOURCE_SUPPORT_FIELD_NOT_GRID',domContract:'LEGACY_STATUS_WRITES_PRESERVED_HIDDEN'},
...
function polish(){
  document.title='OMEGA SAR R4 · Immersive SAR Earth';
  const meta=document.querySelector('meta[name="description"]');if(meta)meta.content='OMEGA SAR R4 R252 — immersive map-first source-backed SAR Earth experience with global Sentinel-1 acquisition fabric, smooth camera motion, Mode188 lemma translation, calibrated regional SAR, exact blade-registered local measurement, measured-source lens and explicit proof boundaries.';
  const identity=document.querySelector('.identity b');if(identity)identity.textContent='OMEGA SAR · R4';
  const sub=document.querySelector('.identity span');if(sub)sub.textContent='IMMERSIVE SAR EARTH / GLOBAL FABRIC / MODE 188 / WOVEN CONTINUITY';
  ...
  const note=document.querySelector('.map-note');if(note)note.textContent='Map-first SAR Earth · M mission · E evidence · A analysis · F clean view';
  const visual=document.querySelector('#visual option[value="earth"]');if(visual)visual.textContent='Immersive SAR Earth';
  ...
  const earthHead=document.querySelector('#omegaEarthAwarenessHud header b');if(earthHead)earthHead.textContent='EARTH AWARENESS · R252';
