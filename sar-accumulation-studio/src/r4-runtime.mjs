...
const runtime={
  ...
  visualState:'ADAPTIVE_MEASURED_REGIONAL_SAR_PLUS_EXACT_BLADE_MESH_PLUS_TOPO_DERIVED_WATER_GEOMETRY_PLUS_OBSERVED_WATER_HISTORY_PLUS_FRAME_SYNCHRONIZED_LIVE_EARTH_EVENTS',
  ...
  boundaries:{footprintIsPixel:false,browseIsMeasurement:false,inferenceIsObservation:false,earthContextIsSar:false,topographicFlowPotentialIsObservedWater:false,jrcRgbTilesAreNumericWaterAnalysis:false,eventProximityImpliesCausation:false,canonIsPhysicalMeasurement:false,realtimeLabelMeansProviderFreshnessNotContinuousRadarSampling:true}
};
globalThis.OMEGA_SAR_R4_RUNTIME=runtime;

function polish(){
  document.title='OMEGA SAR R4 · Continuous SAR Earth Instrument';
  const meta=document.querySelector('meta[name="description"]');if(meta)meta.content='OMEGA SAR R4 — source-backed continuous SAR Earth instrument with adaptive measured Sentinel-1 regional calibration, exact blade-registered local SAR, inverse-Jacobian focus, terrain-derived water geometry, EC JRC historical surface-water context, frame-synchronized live Earth-event awareness, bounded OMEGA reconstruction, and explicit evidence boundaries.';
  const identity=document.querySelector('.identity b');if(identity)identity.textContent='OMEGA SAR · R4';
  const sub=document.querySelector('.identity span');if(sub)sub.textContent='CONTINUOUS SAR EARTH / REGIONAL MEASUREMENT / WATER GEOMETRY / FRAME ↔ WORLD SYNCHRONIZATION';
  const mission=document.querySelector('.mission-banner b');if(mission)mission.textContent='SAR-FIRST EARTH STATE · CALIBRATED REGIONAL + EXACT LOCAL SENTINEL-1 / NISAR · TOPOGRAPHY · DERIVED + OBSERVED WATER · SYNCHRONIZED LIVE EVENTS · BOUNDED Ω';
  const chain=document.querySelector('.truth-chain');if(chain)chain.textContent='TARGET → ACQUISITION → REGIONAL COG → CALIBRATION → GCP/BLADE MESH → EXACT PATCH → TOPOGRAPHY → WATER FLOW POTENTIAL ↔ OBSERVED WATER HISTORY → FRAME ↔ WORLD EVENTS → Ω FIELD → PROOF';
  const note=document.querySelector('.map-note');if(note)note.textContent='SAR-first Earth camera · adaptive measured regional COG · exact local blade mesh · inverse focus · DEM drainage potential · JRC 1984–2024 surface-water context · frame-synchronized sourced Earth events';
  ...
  for(const p of document.querySelectorAll('.truth-body p')){
    if(/Earth imagery is the visual surface/i.test(p.textContent||''))p.innerHTML='<b>SAR is the primary evidence surface.</b> Regional/exact SAR are source measured. JRC historical surface-water RGB, terrain, derived drainage potential, live-event metadata, GIBS, and OMEGA/Canon retain separate provenance; event proximity is relational context, not causal proof.';
  }
}
