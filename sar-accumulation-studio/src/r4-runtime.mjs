...
const runtime={
  ...
  visualState:'BALANCED_VIEWPORT_SAR_EARTH_EXPERIENCE_PLUS_GLOBAL_SOURCE_BACKED_FABRIC_PLUS_SMOOTH_INERTIAL_CAMERA_PLUS_MODE188_LEMMA_TRANSLATOR_PLUS_WOVEN_CONTINUITY_MOTION_PLUS_CALIBRATED_REGIONAL_AND_EXACT_SAR',
  ...
  experience:{defaultMode:'EXPLORE',layout:'BALANCED_VIEWPORT_WITH_COMPACT_CONTROLS_AND_BOUNDED_DRAWERS',modes:['EXPLORE','ANALYZE','PROOF'],cleanView:true,keyboard:['M_MISSION','E_EVIDENCE','A_ANALYSIS','F_CLEAN','ESC_CLOSE'],globalFabricVisual:'SOFT_CONTINUOUS_SOURCE_SUPPORT_FIELD_NOT_GRID',domContract:'LEGACY_STATUS_WRITES_PRESERVED_HIDDEN',viewportRule:'PRIMARY_EARTH_VIEW_REMAINS_VISIBLE_WHILE_CONTROLS_STAY_COMPACT'},
...
function polish(){
  document.title='OMEGA SAR R4 · Balanced SAR Earth';
  const meta=document.querySelector('meta[name="description"]');if(meta)meta.content='OMEGA SAR R4 R253 — compact balanced-viewport source-backed SAR Earth experience with global Sentinel-1 acquisition fabric, calibrated regional SAR, exact blade-registered local measurement, measured-source lens and explicit proof boundaries.';
  const identity=document.querySelector('.identity b');if(identity)identity.textContent='OMEGA SAR · R4';
  const sub=document.querySelector('.identity span');if(sub)sub.textContent='SAR EARTH / SOURCE FABRIC / MODE 188 / WOVEN CONTINUITY';
  const mission=document.querySelector('.mission-banner b');if(mission)mission.textContent='SAR-FIRST EARTH STATE · GLOBAL SOURCE COVERAGE → CALIBRATED REGIONAL → EXACT LOCAL · MODE188 TRANSLATION · WOVEN CONTINUITY';
  const chain=document.querySelector('.truth-chain');if(chain)chain.textContent='CAMERA → GLOBAL SAR FABRIC → LEMMA/MODE188 → REGIONAL COG → CALIBRATION → BLADE MESH → EXACT PATCH ↔ SOURCE LENS → EARTH CONTEXT → Ω FIELD → PROOF';
  const note=document.querySelector('.map-note');if(note)note.textContent='Balanced SAR Earth · M mission · E evidence · A analysis · F clean view';
  const visual=document.querySelector('#visual option[value="earth"]');if(visual)visual.textContent='Balanced SAR Earth';
  ...
  const earthHead=document.querySelector('#omegaEarthAwarenessHud header b');if(earthHead)earthHead.textContent='EARTH AWARENESS · R253';
