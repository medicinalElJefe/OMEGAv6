...
const runtime={
  ...
  computation:{lemmaTranslator:'MODE188_EVIDENCE_ADMISSION_AND_RENDER_STATE_TRANSLATOR',wovenContinuity:'INVARIANT_CARRY_PLUS_SCAR_HISTORY_CARRY_ACROSS_DECLARED_FRAMES',cameraMotion:'FRAME_COALESCED_POINTER_PAN_PLUS_EXPONENTIAL_ZOOM_PLUS_BOUNDED_INERTIA',atlasLod:'12_TO_144_TO_1728_TO_20736_TO_248832_RENDER_ADDRESS_RESOLUTION'},
...
function polish(){
  ...
  const meta=document.querySelector('meta[name="description"]');if(meta)meta.content='OMEGA SAR R4 R251 — source-backed global Sentinel-1 acquisition fabric, frame-coalesced inertial Earth camera, Mode188 lemma-state evidence translator, woven continuity motion projection, adaptive calibrated regional SAR, exact blade-registered local SAR, measured-source lens, water geometry and synchronized Earth-event awareness.';
  ...
  const chain=document.querySelector('.truth-chain');if(chain)chain.textContent='CAMERA → GLOBAL SAR FABRIC → LEMMA/MODE188 → ACQUISITION → REGIONAL COG → CALIBRATION → BLADE MESH → EXACT PATCH ↔ SOURCE LENS → TOPO/WATER → FRAME ↔ WORLD EVENTS → Ω FIELD → PROOF';
