...
function renderPatchStatus(patch){
  const e=$('#rasterEmpty'),stats=$('#rasterStats');if(e)e.style.display='none';
  ...
  const canvas=$('#raster');canvas?.classList.remove('flash');if(canvas){void canvas.offsetWidth;canvas.classList.add('flash');}
  const badge=$('#sarCalProof');if(badge){badge.textContent=`${g.quality} · ${patch.evidence.grade} · ${patch.processing.thermalNoiseCorrectionPerformed?'NOISE CORRECTED':'NOISE STATE UNCONFIRMED'} · ${m?.state||'EARTH MESH PENDING'}`;badge.dataset.state='ready';}
}
...
async function loadCalibratedCurrent({force=false}={}){
  ...
  try{
    ...
    if(my!==generation)return null;if(e){e.style.display='grid';e.textContent=`Calibrated patch unavailable: ${error.message}`;}
    ...
    window.dispatchEvent(new CustomEvent('omega-calibrated-sar-patch-clear'));return null;
  }
}

function queryOptions(){const start=$('#start')?.value,end=$('#end')?.value,aoi=$('#aoi')?.value;return {start:start?`${start}T00:00:00Z`:null,end:end?`${end}T23:59:59Z`:null,intersectsWith:aoi||'',maxResults:Math.min(48,Math.max(1,Number($('#maxResults')?.value||24))),flightDirection:$('#direction')?.value||'',polarization:$('#polarization')?.value||''};}
...
function paintTemporalChart(samples){
  ...
  const x=t=>pad+(w-2*pad)*(maxT===minT?.5:(t-minT)/(maxT-minT)),y=v=>h-pad-(h-2*pad)*(maxV===minV?.5:(v-minV)/(maxV-minV));
...
async function probeCalibratedStack(){
  const target=pointFromUi(),status=$('#pixelProbeStatus'),button=$('#probeStack');if(!target){if(status)status.textContent='Bind a map/AOI point first.';return;}
  ...
  try{
    const result=await fetchSentinel1Cog(queryOptions()),pol=selectedPolarization(),quantity=selectedQuantity(),records=result.records.slice(-Math.min(24,result.records.length)),samples=[];
    for(let i=0;i<records.length;i++){
      if(status)status.textContent=`Calibrating actual target sample ${i+1}/${records.length}…`;
      try{samples.push(await sampleCalibratedSentinel1(records[i],target.lon,target.lat,{polarization:pol,quantity}));}catch(error){samples.push({state:'SAMPLE_ERROR',id:records[i].id,startTime:records[i].startTime,measured:false,error:error.message});}
    }
    const valid=samples.filter(s=>s.state==='CALIBRATED_SENTINEL1_GRD_SAMPLE'&&Number.isFinite(s.db));globalThis.OMEGA_SAR_CALIBRATED_STACK=samples;paintTemporalChart(samples);
  ...
  }catch(error){if(status)status.textContent=`Calibrated stack failed: ${error.message}`;}finally{if(button)button.disabled=false;}
}
...
function installInterceptors(){
  const raster=$('#loadRaster');raster?.addEventListener('click',event=>{event.preventDefault();event.stopImmediatePropagation();loadCalibratedCurrent({force:true});},true);
  const probe=$('#probeStack');probe?.addEventListener('click',event=>{event.preventDefault();event.stopImmediatePropagation();probeCalibratedStack();},true);
  const scene=$('#currentScene');if(scene){let last=scene.textContent;new MutationObserver(()=>{const now=scene.textContent;if(now===last)return;last=now;if(autoPatch&&now&&now!=='—')setTimeout(()=>loadCalibratedCurrent(),0);}).observe(scene,{childList:true,characterData:true,subtree:true});}
}

export function initializeSentinelConsole(){if(initialized||typeof document==='undefined')return;initialized=true;installControls();installInterceptors();globalThis.OMEGA_SAR_SENTINEL={loadCalibratedCurrent,probeCalibratedStack,cache};}
if(typeof document!=='undefined'){if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',initializeSentinelConsole,{once:true});else queueMicrotask(initializeSentinelConsole);}
