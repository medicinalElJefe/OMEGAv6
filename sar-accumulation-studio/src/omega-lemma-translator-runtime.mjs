...
const state={state:'INITIALIZING',field:null,annotated:0,counts:{},lastAt:null,previous:new Map(),timer:null,boundary:'Lemma translation classifies and carries evidence state inside the OMEGA field. It does not change measured values, create missing SAR pixels, or promote contextual/derived state into observation.'};
...
function annotate(){
  const field=globalThis.OMEGA_SAR_CONTINUOUS_FIELD;if(!field?.cells?.length||field===state.field)return false;
  ...
  state.previous=nextPrevious;state.field=field;state.annotated=annotated;state.counts=counts;state.lastAt=new Date().toISOString();state.state='READY';
  window.dispatchEvent(new CustomEvent('omega-lemma-field-update',{detail:snapshot()}));return true;
}
function snapshot(){return {state:state.state,annotated:state.annotated,counts:{...state.counts},lastAt:state.lastAt,boundary:state.boundary};}
function install(){state.state='WATCHING';state.timer=setInterval(annotate,220);queueMicrotask(annotate);}
