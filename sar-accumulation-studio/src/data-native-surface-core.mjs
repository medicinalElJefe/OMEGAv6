...
export function sampleTerrainField(terrain,field,lon,lat){
  ...
  const [w,s,e,n]=terrain.bbox,lo=Number(lon),la=Number(lat),lonSpan=e-w,latSpan=n-s;if(!(lonSpan>0&&latSpan>0)||!finite(lo)||!finite(la)||lo<w||lo>e||la<s||la>n)return NaN;
  const x=(lo-w)/lonSpan*(terrain.width-1),y=(n-la)/latSpan*(terrain.height-1);
...
function sampleDb(db,width,height,x,y){return Number(db[clamp(y,0,height-1)*width+clamp(x,0,width-1)]);}
function localSarTexture(db,width,height,x,y){
  const c=sampleDb(db,width,height,x,y);if(!finite(c))return 0;
  let energy=0,weight=0;
  for(const [radius,w] of [[1,.58],[2,.28],[4,.14]]){
    const l=sampleDb(db,width,height,x-radius,y),r=sampleDb(db,width,height,x+radius,y),u=sampleDb(db,width,height,x,y-radius),d=sampleDb(db,width,height,x,y+radius),gx=(finite(r)?r:c)-(finite(l)?l:c),gy=(finite(d)?d:c)-(finite(u)?u:c);energy+=w*Math.hypot(gx,gy)/Math.max(1,radius);weight+=w;
  }
...
export function buildTerrainReliefSurface(terrain,water,{azimuthDeg=315,elevationDeg=43}={}){
  ...
  return {width,height,rgba,stats:{minElevation:range.min,maxElevation:range.max,elevationSpan:range.span},semantics:'DEM-derived shaded relief using source DEM hypsometry plus DEM-normal lighting and derived drainage-potential modulation. This is a data-shaped terrain image, not optical land-cover synthesis and not an observed-water claim.'};
}
