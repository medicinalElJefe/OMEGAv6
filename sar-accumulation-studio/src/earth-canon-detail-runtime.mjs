...
function draw(){
  ...
  const requested=globalThis.OMEGA_SAR_R258_CALCULUS?.mode||'detail';if(requested!=='detail'){state.state='STANDBY_SPECIALIZED_VIEW';return;}
  if(!ensureImage()){state.state='WAITING_FOR_MEASURED_SAR';return;}const r=renderer(),mesh=activePatch?.geoMesh;if(!r||!mesh?.nodes||mesh.validNodeCount<4){state.state='WAITING_FOR_REGISTERED_MESH';return;}const n=mesh.segments||mesh.nodes.length-1;let cells=0;ctx.save();ctx.globalCompositeOperation='source-over';ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';
