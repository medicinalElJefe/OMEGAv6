...
function hasLocalMeasuredSar(){return renderer()?.sarOverlay?.patch?.evidence?.measured===true;}
function stageScale(value){const requested=Math.max(1,Math.min(8192,Number(value)||160));return hasLocalMeasuredSar()?requested:Math.min(160,requested);}
function centerErrorPixels(point){const r=map?.getBoundingClientRect(),v=renderer()?.view||nav.view,s=Number(v?.scale)||1;if(!r||!point)return null;let dlon=Number(v.centerLon)-Number(point.lon);while(dlon>180)dlon-=360;while(dlon<-180)dlon+=360;const dx=dlon*(r.width/360)*s,dy=(Number(point.lat)-Number(v.centerLat))*(r.height/180)*s;return {dx,dy,distance:Math.hypot(dx,dy)};}
function publishFromRenderer(){const r=renderer();if(!r)return;nav.view={...r.view,bbox:r.viewBounds?.()};}
async function bootstrapRenderer(point){if(renderer())return renderer();const button=$('#jumpLocation');if(!button)return null;bindInputs(point);nav.bootstrapPass=true;button.click();nav.bootstrapPass=false;for(let i=0;i<20&&!renderer();i++)await sleep(10);return renderer();}

async function focusLocation(point,{scale=160,bind=true}={}){
  if(!validPoint(point))return false;point={lon:Number(point.lon),lat:Number(point.lat)};bindInputs(point);const token=++nav.focusToken;nav.focusing=true;
  const r=renderer()||await bootstrapRenderer(point);if(!r||token!==nav.focusToken){nav.focusing=false;return false;}
  // Before exact local SAR exists, stay at a source-SAR regional scale. Once the
  // measured patch exists, the same camera may move to true local scale.
  r.fitLocation(point.lon,point.lat,stageScale(scale));if(bind)r.setPoint(point.lon,point.lat,{emit:true,redraw:true});publishFromRenderer();nav.lastSelectionKey=key(point);nav.focusing=false;return true;
}
function cancelFocus(){nav.focusToken++;nav.focusing=false;}
async function focusScale(target=950,point=parsePoint()){
  const r=renderer();if(!r){if(point)return focusLocation(point,{scale:target,bind:false});return false;}target=stageScale(target);const token=++nav.focusToken;nav.focusing=true;const p=validPoint(point)?{lon:Number(point.lon),lat:Number(point.lat)}:null;if(p)r.fitLocation(p.lon,p.lat,target);else r.zoomBy(target/(Number(r.view.scale)||1));publishFromRenderer();if(token===nav.focusToken)nav.focusing=false;return r.view.scale;
}
...
async function useDevice(){
  ...
  nav.pendingDevice=true;cancelFocus();if(button){button.dataset.busy='true';button.textContent='LOCATING DEVICE…';}const status=$('#status');if(status){status.textContent='Requesting precise browser/device location…';status.dataset.kind='';}
  navigator.geolocation.getCurrentPosition(async pos=>{const point={lon:Number(pos.coords.longitude),lat:Number(pos.coords.latitude)};await focusLocation(point,{scale:160,bind:true});const accuracy=Number(pos.coords.accuracy),error=centerErrorPixels(point);if(status){status.textContent=`Device location bound at ${point.lat.toFixed(5)}, ${point.lon.toFixed(5)}${Number.isFinite(accuracy)?` · ±${Math.round(accuracy)} m browser accuracy`:''}${error?` · camera residual ${error.distance.toFixed(2)} px`:''}. Loading SAR evidence for this point.`;status.dataset.kind='ok';}nav.pendingDevice=false;if(button){button.dataset.busy='false';button.textContent='USE DEVICE LOCATION';}},error=>{nav.pendingDevice=false;if(button){button.dataset.busy='false';button.textContent='USE DEVICE LOCATION';}if(status){status.textContent=`Device location unavailable: ${error.message}. You can still search or enter exact coordinates.`;status.dataset.kind='error';}},{enableHighAccuracy:true,timeout:15000,maximumAge:15000});
}
...
function wireProgrammaticPointSync(){
  ...
  const jump=$('#jumpLocation');if(jump)jump.addEventListener('click',event=>{if(nav.bootstrapPass)return;const p={lon:Number($('#jumpLon')?.value),lat:Number($('#jumpLat')?.value)};if(!validPoint(p))return;if(renderer()){event.preventDefault();event.stopImmediatePropagation();focusLocation(p,{scale:160,bind:true});}else setTimeout(()=>focusLocation(p,{scale:160,bind:true}),0);},true);
...
nav.focusLocation=focusLocation;nav.focusScale=focusScale;nav.focusCurrentTarget=focusCurrentTarget;nav.centerErrorPixels=centerErrorPixels;nav.useDevice=useDevice;nav.cancelFocus=cancelFocus;nav.zoomIn=()=>zoom(1.42);nav.zoomOut=()=>zoom(1/1.42);nav.renderer=renderer;nav.hasLocalMeasuredSar=hasLocalMeasuredSar;
