...
function bboxKey(b,scale){const q=scale<2.5?5:scale<12?2:scale<60?.5:.15;return [...b.map(v=>Math.round(v/q)*q),Math.round(Math.log2(Math.max(1,scale)))].join(':');}
function sectorsFor(b,scale){
  if(scale<=2.4||b[2]-b[0]>150)return [-180,-120,-60,0,60,120].map(w=>[w,-85,Math.min(180,w+60),85]);
  if(b[0]<=-179.99&&b[2]>=179.99)return [-180,-90,0,90].map(w=>[w,b[1],Math.min(180,w+90),b[3]]);
  return [b];
}
async function querySector(bbox,signal,{days=120,limit=80}={}){
...
function buildFabric(records,bbox,view){
  ...
  for(let iy=0;iy<rows;iy++)for(let ix=0;ix<cols;ix++){
    ...
    const mean=hits.length?sumAge/hits.length:null,orbitMix=hits.length?(asc-desc)/hits.length:0,lemma=translateFabricCell({coverage:hits.length,newestAgeHours:newest,meanAgeHours:mean,orbitMix,previous:state.fabric?.cells?.[iy*cols+ix]||null});
...
function colorFor(cell){
  const cov=cell.coverage,lemma=cell.lemma,recency=lemma.temporalSupport||0;if(cov){const alpha=Math.min(.30,.035+.035*Math.log1p(cov)+.11*recency);return `rgba(165,220,234,${alpha})`;}
  const w=cell.wovenDisplaySupport||0;return w>.08?`rgba(108,164,180,${Math.min(.055,.012+.045*w)})`:null;
}
...
async function refresh(){
  ensureLayer();const r=renderer();if(!r||!state.enabled)return;const bbox=normalizeBBox(r.viewBounds?.()),key=bboxKey(bbox,r.view.scale);if(key===state.lastKey&&state.fabric){draw();return;}const cached=cache.get(key);if(cached&&Date.now()-cached.at<300000){state.records=cached.records;state.previousFabric=state.fabric;state.fabric=buildFabric(state.records,bbox,{scale:r.view.scale,width:r.w,height:r.h});state.transitionStart=performance.now();state.lastKey=key;draw();return;}
  ...
  try{const sectors=sectorsFor(bbox,r.view.scale),groups=await Promise.all(sectors.map(b=>querySector(b,controller.signal,{days:r.view.scale<3?180:120,limit:r.view.scale<3?55:120})));if(my!==generation||controller.signal.aborted)return;const byId=new Map();for(const group of groups)for(const rec of group)byId.set(rec.id,rec);const records=prepareRecords([...byId.values()]);cache.set(key,{at:Date.now(),records});if(cache.size>18)cache.delete(cache.keys().next().value);state.previousFabric=state.fabric;state.records=records;state.fabric=buildFabric(records,bbox,{scale:r.view.scale,width:r.w,height:r.h});state.transitionStart=performance.now();state.lastKey=key;state.queries+=sectors.length;state.updatedAt=new Date().toISOString();state.state='READY';if(transitionRaf)cancelAnimationFrame(transitionRaf);draw();window.dispatchEvent(new CustomEvent('omega-global-sar-fabric-update',{detail:snapshot()}));}
