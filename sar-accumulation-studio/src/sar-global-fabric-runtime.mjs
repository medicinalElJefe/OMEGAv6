...
const state={state:'INITIALIZING',records:[],fabric:null,previousFabric:null,transitionStart:0,queries:0,lastKey:null,updatedAt:null,error:null,enabled:true,source:'EARTH_SEARCH_SENTINEL1_GRD_METADATA',inFlight:null,inFlightKey:null,activeController:null,sectorResolved:0,sectorUnresolved:0,scarLedger:[],refining:false,boundary:'Global fabric is source-backed acquisition coverage/time/COG availability plus bounded continuity visualization. It is not a global calibrated SAR mosaic and does not synthesize missing backscatter pixels.'};
...
function sectorsFor(b,scale){
  ...
  if(scale<=2.4||lonSpan>150)return splitBBox([-180,-85,180,85],60,56.6666667);
  if(lonSpan>70||latSpan>42)return splitBBox(b,45,32);
  if(lonSpan>28||latSpan>22)return splitBBox(b,24,18);
  return [b];
}
function scar(bbox,error,status=null){return {bbox:[...bbox],reason:String(error?.message||error||'unresolved'),status:Number.isFinite(Number(status))?Number(status):null,at:new Date().toISOString(),measurementPromotion:false};}
function addScar(entry){state.scarLedger.push(entry);if(state.scarLedger.length>72)state.scarLedger.splice(0,state.scarLedger.length-72);state.sectorUnresolved=state.scarLedger.length;}
async function querySector(bbox,signal,{days=365,limit=80}={}){
  ...
  if(!response.ok)throw Object.assign(new Error(`Global SAR STAC ${response.status}`),{status:response.status});
  const fc=await response.json(),fetchedAt=new Date().toISOString();
  if(fc?.omega?.state==='SOURCE_SECTOR_UNRESOLVED')return {records:[],scar:fc.omega.scar||scar(bbox,'Source sector unresolved')};
  return {records:(fc.features||[]).map(x=>normalizeStacItem(x,fetchedAt)),scar:null};
}
async function querySectors(sectors,signal,options,onProgress){
  const records=[],scars=[];
  for(let i=0;i<sectors.length;i+=6){
    if(signal?.aborted)throw new DOMException('Global SAR query superseded','AbortError');
    const batch=sectors.slice(i,i+6),settled=await Promise.allSettled(batch.map(b=>querySector(b,signal,options)));
    for(let j=0;j<settled.length;j++){
      const result=settled[j],bbox=batch[j];
      if(result.status==='fulfilled'){
        if(result.value.scar)scars.push(result.value.scar);else state.sectorResolved++;
        records.push(...result.value.records);
      }else{
        if(signal?.aborted||result.reason?.name==='AbortError')throw result.reason;
        scars.push(scar(bbox,result.reason,result.reason?.status));
      }
    }
    onProgress?.({records:[...records],scars:[...scars],completed:Math.min(sectors.length,i+batch.length),total:sectors.length});
  }
  return {records,scars};
}
...
function dedupePrepared(records){const byId=new Map();for(const rec of records)if(rec?.id)byId.set(rec.id,rec);return prepareRecords([...byId.values()]);}
...
function colorFor(cell){const cov=cell.coverage,lemma=cell.lemma,recency=lemma.temporalSupport||0;if(cov){const alpha=Math.min(.34,.04+.04*Math.log1p(cov)+.12*recency);return `rgba(165,220,234,${alpha})`;}const w=cell.wovenDisplaySupport||0;return w>.08?`rgba(108,164,180,${Math.min(.06,.012+.05*w)})`:null;}
function drawFabric(fabric,alpha=1){if(!fabric||!ctx)return;const r=renderer();if(!r)return;const {cols,rows,bbox,cells}=fabric,cw=(bbox[2]-bbox[0])/cols,ch=(bbox[3]-bbox[1])/rows;ctx.save();ctx.globalAlpha=alpha;for(const cell of cells){const fill=colorFor(cell);if(!fill)continue;const west=bbox[0]+cell.ix*cw,east=west+cw,north=bbox[3]-cell.iy*ch,south=north-ch,p0=r.project(west,north),p1=r.project(east,south),x=Math.min(p0[0],p1[0]),y=Math.min(p0[1],p1[1]),w=Math.abs(p1[0]-p0[0])+1,h=Math.abs(p1[1]-p0[1])+1;if(w<.2||h<.2)continue;ctx.fillStyle=fill;ctx.fillRect(x,y,w,h);if(cell.coverage>=3&&w>7&&h>5){ctx.strokeStyle=`rgba(213,239,245,${Math.min(.22,.035+.018*cell.coverage)})`;ctx.lineWidth=.5;ctx.strokeRect(x+.25,y+.25,w-.5,h-.5);}}ctx.restore();}
function draw(now=performance.now()){if(!canvas||!ctx||!map)return;const rect=map.getBoundingClientRect();ctx.clearRect(0,0,rect.width,rect.height);if(!state.enabled)return;const t=state.transitionStart?Math.max(0,Math.min(1,(now-state.transitionStart)/520)):1;if(state.previousFabric&&t<1)drawFabric(state.previousFabric,1-t);drawFabric(state.fabric,t);if(t<1){transitionRaf=requestAnimationFrame(draw);}else{state.previousFabric=null;state.transitionStart=0;transitionRaf=0;}updateHud();}
function updateHud(){const h=$('#omegaGlobalSarFabricHud [data-k=state]');if(!h)return;if(state.error)h.textContent=`${state.state} · ${state.error}`;else if(state.fabric){const covered=state.fabric.cells.filter(c=>c.coverage>0).length,total=state.fabric.cells.length,cogs=state.fabric.cells.reduce((s,c)=>s+c.cog,0),scarText=state.scarLedger.length?` · ${state.scarLedger.length} source scars`:'';h.textContent=`${state.records.length} recent scenes · ${covered}/${total} camera cells source-covered · ${cogs} COG supports${scarText} · ${state.fabric.lod.atlasAddress} atlas LOD`;}else h.textContent=state.state.replaceAll('_',' ');}
...
function commitFabric(records,bbox,r,key,{final=false,scars=[]}={}){
  const prepared=dedupePrepared(records),previous=state.fabric;state.records=prepared;state.previousFabric=previous;state.fabric=buildFabric(prepared,bbox,{scale:r.view.scale,width:r.w,height:r.h},previous);state.transitionStart=performance.now();state.lastKey=key;state.updatedAt=new Date().toISOString();for(const s of scars)addScar(s);state.refining=!final;state.state=prepared.length?'READY':final?'READY_NO_SOURCE':'QUERYING_GLOBAL_SAR';state.error=null;if(transitionRaf)cancelAnimationFrame(transitionRaf);draw();window.dispatchEvent(new CustomEvent('omega-global-sar-fabric-update',{detail:snapshot()}));return prepared;
}
async function refresh(){
  ensureLayer();const r=renderer();if(!r||!state.enabled)return;const bbox=normalizeBBox(r.viewBounds?.()),key=bboxKey(bbox,r.view.scale);
  if(key===state.lastKey&&state.fabric){draw();return state.fabric;}
  if(state.inFlight&&state.inFlightKey===key)return state.inFlight;
  const cached=cache.get(key);if(cached&&Date.now()-cached.at<300000){commitFabric(cached.records,bbox,r,key,{final:true,scars:cached.scars||[]});return state.fabric;}
  if(state.activeController&&state.inFlightKey!==key)state.activeController.abort(new DOMException('Camera moved to a different fabric key','AbortError'));
  const my=++generation,controller=new AbortController();state.activeController=controller;state.inFlightKey=key;state.state='QUERYING_GLOBAL_SAR';state.error=null;state.refining=true;state.sectorResolved=0;state.sectorUnresolved=0;state.scarLedger=[];updateHud();
  const sectors=sectorsFor(bbox,r.view.scale),world=r.view.scale<=2.4||bbox[2]-bbox[0]>150,allScars=[];
  const task=(async()=>{
    try{
      const result=await querySectors(sectors,controller.signal,{days:world?365:240,limit:world?24:Math.max(60,Math.ceil(180/Math.max(1,sectors.length)))},progress=>{
        if(my!==generation||controller.signal.aborted)return;const newScars=progress.scars.filter(s=>!allScars.some(q=>q.at===s.at&&q.reason===s.reason&&String(q.bbox)===String(s.bbox)));allScars.push(...newScars);const prepared=commitFabric(progress.records,bbox,r,key,{final:false,scars:newScars});state.queries=progress.completed;state.state=prepared.length?'READY':'QUERYING_GLOBAL_SAR';state.refining=progress.completed<progress.total;updateHud();
      });
      if(my!==generation||controller.signal.aborted)return null;const prepared=commitFabric(result.records,bbox,r,key,{final:true,scars:[]});state.queries=sectors.length;cache.set(key,{at:Date.now(),records:prepared,scars:[...allScars]});if(cache.size>18)cache.delete(cache.keys().next().value);state.state=prepared.length?'READY':'READY_NO_SOURCE';state.refining=false;draw();return state.fabric;
    }catch(error){if(my!==generation||error?.name==='AbortError')return null;state.state='ERROR';state.error=error.message;state.refining=false;updateHud();return null;}
    finally{if(state.inFlight===task){state.inFlight=null;state.inFlightKey=null;if(state.activeController===controller)state.activeController=null;}}
  })();
  state.inFlight=task;return task;
}
function schedule(delay=700){clearTimeout(timer);timer=setTimeout(()=>refresh().catch(()=>{}),delay);}
function snapshot(){return {state:state.state,recordCount:state.records.length,queries:state.queries,updatedAt:state.updatedAt,refining:state.refining,sectorResolved:state.sectorResolved,sectorUnresolved:state.sectorUnresolved,scarCount:state.scarLedger.length,scarLedger:state.scarLedger.slice(-18),fabric:state.fabric?{bbox:state.fabric.bbox,cols:state.fabric.cols,rows:state.fabric.rows,lod:state.fabric.lod,recordCount:state.fabric.recordCount,coveredCells:state.fabric.cells.filter(c=>c.coverage>0).length}:null,boundary:state.boundary};}
