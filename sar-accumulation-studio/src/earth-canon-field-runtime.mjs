...
let timer=null,generation=0,lastSignature='';
...
function signature(field){return field?.state==='READY'?JSON.stringify([field.renderState,field.gate?.decision,field.gate?.admissibility,field.detailPriority,Object.values(field.channels||{}).map(v=>Number(v).toFixed(5)),field.support?.structure,field.support?.terrain,field.support?.temporal]):field?.state||'WAITING';}
function rebuild(){const my=++generation;try{const current=inputs(),field=synthesizeCanonicalVisualField(current.cube,current);if(my!==generation)return;const invariant=field.state==='READY'?visualFieldInvariant(field):{ok:true};if(!invariant.ok)throw new Error(`R260 visual field invariant failed: ${JSON.stringify(invariant)}`);const nextSignature=signature(field),changed=nextSignature!==lastSignature;lastSignature=nextSignature;state.field=field;state.state=field.state;state.updatedAt=new Date().toISOString();state.error=null;globalThis.OMEGA_EARTH_CANON_VISUAL_FIELD_STATE=field;if(changed)window.dispatchEvent(new CustomEvent('omega-earth-canon-visual-field',{detail:{state:field.state,renderState:field.renderState,channels:field.channels,gate:field.gate,detailPriority:field.detailPriority,updatedAt:state.updatedAt}}));}catch(error){if(my!==generation)return;state.state='ERROR';state.error=error.message;window.dispatchEvent(new CustomEvent('omega-earth-canon-visual-field-error',{detail:{error:error.message}}));}}
