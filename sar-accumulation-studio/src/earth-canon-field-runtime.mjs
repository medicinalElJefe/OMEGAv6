...
function inputs(){
  ...
  const terrainCoverage=Number(detail?.terrainCoverage??globalThis.OMEGA_EARTH_CANON_DETAIL?.stats?.terrainCoverage??(t?.state==='READY'?.85:0))||0;
  return {cube,terrainCoverage,temporalReady:temporal?.state==='MEASURED_TEMPORAL_CALCULUS_READY',structureReady:detail?.stats?.structureSamples>0};
}
function rebuild(){const my=++generation;try{const field=synthesizeCanonicalVisualField(inputs().cube,inputs());if(my!==generation)return;const invariant=field.state==='READY'?visualFieldInvariant(field):{ok:true};if(!invariant.ok)throw new Error(`R260 visual field invariant failed: ${JSON.stringify(invariant)}`);state.field=field;state.state=field.state;state.updatedAt=new Date().toISOString();state.error=null;globalThis.OMEGA_EARTH_CANON_VISUAL_FIELD_STATE=field;window.dispatchEvent(new CustomEvent('omega-earth-canon-visual-field',{detail:{state:field.state,renderState:field.renderState,channels:field.channels,gate:field.gate,detailPriority:field.detailPriority,updatedAt:state.updatedAt}}));}catch(error){if(my!==generation)return;state.state='ERROR';state.error=error.message;window.dispatchEvent(new CustomEvent('omega-earth-canon-visual-field-error',{detail:{error:error.message}}));}}
