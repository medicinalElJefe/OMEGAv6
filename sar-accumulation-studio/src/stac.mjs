...
export function buildStacBody({ start, end, bbox, intersectsWith, limit = 250 } = {}) {
  ...
  if (bbox) {
    if (!Array.isArray(bbox) || bbox.length !== 4 || !bbox.every(Number.isFinite) || bbox[0] >= bbox[2] || bbox[1] >= bbox[3]) throw new Error('STAC bbox must have non-zero area [west,south,east,north].');
    body.bbox = bbox;
...
function assetMap(item) {
  ...
  for (const [key, asset] of Object.entries(item.assets || {})) {
    ...
    out[key.toLowerCase()] = { key: key.toLowerCase(), href: s3ToHttps(asset.href), sourceHref: asset.href, type: asset.type || null, title: asset.title || key.toUpperCase(), description: asset.description || null, roles };
  }
  return out;
}
...
function thumbnailOf(item) {
  ...
  for(const key of ['thumbnail','preview','overview','browse','rendered_preview']){
    const asset=assets[key];if(asset?.href)return s3ToHttps(asset.href);
  }
  for(const asset of Object.values(assets)){
    if(!asset?.href)continue;
    const roles=Array.isArray(asset.roles)?asset.roles.map(x=>String(x).toLowerCase()):[];
    const type=String(asset.type||'').toLowerCase(),title=String(asset.title||'').toLowerCase();
    if(roles.some(r=>['thumbnail','overview','visual'].includes(r))||(type.startsWith('image/')&&!type.includes('tiff'))||/thumbnail|browse|preview|overview/.test(title))return s3ToHttps(asset.href);
  }
  const link = (item.links || []).find(l => ['thumbnail','preview','overview'].includes(String(l.rel||'').toLowerCase()));
  return s3ToHttps(link?.href || null);
}
...
export function normalizeStacItem(item, fetchedAt = new Date().toISOString()) {
  ...
  return {
    id: item.id, sceneName: item.id, platform: p.platform || 'sentinel-1', sensor: 'C-SAR', beamMode: p['sar:instrument_mode'] || null,
    polarization: Array.isArray(polarizations) ? polarizations.join('+') : polarizations,
    processingLevel: p['sar:product_type'] || 'GRD', maturity: p['processing:level'] || null,
    flightDirection: p['sat:orbit_state'] ? String(p['sat:orbit_state']).toUpperCase() : null,
    absoluteOrbit: p['sat:absolute_orbit'] ?? null, relativeOrbit: p['sat:relative_orbit'] ?? null, frameNumber: null,
    startTime, stopTime, geometry: item.geometry, bbox: item.bbox || null, browse: thumbnailOf(item), dataAssets: assets,
    projection: { epsg: p['proj:epsg'] ?? null, bbox: p['proj:bbox'] || item.bbox || null, shape: p['proj:shape'] || null, transform: p['proj:transform'] || null },
    measurement: { kind: 'SENTINEL1_GRD_AMPLITUDE', actualPixelsAvailable: Object.keys(assets).length > 0, radiometricCalibrationClaimed: false, interpretation: 'Actual GRD measurement values; display stretch is visual only unless a calibrated transform is explicitly applied.' },
    source: { authority: 'Earth Search STAC / AWS Sentinel-1 GRD', endpoint: EARTH_SEARCH, query: null, fetchedAt, importedFile: null },
    sourceProperties: typeof structuredClone === 'function' ? structuredClone(p) : JSON.parse(JSON.stringify(p)),
    evidence: { grade: 'B', reason: 'Public STAC item with measurement COG asset and acquisition geometry/time; precise ephemeris is not independently bound by this client.' },
...
export async function fetchSentinel1Cog(options = {}, signal) {
  ...
  const items = [], fetchedAt = new Date().toISOString();
  let request = { url: EARTH_SEARCH, method: 'POST', body }, pageCount = 0;
  while (request && items.length < wanted && pageCount < 25) {
    ...
    for (const item of fc.features || []) { const record = normalizeStacItem(item, fetchedAt); if (passesClientFilters(record, options)) items.push(record); if (items.length >= wanted) break; }
    const next = (fc.links || []).find(l => l.rel === 'next'); if (!next || items.length >= wanted) break;
    request = { url: next.href, method: String(next.method || 'GET').toUpperCase(), body: next.body || body }; pageCount++;
  }
  return { records: items.slice(0, wanted), source: { authority: 'Earth Search STAC / AWS Sentinel-1 GRD', endpoint: EARTH_SEARCH, query: body, fetchedAt }, requestBody: body, pageCount: pageCount + 1 };
}
