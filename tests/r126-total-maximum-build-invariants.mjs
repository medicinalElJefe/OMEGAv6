...
const autonomic=fs.readFileSync('src/swarm/swarmAutonomicR125.js','utf8');
...
assert.ok(autonomic.includes("AUTONOMIC_REVISION='R125'"),'R126 must preserve the R125 origin of the autonomic runtime');
assert.ok(wrangler.includes('"OmegaSwarmAutonomicCoordinator": {"type": "durable-object", "storage": "sqlite"}'),'R126 must preserve the already-provisioned R125 autonomic namespace as a live SQLite declarative export');
...
console.log('R126 TOTAL MAXIMUM BUILD invariants PASS · 179 source catalog · 62 canon lenses · 12 projections · 24 system families · 1→12→144→1728→20736 hierarchy · R125 SQLite namespace preserved through declarative lifecycle');