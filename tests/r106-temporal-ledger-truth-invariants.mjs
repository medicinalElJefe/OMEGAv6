...
// Navigation/submenu continuity must survive the R242 successor transform: one canonical route source, six contextual workspaces, no hidden truncation.
must(nav.includes('OMEGA_ALL_ROUTES_R82.map((route,index)=>')&&nav.includes('compileNavigationLemmaR242({routes:routeRecords,query,workspaceFilter,currentRoute:currentPanel})')&&nav.includes('navigationLemma.routes.map')&&nav.includes("className='r105-workspace-filter'")&&!nav.includes('rows.slice('),'R105/R242 submenu/registered-route authority regressed');
...
console.log('R106 TEMPORAL PROOF-LEDGER TRUTH PASS · current runtime/session separated from retained evidence · R50/R53/R54/R55 continuity preserved · R242 canonical route-source/lemma navigation preserved');
