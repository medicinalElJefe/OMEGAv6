...
must(r111Direct||r114PreservesR111||r115PreservesR111,'Wrangler must promote R111 directly or through the additive R114/R115 successor chain');
...
must(!mesh.includes('Math.random')&&!worker.includes('Math.random')&&!worker114.includes('Math.random')&&!worker115.includes('Math.random')&&!sentinel.includes('Math.random'),'fabric truth must be deterministic/probe-derived');
console.log(`R111/R115 FEDERATED CAPABILITY MESH PASS · neutral observer identity preserves four authority observations · scalable services · ${surfaces.length} dynamic route/output contracts · R112 shared one-touch Hybrid proof flow · R114 closure inherited · R115 machine adapters remain service-only · independent advisory sentinel`);
