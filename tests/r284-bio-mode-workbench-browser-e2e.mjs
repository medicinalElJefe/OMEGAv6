...
async function prove(viewport,label){
 ...
 try{
  ...
  const compare=workbench.getByRole('region',{name:'Read-only Heavy Bio mode comparison'});await compare.waitFor({state:'visible',timeout:10000});
  if(await compare.getAttribute('data-read-only')!=='true')throw new Error(`${label}: comparison is not machine-bound read-only`);
  if(await compare.getAttribute('data-measurement-authority-delta')!=='0')throw new Error(`${label}: comparison measurement-authority delta is not zero`);
  const compareText=(await compare.innerText()).toUpperCase();if(!compareText.includes('READ-ONLY COMPARISON')||!compareText.includes('MEASUREMENT AUTHORITY Δ'))throw new Error(`${label}: visible read-only comparison contract missing`);
  const compareTruth=((await compare.locator('p').textContent())||'').trim();if(compareTruth.length<20)throw new Error(`${label}: comparison truth boundary missing`);
...
console.log('R284 BROWSER PASS · 241-channel menus · 179/62 family filters · exact selected-key state transition · accessible visible truth boundary exactly matches machine-bound truth value · machine-visible zero measurement authority · mode-specific metadata visual · evidence ladder · machine-bound read-only comparison with zero measurement-authority delta · desktop/mobile containment');
