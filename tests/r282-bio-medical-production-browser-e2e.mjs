...
async function prove(viewport,label){
 ...
 try{
  ...
  const medical=page.locator('.bio282'),surface=page.locator('.bio281');
  ...
  await page.waitForFunction(()=>document.querySelector('.bio282-release')?.textContent?.includes('AUTHORIZED CLINICAL RELEASE READY'),{timeout:10000});
  await page.waitForFunction(()=>document.querySelector('.bio282')?.textContent?.includes('AUDIT CHAIN')&&document.querySelector('.bio282')?.textContent?.includes('PASS'),{timeout:10000});
  const after=await medical.innerText();
  for(const token of ['AUTHORIZED CLINICAL RELEASE READY','MEASUREMENT GATE','RISK GATE','AUTHORIZATION','PASS','R282-BROWSER-IU','MEASUREMENT ONLY','TEST-AUTHORIZATION-FIXTURE'])if(!after.includes(token))throw new Error(`${label} R282 authorized fixture missing ${token}`);
