...
for(const token of ['GrowthSequenceRail','Seed Intake','Bind Authority','Admit','Construct','Prune','Route','Render','Persist','Replay','Package','Escalate','144-SEQUENCE GROWTH']){const source=token==='GrowthSequenceRail'?app:rail;if(!source.includes(token))throw new Error(`144-sequence visual grammar missing: ${token}`)}
for(const token of ['growth-sequence-rail','.gsr-track','.gsr-progress','.gsr-step.active','@media(max-width:680px)'])if(!railCss.includes(token))throw new Error(`growth rail style missing: ${token}`);
