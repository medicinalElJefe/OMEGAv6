...
assert.ok(workstation.includes("window.setTimeout(()=>{const root=document.documentElement;if(root.dataset.omegaRouteEpoch===routeLifecycle.epoch&&root.dataset.omegaRouteState==='REQUESTED'&&root.dataset.omegaRouteTarget===next&&root.dataset.omegaRouteCurrent!==next)setPanel(next)},240)"),'R356 route lifecycle must escalate a starved transition on the same epoch/target instead of remaining REQUESTED indefinitely');
