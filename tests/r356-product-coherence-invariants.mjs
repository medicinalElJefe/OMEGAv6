...
for(const token of ["data-r356-interaction-ready={interactionReady?'true':'false'}","data-r356-capability-ready","setInteractionReady(true)","className='r356-proof-toggle'","aria-expanded={proofOpen}","aria-controls={`r356-proof-${slug(panel)}`}","className='r356-proof-region'","hidden={!proofOpen}","data-r356-lightweight-proof='true'","className='r356-load-deep-proof'","setProofDeep(true)","proofDeep&&<div className='r356-proof-lineage-deep'","<SurfaceProvenanceR94 surface={panel}/>"])assert.ok((surface+capabilityField).includes(token),`R356 interaction/proof readiness contract missing: ${token}`);
