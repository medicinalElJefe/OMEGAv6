...
for(const token of ["window.setTimeout(()=>setProofOpen(open),0)","data-r356-lightweight-proof='true'","proofDeep&&<div className='r356-proof-lineage-deep'","<SurfaceProvenanceR94 surface={panel}/>","window.setTimeout(()=>setProofDeep(true),120)"])assert.ok(surface.includes(token),`R356 proof-lineage disclosure must stay lightweight before deep lineage/calculus mount: ${token}`);
