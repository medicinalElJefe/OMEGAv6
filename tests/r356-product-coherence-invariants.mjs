...
for(const token of ["data-r356-interaction-ready={interactionReady?'true':'false'}","requestIdleCallback","setInteractionReady(true)","ref={proofDetailsRef}","onToggle={e=>onProofToggle(e.currentTarget.open)}","window.setTimeout(()=>{if(!proofDetailsRef.current?.open)return;setProofOpen(true)},240)","window.setTimeout(()=>{if(!proofDetailsRef.current?.open)return;setProofOpen(true);setProofDeep(true)},900)","data-r356-lightweight-proof='true'","proofDeep&&<div className='r356-proof-lineage-deep'","<SurfaceProvenanceR94 surface={panel}/>"])assert.ok(surface.includes(token),`R356 proof-lineage disclosure must defer React/deep lineage work until after native toggle completion: ${token}`);
