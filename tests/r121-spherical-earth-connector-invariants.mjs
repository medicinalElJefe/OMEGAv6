...
const delegatedSphere=membrane.includes("import OmegaPhysicsManifoldR132 from './OmegaPhysicsManifoldR132'")&&membrane.includes('<OmegaPhysicsManifoldR132 address={address} onAddress={onAddress} projection={projection} view={view}/>')&&physics.includes("import OmegaEarthRelativitySphereR121 from './OmegaEarthRelativitySphereR121'")&&physics.includes('<OmegaEarthRelativitySphereR121 address={address} onAddress={onAddress} projection={projection} view={view}/>');
must(directSphere||delegatedSphere,'whole-sphere instrument must remain reachable through canonical Home visual authority, directly or through the R132 relational manifold');
...
must(directSphere?sphereCss.includes('Whole-sphere integrative view is primary;'):homeAuthority.includes('Relational physics manifold is primary; Earth remains a source-backed reference/observed lens;'),'Home projection copy must communicate current manifold/Earth/membrane authority correctly');
