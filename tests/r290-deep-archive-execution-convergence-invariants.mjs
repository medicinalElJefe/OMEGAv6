...
must(model.includes('routeMismatches')&&model.includes('missingGenomeRefs')&&model.includes('unknownMenus')&&model.includes('duplicateIds'),'R290 audit must fail closed on route/menu/genome/identity drift');
...
must(ui.includes('MASTER MENU')&&ui.includes('CURRENT SUCCESSOR')&&ui.includes('Proof required')&&ui.includes('Drive provenance'),'UI must expose menu/successor/provenance/proof context');
...
console.log('R290 DEEP ARCHIVE EXECUTION CONVERGENCE PASS · 7 deduplicated context bindings · Drive provenance + current successor + 12-menu placement + proof requirements · no new authority');
