...
const worker=read('src/workerR32.js'),worker33=read('src/workerR33.js'),worker34=fs.existsSync('src/workerR34.js')?read('src/workerR34.js'):'',worker101=fs.existsSync('src/workerR101.js')?read('src/workerR101.js'):'',config=read('wrangler.jsonc'),adapter=read('src/platformAdapter.ts'),hybrid=read('src/HybridMissionControlR8.tsx'),agent=read('public/omega-hybrid-agent.py'),shell=read('src/SingleFrameRuntimeShellR27.tsx');
...
must((direct||successor33||successor34||successor101)&&config.includes('OMEGA_RUNTIME')&&config.includes('OmegaRuntime')&&config.includes('new_sqlite_classes'),'Wrangler durable runtime binding or R33/R34/R101 successor missing');
...
must(shell.includes("'WORK'|'EXPLORE'|'INTELLIGENCE'|'EVIDENCE'|'SYSTEM'"),'R31 human navigation hierarchy must survive R32/R33/R34/R101');
console.log('OMEGA R32 ENACTED RUNTIME PASS · durable event/action bus + governed AI + authenticated Hybrid proof transport preserved under R101');
