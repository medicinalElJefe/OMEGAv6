...
// Integrated R147/R195/R196 dynamic process. R197 may wrap R196, but cannot raise or replace the R196 hard scheduler.
...
const source=fs.readFileSync('src/execution/unifiedExecutorFabricR147.js','utf8'),r196=fs.readFileSync('src/execution/boundedPartitionSchedulerR196.js','utf8'),r197=fs.readFileSync('src/execution/adaptivePartitionBackpressureR197.js','utf8');for(const token of ['executeAdaptivePartitionsR197','R196_BOUNDED_PARTITION_BATCH_FAILURE','schedulerRevision:\'R196\'','boundedPartitionParallelism:manifestR196()','R196_STAY_TURN_IS_SCHEDULING_BEHAVIOR_NOT_TRUTH_PROMOTION'])assert.ok(source.includes(token),`R147 missing composed R196 runtime integration ${token}`);assert.ok(r197.includes('executeBoundedPartitionsR196'),'R197 must delegate actual bounded dirty-work execution to R196');for(const token of ['R196_HARD_CONCURRENCY_MAX=12','replanOnEveryRun:true','turn=dirty===0?\'STAY\':\'TURN\'','FIRST_FAILURE_STOPS_NEW_ASSIGNMENTS_ALREADY_RUNNING_BOUNDED_CALLS_SETTLE_NO_R195_PUBLICATION','selfEscalation:false'])assert.ok(r196.includes(token),`R196 scheduler invariant missing ${token}`);assert.ok(!r196.includes('canonicalMutation:true'),'R196 must never mutate CanonState');
console.log('R196 BOUNDED PARTITION PARALLELISM PASS · R193 compute-axis ceilings dynamically replan each R195 dirty set; STAY carries without provider work, TURN executes only dirty partitions through the R196 capped worker pool even when R197 composes adaptive backpressure above it, ordered reassembly is preserved, failure stops new scheduling and publishes no partial carry, and R147/R146/R182/R159/R125 authority remains distinct');
