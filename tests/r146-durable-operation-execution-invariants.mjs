...
const worker=fs.readFileSync('src/workerR116.js','utf8'),wrangler=fs.readFileSync('wrangler.jsonc','utf8'),field=fs.readFileSync('src/OmegaCapabilityFieldR138.tsx','utf8'),client=fs.readFileSync('src/durableExecutionClientR146.ts','utf8');
for(const token of ["from './execution/durableOperationExecutionR146.js'",'/api/execution/r146/manifest','/api/execution/runs','executionRuntimeProxyR146','PAIR_AUTH_FAILED','export class OmegaRuntime extends OmegaRuntimeR115','return super.fetch(request)'])must(worker.includes(token),'proven R116 worker additive mount missing '+token);
must(wrangler.includes('"main": "src/workerR116.js"'),'Wrangler must retain proven R116 production entrypoint');
must(!fs.existsSync('src/workerR146.js'),'R146 must not create a shadow production Worker entrypoint');
...
console.log('R146 DURABLE OPERATION EXECUTION PASS · one proven R116 spine · R143 contracts persisted · R142 lifecycle preserved · R141 Hybrid proof required · R144/R145 evidence separated · hash replay verified · R125 admission unchanged');
