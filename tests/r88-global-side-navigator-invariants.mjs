...
for(const token of ['/commits/${sha}/pulls','/actions/runs?head_sha=${candidate}&event=pull_request','duplicate revision identity','OMEGA Cloud Bridge CI','R170 Current Convergence','R202 Operational Source Authority','parents.length!==2'])must(releaseGuard.includes(token),`R210 release-correlation guard missing ${token}`);
for(const token of ['actions: read','pull-requests: read','contents: read','group: omega-r210-release-controller','cancel-in-progress: false','collision-free exact-base candidate identity','exact merged PR head and required green candidate workflows'])must(releaseWorkflow.includes(token),`R210 release-controller workflow missing ${token}`);
must(!releaseWorkflow.includes('contents: write')&&!releaseWorkflow.includes('deploy')&&!releaseWorkflow.includes('wrangler'),'R210 controller is proof-only and must not gain deployment/source mutation authority');
...
console.log(`R88/R89/R104/R210 CONVERGENCE PASS · persistent readable side toolbar · ${routes.length} destinations · focus/outside-close/live-count accessibility · readable mobile overlay · exact-head release correlation · stale DO config removed · no new navigation/deploy/Canon authority`);
