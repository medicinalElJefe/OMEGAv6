...
const must=(ok,msg)=>{if(!ok)throw new Error('R88/R89/R229/R299/R302 '+msg)};
...
for(const token of ['scrollbar-gutter:stable',"[tabindex]:not([tabindex='-1'])",'touch-action:manipulation','@media(any-pointer:coarse)','min-width:44px!important','min-height:44px!important','animation:none!important','scroll-behavior:auto!important'])must(css210.includes(token),`R302 navigator polish missing ${token}`);
...
must(css210.includes(':where(.r88-head-actions button,.r89-nav-mode button,.r94-rail-action)'),'R302 coarse-pointer touch-target rule must cover inherited undersized navigator controls');
must(css.includes('.r88-head-actions button{width:36px;height:36px')&&css.includes('min-height:32px'),'R302 repair must remain anchored to real inherited sub-44px controls');
must(!css210.includes('/api/')&&!css210.includes('fetch(')&&!polish.includes('/api/')&&!polish.includes('fetch('),'R302 visual polish must remain presentation-only');
