...
const agent=read('public/omega-hybrid-agent.py'),worker=read('src/workerR32.js'),html=read('public/omega-pc-proof-r205.html'),client=read('public/omega-pc-proof-r205.js'),wrangler=read('wrangler.jsonc'),r141=read('public/omega-hybrid-agent-r141.py');
...
for(const retired of ['OmegaMissionLedgerR201','OmegaHybridMissionLedgerR203']){must(wrangler.includes(`"${retired}": {"type": "durable-object", "state": "deleted"}`),`${retired} retirement tombstone missing`);must(!wrangler.includes(`"class_name": "${retired}"`),`${retired} regained a live binding`)}
...
const syntax=spawnSync('python3',['-c',"import ast,pathlib;ast.parse(pathlib.Path('public/omega-hybrid-agent.py').read_text())"],{encoding:'utf8'});assert.equal(syntax.status,0,`R205 agent Python syntax failed: ${syntax.stderr}`);
const simulation=spawnSync('python3',['-c',String.raw`import importlib.util,json,tempfile
from pathlib import Path
spec=importlib.util.spec_from_file_location('omega_r205','public/omega-hybrid-agent.py');m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m)
...
console.log('R205 PC PROOF CLOSURE PASS · AT09 authenticated desktop/runtime smoke + AT10 complete bounded forensic ledger · R34.1/R132 preserved · old agents fail closed · R125 unchanged');