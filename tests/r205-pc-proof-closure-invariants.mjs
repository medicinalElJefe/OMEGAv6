...
for(const retired of ['OmegaMissionLedgerR201','OmegaHybridMissionLedgerR203']){must(wrangler.includes(`"${retired}": {"type": "durable-object", "state": "deleted"}`),`${retired} deleted-only retirement tombstone required under R228.1`);must(!wrangler.includes(`"class_name": "${retired}"`),`${retired} regained a live binding`);must(!wrangler.includes(`"${retired}": {"type": "durable-object", "storage": "sqlite"}`),`${retired} regained live sqlite authority`)}
...
console.log('R205/R228.1 PC PROOF CLOSURE PASS · frozen R34.1/R132/R205 base retains AT09/AT10 executor proof · R207 wrapper preserved · R201/R203 deleted-only retirement reconciled · R125 unchanged');