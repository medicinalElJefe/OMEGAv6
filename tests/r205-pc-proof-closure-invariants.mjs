...
for(const retired of ['OmegaMissionLedgerR201','OmegaHybridMissionLedgerR203']){must(wrangler.includes(`"${retired}": {"type": "durable-object", "state": "deleted"}`),`${retired} provider-required deleted tombstone missing under R314.1`);must(!wrangler.includes(`"class_name": "${retired}"`),`${retired} regained a live binding`);must(!wrangler.includes(`"${retired}": {"type": "durable-object", "storage": "sqlite"}`),`${retired} regained live storage`)}
...
console.log('R205/R229/R299/R314.1 PC PROOF CLOSURE PASS · frozen R34.1/R132/R205 base retains AT09/AT10 executor proof · R207 wrapper preserved · R201/R203 retired from executable/live authority with provider-required deleted tombstone scar · R125 unchanged');