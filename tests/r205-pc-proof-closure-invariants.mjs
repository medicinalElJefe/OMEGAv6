...
for(const retired of ['OmegaMissionLedgerR201','OmegaHybridMissionLedgerR203']){must(!wrangler.includes(`"${retired}": {"type": "durable-object", "state": "deleted"}`),`${retired} stale retirement tombstone must be absent under R210`);must(!wrangler.includes(`"class_name": "${retired}"`),`${retired} regained a live binding`)}
...
console.log('R205/R210 PC PROOF CLOSURE PASS · frozen R34.1/R132/R205 base retains AT09/AT10 executor proof · R207 wrapper preserved · stale R201/R203 config removed · R125 unchanged');