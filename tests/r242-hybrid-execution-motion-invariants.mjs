...
const must=(ok,msg)=>assert.ok(ok,`R242 ${msg}`);
...
const ui=read('src/HybridExecutionMotionR242.tsx');
...
must(!wrapper.includes('shell=True'),'R242 wrapper must not add arbitrary shell authority');
must(!/pip\s+install|python\s+-m\s+pip|conda\s+install/i.test(wrapper),'R242 must not auto-install dependencies');
const baseSha=crypto.createHash('sha256').update(base).digest('hex');
assert.equal(baseSha,'49a3be453b1e67e5eb7e8e29411e82f07d30d2fd45fa52fa0bf28ef57774a046','R242 must leave immutable R205 executor byte-identical');

for(const token of ['data-r242-motion','MOTION_PROVED','STALL_DETECTED','LEGACY_RUNNING_NO_LEASE','CURRENT STEP','MOTION / LEASE','RETURNED STEP COUNT','R141 returned proof'])must(ui.includes(token),`operator execution-motion surface missing ${token}`);
must(link.includes("import HybridExecutionMotionR242 from './HybridExecutionMotionR242'"),'Hybrid Link must import R242 motion surface');
must(link.includes('<HybridExecutionMotionR242/>'),'Hybrid Link must mount R242 motion surface');
must(link.indexOf('<HybridWovenContinuityR238/>')<link.indexOf('<HybridExecutionMotionR242/>')&&link.indexOf('<HybridExecutionMotionR242/>')<link.indexOf('<HybridHostIntelligenceR238/>'),'R242 motion must live inside the shared snapshot frame before returned host intelligence');
...
await runtime.recoverStalledJobsR242('pc-r242');jobs=await storage.get('jobs');assert.equal(jobs.find(j=>j.id==='job-mutation').status,'FAILED');must(!jobs.some(j=>j.recoveryOf==='job-mutation'),'R242 must never blind-replay a stale mutation');

console.log('OMEGA R242 HYBRID EXECUTION MOTION PASS · authenticated 3s host step pulses renew a bounded RUNNING lease and heartbeat · current step/elapsed/progress are operator-visible · expired legacy RUNNING claims fail closed · only bounded non-mutating discovery can auto-recover · root HASH_TREE is stripped from initial/recovery discovery · mutation is never blindly replayed · immutable R205 + R141/R240/R146/R147/R125 authority preserved');
