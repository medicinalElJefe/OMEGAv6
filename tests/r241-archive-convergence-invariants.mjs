...
for(const token of ["data-r237-correlation","['LOCKED','HELD']","correlation==='LOCKED'",'HOST / JOB / MISSION / EPOCH LOCKED','EXECUTION CONTEXT HELD','OMEGA_EXPECTED_WORKER_VERSION_ID','Cloudflare-Workers-Version-Overrides','SPA_FALLBACK_NOT_RELEASE_EVIDENCE','closingVersion!==servedVersion'])must(commandVerifier.includes(token),`R241 R240 repair missing semantic/runtime-identity token ${token}`);
...
for(const token of ["data-r237-correlation","HOST / JOB / MISSION / EPOCH LOCKED","EXECUTION CONTEXT HELD","activeJob.targetDeviceId!==device?.id","targetForMission(currentMission,missionJob)!==device.id","snapshotCurrent","requireCurrentSnapshot","exact Worker-version live pin + governed staged SHA receipt"])must(commandInvariant.includes(token),`R241 must pin the R237 semantic LOCKED/HELD and exact-runtime fail-closed truth behavior in focused invariants: ${token}`);

for(const token of ["Prove R240.1 semantic verifier against exact served runtime","/api/core-health?r241=","OMEGA_CANONICAL_CORE_HEALTH_R163","R163-FIRST-HAND","body?.runtimeVersion?.id","OMEGA_EXPECTED_WORKER_VERSION_ID=\"$SERVED_VERSION\"","OMEGA_WORKER_VERSION_ID=\"$SERVED_VERSION\"","for attempt in 1 2 3","internally consistent exact Worker-version semantic proof"])must(workflow.includes(token),`R241.1 workflow exact-runtime race closure missing ${token}`);
...
must(!workflow.includes("const response=await fetch(base+'/omega-build-receipt.json'"),'R241.1 workflow must not treat the SPA-fallback build-receipt path as authoritative production identity');
...
must(governor.selfPromotion?.exactTwoParentMainUpdate===true&&governor.selfPromotion?.canonicalDeploymentPushTrigger===false&&governor.selfPromotion?.canonicalDeploymentDispatchTrigger===true&&governor.selfPromotion?.canonicalDeploymentDispatchAllowed===true,'R340 promotion must preserve exact two-parent source lineage while explicitly dispatching the sole canonical writer after a token-authored main update');
