...
for(const token of [
 ...
 '/tmp/r305-browser-suite.log',
 'Upload R305 browser failure diagnostic',
 'if: failure()',
 'uses: actions/upload-artifact@v4',
 'name: r305-browser-failure-${{ github.run_id }}',
 'if-no-files-found: ignore',
 'retention-days: 3'
])must(workflow.includes(token),`R305 bounded browser diagnostic pipeline missing ${token}`);
const browserSuites=[...workflow.matchAll(/run_browser tests\/(.+?-browser-e2e\.mjs)/g)].map(m=>m[1]);
must(browserSuites.length===9&&new Set(browserSuites).size===9,'R305 must preserve all nine inherited real-browser suites while bounding each one');
must(workflow.includes("trap 'kill \"$VITE_PID\" 2>/dev/null || true' EXIT"),'R305 browser harness must clean up the preview server');
must(workflow.includes('exit "$status"'),'R305 bounded browser failure must propagate exact suite/timeout status');
must(!workflow.includes('continue-on-error: true'),'R305 must fail closed rather than suppress browser proof failures');
