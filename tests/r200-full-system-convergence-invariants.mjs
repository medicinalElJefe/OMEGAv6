...
for(const retired of ['OmegaMissionLedgerR201','OmegaHybridMissionLedgerR203']){assert.ok(wrangler.includes(`"${retired}": {"type": "durable-object", "state": "deleted"}`),`R228.1 provisioned historical namespace must remain deleted-only ${retired}`);assert.ok(!wrangler.includes(`"class_name": "${retired}"`),`R200 retired namespace regained a live binding ${retired}`);assert.ok(!wrangler.includes(`"${retired}": {"type": "durable-object", "storage": "sqlite"}`),`R200 retired namespace regained live sqlite authority ${retired}`)}
...
console.log('R200/R228.1 FULL SYSTEM CONVERGENCE PASS · deleted-only Cloudflare namespace retirement preserved · R199.1 performance preserved · exact-build desktop/mobile browser proof · operator-invoked 20,736-state analysis · authority chain preserved');