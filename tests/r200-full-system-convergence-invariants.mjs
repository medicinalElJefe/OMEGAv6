...
for(const retired of ['OmegaMissionLedgerR201','OmegaHybridMissionLedgerR203']){assert.ok(wrangler.includes(`"${retired}": {"type": "durable-object", "state": "deleted"}`),`R314.1 provider-required deleted tombstone missing ${retired}`);assert.ok(!wrangler.includes(`"class_name": "${retired}"`),`R200 retired namespace regained a live binding ${retired}`);assert.ok(!wrangler.includes(`"${retired}": {"type": "durable-object", "storage": "sqlite"}`),`R200 retired namespace regained live storage ${retired}`)}
...
console.log('R200/R229/R299/R314.1 FULL SYSTEM CONVERGENCE PASS · retired R201/R203 absent from executable/live authority while provider-required deleted tombstones preserve provider scar · R199.1 performance preserved · exact-build desktop/mobile browser proof · operator-invoked 20,736-state analysis · authority chain preserved');
