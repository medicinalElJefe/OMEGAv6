...
for(const retired of ['OmegaMissionLedgerR201','OmegaHybridMissionLedgerR203']){assert.ok(!wrangler.includes(`"${retired}": {"type": "durable-object", "state": "deleted"}`),`R210 stale historical tombstone must be absent ${retired}`);assert.ok(!wrangler.includes(`"class_name": "${retired}"`),`R200 retired namespace regained a live binding ${retired}`)}
...
console.log('R200/R210 FULL SYSTEM CONVERGENCE PASS · deployment cleanup preserved · R199.1 performance preserved · exact-build desktop/mobile browser proof · operator-invoked 20,736-state analysis · authority chain preserved');
