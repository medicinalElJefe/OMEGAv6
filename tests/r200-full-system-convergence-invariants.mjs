...
for(const retired of ['OmegaMissionLedgerR201','OmegaHybridMissionLedgerR203']){assert.ok(wrangler.includes(`"${retired}": {"type": "durable-object", "state": "deleted"}`),`R229 provider-required tombstone must remain ${retired}`);assert.ok(!wrangler.includes(`"class_name": "${retired}"`),`R200 retired namespace regained a live binding ${retired}`);assert.ok(!wrangler.includes(`"${retired}": {"type": "durable-object", "storage": "sqlite"}`),`R200 retired namespace regained live storage ${retired}`)}
...
console.log('R200/R229 FULL SYSTEM CONVERGENCE PASS · provider-required retired tombstones preserved without live authority · R199.1 performance preserved · exact-build desktop/mobile browser proof · operator-invoked 20,736-state analysis · authority chain preserved');
