...
const routes=['Command Center','Visual Instrument','Reality Lab','Convergence','Evidence & Proof','System'];
 ...
 for(const[profile,opts]of profiles){
  ...
  for(const name of routes){
   ...
   if(profile==='mobile'){
    ...
    if(!geometry.primary||!geometry.context||!geometry.workspace)throw new Error(profile+'/'+name+': desktop product geometry missing '+JSON.stringify(geometry));
    if(geometry.primary.left>=geometry.context.left||geometry.primary.right>geometry.context.left+2)throw new Error(profile+'/'+name+': primary instrument is not confined to flexible left track '+JSON.stringify(geometry));
    if(geometry.primary.width<=geometry.context.width)throw new Error(profile+'/'+name+': bounded context rail has displaced the primary instrument '+JSON.stringify(geometry));
   }
