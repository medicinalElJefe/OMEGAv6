...
const must=(ok,msg)=>{if(!ok)throw new Error(`R31 shell invariant failed: ${msg}`)};

for(const label of ['Home','Ask','Explore','Earth','Menu']) must(shell.includes(`<span>${label}</span>`),`mobile primary navigation must include ${label}`);
...
for(const shortcut of ['Ask OMEGA','Continue work','Explore Earth']) must(shell.includes(shortcut),`direct shortcut missing ${shortcut}`);
...
console.log('R31 shell invariants PASS');
