...
const must=(ok,msg)=>{if(!ok)throw new Error(`R31/R32/R33/R156 shell invariant failed: ${msg}`)};
...
const r32Labels=shell.includes('<span>Home</span>')&&shell.includes('<span>Connect</span>')&&shell.includes('<span>More</span>');
const r33Labels=shell.includes('<span>Now</span>')&&shell.includes('<span>PC</span>')&&shell.includes('<span>Tools</span>');
const r156LiveLabels=shell.includes('<span>Now</span>')&&shell.includes("<span>{live.pcOnline?'PC ✓':'PC'}</span>")&&shell.includes('<span>Tools</span>');
must(r32Labels||r33Labels||r156LiveLabels,'mobile labels must retain the Home/Now, Hybrid PC/Connect and More/Tools destination semantics, including the R156 proved-live PC refinement');
...
console.log('R31/R32/R33/R156 shell invariants PASS · destination semantics preserved while labels evolve toward live enacted state');
