...
const must=(ok,msg)=>{if(!ok)throw new Error(`R289/R305 recovered-menu invariant failed: ${msg}`)};
...
must(routeNames.length>0,'route registry must remain non-empty');
must(new Set(routeNames).size===routeNames.length,'route names must remain unique');
must(routeMap.length===routeNames.length,'every current route must have exactly one recovered master-menu assignment');
must(new Set(routeMap.map(x=>x.route)).size===routeMap.length,'master-menu mapping must not duplicate routes');
for(const name of routeNames)must(routeMap.some(x=>x.route===name),`orphan route ${name}`);
for(const mapped of routeMap)must(routeNames.includes(mapped.route),`master-menu mapping references non-route ${mapped.route}`);
...
for(const token of ['OMEGA_RECOVERED_MASTER_MENU_NAVIGATION_R289','ONE_CURRENT_ROUTE_AUTHORITY_PRESENTED_THROUGH_12_RECOVERED_MASTER_MENUS','OMEGA_MASTER_MENU_NAVIGATION_R289','omegaMasterMenuForRouteR289'])must(nav.includes(token),`navigation contract missing ${token}`);
must(!nav.includes('ONE_44_ROUTE_AUTHORITY_PRESENTED_THROUGH_12_RECOVERED_MASTER_MENUS'),'recovered-menu contract must not freeze a historical route count');
...
must(launcher.includes('{LAUNCHER_SURFACES.length} routes · {LAUNCHER_MASTER_MENUS_R289.length} recovered menus · ALL MODES'),'launcher must expose the dynamic complete navigation contract');
must(!launcher.includes('44 routes · 12 recovered menus · ALL MODES'),'launcher must not freeze historical route-count copy');
...
console.log(`R289/R305 RECOVERED MASTER-MENU NAVIGATION PASS · ${routeNames.length} current routes · ${menus.length} recovered menus · ${new Set(bindRoutes).size} recovery-bound surfaces · live R88/R239 organization is downstream of R242 lemma · workspace/search filters pointer-safe · zero orphan routes/bindings · launcher route/favorite controls use independent native semantics · no historical route-count ceiling`);
