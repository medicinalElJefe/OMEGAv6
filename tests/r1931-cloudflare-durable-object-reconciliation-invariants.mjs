...
for(const retired of ['OmegaMissionLedgerR201','OmegaHybridMissionLedgerR203']){
 ...
 assert.ok(config.includes(`"${retired}": {"type": "durable-object", "state": "deleted"}`),`provider-reported provisioned retired namespace must retain deleted reconciliation tombstone ${retired}`);
...
assert.ok(config.includes('OMEGA R235 preserves the proven R116 Worker spine'),'R235 deployment reconciliation provenance missing');
assert.ok(config.includes('Cloudflare production reconciliation on R234 proved both provider namespaces still exist'),'R235 provider-state reconciliation evidence law missing');
assert.ok(config.includes('reconciliation metadata only'),'R235 tombstone truth boundary missing');
assert.ok(config.includes('R125 remains canonical admission authority'),'R125 admission authority guardrail missing');
console.log('R193.1/R235 CLOUDFLARE DURABLE OBJECT RECONCILIATION PASS · R201/R203 executable classes, live bindings, and sqlite exports absent · provider-required deleted tombstones retained · all seven live R116 durable authorities intact');
