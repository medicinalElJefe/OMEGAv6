...
for(const token of ["import OmegaDataLexiconR46 from './OmegaDataLexiconR46'","data-lexicon-adapter"])must(plugins.includes(token),`Plugins data/lexicon integration missing ${token}`);
must(/<OmegaDataLexiconR46(?:\s+[^>]*)?\s*\/>/.test(plugins),'Plugins data/lexicon integration missing OmegaDataLexiconR46 mount');
