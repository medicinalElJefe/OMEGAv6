...
import {
 ...
 opticalCandidateIdentityR316,routeOpticalSolverR316,evaluateOpticalPromotionR316,buildExperimentEpisodeR316,
 buildSpectralObservationR316,buildAcquisitionReceiptR316,scoreHardwareTargetR316,compileResearchAdvancementR316
...
const held=evaluateOpticalPromotionR316({fast:{efficiency:.90,phaseRad:1},rcwa:{efficiency:.70,phaseRad:1.8,converged:true,energyResidual:.01},fabricationRobustness:.92});
...
assert.equal(Object.values(compiled.capabilities).filter(Boolean).length,10);
assert.equal(compiled.authority.dispatch,'R147');

console.log('R316 research advancement invariants PASS');
