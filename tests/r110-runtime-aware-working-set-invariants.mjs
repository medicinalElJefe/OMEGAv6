...
for(const retired of ['OmegaMissionLedgerR201','OmegaHybridMissionLedgerR203']){
 must(!wrangler.includes(`"${retired}": {"type": "durable-object", "state": "deleted"}`),`R299 stale Cloudflare retirement tombstone must be absent ${retired}`);
...
console.log(`R110/R229/R270/R288/R289/R290/R292/R299/R301 RUNTIME-AWARE WORKING SET PASS · ${surfaces.length} registered destinations preserved dynamically · archive-native + unified R289 convergence/navigation gates chained · R290 Singmaster exact proof-atlas gate chained · R292 reusable proof-carry fabric chained into Woven and proof-aware ALL MODES · R301 HTML/SVG semantic capability recovery chained · hidden/Save-Data/2G suppression · low-power/3G budget reduction · direct route demand preserved · Home deferred · exactly one branded suspense boundary · workstation-only shared R238 provider · 500 KiB initial-entry budget enforced · R201/R203 fully retired without live bindings · one route/state/proof authority retained`);
