...
for(const retired of ['OmegaMissionLedgerR201','OmegaHybridMissionLedgerR203']){
 must(wrangler.includes(`"${retired}": {"type": "durable-object", "state": "deleted"}`),`R228.1 must retain deleted-only Cloudflare retirement tombstone ${retired}`);
 must(!wrangler.includes(`"${retired}": {"type": "durable-object", "storage": "sqlite"}`),`R228.1 must not restore retired namespace ${retired} as live storage`);
 must(!wrangler.includes(`"class_name": "${retired}"`),`R228.1 must not restore retired namespace ${retired} as live binding`);
}

console.log(`R110/R228.1 RUNTIME-AWARE WORKING SET PASS · ${surfaces.length} registered destinations preserved dynamically · hidden/Save-Data/2G suppression · low-power/3G budget reduction · direct route demand preserved · Home deferred · 500 KiB initial-entry budget enforced · R201/R203 deleted-only reconciliation · one route/state/proof authority retained`);