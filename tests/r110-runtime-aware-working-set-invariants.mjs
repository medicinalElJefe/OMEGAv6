...
for(const retired of ['OmegaMissionLedgerR201','OmegaHybridMissionLedgerR203']){
 must(wrangler.includes(`"${retired}": {"type": "durable-object", "state": "deleted"}`),`R235 provider reconciliation tombstone missing ${retired}`);
 must(!wrangler.includes(`"${retired}": {"type": "durable-object", "storage": "sqlite"}`),`R235 must not restore retired namespace ${retired} as live storage`);
 must(!wrangler.includes(`"class_name": "${retired}"`),`R235 must not restore retired namespace ${retired} as a live binding`);
}

console.log(`R110/R235 RUNTIME-AWARE WORKING SET PASS · ${surfaces.length} registered destinations preserved dynamically · hidden/Save-Data/2G suppression · low-power/3G budget reduction · direct route demand preserved · Home deferred · 500 KiB initial-entry budget enforced · R201/R203 retired with provider-required reconciliation tombstones and no live bindings/storage · one route/state/proof authority retained`);
