...
for(const token of ['setTimeout(resolve,0)','field.count','cache||createAnalysisCacheR189(field)','quantiles','hotspots','basins','clusters'])assert.ok(globalAtlas.includes(token),`R188 bounded global analysis invariant missing: ${token}`);
...
for(const token of ['CANONICAL_OMEGA_API_ORIGIN','/api/'])assert.ok(apiTransport.includes(token),`Promoted canonical API routing missing: ${token}`);
