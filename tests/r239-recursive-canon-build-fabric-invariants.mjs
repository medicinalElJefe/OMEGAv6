...
for(const token of [
 ...
 "r239ReadyFrontier",
 "r239MergeAdmissible",
 "MAXIMUM_INTERNAL_EVOLUTION + STRICT_EXTERNAL_ADMISSION",
...
assert.ok(!/physical dimensions|physical dimensionality/i.test(core)||core.includes('does not directly mutate main'),'R239 core truth boundary must remain explicit');
...
for(const token of [
 ...
 "rankDependencyReadyCapsules",
 "const capsule=candidates[0]||null",
 "R239 ranks the whole dependency-ready sparse frontier",
 "schedulerRevision:'R239'",
 "canonicalAdmission:false"
])assert.ok(engine.includes(token),`R239 R170 integration missing ${token}`);
...
assert.equal(governor.currentCapabilityFloor,'R237','R239 source candidate must not falsify promoted workflow capability floor before production proof');

for(const token of [
 "data-r239-recursive-build='SPARSE_PARALLEL_PLAN_STRICT_ADMISSION'",
 'R239 · FULL OVERALL CANON · RECURSIVE BUILD FABRIC',
 'R239_RESOLUTION.cells.toLocaleString()',
 "fetch('/omega-r170-selfbuild-state.json'",
 'R147 remains dispatch authority',
 'R125 sole CanonState admission authority'
])assert.ok(ui.includes(token),`R239 operator surface missing ${token}`);
...
console.log('OMEGA R239 RECURSIVE CANON BUILD FABRIC PASS · Woven Continuity applied to software evolution · 12/144/1728/20736/248832 logical addressing · sparse dependency frontier · pressure-directed ranking · R170 one-candidate mutation boundary preserved · R141/R146/R147/R125 authority preserved');
