...
if(boundedR239Browser){
 must(workflow.includes('run_browser() {')&&workflow.includes('timeout --signal=TERM --kill-after=15s 300s'),'R305 bounded R239 browser invocation must retain the fail-closed TERM→KILL 300-second timeout runner');
 must(workflow.includes('exit "$status"'),'R305 bounded browser runner must propagate the exact nonzero/timeout status');
 must(!workflow.includes('continue-on-error: true'),'R305 bounded browser runner must not convert failure into success');
}
