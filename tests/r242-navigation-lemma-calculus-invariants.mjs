...
const must=(ok,msg)=>{if(!ok)throw new Error(`R242.1 ${msg}`)};
...
const r239Browser=read('tests/r239-user-navigation-browser-e2e.mjs');
...
must(governance.routes[0]?.name==='Governance','exact Governance identity must outrank governance metadata matches');
...
for(const token of ['node tests/r242-navigation-lemma-calculus-invariants.mjs','node tests/r239-user-navigation-browser-e2e.mjs','node tests/r242-navigation-browser-e2e.mjs'])must(workflow.includes(token),`R241 bounded proof workflow must execute ${token}`);
...
must(r239Browser.includes('data-route-id')&&r239Browser.includes('ancestor-aware visible-canvas sanity'),'R239.1 machine-route identity and ancestor-aware browser proof must remain preserved rather than replaced by R242');

console.log(`R242.1 NAVIGATION LEMMA CALCULUS PASS · ${routeRecords.length} source routes conserved · exact identity > prefix/token/metadata · Governance/System ambiguity closed · all workspace partitions conserved · unresolved/duplicate identities carried as residuals · R239.1 machine identity proof preserved · navigation remains read-only and non-Canon`);
