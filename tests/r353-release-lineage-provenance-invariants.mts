...
import{CANONICAL_RELEASES_R353,HISTORICAL_PROVENANCE_DONORS_R353,compileReleaseLineageR353,releaseLineageSha256R353,R353_BOUNDARY,R353_SCHEMA}from'../src/system/releaseLineageR353';
...
for(const token of['/api/release-evidence','/api/runtime-attestation','/omega-build-receipt.json','CURRENT PROOF HOLD','Historical provenance donors'])assert.ok(surface.includes(token),`R353 surface missing ${token}`);
