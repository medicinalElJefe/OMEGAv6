...
for(const token of ['min-height:100dvh','env(safe-area-inset-top,0px)',':focus-visible','@media(hover:hover)','@media(forced-colors:active)','min-height:44px','overscroll-behavior-inline:contain','position:relative;z-index:30'])if(!experienceCss.includes(token))fail(`R260.1 responsive/accessibility/stacking polish missing ${token}`);

console.log('startup experience invariants: PASS · 44 registered routes + source-backed modes + R27 single frame + R259 atomic continuity + R260 live Home coherence + R260.1 browser polish/error closure + shell-above-observer navigator precedence');
