...
async function desktop(browser){
 ...
 for(const token of ['R168 · FULL RESTORATION / CURRENT EXECUTION CONVERGENCE','CURRENT SUCCESSOR EXECUTION · R48/R153/R168','TRUTH GATED','R166 DEVELOPMENT RESIDUAL → LIVING WORLD','12 MASTER OPERATIONAL INTENTS'])if(!text.includes(token))throw new Error(`R168 desktop missing ${token}`);
...
const browser=await chromium.launch({headless:true});try{await desktop(browser);await mobile(browser);console.log('R168 BROWSER PASS · current successor truth + R166 residual world + 24 families + 12 intents · Matter opens real biology executor · desktop/mobile contained')}finally{await browser.close()}
