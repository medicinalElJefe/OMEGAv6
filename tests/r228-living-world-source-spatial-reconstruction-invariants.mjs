...
for(const forbidden of ['fetch(','XMLHttpRequest','WebSocket','navigator.gpu','POST','PUT','PATCH','DELETE'])must(!core.includes(forbidden),`core must remain local/bounded and not contain ${forbidden}`);
...
for(const forbidden of ['fetch(','/api/','method:\'POST\'','method:\"POST\"','method:\'PUT\'','method:\'PATCH\'','method:\'DELETE\''])must(!ui.includes(forbidden),`UI must not introduce remote execution/mutation path ${forbidden}`);
