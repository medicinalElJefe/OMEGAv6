...
for(const name of retired){
  assert.ok(!wrangler.includes(`\"${name}\": {\"type\": \"durable-object\", \"state\": \"deleted\"}`),`completed provider retirement must not retain stale tombstone ${name}`);
...
console.log('R229/R230.1 CLOUDFLARE RETIREMENT RECONCILIATION PASS · R201/R203 remain fully retired with no stale export tombstones/live binding/storage/class · R125 preserved');
