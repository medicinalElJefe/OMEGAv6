...
for(const implementation of ['def apply_patch(','def write_text(','def build(','def desktop_health(','def forensic_hash_ledger('])must(!wrapper.includes(implementation),`R207 wrapper must not create a second executor implementation: ${implementation}`);
...
const syntax=spawnSync('python3',['-c',"import ast,pathlib;[ast.parse(pathlib.Path(p).read_text()) for p in ['public/omega-hybrid-agent.py','public/omega-hybrid-agent-r141.py','public/omega-hybrid-agent-base-r205.py']]"],{encoding:'utf8'});assert.equal(syntax.status,0,`R207/R141/R205 Python syntax failed: ${syntax.stderr}`);
...
out=m.wrap_packet(packet,m.EXPECTED_BASE_SHA256);core={k:out.get(k) for k in ('jobId','ok','stepProofs','outputPaths','log','evaluation','promotion','capabilityRevision')};payload=json.dumps(core,sort_keys=True,separators=(',',':'),ensure_ascii=False);assert out['resultFingerprintSchema']=='OMEGA_AGENT_RETURN_FINGERPRINT_R141';assert out['resultFingerprintR141Payload']==payload;assert out['resultFingerprintR141']==hashlib.sha256(payload.encode()).hexdigest();assert out['proofClosureRevision']=='R141';assert out['baseAgentSha256']==m.EXPECTED_BASE_SHA256;assert out['proofExtensions']==['R205','R207'];assert out['resultFingerprint']=='legacy';print('R207_R141_ENVELOPE_SIM_PASS')`],{encoding:'utf8'});assert.equal(simulation.status,0,`R207 R141 envelope simulation failed: ${simulation.stderr||simulation.stdout}`);must(simulation.stdout.includes('R207_R141_ENVELOPE_SIM_PASS'),'R207 simulation receipt missing');
console.log('R207 R141-NATIVE HOST EVIDENCE PASS · runtime-pinned immutable R205 executor SHA · inherited executor contract validated before load · R117/R141 direct-base path proven · exact semantic return envelope · no expanded execution authority · R125 preserved');
