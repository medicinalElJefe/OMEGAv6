...
out=m.wrap_packet(packet,m.EXPECTED_BASE_SHA256);core={k:out.get(k) for k in ('jobId','ok','stepProofs','outputPaths','log','evaluation','promotion','capabilityRevision')};payload=json.dumps(core,sort_keys=True,separators=(',',':'),ensure_ascii=False);assert out['resultFingerprintSchema']=='OMEGA_AGENT_RETURN_FINGERPRINT_R141';assert out['resultFingerprintR141Payload']==payload;assert out['resultFingerprintR141']==hashlib.sha256(payload.encode()).hexdigest();assert out['proofClosureRevision']=='R141';assert out['baseAgentSha256']==m.EXPECTED_BASE_SHA256;assert out['proofExtensions']==['R205','R207','R243','R345'];assert out['resultFingerprint']=='legacy';print('R207_R141_R243_R345_ENVELOPE_SIM_PASS')`],{encoding:'utf8'});assert.equal(simulation.status,0,`R207/R243 R141 envelope simulation failed: ${simulation.stderr||simulation.stdout}`);must(simulation.stdout.includes('R207_R141_R243_ENVELOPE_SIM_PASS'),'R207/R243/R345 simulation receipt missing');
