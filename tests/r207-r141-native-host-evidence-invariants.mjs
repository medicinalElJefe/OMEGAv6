...
for(const token of ["VERSION='R207'","BASE_TRANSPORT_VERSION='R34.1'","BASE_CAPABILITY_REVISION='R132'","BASE_PROOF_EXTENSION='R205'","PROOF_CLOSURE_REVISION='R141'","FINGERPRINT_SCHEMA='OMEGA_AGENT_RETURN_FINGERPRINT_R141'","BASE_PATH='/omega-hybrid-agent-base-r205.py'",`EXPECTED_BASE_SHA256='${baseSha}'`,'if digest!=EXPECTED_BASE_SHA256','R207 immutable R205 base SHA-256 mismatch','canonical_json','exact_payload','wrap_packet','OMEGA Hybrid Link agent compatibility signature'])must(wrapper.includes(token),`R207 wrapper missing ${token}`);
...
console.log('R207 R141-NATIVE HOST EVIDENCE PASS · runtime-pinned immutable R205 executor SHA · exact semantic return envelope · no expanded execution authority · R125 preserved');
