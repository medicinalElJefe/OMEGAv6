...
const convergenceStart=suite.indexOf("if(panel==='Convergence')return wrap("),governanceStart=suite.indexOf('GOVERNANCE + ADVANCEMENT',convergenceStart),governanceEnd=suite.indexOf("</section>",governanceStart),governanceGroup=suite.slice(governanceStart,governanceEnd);assert.ok(convergenceStart>=0&&governanceStart>convergenceStart,'Convergence route must retain an explicit governance/advancement group');const convergenceOrder=['<AppliedCalculusR168/>','<CalculusAddressFabricR240 record={record}/>','<OmegaResearchAdvancementR316/>','<RecursiveSelfBuildR240/>'].map(token=>governanceGroup.indexOf(token));assert.ok(convergenceOrder.every(x=>x>=0)&&convergenceOrder.every((x,i)=>i===0||x>convergenceOrder[i-1]),'Convergence route must retain the established calculus → address → research → self-build semantic order inside its governance group');
