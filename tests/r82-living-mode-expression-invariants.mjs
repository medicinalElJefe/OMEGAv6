...
must(home.includes('omega-r88-open-navigator')&&nav.includes("placeholder='Search tools, surfaces, or workflows'"),'Home must delegate complete application search to the one shared navigator');
...
must(nav.includes('R239_USER_NAV_REVISION')&&nav.includes('YOU ARE HERE'),'deep historical route reachability must remain explicit in the persistent rail/browser');
...
must(nav.includes("aria-label='Browse all registered OMEGA tools'")&&nav.includes("aria-label={activeWorkspace?`${activeWorkspace.label} OMEGA applications`:'All registered OMEGA applications'}"),'complete route reachability must remain exposed to assistive/browser navigation');
...
console.log(`R82/R239 LIVING MODE EXPRESSION PASS · ${routes.length} current registered destinations · complete unsliced All Tools reachability · 8 projection grammars · individual mode expressions · state-bound surfaces preserved`);
