...
const r139=fs.existsSync('src/unifiedCapabilityEngineR139.ts')?read('src/unifiedCapabilityEngineR139.ts'):'';
...
for(const token of ['corpusState','calculusVisualLaw','onAddress','onNavigate','Admitted next','CAPABILITY_FIRST_NO_STALE_CHART_PRIMARY'])must(field.includes(token),'active capability field missing '+token);
...
must(field.includes("kind:'STATE'")&&field.includes('compileUnifiedCapabilityRuntimeR139'),'capability field must preserve state semantics and consume the unified route/action runtime');
...
if(r139){
 for(const route of ['Hybrid Link','Build Out','Evidence & Proof'])must(r139.includes(`'${route}'`)||r139.includes(`${route}`),'unified R139 action runtime must preserve '+route);
 for(const kind of ["return'EXECUTE'","return'PROVE'","return'BUILD'"])must(r139.includes(kind),'unified capability runtime must preserve '+kind);
 must(!field.includes('ROUTE_BY_PANEL'),'R139 must remove duplicate static capability routing from the R138 field');
}else{
 for(const token of ['Hybrid Link','Build Out','Evidence & Proof',"return'EXECUTE'","return'PROVE'","return'BUILD'"])must(field.includes(token),'R138 direct capability field missing '+token);
}
...
console.log('R138 CAPABILITY-FIRST VISUAL RUNTIME PASS · stale plot-first surfaces retired · canonical manifold retained · actionable living topology mounted · route/state/build/proof execution preserved directly or through R139 · R137/R136/R125 authorities unchanged');
