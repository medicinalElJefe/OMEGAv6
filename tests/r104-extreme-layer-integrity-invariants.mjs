...
// Readable, flat, reserved-space global navigation. R242 owns the current query/workspace transform without weakening R104 layout/provenance guarantees.
...
must(nav.includes('compileNavigationLemmaR242({routes:routeRecords,query,workspaceFilter,currentRoute:currentPanel})')&&nav.includes('rows.map(route=>')&&!nav.includes('rows.slice('),'all registered routes must remain one flat searchable list through the R242 calculus projection');
must(lemma.includes('WORKSPACE_PARTITION_MUST_CONSERVE_THE_COMPLETE_ROUTE_SET')&&lemma.includes('QUERY_TRANSFORM_MAY_REORDER_PRESENTATION_BUT_MAY_NOT_RENAME_OR_DUPLICATE_ROUTES')&&lemma.includes('CURRENT_ROUTE_IDENTITY_MUST_SURVIVE_VIEW_AND_SEARCH_TRANSFORMS'),'R242 navigation transform must conserve R104 route, query and current-location semantics');
...
console.log('R104/R239.1/R242 EXTREME LAYER INTEGRITY PASS · route/layer/provenance coverage · calculus-conserved user-explained reserved-space navigation · source-driven visual modes · current zero-drift Hybrid transport + compatibility path · task-first federation preserved');
