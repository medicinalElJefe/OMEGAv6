...
function usableSnapshot(){
 ...
 const buried=interactives.filter(el=>{const r=el.getBoundingClientRect();if(r.right<=0||r.left>=innerWidth||r.bottom<=0||r.top>=innerHeight)return false;const x=Math.max(0,Math.min(innerWidth-1,r.left+r.width/2)),y=Math.max(0,Math.min(innerHeight-1,r.top+r.height/2)),hit=document.elementFromPoint(x,y);return Boolean(hit&&!el.contains(hit)&&!hit.contains(el))}).map(el=>{const r=el.getBoundingClientRect(),x=Math.max(0,Math.min(innerWidth-1,r.left+r.width/2)),y=Math.max(0,Math.min(innerHeight-1,r.top+r.height/2)),hit=document.elementFromPoint(x,y),label=(el.textContent||el.getAttribute('aria-label')||el.getAttribute('placeholder')||el.tagName).replace(/\s+/g,' ').trim().slice(0,64),blocker=(hit?.textContent||hit?.getAttribute?.('aria-label')||hit?.className||hit?.tagName||'unknown').toString().replace(/\s+/g,' ').trim().slice(0,64);return`${label} <- ${blocker}`});
