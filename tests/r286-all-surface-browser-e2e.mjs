...
async function verifyNavigatorModeTouchTargets(page,viewportName){
  ...
  if(state.buttons.length!==2)throw new Error(`mobile: R305 expected two navigator-mode controls, received ${state.buttons.length}`);
  const undersized=state.buttons.filter(x=>x.width<43.5||x.height<43.5);
  if(undersized.length)throw new Error(`mobile: R305 navigator-mode targets below 44px: ${undersized.map(x=>`${x.label} ${x.width.toFixed(1)}×${x.height.toFixed(1)}`).join(' | ')}`);
}
...
try{
  for(const [name,contextOptions] of profiles){
    ...
    await verifyR305InteractionEnvelope(page,name);
  ...
  console.log('R286/R305 ALL-SURFACE BROWSER PASS · exact two-button navigator-mode controls >=44×44px under coarse-pointer emulation · ALL + six contextual workspace submenus · 44/44 routes desktop + 390px 2×DPR touch mobile · every activated workstation horizontally contained · coarse-pointer 44×44 action + 44px-high form-control proof · center-point layer-occlusion proof for visible interactive controls · reduced-motion navigator proof · exact panel transitions · visible-content proof · exact 78×78/6084-cell SAR geometry · Escape/reopen · no page errors.');
