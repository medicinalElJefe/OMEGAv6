...
import {
  R242_CONTINUITY_OPERATOR,R242_RESOLUTION,R242_ROLES,R242_INVARIANT_CARRY,
  addressForCellR242,validateRoadmapDagR242,carryScarsR242,buildWorkCellR242,rolePacketsForCellR242,planWovenBuildFabricR242
} from '../scripts/lib/r242-woven-selfbuild-fabric.mjs';

const policy=JSON.parse(fs.readFileSync('public/omega-r242-woven-selfbuild-fabric.json','utf8'));
...
assert.deepEqual(policy.fabric.typedRoles,R242_ROLES);
assert.equal(policy.continuityOperator,R242_CONTINUITY_OPERATOR);
assert.equal(policy.resolution.physicalDimensionsClaimed,false);
assert.equal(R242_RESOLUTION.organs,12);
assert.equal(R242_RESOLUTION.branches,144);
assert.equal(R242_RESOLUTION.cells,1728);
assert.equal(R242_RESOLUTION.lanes,20736);
assert.equal(R242_RESOLUTION.deepAddress,248832);

const addresses=Array.from({length:1728},(_,i)=>addressForCellR242(i));
assert.equal(new Set(addresses.map(x=>x.address)).size,1728,'every R242 cell address must be unique');
assert.equal(new Set(addresses.map(x=>x.branch)).size,144,'R242 must expose exactly 144 branches');
assert.equal(addresses[0].laneStart,0);
assert.equal(addresses.at(-1).laneEnd,20735);
assert.equal(addresses.at(-1).deepEnd,248831);
assert.throws(()=>addressForCellR242(1728),RangeError);
...
assert.equal(validateRoadmapDagR242(roadmap).valid,true);
assert.equal(validateRoadmapDagR242([{id:'A',prerequisites:['MISSING']}]).valid,false);
assert.equal(validateRoadmapDagR242([{id:'A',prerequisites:['B']},{id:'B',prerequisites:['A']}]).valid,false);
assert.equal(validateRoadmapDagR242([{id:'A'},{id:'A'}]).valid,false);
...
const scars=carryScarsR242([{scarId:'OLD',severity:'LOW',mode:'OBSERVE_ONLY',summary:'older scar',provenance:'R146_DURABLE_HISTORY'}],evidence);
...
let plan=planWovenBuildFabricR242({roadmap,admitted:[],maxParallel:12,evidence,previousScars:scars.filter(x=>x.scarId==='OLD')});
assert.equal(plan.schema,'OMEGA_WOVEN_SELFBUILD_FABRIC_R242');
assert.equal(plan.revision,'R242');
...
for(const cell of plan.activeCells){
  ...
  assert.equal(cell.rolePackets.length,R242_ROLES.length);
  assert.deepEqual(cell.rolePackets.map(x=>x.role),R242_ROLES);
...
assert.ok(R242_INVARIANT_CARRY.includes('CI_YML_SOLE_CANONICAL_PRODUCTION_WRITER'));
assert.ok(R242_INVARIANT_CARRY.includes('SOURCE_MUTATION_REQUIRES_EXACT_APPLY_PATCH_OR_WRITE_TEXT_RETURN_PROOF'));

plan=planWovenBuildFabricR242({roadmap,admitted:['A'],maxParallel:12,evidence,previousScars:[]});
assert.ok(plan.activeCells.some(x=>x.id==='C'),'C must become ready only after A is admitted');
const badPlan=planWovenBuildFabricR242({roadmap:[{id:'A',prerequisites:['B']},{id:'B',prerequisites:['A']}],admitted:[],evidence});
