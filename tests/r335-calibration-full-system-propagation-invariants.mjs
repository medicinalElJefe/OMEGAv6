...
const receiptBlock=(master.match(/promotionReceipts:\[([^\]]+)\]/)||[])[1]||'';
assert.ok(receiptBlock.includes('R335_PROPAGATION_RECEIPT')&&receiptBlock.includes('R334_B06_PROGRESS_RECEIPT'),'R335 and inherited R334 receipts must remain in the promotion receipt chain');
assert.ok(receiptBlock.indexOf('R334_B06_PROGRESS_RECEIPT')>receiptBlock.indexOf('R335_PROPAGATION_RECEIPT'),'R335 propagation receipt must remain ahead of inherited R334 receipt even when newer successor receipts are prepended');
