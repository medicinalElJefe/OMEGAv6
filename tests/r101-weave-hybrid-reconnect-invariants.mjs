...
must(bootstrap117.includes("fetch('/api/hybrid/bootstrap'")&&bootstrap117.includes('const session=runtimeSessionId()')&&bootstrap117.includes("'x-omega-session-id':session")&&bootstrap117.includes('if(current?.bridgeId===session&&current.secret)')&&bootstrap117.includes("headers['x-omega-bridge-secret']=current.secret")&&bootstrap117.includes('body:JSON.stringify({rotate:true})')&&bootstrap117.includes('saveHybridBridge'),'R117 must reject stale cross-session bridge headers, rotate same-session credentials explicitly, and persist the exact returned bridge');
