...
const worker=read('src/workerR33.js'),worker32=read('src/workerR32.js'),worker34=fs.existsSync('src/workerR34.js')?read('src/workerR34.js'):'',worker101=fs.existsSync('src/workerR101.js')?read('src/workerR101.js'):'',worker102=fs.existsSync('src/workerR102.js')?read('src/workerR102.js'):'',worker111=fs.existsSync('src/workerR111.js')?read('src/workerR111.js'):'',agent=read('public/omega-hybrid-agent.py'),shell=read('src/SingleFrameRuntimeShellR27.tsx'),pulse=read('src/LivingRuntimePulseR33.tsx'),config=read('wrangler.jsonc');
...
const r102PreservesR33=config.includes('"main": "src/workerR102.js"')&&worker102.includes("from './workerR101.js'")&&worker101.includes("from './workerR34.js'")&&worker34.includes("from './workerR33.js'")&&worker34.includes('return r33.fetch(request,env)');
const r111PreservesR33=config.includes('"main": "src/workerR111.js"')&&worker111.includes("from './workerR102.js'")&&worker102.includes("from './workerR101.js'")&&worker101.includes("from './workerR34.js'")&&worker34.includes("from './workerR33.js'")&&worker34.includes('return r33.fetch(request,env)');
must(directR33||r34PreservesR33||r101PreservesR33||r102PreservesR33||r111PreservesR33,'R33 living runtime must remain active directly or through the proven R34/R101/R102/R111 successor chain');
...
for(const source of [worker,worker32,worker34,worker101,worker102,worker111,agent,shell,pulse,config])must(!source.includes('@appdeploy/client')&&!source.includes('appdeploy.ai'),'living runtime must remain AppDeploy-free');
console.log('OMEGA R33b LIVING RUNTIME PASS · preserved through R34/R101/R102/R111 federation + reconnect successor');