...
for(const token of ["['RETURNED_HOST_PROOF','AWAITING_RETURNED_PROFILE'].includes(intelligenceState)",'data-r238-snapshot-epoch',"if(intelligenceState==='RETURNED_HOST_PROOF')",'proof-only R141 provenance','R141 exact return closure'])must(liveVerifier.includes(token),`R240 R238 live verifier missing state-semantic proof marker ${token}`);
must(liveVerifier.includes("else if(text.includes('Proof source: selected authenticated Hybrid device')||text.includes('R141 exact return closure'))"),'R240 R238 live verifier must reject proof-only provenance while awaiting a returned profile');
must(!liveVerifier.includes("['R238 · HYBRID HOST INTELLIGENCE','Use the machine you actually have.','RCWA PYTHON DEPENDENCY','LOCAL MACRO STORE','R141 exact return closure']"),'R240 R238 live verifier must not require proof-only R141 footer prose in AWAITING_RETURNED_PROFILE state');
...
console.log('OMEGA R238/R240 HYBRID HOST INTELLIGENCE PASS · immutable R205 byte SHA preserved · R141 exact-return proof preserved · live verifier bound to semantic R238 state and only requires R141 proof footer when returned proof exists · one shared selected-host snapshot epoch · no duplicate polling reality · no screenshot constants · Windows CPU/RAM/GPU/storage/Python/RCWA evidence · no dependency installation · bounded macro metadata/schema/hash/count/order/time/coordinate/window preflight · Windows runtime CI · built-browser cross-host isolation · v7 Actions hygiene');
