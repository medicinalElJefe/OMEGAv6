...
for(const token of ["api.get<any>('/api/hybrid/status')","omega:hybrid:selectedDeviceId","String(s?.op||'').toUpperCase()==='DESKTOP_HEALTH'",'s?.result?.hostProfileR238','RETURNED_HOST_PROOF','RCWA PYTHON DEPENDENCY','LOCAL MACRO STORE','Hardware presence does not prove CUDA runtime','data-r238-selected-device'])must(ui.includes(token),`R238 Hybrid UI missing truth-bound token ${token}`);
must(ui.includes('onlineDevices.find((d:any)=>d?.id===selectedDeviceId)')&&ui.includes('targetDeviceId===device.id'),'R238 must bind returned host intelligence to the same exact selected authenticated device as R237');
