...
try{
 for(const [label,viewport,dpr] of viewports){
  ...
  await page.route('**/api/earth/gibs/global*',route=>route.fulfill({status:200,contentType:'image/png',body:R283_TEXTURE,headers:{'x-omega-source':'NASA-GIBS-VIIRS-SNPP-TRUECOLOR-GLOBAL','x-omega-date':'2026-09-09','x-omega-crs':'EPSG:4326','x-omega-bbox':'-180,-90,180,90','x-omega-truth':'RETURNED_GLOBAL_OBSERVATION'}}));
  await page.goto(`${base}/?r283=${Date.now()}-${label}`,{waitUntil:'domcontentloaded',timeout:45000});
  ...
  for(const [name,selector] of EXPECT){
   ...
   if(name==='Planet'){
    ...
    const planet=page.locator('.earth-r281-globe'),globe=planet.locator('canvas'),rect=await globe.boundingBox();
    if(!rect||rect.width<260||rect.height<420)throw new Error(`${label}: R283 observed globe canvas unusable ${JSON.stringify(rect)}`);
    if(await planet.getAttribute('data-projection')!=='WGS84_ELLIPSOID_ORTHOGRAPHIC')throw new Error(`${label}: R283 WGS84 projection identity missing`);
    if(await planet.getAttribute('data-source-crs')!=='EPSG:4326')throw new Error(`${label}: R283 source CRS identity missing`);
    const backing=await globe.evaluate(c=>({width:c.width,height:c.height,render:Number(c.dataset.surfaceRender||0),ratio:Number(c.dataset.devicePixelRatio||0)}));
    if(backing.width<rect.width*Math.max(1,dpr)*.9)throw new Error(`${label}: R283 canvas backing resolution too low ${JSON.stringify({rect,backing,dpr})}`);
    if(backing.render<Math.min(420,rect.width))throw new Error(`${label}: R283 ellipsoid render surface too low ${JSON.stringify(backing)}`);
    const sampled=await globe.evaluate(c=>{const ctx=c.getContext('2d');if(!ctx)return{unique:0,width:c.width,height:c.height};const d=ctx.getImageData(0,0,c.width,c.height).data,unique=new Set();for(let y=0;y<c.height&&unique.size<64;y+=Math.max(1,Math.floor(c.height/36)))for(let x=0;x<c.width&&unique.size<64;x+=Math.max(1,Math.floor(c.width/36))){const i=(y*c.width+x)*4;unique.add(`${d[i]},${d[i+1]},${d[i+2]},${d[i+3]}`)}return{unique:unique.size,width:c.width,height:c.height}});
    if(sampled.unique<12)throw new Error(`${label}: R283 globe did not render a materially varied observed texture (${sampled.unique} sampled colors)`);
    let text=await planet.innerText();for(const token of ['OBSERVED TEXTURE','EPSG:4326','WGS84 ELLIPSOID','SOURCE BRIGHTNESS','DERIVED UTC GEOMETRY','Truth boundary'])if(!text.includes(token))throw new Error(`${label}: R283 source/projection truth missing ${token}`);
    ...
    const grid=page.getByRole('button',{name:'Hide geodetic grid'}),terminator=page.getByRole('button',{name:'Hide UTC terminator'});if(!await grid.isVisible()||!await terminator.isVisible())throw new Error(`${label}: R283 geodetic overlay controls missing`);
   }
  }
 ...
 console.log('R279/R283 EARTH VIEW BROWSER PASS · desktop + 2×DPR mobile route to Earth Now · seven Earth menu buttons mount distinct surfaces · R283 Planet renders varied returned-source pixels on WGS84 ellipsoid projection · source/derived controls are explicit · reset/query target works · no page errors or viewport overflow');
