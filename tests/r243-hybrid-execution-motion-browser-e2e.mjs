...
const status={state:'VERIFIED_DEVICE_ONLINE',nativeExecutionClaimed:true,executionMotionRevision:'R243',runningLeaseMs:20000,devices:[{id:'dev-r243',name:'OMEGA R243 MOTION PC',online:true,revoked:false,lastSeen:now-500,rootLabel:'J:/',capabilities:caps,proofExtensions:['R205','R238','R240','R243']}],jobs:[running]};
...
for(const token of ['MOTION PROVED','HASH_TREE','S02','step 2/3','1 / 3','transport R243','R141 returned proof'])if(!text.includes(token))throw new Error(`R243 motion surface missing ${token}: ${text.slice(0,1800)}`);
...
if(await motion.getAttribute('data-r243-transport')!=='R243')throw new Error('R243 MOTION_PROVED must bind the native R243 wire protocol');
...
console.log('OMEGA R243 BUILT BROWSER PASS · selected authenticated host exposes native R243 renewable lease transport · exact HASH_TREE S02 motion · 1/3 returned-step boundary · selected-host identity retained · no prose-only INVOKED ambiguity');
await browser.close();
