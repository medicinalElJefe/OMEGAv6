...
assert.deepEqual(pushBranches,['r180-optical-fullwave-current-r1703']);
...
assert.equal(governor.currentCapabilityFloor,'R180');
assert.ok(governor.promotedSuccessorContinuity.includes('R180'));
assert.equal(governor.selfBuild.observationCadence,'HOURLY');
assert.equal(governor.selfBuild.expensiveProofMode,'PROPOSE_ONLY');
assert.equal(governor.preservedRuntime.opticalMachine,'omega-optical-machine-r1532');
assert.equal(governor.preservedRuntime.opticalVersion,'R153.2');
assert.equal(governor.preservedRuntime.opticalCandidateMachine,'omega-optical-machine-r1533');
assert.equal(governor.preservedRuntime.opticalCandidateAuthority,'SCREEN_ONLY_WITH_FULLWAVE_ADMISSIBILITY_GATE_PREPARED_NOT_BOUND');

console.log('R180 OPTICAL FULLWAVE ADMISSIBILITY INTEGRATION PASS · R170.3 preserved · R153.3 identity separated from inherited R152 · manifold gate deterministic · active binding remains R153.2 until separate live proof');
