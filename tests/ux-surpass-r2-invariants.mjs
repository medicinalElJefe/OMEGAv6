...
const names=[...launcher.matchAll(/name:'([^']+)'/g)].map(x=>x[1]);
must(names.length===44,`Nexus must expose exactly 44 destinations, got ${names.length}`);
...
for(const group of ['STUDIO','OPERATIONS','WORK','INTELLIGENCE','GOVERNANCE','SYSTEM'])must(launcher.includes(`'${group}'`),`Nexus missing group ${group}`);
