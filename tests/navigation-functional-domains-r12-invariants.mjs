...
const css=fs.readFileSync('src/omegaLauncherR12.css','utf8');
...
assert(launcher.includes('44 real routes · 7 functional domains'),'launcher must disclose its organization contract');
assert(css.includes("grid-template-columns:repeat(2")&&css.includes('@media(max-width:760px)'),'domain launcher must have explicit desktop and mobile layouts');
