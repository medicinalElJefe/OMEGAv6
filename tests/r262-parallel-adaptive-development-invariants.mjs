...
const r264=fs.existsSync('tests/r264-zero-constriction-hybrid-invariants.mjs')?read('tests/r264-zero-constriction-hybrid-invariants.mjs'):'';
...
const legacyWatchdog=surface.includes("window.setInterval(tick,5000)");
const r264Watchdog=surface.includes('const WATCHDOG_MS=30_000')&&surface.includes('window.setInterval(tick,WATCHDOG_MS)')&&surface.includes('startTransition(()=>setCorpus(read()))')&&r264.includes('R264 ZERO-CONSTRICTION HYBRID PASS');
assert.ok((legacyWatchdog||r264Watchdog)&&surface.includes("window.addEventListener('storage',sync)")&&surface.includes("window.addEventListener(CORPUS_EVENT,sync)")&&surface.includes("document.addEventListener('visibilitychange',tick)"),'R262 bounded/event-first/visibility-aware observation missing');
...
console.log(`R262 PARALLEL ADAPTIVE DEVELOPMENT PASS · current R261.1 lineage preserved · simultaneous use/observe/calibrate/develop · same-tab + cross-tab corpus signaling · ${r264Watchdog?'R264 event-first 30s successor watchdog':'legacy 5s watchdog'} · stale-data exclusion · Wilson confidence hardening · no new execution/mutation/promotion/production/Canon authority`);
