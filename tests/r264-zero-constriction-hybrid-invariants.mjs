...
assert.ok(link.includes("data-r264-operational-surfaces='FULL'")&&link.includes("data-r264-responsive='DESKTOP_MOBILE'"),'R264 full-surface/desktop-mobile truth markers missing');
...
for(const [name,path] of ordinary){
 assert.ok(link.includes(`import ${name} from '${path}'`),`R264 must keep ordinary Hybrid surface eager/visible: ${name}`);
 assert.ok(link.includes(`<${name}/>`),`R264 must keep ordinary Hybrid surface mounted: ${name}`);
 assert.ok(!link.includes(`lazy(()=>import('${path}'))`),`R264 may not defer ordinary Hybrid surface: ${name}`);
}
...
assert.ok(link.includes("const HybridMissionControlR8=lazy(()=>import('./HybridMissionControlR8'))"),'R264 may code-split only the explicitly collapsed R8 donor surface');
assert.ok(link.includes('deepOpen&&<Suspense')&&link.includes('<HybridMissionControlR8 status={status} record={record}/>'),'R264 R8 donor surface must remain explicit-demand reachable');
...
assert.ok(!linkCss.includes('content-visibility:auto')&&!linkCss.includes('contain-intrinsic-size'),'R264 may not hide ordinary Hybrid layers through content-visibility optimization');
