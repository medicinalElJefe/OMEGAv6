...
assert.equal(governor.currentSuccessorFloor,'R174');for(const revision of ['R171','R172','R173','R174'])assert.ok(governor.promotedSuccessorContinuity.includes(revision),`missing promoted successor ${revision}`);
...
console.log(`R170.2 / R175.1 GOVERNED SELF-BUILD INVARIANTS PASS · generation ${state.generation}/${state.maxAutonomousGenerations} · admitted ${admitted.length} · floor ${governor.currentSuccessorFloor}`);
