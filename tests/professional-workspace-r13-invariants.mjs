...
assert(nav.includes('OMEGA_WORKSPACES_R82')&&nav.includes('OMEGA_ALL_ROUTES_R82'),'navigator must consume the shared 44-route experience registry');
...
assert(nav.includes('Search all 44 OMEGA applications'),'professional navigator must provide global application search');
assert(nav.includes('One scroll owner')&&nav.includes('44 canonical routes'),'historical route reachability must remain governed and explicit');
assert(nav.includes('OMEGA V6')&&nav.includes('APPLICATIONS')&&nav.includes('Complete software system'),'navigation must expose readable product/application/software labels');
...
console.log('PASS professional-workspace invariant · R88 pop-out navigation + viewport-first authority accepted');
