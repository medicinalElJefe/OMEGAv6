...
// R319 reconciliation: R317 intentionally retired the duplicate visible R257 outer shell from App composition.
// Preserve R257 as the experience/truth context around the actual application while R317 owns the single visible navigator.
assert.ok(app.includes('<OmegaExperienceProviderR257>')&&app.includes("<main className='r317-product-root' data-r317-composition='SINGLE_CANONICAL_NAVIGATOR'>"),'R257 provider must wrap the R317 canonical Home/specialist application surface');
assert.ok(!app.includes('<OmegaExperienceShellR257'),'R317 single-canonical-navigator composition must not remount duplicate R257 visible shell');
...
console.log('R257/R317 RECONCILED EXPERIENCE PROOF PASS · R257 context/truth boundaries preserved · R317 single canonical navigator owns visible composition · no duplicate shell authority');
