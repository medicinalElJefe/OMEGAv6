...
const must=(ok,msg)=>{if(!ok)throw new Error('R105/R242 '+msg)};
...
// Contextual submenus partition one canonical registered route inventory; historical 44 remains telemetry, never a ceiling.
...
// R242 replaces ad-hoc Array.filter routing with the same flat source inventory passed through an invariant-preserving lemma transform.
for(const token of ['compileNavigationLemmaR242({routes:routeRecords,query,workspaceFilter,currentRoute:currentPanel})','navigationLemma.routes.map','rows.map(route=>','data-route-name={route}'])must(nav.includes(token),'lemma-bound submenu reachability missing '+token);
for(const token of ['WORKSPACE_PARTITION_MUST_CONSERVE_THE_COMPLETE_ROUTE_SET','QUERY_TRANSFORM_MAY_REORDER_PRESENTATION_BUT_MAY_NOT_RENAME_OR_DUPLICATE_ROUTES','ROUTE_IDENTITY_BEFORE_PRESENTATION_MATCH'])must(lemma.includes(token),'R242 navigation law missing '+token);
must(!nav.includes('rows.slice('),'submenu projection must never slice away registered routes');
...
console.log(`R105/R242 LIVE DATA TRUTH + SUBMENU PASS · ${registeredRoutes.length} registered routes partition-conserved by R242 lemma calculus · 6 contextual submenus · complete All Tools recovery · current canonical recurrence separated from 2025 donor samples · current-session performance truth enforced`);
