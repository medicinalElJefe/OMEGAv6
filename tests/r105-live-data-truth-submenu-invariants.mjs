...
const everywhereLabel=nav.includes('Everywhere <b>{routeCount}</b>')||nav.includes('Operate <b>{routeCount}</b>');
must(nav.includes('routeCount=OMEGA_ROUTE_INVENTORY_R107.currentCount')&&nav.includes("setWorkspaceFilter('ALL');open('EVERYWHERE')")&&everywhereLabel,'global EVERYWHERE path must restore the complete registered route inventory dynamically; adaptive UI may label that same path Operate');
...
console.log(`R105/R156 LIVE DATA TRUTH + SUBMENU PASS · ${registeredRoutes.length} registered routes preserved · 6 contextual submenus · EVERYWHERE/Operate full-registry semantics preserved · current canonical recurrence separated from 2025 donor samples · current-session performance truth enforced`);
