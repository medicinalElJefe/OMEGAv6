...
// Contextual submenus repartition one canonical registered route inventory; historical 44 remains telemetry, never a ceiling.
...
must(nav.includes('compileNavigationLemmaR242({routes:routeRecords,query,workspaceFilter,currentRoute:currentPanel})')&&nav.includes('rows.map(route=>')&&!nav.includes('rows.slice('),'submenu filtering must pass through the R242 partition-conserving lemma transform while preserving direct reachability');
must(lemma.includes('WORKSPACE_PARTITION_MUST_CONSERVE_THE_COMPLETE_ROUTE_SET')&&lemma.includes("requestedWorkspace==='ALL'?source:source.filter(route=>route.workspaceId===requestedWorkspace)"),'R242 workspace repartition must conserve the canonical registered route inventory instead of introducing a shadow filter');
...
console.log(`R105/R239/R242 LIVE DATA TRUTH + SUBMENU PASS · ${registeredRoutes.length} registered routes preserved through canonical R242 lemma repartition · 6 contextual submenus · complete All Tools recovery · current canonical recurrence separated from 2025 donor samples · current-session performance truth enforced`);
