...
must(nav.includes('OMEGA_ALL_ROUTES_R82.filter')&&nav.includes('rows.map(route=>')&&!nav.includes('rows.slice('),'submenu filtering must preserve flat 44-route registry and direct reachability');
must(nav.includes("setWorkspaceFilter('ALL');open('EVERYWHERE')")&&nav.includes('Everywhere <b>44</b>'),'global Everywhere path must always restore all 44 routes');
