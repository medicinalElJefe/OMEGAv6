...
const must=(ok,msg)=>{if(!ok)throw new Error('R105/R239/R242 '+msg)};
...
// Contextual submenus transform one canonical registered route inventory; historical 44 remains telemetry, never a ceiling.
...
must(nav.includes('OMEGA_ALL_ROUTES_R82.map((route,index)=>')&&nav.includes('compileNavigationLemmaR242({routes:routeRecords,query,workspaceFilter,currentRoute:currentPanel})')&&nav.includes('navigationLemma.routes.map')&&nav.includes('rows.map(route=>')&&!nav.includes('rows.slice('),'R242 lemma transform must preserve the flat canonical registered route source and direct reachability without hidden slicing');
must(nav.includes('routeCount=OMEGA_ROUTE_INVENTORY_R107.currentCount')&&nav.includes("setWorkspaceFilter('ALL');setMasterMenu('ALL');open('EVERYWHERE')")&&nav.includes("title='All tools'"),'global All Tools path must restore the complete registered route inventory and clear both workspace + recovered master-menu filters dynamically');
...
console.log(`R105/R239/R242/R289 LIVE DATA TRUTH + SUBMENU PASS · ${registeredRoutes.length} registered routes preserved through canonical R242 lemma transform · 6 contextual workspaces + recovered master-menu filter · complete All Tools recovery · current canonical recurrence separated from 2025 donor samples · current-session performance truth enforced`);
