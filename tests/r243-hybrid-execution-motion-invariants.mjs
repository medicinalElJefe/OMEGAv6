...
for(const token of [
 ...
 "state:'CLAIMED'","schema:'OMEGA_HYBRID_EXECUTION_PROGRESS_R243'",'leaseUntil:t+RUNNING_LEASE_MS'
...
// R243 intentionally reuses the Windows-proven R242 host pulse wrapper byte contract as an inherited component.
for(const token of [
...
])must(wrapper.includes(token),`inherited Windows motion/continuity component missing ${token}`);
must(!wrapper.includes('shell=True'),'motion wrapper must not add arbitrary shell authority');
must(!/pip\s+install|python\s+-m\s+pip|conda\s+install/i.test(wrapper),'R243 must not auto-install dependencies');
...
must(link.includes('Build success is not source mutation'),'R212 source-mutation truth boundary must remain explicit');
must(link.includes('Native execution is claimed only while an authenticated agent heartbeat is current'),'R141 native heartbeat truth gate must remain explicit');
must(link.includes('R242 navigation calculus remains read-only'),'R243 must preserve R242 navigation as read-only');
...
console.log('OMEGA R243 HYBRID EXECUTION MOTION PASS · authenticated host step pulses renew a bounded RUNNING lease and heartbeat · current step/elapsed/progress are operator-visible · expired legacy RUNNING claims fail closed · only bounded non-mutating discovery can auto-recover · root HASH_TREE is stripped from initial/recovery discovery · mutation is never blindly replayed · immutable R205 + R141/R240/R242/R146/R147/R125 authority preserved');
