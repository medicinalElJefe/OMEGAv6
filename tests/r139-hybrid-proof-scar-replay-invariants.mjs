...
import {agentPacketCoreR139,closeHybridReturnR139,replayHybridClosureR139,manifestR139} from '../src/hybridProofClosureR139.js';
import {sha256R134} from '../src/world/canonicalWorldContinuityR134.js';

const read=p=>fs.readFileSync(p,'utf8');
const must=(ok,msg)=>assert.ok(ok,'R139 '+msg);
const worker=read('src/workerR116.js'),agent=read('public/omega-hybrid-agent.py'),css=read('src/capabilityFirstR138.css'),r138=read('tests/r138-capability-first-visual-runtime.mjs'),ui=read('src/HybridProofClosureR139.tsx'),hybrid=read('src/HybridLinkR32.tsx'),wrangler=read('wrangler.jsonc'),r134=read('src/world/canonicalWorldContinuityR134.js'),r136=read('src/world/livingWorldFrameR136.js');
...
body.resultFingerprint=await sha256R134(agentPacketCoreR139(body));
...
must(closure.fingerprint.verified===true,'recomputed agent packet fingerprint must verify');
...
must(replay?.ok===true&&replay.headMatch===true&&replay.fingerprintMatch===true,'deterministic replay must reproduce both fingerprint and final continuity head');
...
const mismatchRuntime=new RuntimeMock([{id:'device_r139_pc',online:true,revoked:false,lastSeen:1788731700000,capabilityRevision:'R132'}]);
mismatchRuntime.store.set('jobs',[structuredClone(job)]);
const bad={...body,resultFingerprint:'0'.repeat(64)};
const mismatch=await closeHybridReturnR139(mismatchRuntime,job,bad);
must(mismatch.state==='FINGERPRINT_MISMATCH'&&mismatch.fingerprint.verified===false,'tampered return fingerprint must be held, never accepted as execution proof');
must(mismatch.livingWorldFrame?.head?.lastEvent?.proofIds?.length===0||mismatch.input.frame.federation.proofIds.length===0,'fingerprint mismatch must not enter the return as a proof id');

const manifest=manifestR139();
for(const law of ['AGENT_FINGERPRINT_MUST_RECOMPUTE_BEFORE_PROOF','HOST_RETURN_IS_EVIDENCE_NOT_CANONSTATE','EVERY_RETURN_CARRIES_SCAR_HISTORY','REPLAY_MUST_REPRODUCE_THE_SAME_FINAL_CONTINUITY_HEAD','R125_REMAINS_CANONICAL_ADMISSION_AUTHORITY'])must(manifest.laws.includes(law),'manifest missing law '+law);
...
for(const route of ['/api/hybrid/agent/register','/api/hybrid/agent/heartbeat','/api/hybrid/agent/poll','/api/hybrid/agent/result'])must(agent.includes(route),'existing authenticated agent transport missing '+route);
must(agent.includes('root-confined')&&agent.includes('shell=False'),'local execution must remain root-confined without arbitrary shell execution');
must(agent.includes("packet['resultFingerprint']=sha_json(packet)"),'agent must continue producing the deterministic return fingerprint R139 recomputes');
...
console.log('R139 HYBRID PROOF→SCAR→REPLAY PASS · agent fingerprint recomputed · tamper held · R134 scar head durable · R136 evidence frame bound · deterministic replay reproduces head · R138 transition regression repaired · R116/R117 execution spine + R125 admission authority preserved');
