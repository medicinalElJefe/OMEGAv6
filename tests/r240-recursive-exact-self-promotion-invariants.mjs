...
assert.equal(policy.selfPromotion.repositoryPrCreationRequired,false);
assert.equal(policy.selfPromotion.candidatePullRequestRequired,false);
assert.equal(policy.selfPromotion.exactTwoParentLeaseGuardedMainUpdate,true);
...
assert.ok(workflow.includes('gh workflow run ci.yml --repo "$GITHUB_REPOSITORY" --ref main'),'R339 must explicitly dispatch the sole canonical writer after an Actions-token main merge');
assert.ok(workflow.includes("--event workflow_dispatch"),'R339 must bind the resulting canonical CI run by workflow_dispatch event');
assert.ok(workflow.includes("r.event==='workflow_dispatch'"),'R339 production proof must require the dispatched run event on the exact merge SHA');
assert.ok(!workflow.includes("Canonical ci.yml push run did not appear for exact promoted merge"),'R339 must not wait for an impossible recursive GITHUB_TOKEN push trigger');
