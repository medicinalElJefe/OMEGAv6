...
for(const terminal of ['REPAIR_REQUIRED','HOLD_PROJECT_NOT_FOUND','HOLD_NO_DECLARED_VERIFICATION','HOLD_MAX_CYCLES','HOLD_DEVICE_PROOF','HOLD_PATCH_PROPOSAL','FAILED_PACKAGE'])must(executor.includes(`'${terminal}'`),`executor must recognize adaptive terminal/hold state ${terminal}`);
for(const token of ['MISSION_TERMINAL','JOB_TERMINAL','missionTerminal(missionState)','START NEW BOUNDED RETRY','previousMission:mission&&missionTerminal(missionState)','holdReason'])must(executor.includes(token),`mission lifecycle recovery missing ${token}`);
must(!executor.includes("active=mission&&(['ACTIVE','PAUSED'].includes(missionState)||!terminal(jobState))"),'failed/held missions must not remain falsely active merely because currentJob is absent');
...
console.log('R168 BUILD OUT ADAPTIVE MISSION SPINE PASS · Build Out and System Atlas share one R151→R153 executor · 12-cycle bounded proof-driven repair/build/test/package · hold states stop fake activity and permit a new bounded retry · current PC proof required · no shadow execution authority');
