...
const bioModeCss=read('src/bioModeWorkbenchR284.css');
...
must(bioModeCss.includes('.bio284-compare span{text-transform:none}'),'Heavy Bio read-only comparison labels must preserve authored case so the visible zero-authority comparison contract remains exact');
...
console.log('R286 UI INTERACTION INTEGRITY PASS · modern navigation hierarchy + 44 canonical surfaces + shared menu event + normalized persisted panel identity + R284/R284.1 truth contracts + Heavy Bio authored-case comparison contract + exact 78×78 mobile field geometry preserved.');
