...
must(browserProof.includes("animationName:style?.animationName||''")&&browserProof.includes("state.animationName&&state.animationName!=='none'"),'reduced-motion browser proof must reject a real animation by computed animation-name semantics');
must(!browserProof.includes("state.animationDuration&&state.animationDuration!=='0s'"),'reduced-motion proof must not confuse an inert inherited duration token with an active animation');
