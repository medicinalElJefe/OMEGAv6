...
const must=(ok,msg)=>{if(!ok)throw new Error('R112/R118/R120 '+msg)};
...
must(sovereign117.includes('J-ROOT SAFE')&&sovereign117.includes('does not silently fall back to C:'),'connection surface must make the approved non-C write boundary explicit');
...
const cReject=launcher117.indexOf('if /I "%OMEGA_ROOT:~0,2%"=="C:" goto :root_fail'),mkdir=launcher117.indexOf('mkdir "%OMEGA_BIN%"');
must(cReject>=0&&mkdir>cReject,'R117 must reject C: before creating any OMEGA runtime directories');
must(launcher117.includes('set /p "OMEGA_FIRST_LINE="')&&launcher117.includes('findstr /L /C:"OMEGA R34 local Hybrid Link agent"')&&launcher117.includes('findstr /L /C:"DEFAULT_SERVER=\'https://omegav6.jeffdeweyeljefe.workers.dev\'"'),'R117 launcher must validate canonical Python identity with CMD-safe literal checks');
must(!launcher117.includes("$s.Contains('OMEGA R34 local Hybrid Link agent')"),'R117 launcher must not retain the Windows PowerShell parser failure path');
...
must(!launcher117.includes('pip install numpy grcwa')&&!launcher117.includes('winget install'),'connection bootstrap must not silently alter the Python/Windows installation');
must(launcher117.includes('OMEGA_CONNECT_PC_R117.log')&&launcher117.includes('OMEGA_LOGDIR'),'R117 launcher must leave its diagnostic log on the approved root');
must(launcher117.includes('set "TEMP=%OMEGA_TMP%"')&&launcher117.includes('PIP_CACHE_DIR=%OMEGA_CACHE%'),'launcher must redirect OMEGA temp and Python package cache writes to approved-root storage');
...
must(law.includes('globalModeInfluenceR107')&&law.includes('sourceModeInfluence'),'R117/R118/R120 must consume the established full calculus/mode fabric rather than inventing a parallel decorative motion system');

console.log('R112/R118/R120 SOVEREIGN + LIVING FABRIC PASS · fresh durable bootstrap · J-root-safe CMD validation · current-heartbeat truth · retained local learning/RCWA · task-first federation · calculus-driven living motion · advanced donor diagnostics lazy-mounted · R112 donor preserved');
