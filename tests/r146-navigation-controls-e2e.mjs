...
async function mobile(browser){
 ...
 await expand(page);const backdrop=page.getByRole('button',{name:'Close OMEGA navigator backdrop'});if(!await backdrop.count())throw new Error('mobile: touch backdrop missing');const box=await backdrop.boundingBox();if(!box||box.width<40||box.height<600)throw new Error(`mobile: reserved dismiss strip is not a usable touch target ${JSON.stringify(box)}`);await backdrop.click({position:{x:box.width/2,y:Math.min(box.height-20,420)}});await page.waitForFunction(()=>document.documentElement.dataset.omegaNavExpanded==='false');
