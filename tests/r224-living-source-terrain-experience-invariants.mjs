...
let calls=0;
const fetcher=async url=>{calls++;return new Response(JSON.stringify({levels:{GROUND:{state:'RETURNED_USGS_ELEVATION',elevationM:700+calls,source:'USGS_3DEP_TEST'}}}),{status:200,headers:{'content-type':'application/json'}})};
const a=await assembleLivingTerrainExperienceR224({field,fetcher,origin:'https://omegav6.jeffdeweyeljefe.workers.dev'});
assert.equal(a.state,'LIVING_SOURCE_BACKED_TERRAIN_READY');assert.equal(a.experienceReady,true);assert.equal(a.canonicalWorkerMediated,true);assert.equal(a.r218GridShapeCarried,true);assert.equal(a.vertexCount,25);assert.equal(a.triangleCount,32);assert.equal(a.visual.points.length,25);assert.ok(a.visual.segments.length>32);assert.equal(calls,25);
...
const missing=samples[7];let n=0;const incomplete=async url=>{n++;const u=new URL(url),lat=Number(u.searchParams.get('lat')),lon=Number(u.searchParams.get('lon')),isMissing=Math.abs(lat-missing.lat)<1e-10&&Math.abs(lon-missing.lon)<1e-10;if(isMissing)return new Response(JSON.stringify({levels:{GROUND:{state:'NO_SOURCE_ELEVATION'}}}),{status:200,headers:{'content-type':'application/json'}});return new Response(JSON.stringify({levels:{GROUND:{state:'RETURNED_USGS_ELEVATION',elevationM:710+n,source:'USGS_3DEP_TEST'}}}),{status:200,headers:{'content-type':'application/json'}})};
const held=await assembleLivingTerrainExperienceR224({field,fetcher:incomplete,origin:'https://omegav6.jeffdeweyeljefe.workers.dev'});assert.equal(held.state,'HELD_FOR_SOURCE_BACKED_TERRAIN_EXPERIENCE');assert.equal(held.experienceReady,false);assert.equal(held.reason,'R221_CANONICAL_ELEVATION_NOT_READY');assert.ok(n>25,'persistent missing sample should exercise bounded retry');
