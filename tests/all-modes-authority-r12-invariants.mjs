...
const names=[...authority.matchAll(/^'([^']+)'(?:,|$)/gm)].map(x=>x[1]);
assert.equal(names.length,62,'R12 must retain all 62 repaired canon/calculus authority names');
for(const required of ['OVERALL CANON MODE','Unified Coherence Mode','Mode 188','Deep Mother Mode','High Father Mode','Prune / Heavy Prune Mode','Guidance Field Mode','FULL SPHERE Mode','Dewey Calculus Mode','No-Nothing Truth Mode','Alpha Mode','Seven-Star Governance Layer','Future Plasticity Law','Ledger / Scar Persistence Law','Horizon Limits vs Execution Limits','Basin / Drain / Loop / Fracture Action Families'])assert(authority.includes(`'${required}'`),`missing canon authority: ${required}`);
...
assert(authority.includes('not additional corpus rows')||authority.includes('not additional corpus rows'.toUpperCase()),'R12 must prohibit false executor double-counting');
