...
must(home.includes('unifiedFromRecord'),'home must bind unified calculus for water signal');
...
for(const v of ['--oh-c','--oh-phi','--oh-q','--oh-burden','--oh-motion','--oh-evidence','--oh-scar','--oh-water'])must(home.includes(v)&&css.includes(v),`missing visual source variable ${v}`);
...
must(home.includes('ALL 44 IN WORKSTATION'),'home must hand off to complete workstation navigation');
...
must(css.includes('.node.water'),'water conductance must have a distinct visual channel');
