...
assert.ok(motion.includes('same canonical packet')||motion.includes('same canonical packet'.replace('same ','shared '))||motion.includes('same canonical packet'.toUpperCase())||motion.includes('sharedCanonicalPacket:true'),'R310 scale identity carry must be explicit');
...
console.log('R310/R182 VISUAL COHERENCE PASS · packet-locked motion · eight distinct formal Matter scale lenses · no physical reconstruction overclaim · persistent orientation · retained functions · progressive disclosure');