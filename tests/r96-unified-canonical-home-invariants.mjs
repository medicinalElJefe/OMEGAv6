...
const must=(ok,msg)=>{if(!ok)throw new Error('R96/R239 '+msg)};
...
must(home.includes('<OmegaSideNavigatorR88')&&home.includes('<Search/>All tools')&&home.includes('OMEGA_ALL_ROUTES_R82'),'all registered routes must remain behind the shared All Tools catalog');
must(home.includes("aria-label='All tools'"),'visible and accessible All Tools language must agree');
...
console.log('R96/R239 UNIFIED CANONICAL HOME PASS · one frame · controlled living membrane · All Tools accessibility · truthful federation · exact Hybrid bytes');
