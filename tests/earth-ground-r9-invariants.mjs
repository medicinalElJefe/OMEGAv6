...
for(const token of ['sourceEnvelope','transportOk','evidenceReturned','REACHABLE_NO_IMAGE','REACHABLE_NO_NUMERIC_ELEVATION','RETURNED_EMPTY','Provider reachability is not evidence return'])must(worker.includes(token),`R309 provider/evidence separation missing ${token}`);
must(worker.includes('kartaMetadata?.imageUrl?kartaMetadata:null'),'R309 must not claim a KartaView photo without an admissible returned image asset');
must(worker.includes("state:kartaPhoto?'RETURNED_KARTAVIEW_PHOTO':address?.road?'RETURNED_OSM_ROAD_ONLY':'UNAVAILABLE'"),'R309 street hierarchy must distinguish returned imagery from OSM road-only evidence');
must(worker.includes('boundedGibsWindow')&&worker.includes("'x-omega-effective-span'")&&worker.includes("'x-omega-boundary-limited'")&&worker.includes("'x-omega-center-preserved'"),'R309 GIBS boundary-safe viewport contract missing');
...
must(ground.includes('REACHABLE · NO EVIDENCE')&&ground.includes('EVIDENCE RETURNED'),'R309 UI must distinguish provider reachability from actual evidence return');
must(ground.includes('Boundary-safe viewport')&&ground.includes('gibsBoundaryLimited'),'R309 UI must disclose dateline/pole viewport narrowing');
...
const r114=fs.existsSync('src/workerR114.js')?fs.readFileSync('src/workerR114.js','utf8'):'';
const r115=fs.existsSync('src/workerR115.js')?fs.readFileSync('src/workerR115.js','utf8'):'';
const r116=fs.existsSync('src/workerR116.js')?fs.readFileSync('src/workerR116.js','utf8'):'';
...
const r114WrappedR9=wrangler.includes('"main": "src/workerR114.js"')&&r114.includes("from './workerR111.js'")&&r111.includes("from './workerR102.js'")&&r102.includes("from './workerR101.js'")&&r101.includes("from './workerR34.js'")&&r34.includes("from './workerR33.js'")&&r33.includes("from './workerR32.js'")&&r32.includes("import r27 from './workerR27.js'")&&r27.includes("import r9 from './workerR9.js'")&&r27.includes('return r9.fetch(request,env)');
const r115WrappedR9=wrangler.includes('"main": "src/workerR115.js"')&&r115.includes("from './workerR114.js'")&&r114.includes("from './workerR111.js'")&&r111.includes("from './workerR102.js'")&&r102.includes("from './workerR101.js'")&&r101.includes("from './workerR34.js'")&&r34.includes("from './workerR33.js'")&&r33.includes("from './workerR32.js'")&&r32.includes("import r27 from './workerR27.js'")&&r27.includes("import r9 from './workerR9.js'")&&r27.includes('return r9.fetch(request,env)');
const r116WrappedR9=wrangler.includes('"main": "src/workerR116.js"')&&r116.includes("from './workerR115.js'")&&r115.includes("from './workerR114.js'")&&r114.includes("from './workerR111.js'")&&r111.includes("from './workerR102.js'")&&r102.includes("from './workerR101.js'")&&r101.includes("from './workerR34.js'")&&r34.includes("from './workerR33.js'")&&r33.includes("from './workerR32.js'")&&r32.includes("import r27 from './workerR27.js'")&&r27.includes("import r9 from './workerR9.js'")&&r27.includes('return r9.fetch(request,env)');
must(directR9||wrappedR9||r32WrappedR9||r33WrappedR9||r34WrappedR9||r101WrappedR9||r102WrappedR9||r111WrappedR9||r114WrappedR9||r115WrappedR9||r116WrappedR9,'Cloudflare must execute R9 directly or through a proven successor wrapper that preserves R9');
console.log('R309 EARTH GROUND PASS · provider reachability separated from evidence return · actual KartaView image asset required · USGS numeric elevation required · returned-empty OSM preserved · boundary-safe GIBS viewport · successor chain preserved');