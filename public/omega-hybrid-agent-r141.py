...
before closing the return into R134/R136 evidence. R139/R140, R146/R147, the separate R206
world-mission continuity bridge and R125 Canon admission remain preserved layers.
"""
...
def canonical_base_source(server):
    ...
    req=urllib.request.Request(url,method='GET',headers={'cache-control':'no-cache','user-agent':'OMEGA-Hybrid-R141-R206.1-Wrapper/1'})
    ...
    for token in required:
        if token not in text:raise RuntimeError('R141/R206.1 base agent contract missing '+token)
...
def load_base(server):
    ...
    if not {'DESKTOP_HEALTH','FORENSIC_HASH_LEDGER'}.issubset(caps):raise RuntimeError('R206.1 host proof operations are not advertised by canonical base agent.')
...
def main():
    ...
    print('OMEGA Hybrid Link proof wrapper',PROOF_CLOSURE_REVISION,'· base',base.VERSION,'execution',base.CAPABILITY_REVISION,'host proof',HOST_PROOF_EXTENSION,'continuity',HOST_EVIDENCE_CONTINUITY_REVISION)
    print('Exact return payload SHA-256 is enabled; R205 host proof, R206 world-mission continuity and R125 admission boundaries remain intact.')
    base.main()
