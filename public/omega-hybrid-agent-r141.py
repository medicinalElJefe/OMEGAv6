...
hashed before closing the return into R134/R136 evidence. R139 unified capability routing
and R140 living-world operation bridging remain separate preserved layers.
"""
...
def canonical_base_source(server):
    ...
    req=urllib.request.Request(url,method='GET',headers={'cache-control':'no-cache','user-agent':'OMEGA-Hybrid-R141-Wrapper/1'})
    ...
    for token in ("VERSION='R34.1'","CAPABILITY_REVISION='R132'",BASE_IDENTITY_MARKER,PAIRING_IDENTITY_MARKER,'root-confined','shell=False',"'/api/hybrid/agent/poll'","'/api/hybrid/agent/result'"):
        if token not in text:raise RuntimeError('R141 base agent contract missing '+token)
...
def main():
    ...
    print('Exact return payload SHA-256 is enabled; R139 capability routing and R140 living-world operation bridge remain intact.')
    base.main()
