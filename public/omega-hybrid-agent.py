...
R127 sealed successor. Stdlib-first, root-confined, allow-listed execution.
...
R127 pins the exact served agent SHA-256, binds a process boot session, rejects
...
VERSION='R127.0'
HYBRID_PROTOCOL='OMEGA_HYBRID_PROTOCOL_R127'
...
def validate_root(value):
    ...
    if os.name=='nt' and root.drive.upper()=='C:': raise AgentError('R127 refuses C: as the OMEGA approved runtime root.')
    return root
...
def main():
    ap=argparse.ArgumentParser(description='OMEGA R127 sealed Hybrid Link local agent')
    ...
    def write_state(stage,**extra):
        atomic_json(state_file,{'schema':'OMEGA_HYBRID_LOCAL_STATE_R127','stage':stage,'updatedAt':time.time(),'server':server,'deviceId':device_id,'bootId':boot_id,'protocol':HYBRID_PROTOCOL,'version':VERSION,'agentSha256':agent_sha256,'rootIdentity':root_identity,'rootLabel':root.name or root.anchor,'heartbeatSeq':heartbeat_seq,'secretStored':False,**extra})
    ...
    try:
        ...
        if e.code=='PAIR_AUTH_FAILED': print('      Pairing credential is rejected. Use OMEGA to prepare the current R127 connector; this agent will not retry a dead credential.',file=sys.stderr)
        raise SystemExit(22)
    except Exception as e:
