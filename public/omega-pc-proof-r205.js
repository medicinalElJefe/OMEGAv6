...
async function refresh(){
 try{
  ...
  state.closure=null;
  if(state.job?.id&&(state.job?.returnPacket||terminalJob(state.job))){try{const closed=await get(`/api/hybrid/jobs/${encodeURIComponent(state.job.id)}/closure`);state.closure=closed?.closure||null}catch{state.closure=null}}
  render();
...
async function runProof({automatic=false}={}){
 ...
 try{
  ...
  const data=await post('/api/missions',{objective,threadId:'',stateContext:JSON.stringify({surface:'OMEGA_PC_PROOF_CLOSURE_R205',handoff:'R300_ARMED_EXPLICIT_CONFIRMATION',archiveResiduals:['AT09','AT10'],transport:'R34.1',capability:'R132',proofExtension:'R205',proofClosure:'R141',hostEvidenceProjection:'R206.1',canonicalAdmissionAuthority:'R125',truth:'returned host proof requires exact R141 payload verification'}),draft,targetDeviceId:state.device.id,allowedOps:['DESKTOP_HEALTH','FORENSIC_HASH_LEDGER'],maxCycles:2,confirmedMission:true});
