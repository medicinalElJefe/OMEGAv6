...
const ci=contents.get('ci.yml');assert.match(ci,/Promoted main commit must be an exact two-parent merge commit/);assert.match(ci,/verify_federation_live_r1681\.mjs/);assert.match(ci,/R153\.2|omega-optical-machine-r1532/);
...
console.log(JSON.stringify({schema:'OMEGA_WORKFLOW_TOPOLOGY_R170_1',activeCount:active.length,active,archivedCount:archived.length,directMainCandidateMutation:false,recursiveWorkflowRunFanout:false,productionProofRequired:true,canonicalAdmissionAuthority:'R125',result:'PASS'},null,2));
