...
assert.match(selfbuild,/workflow_dispatch:/);assert.equal(triggerBlock(selfbuild,'schedule'),'');assert.ok(!/^\s*push\s*:/m.test(selfbuild));assert.ok(!/^\s*pull_request\s*:/m.test(selfbuild));assert.doesNotMatch(selfbuild,/gh\s+pr\s+create/);assert.match(selfbuild,/gh run list/);assert.match(selfbuild,/production_ready/);assert.match(selfbuild,/OBSERVE_ONLY/);assert.match(selfbuild,/prove_successor_workflow_invariants_r175\.mjs/);assert.match(selfbuild,/git commit-tree/);assert.match(selfbuild,/--force-with-lease=/);
