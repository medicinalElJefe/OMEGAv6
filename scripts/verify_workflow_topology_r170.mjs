...
const promoted=Array.isArray(governor.promotedSuccessorContinuity)?governor.promotedSuccessorContinuity:[];
for(const successor of successors)assert.ok(promoted.includes(`R${successor.revision}`)||successor.revision>Number(String(governor.currentSuccessorFloor||'R170').replace(/^R/,'')),`successor ${successor.name} is neither promoted nor above current successor floor`);
console.log(JSON.stringify({schema:'OMEGA_WORKFLOW_TOPOLOGY_R175_1',activeCount:active.length,coreCount:coreRequired.length,successorCount:successors.length,successors,archivedCount:archived.length,maxActive,currentSuccessorFloor:governor.currentSuccessorFloor,directMainCandidateMutation:false,recursiveWorkflowRunFanout:false,productionProofRequired:true,successorPolicy:'READ_ONLY_BRANCH_OR_PR_PROOF_AUTHORITIES',canonicalAdmissionAuthority:'R125',result:'PASS'},null,2));
