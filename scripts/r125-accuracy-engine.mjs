...
const R164_MANIFEST='public/omega-r164-development-residual-graph.json';
...
const r164Required=['src/system/developmentResidualGraphR164.js','tests/r164-development-residual-graph-invariants.mjs','.github/workflows/r164-development-residual-graph.yml',R164_MANIFEST];
const r164Missing=r164Required.filter(x=>!exists(x));
let r164Manifest=null;
if(exists(R164_MANIFEST)){
 try{r164Manifest=readJson(R164_MANIFEST)}catch{r164Missing.push(`${R164_MANIFEST}#VALID_JSON`)}
}
if(r164Manifest){
 if(r164Manifest.schema!=='OMEGA_DEVELOPMENT_RESIDUAL_GRAPH_R164'||r164Manifest.revision!=='R164')r164Missing.push(`${R164_MANIFEST}#SCHEMA_REVISION`);
 if(r164Manifest.canonicalMutation!==false)r164Missing.push(`${R164_MANIFEST}#CANONICAL_MUTATION_FALSE`);
 if(r164Manifest.autonomousMutationAuthority!==false)r164Missing.push(`${R164_MANIFEST}#AUTONOMOUS_MUTATION_AUTHORITY_FALSE`);
 if(r164Manifest.canonicalAdmissionAuthority!=='R125')r164Missing.push(`${R164_MANIFEST}#R125_ADMISSION_AUTHORITY`);
}
if(r164Missing.length)residuals.push({id:'R-R164-DEVELOPMENT-RESIDUAL-GRAPH-MISSING',kind:'DEVELOPMENT_RESIDUAL_GRAPH_FAILURE',severity:'HIGH',summary:'R164 development residual/evidence graph authority is incomplete or violates its read-only R125 admission boundary.',evidence:r164Missing.map(x=>evidence('SOURCE',x,'required R164 residual-graph sensor authority is intact',false,true)),affected:r164Missing,reproducible:true});

const r164ReflexRequired=['src/execution/reflexAutonomicSwarmR164.js','src/ReflexAutonomicR164.tsx','tests/r164-reflex-autonomic-swarm-convergence-invariants.mjs','.github/workflows/r164-reflex-autonomic-swarm-convergence.yml'];
const r164ReflexMissing=r164ReflexRequired.filter(x=>!exists(x));
if(r164ReflexMissing.length)residuals.push({id:'R-R164-REFLEX-AUTONOMIC-SWARM-MISSING',kind:'REFLEX_AUTONOMIC_SWARM_FAILURE',severity:'HIGH',summary:'R164 returned-reflex to governed R125 autonomic-swarm bridge is incomplete on disk.',evidence:r164ReflexMissing.map(x=>evidence('SOURCE',x,'required R164 reflex/autonomic bridge authority exists',false,true)),affected:r164ReflexMissing,reproducible:true});
...
for(const run of Array.isArray(runs)?runs:[]){
 if(run.status==='completed'&&run.conclusion&&run.conclusion!=='success'&&run.conclusion!=='skipped'){
  ...
  const kind=/R164 Development Residual Graph/i.test(workflowName)?'DEVELOPMENT_RESIDUAL_GRAPH_FAILURE':/R164 Reflex Autonomic Swarm Convergence/i.test(workflowName)?'REFLEX_AUTONOMIC_SWARM_FAILURE':/R154 Relative Capacity Fabric/i.test(workflowName)?'RELATIVE_CAPACITY_FAILURE':/R153 Lemma Motion NOW Continuity/i.test(workflowName)?'CAUSAL_NOW_LEMMA_FAILURE':/R152 Universal Truth Envelope/i.test(workflowName)?'UNIVERSAL_TRUTH_FAILURE':/R151 All Modes Truth Fusion/i.test(workflowName)?'ALL_MODES_FUSION_FAILURE':/R144 Runtime Deployment Attestation|Release Evidence Live Verify/i.test(workflowName)?'DEPLOYMENT_ATTESTATION_FAILURE':/R143 Authoritative UI Operation Chain/i.test(workflowName)?'UI_OPERATION_CHAIN_FAILURE':/R142 Proof Aware Capability Lifecycle/i.test(workflowName)?'EXECUTION_LIFECYCLE_FAILURE':'TEST_FAILURE';
  const severity=['DEVELOPMENT_RESIDUAL_GRAPH_FAILURE','REFLEX_AUTONOMIC_SWARM_FAILURE','RELATIVE_CAPACITY_FAILURE','CAUSAL_NOW_LEMMA_FAILURE','UNIVERSAL_TRUTH_FAILURE','ALL_MODES_FUSION_FAILURE','DEPLOYMENT_ATTESTATION_FAILURE','UI_OPERATION_CHAIN_FAILURE','EXECUTION_LIFECYCLE_FAILURE'].includes(kind)?'HIGH':'MEDIUM';
...
const semantic={residuals:ranked.map(r=>({id:r.id,kind:r.kind,severity:r.severity,mode:r.mode,confidence:r.confidence,affected:r.affected,evidence:r.evidence.map(e=>({kind:e.kind,source:e.source,claim:e.claim,verified:e.verified,value:e.value}))})),selected:selected?.id??null,repair:repair?{changed:repair.changed,proof:repair.proof}:null,r124Generation:s124?.generation??null,r124Admitted:s124?.admitted??[],r143AuthorityComplete:r143Missing.length===0,r144AttestationComplete:r144Missing.length===0,r164ResidualGraphComplete:r164Missing.length===0,r164ReflexAutonomicComplete:r164ReflexMissing.length===0,advancedAuthorityState,githubRunsObserved:runs.length};
const state={schema:'omega.accuracy.r125.v3',authority:'OMEGAV6',observedAt:now,mode:repair?'REPAIRED':selected?'PROPOSE':ranked.length?'OBSERVE':'HEALTHY',accuracyPolicy:{mutationRequiresVerifiedEvidence:true,autoRepairMinConfidence:.92,autoRepairRisk:'LOW_ONLY',criticalResidualBlocks:true,unreproducibleResidualAutoRepair:false,observationOnlyNeverMutatesMain:true,advancedTruthCapacityFailuresNeverAutoRepair:true,developmentResidualGraphNeverAuthorizesRepair:true,r164SensorFailuresNeverAutoRepair:true},residuals:ranked,selected:selected?selected.id:null,repair,sourceState:{r124Generation:s124?.generation??null,r124Admitted:s124?.admitted??[],r143AuthorityComplete:r143Missing.length===0,r144AttestationComplete:r144Missing.length===0,r164ResidualGraphComplete:r164Missing.length===0,r164ReflexAutonomicComplete:r164ReflexMissing.length===0,advancedAuthorityState,githubRunsObserved:runs.length},lineage:{baseSha:process.env.GITHUB_SHA||'UNKNOWN',semanticFingerprint:hash(semantic),stateSha256:''}};
state.lineage.stateSha256=hash({...state,lineage:{...state.lineage,stateSha256:''}});
fs.writeFileSync(OUT,JSON.stringify(state,null,2)+'\n');
fs.writeFileSync(PROPOSAL,JSON.stringify({schema:'omega.accuracy.proposal.r125.v3',observedAt:now,semanticFingerprint:state.lineage.semanticFingerprint,selected:selected?{id:selected.id,kind:selected.kind,mode:selected.mode,confidence:selected.confidence,summary:selected.summary}:null,repairApplied:!!repair,repair},null,2)+'\n');
console.log(JSON.stringify({status:repair?'REPAIRED':selected?'PROPOSE':ranked.length?'OBSERVE':'HEALTHY',residualCount:ranked.length,selected:selected?.id??null,selectedConfidence:selected?.confidence??null,repairApplied:!!repair,r143AuthorityComplete:r143Missing.length===0,r144AttestationComplete:r144Missing.length===0,r164ResidualGraphComplete:r164Missing.length===0,r164ReflexAutonomicComplete:r164ReflexMissing.length===0,advancedAuthorityState,semanticFingerprint:state.lineage.semanticFingerprint},null,2));