...
const r145Required=['src/computation/advancedComputationKernelR145.js','src/AdvancedComputationR145.tsx','tests/r145-advanced-computation-fabric-invariants.mjs','.github/workflows/r145-advanced-computation-fabric.yml','.github/workflows/r145-computation-live.yml'];
const r145Missing=r145Required.filter(x=>!exists(x));
const r145Kernel=exists('src/computation/advancedComputationKernelR145.js')?fs.readFileSync('src/computation/advancedComputationKernelR145.js','utf8'):'';
const r145Optical=exists('services/opticalMachineR115.js')?fs.readFileSync('services/opticalMachineR115.js','utf8'):'';
if(!r145Kernel.includes('R145_PHYSICS_INFORMED_REDUCED_ORDER_V1'))r145Missing.push('src/computation/advancedComputationKernelR145.js#R145_ENGINE');
if(!r145Optical.includes('/api/computation/r145/screen-batch'))r145Missing.push('services/opticalMachineR115.js#/api/computation/r145/screen-batch');
if(r145Missing.length)residuals.push({id:'R-R145-ADVANCED-COMPUTATION-MISSING',kind:'ADVANCED_COMPUTATION_FAILURE',severity:'HIGH',summary:'R145 advanced computation fabric is incomplete on disk.',evidence:r145Missing.map(x=>evidence('SOURCE',x,'required R145 advanced computation authority exists',false,true)),affected:r145Missing,reproducible:true});
...
for(const run of Array.isArray(runs)?runs:[]){
 if(run.status==='completed'&&run.conclusion&&run.conclusion!=='success'&&run.conclusion!=='skipped'){
  ...
  const kind=/R145 Advanced Computation Fabric|R145 Computation Live Verify/i.test(workflowName)?'ADVANCED_COMPUTATION_FAILURE':/R144 Runtime Deployment Attestation|Release Evidence Live Verify/i.test(workflowName)?'DEPLOYMENT_ATTESTATION_FAILURE':/R143 Authoritative UI Operation Chain/i.test(workflowName)?'UI_OPERATION_CHAIN_FAILURE':/R142 Proof Aware Capability Lifecycle/i.test(workflowName)?'EXECUTION_LIFECYCLE_FAILURE':'TEST_FAILURE';
  const severity=['ADVANCED_COMPUTATION_FAILURE','DEPLOYMENT_ATTESTATION_FAILURE','UI_OPERATION_CHAIN_FAILURE','EXECUTION_LIFECYCLE_FAILURE'].includes(kind)?'HIGH':'MEDIUM';
...
const semantic={residuals:ranked.map(r=>({id:r.id,kind:r.kind,severity:r.severity,mode:r.mode,confidence:r.confidence,affected:r.affected,evidence:r.evidence.map(e=>({kind:e.kind,source:e.source,claim:e.claim,verified:e.verified,value:e.value}))})),selected:selected?.id??null,repair:repair?{changed:repair.changed,proof:repair.proof}:null,r124Generation:s124?.generation??null,r124Admitted:s124?.admitted??[],r143AuthorityComplete:r143Missing.length===0,r144AttestationComplete:r144Missing.length===0,r145ComputationComplete:r145Missing.length===0,githubRunsObserved:runs.length};
const state={schema:'omega.accuracy.r125.v2',authority:'OMEGAV6',observedAt:now,mode:repair?'REPAIRED':selected?'PROPOSE':ranked.length?'OBSERVE':'HEALTHY',accuracyPolicy:{mutationRequiresVerifiedEvidence:true,autoRepairMinConfidence:.92,autoRepairRisk:'LOW_ONLY',criticalResidualBlocks:true,unreproducibleResidualAutoRepair:false,observationOnlyNeverMutatesMain:true},residuals:ranked,selected:selected?selected.id:null,repair,sourceState:{r124Generation:s124?.generation??null,r124Admitted:s124?.admitted??[],r143AuthorityComplete:r143Missing.length===0,r144AttestationComplete:r144Missing.length===0,r145ComputationComplete:r145Missing.length===0,githubRunsObserved:runs.length},lineage:{baseSha:process.env.GITHUB_SHA||'UNKNOWN',semanticFingerprint:hash(semantic),stateSha256:''}};
...
console.log(JSON.stringify({status:repair?'REPAIRED':selected?'PROPOSE':ranked.length?'OBSERVE':'HEALTHY',residualCount:ranked.length,selected:selected?.id??null,selectedConfidence:selected?.confidence??null,repairApplied:!!repair,r143AuthorityComplete:r143Missing.length===0,r144AttestationComplete:r144Missing.length===0,r145ComputationComplete:r145Missing.length===0,semanticFingerprint:state.lineage.semanticFingerprint},null,2));