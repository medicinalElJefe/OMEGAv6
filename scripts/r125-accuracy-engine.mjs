...
const EXECUTION_EVIDENCE_PATH=process.env.OMEGA_R145_EXECUTION_EVIDENCE_PATH||'';
...
if(executionEvidence?.schema==='omega.execution.evidence.r145.v1'){
 for(const probe of Array.isArray(executionEvidence.probes)?executionEvidence.probes:[]){
  ...
  const refs=[
   evidence('DEPLOYMENT',source,`R145 bounded first-hand probe ${probe?.id||'unknown'} failed`,probe?.detail||false,true,observedAt),
   evidence('SOURCE',probe?.capabilityId||'unknown-capability','R145 execution evidence identifies the affected registered capability',probe?.capabilityId||'unknown-capability',true,observedAt)
  ];
  if(probe?.sourceSha256||probe?.expectedSha256||probe?.packageReceiptSha256)refs.push(evidence('PROOF',probe?.path||source,'R145 source/package lineage comparison was observed',`source=${probe?.sourceSha256||'none'} expected=${probe?.expectedSha256||'none'} package=${probe?.packageReceiptSha256||'none'}`,true,observedAt));
  residuals.push({id:`R145-${probe?.id||hash(probe).slice(0,10)}`,kind:probe?.residualKind||'DEPLOYMENT_UNPROVEN',severity:probe?.severity||'HIGH',summary:`R145 live execution residual: ${probe?.capabilityId||probe?.id||'unknown'} · ${probe?.detail||'probe failed'} · root cause not inferred`,evidence:refs,affected:[probe?.capabilityId||'unknown-capability',probe?.path||'unknown-path'],reproducible:true});
 }
}
...
const semantic={residuals:ranked.map(r=>({id:r.id,kind:r.kind,severity:r.severity,mode:r.mode,confidence:r.confidence,affected:r.affected,evidence:r.evidence.map(e=>({kind:e.kind,source:e.source,claim:e.claim,verified:e.verified,value:e.value}))})),selected:selected?.id??null,repair:repair?{changed:repair.changed,proof:repair.proof}:null,r124Generation:s124?.generation??null,r124Admitted:s124?.admitted??[],r143AuthorityComplete:r143Missing.length===0,r144AttestationComplete:r144Missing.length===0,githubRunsObserved:runs.length,executionEvidenceObserved,executionEvidenceSchema:executionEvidence?.schema||null};
const state={schema:'omega.accuracy.r125.v2',authority:'OMEGAV6',observedAt:now,mode:repair?'REPAIRED':selected?'PROPOSE':ranked.length?'OBSERVE':'HEALTHY',accuracyPolicy:{mutationRequiresVerifiedEvidence:true,autoRepairMinConfidence:.92,autoRepairRisk:'LOW_ONLY',criticalResidualBlocks:true,unreproducibleResidualAutoRepair:false,observationOnlyNeverMutatesMain:true},residuals:ranked,selected:selected?selected.id:null,repair,sourceState:{r124Generation:s124?.generation??null,r124Admitted:s124?.admitted??[],r143AuthorityComplete:r143Missing.length===0,r144AttestationComplete:r144Missing.length===0,githubRunsObserved:runs.length,executionEvidenceObserved,executionEvidenceSchema:executionEvidence?.schema||null},lineage:{baseSha:process.env.GITHUB_SHA||'UNKNOWN',semanticFingerprint:hash(semantic),stateSha256:''}};
...
console.log(JSON.stringify({status:repair?'REPAIRED':selected?'PROPOSE':ranked.length?'OBSERVE':'HEALTHY',residualCount:ranked.length,selected:selected?.id??null,selectedConfidence:selected?.confidence??null,repairApplied:!!repair,r143AuthorityComplete:r143Missing.length===0,r144AttestationComplete:r144Missing.length===0,executionEvidenceObserved,executionEvidenceSchema:executionEvidence?.schema||null,semanticFingerprint:state.lineage.semanticFingerprint},null,2));
