...
if(plan.state==='BLOCKED_BY_R243_FABRIC'){
 console.log(JSON.stringify({status:'BLOCKED_BY_R243_FABRIC',generation:state.generation,governedContract:R245_GOVERNED_SELFBUILD_CONTRACT,gate,fabric:plan.woven,dimensionalRelativityR265:r265Summary},null,2));process.exit(0);
}
if(plan.state==='BLOCKED_R243_R240_SELECTION_DIVERGENCE'){
 console.log(JSON.stringify({status:'BLOCKED_BY_R243_R240_SELECTION_DIVERGENCE',generation:state.generation,governedContract:R245_GOVERNED_SELFBUILD_CONTRACT,gate,r240Candidate:plan.r240CandidateId,r243Candidate:plan.r243CandidateId,frontier:plan.frontier,fabric:plan.woven,dimensionalRelativityR265:r265Summary},null,2));process.exit(0);
}
if(plan.state!=='PROPOSE'||!plan.capsule){
 console.log(JSON.stringify({status:'OBSERVE',reason:plan.reason||'no dependency-ready predefined capsule',generation:state.generation,governedContract:R245_GOVERNED_SELFBUILD_CONTRACT,gate,frontier:plan.frontier,fabric:plan.woven,dimensionalRelativityR265:r265Summary},null,2));process.exit(0);
}

const capsule=plan.capsule;
const generation=Number(state.generation||0)+1;
if(!APPLY){
 console.log(JSON.stringify({status:'PROPOSE',generation,capsuleId:capsule.id,title:capsule.title,target:capsule.target,score:plan.score,governedContract:R245_GOVERNED_SELFBUILD_CONTRACT,generatorContract:R245_CAPSULE_GENERATOR_REVISION,residualPolicy:state.residualPolicy?.schema||null,gate,frontier:plan.frontier,fabric:plan.woven,dimensionalRelativityR265:r265Summary,selectionLaw:plan.selectionLaw},null,2));process.exit(0);
}
...
fs.writeFileSync(CANDIDATE_PATH,JSON.stringify({schema:'OMEGA_SELFBUILD_CANDIDATE_R170',revision:'R170.2',schedulerRevision:'R240',planningFabricRevision:'R243',dimensionalRelativityRevision:'R265',governedContract:R245_GOVERNED_SELFBUILD_CONTRACT,generatorContract:R245_CAPSULE_GENERATOR_REVISION,residualPolicy:state.residualPolicy?.schema||null,capsule,score:plan.score,frontier:plan.frontier,wovenFabric:wovenSummary,wovenDimensionalRelativityR265:r265Summary,receipt},null,2)+'\n','utf8');
console.log(JSON.stringify({status:'SANDBOX',generation,capsuleId:capsule.id,target:capsule.target,score:plan.score,governedContract:R245_GOVERNED_SELFBUILD_CONTRACT,generatorContract:R245_CAPSULE_GENERATOR_REVISION,gate,frontier:plan.frontier,fabric:wovenSummary,dimensionalRelativityR265:r265Summary},null,2));
