...
export function auditR314(root=process.cwd()){
 ...
 const r340AdvancePath=path.join(root,'public/canon/Dewey_OMEGA_CERN_ADV05_ADV06_Ablation_RoundTrip_Forecast_v4_2026-09-19.csv');
 ...
 const r340Advance=calibrationMeta(r340AdvancePath,'d4eeab6ec5f4310cb0554973538d60ce333a981ad0b8a3c301f8d359b692a410',25);
 ...
 if(!r340Advance.present)residuals.push({id:'R340-ADVANCE-MISSING',severity:'CRITICAL',mode:'BLOCK',summary:`R340 advancement dataset missing ${r340Advance.path}`,source:r340Advance.path});
 else if(r340Advance.sha256!==r340Advance.expectedSha)residuals.push({id:'R340-ADVANCE-HASH',severity:'CRITICAL',mode:'BLOCK',summary:`R340 advancement dataset SHA mismatch ${r340Advance.sha256}`,source:r340Advance.path});
 if(r340Advance.present&&r340Advance.records!==r340Advance.expectedRecords)residuals.push({id:'R340-ADVANCE-CENSUS',severity:'CRITICAL',mode:'BLOCK',summary:`R340 advancement dataset records ${r340Advance.records} != ${r340Advance.expectedRecords}`,source:r340Advance.path});
 if(!masterSource.includes('R340_PROPAGATION_RECEIPT'))residuals.push({id:'R340-CONVERGENCE-BINDING',severity:'CRITICAL',mode:'BLOCK',summary:'R314 convergence master does not carry the R340 ablation/forecast propagation receipt',source:'src/convergenceMasterR314.ts'});
 ...
 return {
  ...
  calibratedAblationForecast:{revision:'R340',advance:r340Advance,masterRecords:4285,sourceExactRecords:4105,derivedNoOverwriteRecords:180,deltaFromR334Master:25,forecastFrozenAt:'2026-09-19',noRetuning:true,canonicalAdmission:false},
