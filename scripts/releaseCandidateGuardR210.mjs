...
const expectedHead=process.env.OMEGA_EXPECTED_HEAD_SHA||'';
...
if(mode==='CANDIDATE'){
 if(!expectedBase||!expectedHead)throw new Error('R210 dispatched candidate fence requires expected base and head SHA');
 const currentMain=await api('/branches/main');
 if(currentMain?.commit?.sha!==expectedBase)throw new Error(`R210 current main drifted: expected ${expectedBase}, got ${currentMain?.commit?.sha||'NONE'}`);
 const localHead=execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim();
 if(localHead!==expectedHead||sha!==expectedHead)throw new Error(`R210 exact candidate head mismatch local=${localHead} event=${sha} expected=${expectedHead}`);
 try{execFileSync('git',['merge-base','--is-ancestor',expectedBase,expectedHead],{stdio:'ignore'})}
 catch{throw new Error(`R210 candidate ${expectedHead} is not descended from exact base ${expectedBase}`)}
 console.log(`R210 DISPATCHED CANDIDATE FENCE PASS · base ${expectedBase} · head ${expectedHead} · exact main unchanged · ancestry valid`);
 process.exit(0);
}
