...
const HEADLESS_BOUNDARY_R295='R295 never edits source, closes theorem truth, writes production, mutates CanonState, or bypasses governed review; it only compiles source-admission proposals from exact returned-evidence identity.';
if(!INPUT){console.log(JSON.stringify({status:'OBSERVE',reason:'OMEGA_R295_RETURN_RECONCILIATION not supplied',boundary:`${HEADLESS_BOUNDARY_R295} ${PROOF_ADMISSION_BOUNDARY_R295}`}));process.exit(0)}
...
const packet={schema:'OMEGA_PROOF_ADMISSION_DIRECTIVE_R295',generatedAt:new Date().toISOString(),sourcePacket:INPUT,requestedBaseSha:BASE_SHA||null,admission,boundary:HEADLESS_BOUNDARY_R295};
