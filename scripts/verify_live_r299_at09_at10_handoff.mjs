...
async function text(path){
 ...
 if(!r.ok)throw new Error(`R299 ${path} HTTP ${r.status}: ${body.slice(0,240)}`);
 return body;
}

const [html,client,contractRaw,statusRaw]=await Promise.all([
 text('/omega-pc-proof-r205.html'),
 text('/omega-pc-proof-r205.js'),
 text('/omega-r205-pc-proof-contract.json'),
 text('/api/hybrid/status')
]);
const contract=JSON.parse(contractRaw),status=JSON.parse(statusRaw);
const must=(ok,msg)=>{if(!ok)throw new Error(`R299 live handoff failed: ${msg}`)};

must(html.includes('data-r299-handoff="ARMED_EXPLICIT_CONFIRMATION"'),'served proof surface lacks R299 handoff identity');
...
])must(client.includes(token),`served client missing ${token}`);
...
if(status.state==='VERIFIED_DEVICE_ONLINE')must(status.nativeExecutionClaimed===true&&current.length>0,'live status claims verified device without current authenticated heartbeat');
else if(status.state==='DEVICE_PROOF_REQUIRED')must(status.nativeExecutionClaimed===false&&current.length===0,'device-proof-required state conflicts with current heartbeat truth');
else throw new Error(`R299 live handoff received unsupported Hybrid truth state ${status.state}`);

console.log(`R299 LIVE AT09/AT10 HANDOFF PASS · ${base}/omega-pc-proof-r205.html · ${current.length} current authenticated host(s) · armed state remains non-execution · one governed /api/missions path · DESKTOP_HEALTH + FORENSIC_HASH_LEDGER only · R141 verification + R125 Canon authority preserved`);
