...
shards="${R286_PROOF_SHARDS:-8}"
max_parallel="${R286_SHARD_MAX_PARALLEL:-4}"
child_timeout="${R286_SHARD_TIMEOUT_SEC:-270}"
if ! [[ "$shards" =~ ^[1-9][0-9]*$ ]] || [ "$shards" -gt 16 ]; then
  echo "R286_PROOF_SHARDS must be an integer 1..16" >&2
  exit 2
fi
if ! [[ "$max_parallel" =~ ^[1-9][0-9]*$ ]] || [ "$max_parallel" -gt "$shards" ]; then
  echo "R286_SHARD_MAX_PARALLEL must be an integer 1..R286_PROOF_SHARDS" >&2
  exit 2
fi
...
for ((wave_start=0; wave_start<shards; wave_start+=max_parallel)); do
  ...
  for ((i=wave_start;i<wave_end;i++)); do
    ...
    indices+=("$i")
    logs+=("$log")
    echo "R286 shard $((i+1))/$shards started pid=${pids[-1]} wave=$((wave_start/max_parallel+1))"
  done
...
echo "R286/R313 PARTITIONED PROOF PASS · $shards deterministic shards recombined in waves of $max_parallel · complete 44-route × desktop/mobile assignment preserved · every child proof passed"
