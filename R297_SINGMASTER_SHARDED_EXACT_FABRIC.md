...
R297 scales the R296 exact finite-box kernel without weakening its arithmetic guarantees. The computation is partitioned by nontrivial Pascal columns, but equality is reconstructed globally by deterministic buckets computed from the **exact integer coefficient value**. This preserves collisions that cross shard boundaries.
